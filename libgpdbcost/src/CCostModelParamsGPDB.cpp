...
//
//---------------------------------------------------------------------------
CCostModelParamsGPDB::CCostModelParamsGPDB(IMemoryPool *pmp) : m_pmp(pmp)
{
	...
	m_rgpcp[EcpRedistributeRecvCostUnit] = GPOS_NEW(pmp) SCostParam(
	...
	m_rgpcp[EcpHJHashingTupWidthCostUnit] = GPOS_NEW(pmp)
	...
	m_rgpcp[EcpHJFeedingTupColumnSpillingCostUnit] =
		GPOS_NEW(pmp) SCostParam(EcpHJFeedingTupColumnSpillingCostUnit,
	...
	m_rgpcp[EcpHJFeedingTupWidthSpillingCostUnit] =
		GPOS_NEW(pmp) SCostParam(EcpHJFeedingTupWidthSpillingCostUnit,
...
CCostModelParamsGPDB::PcpLookup(ULONG ulId) const
{
...
//---------------------------------------------------------------------------
void
CCostModelParamsGPDB::SetParam(ULONG ulId, CDouble dVal, CDouble dLowerBound,
							   CDouble dUpperBound)
{
	ECostParam ecp = (ECostParam) ulId;
	GPOS_ASSERT(EcpSentinel > ecp);
	...
	m_rgpcp[ecp] = NULL;
	m_rgpcp[ecp] =
		GPOS_NEW(m_pmp) SCostParam(ecp, dVal, dLowerBound, dUpperBound);
}
							   ...
							   CDouble dLowerBound, CDouble dUpperBound)
{
	...
	for (ULONG ul = 0; ul < EcpSentinel; ul++)
	{
		if (0 == clib::IStrCmp(szName, rgszCostParamNames[ul]))
		{
			...
			m_rgpcp[ul] = NULL;
			m_rgpcp[ul] =
				GPOS_NEW(m_pmp) SCostParam(ul, dVal, dLowerBound, dUpperBound);
...
CCostModelParamsGPDB::OsPrint(IOstream &os) const
{
	for (ULONG ul = 0; ul < EcpSentinel; ul++)
	{
		...
		os << rgszCostParamNames[ul] << " : " << pcp->DVal() << "  ["
...
BOOL
CCostModelParamsGPDB::FEquals(ICostModelParams *pcm) const
{
