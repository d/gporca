...
public:
	...
	// ctor
	CColRef(const IMDType *pmdtype, const INT iTypeModifier, ULONG ulId,
			const CName *pname);
	...
	static ULONG
	UlHash(const CColRef *pcr);

	// equality function for hash table
	static BOOL
	FEqual(const ULONG &ulKey, const ULONG &ulKeyOther)
	{
	...
	// equality function
	static BOOL
	FEqual(const CColRef *pcrFirst, const CColRef *pcrSecond)
	{
	...
	static DrgPul *
	Pdrgpul(IMemoryPool *pmp, DrgPcr *pdrgpcr);

	// check if the the array of column references are equal
	static BOOL
	FEqual(const DrgPcr *pdrgpcr1, const DrgPcr *pdrgpcr2);

	// check if the the array of column reference arrays are equal
	static BOOL
	FEqual(const DrgDrgPcr *pdrgdrgpcr1, const DrgDrgPcr *pdrgdrgpcr2);
