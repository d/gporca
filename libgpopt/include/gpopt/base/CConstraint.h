...
typedef CHashMap<CColRef, DrgPcnstr, gpos::UlHash<CColRef>,
				 gpos::FEqual<CColRef>, CleanupNULL<CColRef>,
				 CleanupRelease<DrgPcnstr> >
	HMColConstr;
...
// of pointer values
typedef CHashMap<CConstraint, BOOL, gpos::UlHashPtr<CConstraint>,
				 gpos::FEqualPtr<CConstraint>, CleanupNULL<CConstraint>,
				 CleanupNULL<BOOL> >
	HMConstraintContainment;
...
private:
	...
	static void
	AddColumnToEquivClasses(IMemoryPool *pmp, const CColRef *pcr,
							DrgPcrs **ppdrgpcrs);

	// create constraint from scalar comparison
	static CConstraint *
	PcnstrFromScalarCmp(IMemoryPool *pmp, CExpression *pexpr,
						DrgPcrs **ppdrgpcrs);

	// create constraint from scalar boolean expression
	static CConstraint *
	PcnstrFromScalarBoolOp(IMemoryPool *pmp, CExpression *pexpr,
						   DrgPcrs **ppdrgpcrs);

	// create conjunction/disjunction from array of constraints
	static CConstraint *
	PcnstrConjDisj(IMemoryPool *pmp, DrgPcnstr *pdrgpcnstr, BOOL fConj);
...
protected:
	...
	// print
	IOstream &
	PrintConjunctionDisjunction(IOstream &os, DrgPcnstr *pdrgpcnstr) const;

	// construct a conjunction or disjunction scalar expression from an
	// array of constraints
	CExpression *
	PexprScalarConjDisj(IMemoryPool *pmp, DrgPcnstr *pdrgpcnstr,
						BOOL fConj) const;

	// flatten an array of constraints to be used as constraint children
	DrgPcnstr *
	PdrgpcnstrFlatten(IMemoryPool *pmp, DrgPcnstr *pdrgpcnstr,
					  EConstraintType ect) const;

	// combine any two or more constraints that reference only one particular column
	DrgPcnstr *
	PdrgpcnstrDeduplicate(IMemoryPool *pmp, DrgPcnstr *pdrgpcnstr,
						  EConstraintType ect) const;

	// mapping between columns and arrays of constraints
	HMColConstr *
	Phmcolconstr(IMemoryPool *pmp, CColRefSet *pcrs,
				 DrgPcnstr *pdrgpcnstr) const;

	// return a copy of the conjunction/disjunction constraint for a different column
	CConstraint *
	PcnstrConjDisjRemapForColumn(IMemoryPool *pmp, CColRef *pcr,
								 DrgPcnstr *pdrgpcnstr, BOOL fConj) const;

	// create constraint from scalar array comparison expression
	static CConstraint *
	PcnstrFromScalarArrayCmp(IMemoryPool *pmp, CExpression *pexpr,
							 CColRef *pcr);

public:
	// ctor
	explicit CConstraint(IMemoryPool *pmp);
	...
	virtual BOOL
	FContains(CConstraint *pcnstr);

	// equality function
	virtual BOOL
	FEquals(CConstraint *pcnstr);
	...
	// scalar expression
	virtual CExpression *
	PexprScalar(IMemoryPool *pmp) = 0;

	// check if there is a constraint on the given column
	virtual BOOL
	FConstraint(const CColRef *pcr) const = 0;
	...
	virtual CConstraint *
	PcnstrRemapForColumn(IMemoryPool *pmp, CColRef *pcr) const = 0;

	// print
	virtual IOstream &
	OsPrint(IOstream &os) const = 0;

	// create constraint from scalar expression and pass back any discovered
	// equivalence classes
	static CConstraint *
	PcnstrFromScalarExpr(IMemoryPool *pmp, CExpression *pexpr,
						 DrgPcrs **ppdrgpcrs);

	// create conjunction from array of constraints
	static CConstraint *
	PcnstrConjunction(IMemoryPool *pmp, DrgPcnstr *pdrgpcnstr);

	// create disjunction from array of constraints
	static CConstraint *
	PcnstrDisjunction(IMemoryPool *pmp, DrgPcnstr *pdrgpcnstr);

	// merge equivalence classes coming from children of a bool op
	static DrgPcrs *
	PdrgpcrsMergeFromBoolOp(IMemoryPool *pmp, CExpression *pexpr,
							DrgPcrs *pdrgpcrsFst, DrgPcrs *pdrgpcrsSnd);

	// subset of the given constraints, which reference the given column
	static DrgPcnstr *
	PdrgpcnstrOnColumn(IMemoryPool *pmp, DrgPcnstr *pdrgpcnstr, CColRef *pcr,
					   BOOL fExclusive);
#ifdef GPOS_DEBUG
	void
	DbgPrint() const;
