...
private:
	...
	void
	AddRemainingRanges(IMemoryPool *pmp, DrgPrng *pdrgprngSrc, ULONG ulStart,
					   DrgPrng *pdrgprngDest);

	// append the given range to the array or extend the last element
	void
	AppendOrExtend(IMemoryPool *pmp, DrgPrng *pdrgprng, CRange *prange);

	// difference between two ranges on the left side only -
	// any difference on the right side is reported as residual range
	CRange *
	PrangeDiffWithRightResidual(IMemoryPool *pmp, CRange *prangeFirst,
								CRange *prangeSecond, CRange **pprangeResidual,
	...
	virtual CExpression *
	PexprConstructScalar(IMemoryPool *pmp) const;

	virtual CExpression *
	PexprConstructArrayScalar(IMemoryPool *pmp) const;

	// create interval from scalar comparison expression
	static CConstraintInterval *
	PciIntervalFromScalarCmp(IMemoryPool *pmp, CExpression *pexpr,
							 CColRef *pcr);

	static CConstraintInterval *
	PciIntervalFromScalarIDF(IMemoryPool *pmp, CExpression *pexpr,
							 CColRef *pcr);

	// create interval from scalar bool operator
	static CConstraintInterval *
	PciIntervalFromScalarBoolOp(IMemoryPool *pmp, CExpression *pexpr,
								CColRef *pcr);

	// create interval from scalar bool AND
	static CConstraintInterval *
	PciIntervalFromScalarBoolAnd(IMemoryPool *pmp, CExpression *pexpr,
								 CColRef *pcr);

	// create interval from scalar bool OR
	static CConstraintInterval *
	PciIntervalFromScalarBoolOr(IMemoryPool *pmp, CExpression *pexpr,
								CColRef *pcr);

	// create interval from scalar null test
	static CConstraintInterval *
	PciIntervalFromScalarNullTest(IMemoryPool *pmp, CExpression *pexpr,
								  CColRef *pcr);

	// creates a range like [x,x] where x is a constant
	static DrgPrng *
	PciRangeFromColConstCmp(IMemoryPool *pmp, IMDType::ECmpType ecmpt,
...
public:
	// ctor
	CConstraintInterval(IMemoryPool *pmp, const CColRef *pcr, DrgPrng *pdrgprng,
						BOOL fIsNull);
	...
	CConstraintInterval *
	PciIntersect(IMemoryPool *pmp, CConstraintInterval *pci);

	// interval union
	CConstraintInterval *
	PciUnion(IMemoryPool *pmp, CConstraintInterval *pci);

	// interval difference
	CConstraintInterval *
	PciDifference(IMemoryPool *pmp, CConstraintInterval *pci);

	// interval complement
	CConstraintInterval *
	PciComplement(IMemoryPool *pmp);

	// does the current interval contain the given interval?
	BOOL
	FContainsInterval(IMemoryPool *pmp, CConstraintInterval *pci);
	...
	CExpression *
	PexprConstructDisjunctionScalar(IMemoryPool *pmp) const;
	...
	static CConstraintInterval *
	PciUnbounded(IMemoryPool *pmp, const CColRef *pcr, BOOL fIncludesNull);

	// create an unbounded interval on any column from the given set
	static CConstraintInterval *
	PciUnbounded(IMemoryPool *pmp, const CColRefSet *pcrs, BOOL fIncludesNull);

	// helper for create interval from comparison between a column and a constant
	static CConstraintInterval *
	PciIntervalFromColConstCmp(IMemoryPool *pmp, CColRef *pcr,
							   IMDType::ECmpType ecmpt,
							   CScalarConst *popScConst);

	// create interval from scalar expression
	static CConstraintInterval *
	PciIntervalFromScalarExpr(IMemoryPool *pmp, CExpression *pexpr,
							  CColRef *pcr);

	// create interval from any general constraint that references
	// only one column
	static CConstraintInterval *
	PciIntervalFromConstraint(IMemoryPool *pmp, CConstraint *pcnstr,
							  CColRef *pcr = NULL);

	// generate a ConstraintInterval from the given expression
	static CConstraintInterval *
	PcnstrIntervalFromScalarArrayCmp(IMemoryPool *pmp, CExpression *pexpr,
									 CColRef *pcr);
