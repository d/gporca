...
private:
	...
	static void
	BreakCostTiesForJoinPlans(const CCostContext *pccFst,
...
public:
	...
	// ctor
	CCostContext(IMemoryPool *pmp, COptimizationContext *poc, ULONG ulOptReq,
	...
	void
	DerivePlanProps(IMemoryPool *pmp);
	...
	// set child contexts
	void
	SetChildContexts(DrgPoc *pdrgpoc)
	{
	...
	BOOL
	FValid(IMemoryPool *pmp);

	// comparison operator
	BOOL
	operator==(const CCostContext &cc) const;

	// compute cost
	CCost
	CostCompute(IMemoryPool *pmp, DrgPcost *pdrgpcostChildren);

	// is current context better than the given equivalent context based on cost?
	BOOL
	FBetterThan(const CCostContext *pcc) const;

	// equality function
	static BOOL
	FEqual(const CCostContext &ccLeft, const CCostContext &ccRight)
	{
	...
	// equality function
	static BOOL
	FEqual(const CCostContext *pccLeft, const CCostContext *pccRight)
	{
