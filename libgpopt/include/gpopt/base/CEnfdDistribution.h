...
public:
	...
	// matching function
	BOOL
	FMatch(CEnfdDistribution *ped)
	{
		GPOS_ASSERT(NULL != ped);

		return m_edm == ped->Edm() && m_pds->FMatch(ped->PdsRequired());
	}
	...
	BOOL
	FCompatible(CDistributionSpec *pds) const;
	...
	Epet(CExpressionHandle &exprhdl, CPhysical *popPhysical,
		 CPartitionPropagationSpec *pppsReqd, BOOL fDistribReqd) const;
