...
public:
	...
	// matching function
	BOOL
	FMatch(CEnfdPartitionPropagation *pepp)
	{
		GPOS_ASSERT(NULL != pepp);

		return m_eppm == pepp->Eppm() && m_ppps->FMatch(pepp->PppsRequired()) &&
	...
	static const CHAR *
	SzPropagationMatching(EPartitionPropagationMatching eppm);
