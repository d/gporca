...
public:
	...
	BOOL
	FIncluded(CColRefSet *pcrs) const;
	...
	// equality function
	BOOL
	FEqual(const CFunctionalDependency *pfd) const;

	// do the given arguments form a functional dependency
	BOOL
	FFunctionallyDependent(CColRefSet *pcrsKey, CColRef *pcr)
	{
	...
	// hash function
	static ULONG
	UlHash(const DrgPfd *pdrgpfd);

	// equality function
	static BOOL
	FEqual(const DrgPfd *pdrgpfdFst, const DrgPfd *pdrgpfdSnd);

	// create a set of all keys in the passed FD's array
	static CColRefSet *
	PcrsKeys(IMemoryPool *pmp, const DrgPfd *pdrgpfd);

	// create an array of all keys in the passed FD's array
	static DrgPcr *
	PdrgpcrKeys(IMemoryPool *pmp, const DrgPfd *pdrgpfd);
