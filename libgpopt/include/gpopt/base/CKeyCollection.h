...
public:
	// ctors
	explicit CKeyCollection(IMemoryPool *pmp);
	...
	BOOL
	FKey(const CColRefSet *pcrs, BOOL fExactMatch = true) const;

	// check if an array of columns constitutes a key
	BOOL
	FKey(IMemoryPool *pmp, const DrgPcr *pdrgpcr) const;

	// trim off non-key columns
	DrgPcr *
	PdrgpcrTrim(IMemoryPool *pmp, const DrgPcr *pdrgpcr) const;

	// extract a key
	DrgPcr *
	PdrgpcrKey(IMemoryPool *pmp) const;

	// extract a hashable key
	DrgPcr *
	PdrgpcrHashableKey(IMemoryPool *pmp) const;

	// extract key at given position
	DrgPcr *
	PdrgpcrKey(IMemoryPool *pmp, ULONG ul) const;

	// extract key at given position
	CColRefSet *
	PcrsKey(IMemoryPool *pmp, ULONG ul) const;
