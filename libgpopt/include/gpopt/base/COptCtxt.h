...
public:
	// ctor
	COptCtxt(IMemoryPool *pmp, CColumnFactory *pcf, CMDAccessor *pmda,
	...
	void
	SetReqdSystemCols(DrgPcr *pdrgpcrSystemCols)
	{
	...
	// factory method
	static COptCtxt *
	PoctxtCreate(IMemoryPool *pmp, CMDAccessor *pmda,
				 IConstExprEvaluator *pceeval, COptimizerConfig *poconf);
	...
	PoctxtFromTLS()
	{
		return reinterpret_cast<COptCtxt *>(ITask::PtskSelf()->Tls().Ptlsobj(
			CTaskLocalStorage::EtlsidxOptCtxt));
	}

	// return true if all enforcers are enabled
	static BOOL
	FAllEnforcersEnabled();
