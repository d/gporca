...
private:
	...
	public:
		// ctor
		COrderExpression(gpmd::IMDId *pmdid, const CColRef *pcr,
						 ENullTreatment ent);
		...
		BOOL
		FMatch(const COrderExpression *poe) const;
	...
	void
	ExtractCols(CColRefSet *pcrs) const;

public:
	// ctor
	explicit COrderSpec(IMemoryPool *pmp);
	...
	void
	Append(gpmd::IMDId *pmdid, const CColRef *pcr, ENullTreatment ent);

	// extract colref set of order columns
	virtual CColRefSet *
	PcrsUsed(IMemoryPool *pmp) const;
	...
	// check if order specs match
	BOOL
	FMatch(const COrderSpec *pos) const;

	// check if order specs satisfies req'd spec
	BOOL
	FSatisfies(const COrderSpec *pos) const;

	// append enforcers to dynamic array for the given plan properties
	virtual void
	AppendEnforcers(IMemoryPool *pmp, CExpressionHandle &exprhdl,
					CReqdPropPlan *prpp, DrgPexpr *pdrgpexpr,
					CExpression *pexpr);
	...
	virtual COrderSpec *
	PosCopyWithRemappedColumns(IMemoryPool *pmp, HMUlCr *phmulcr,
							   BOOL fMustExist);

	// return a copy of the order spec after excluding the given columns
	virtual COrderSpec *
	PosExcludeColumns(IMemoryPool *pmp, CColRefSet *pcrs);
