...
private:
	...
	void
	SetSystemCols(IMemoryPool *pmp);

	// return top level operator in the given expression
	static COperator *
	PopTop(CExpression *pexpr);
...
public:
	// ctor
	CQueryContext(IMemoryPool *pmp, CExpression *pexpr, CReqdPropPlan *prpp,
				  DrgPcr *pdrgpcr, DrgPmdname *pdrgpmdname, BOOL fDeriveStats);
	...
	static void
	MapComputedToUsedCols(CColumnFactory *pcf, CExpression *pexpr);
