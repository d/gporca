...
private:
	...
	CExpression *
	PexprEquality(IMemoryPool *pmp, const CColRef *pcr);

	// construct a scalar comparison expression from one of the ends
	CExpression *
	PexprScalarCompEnd(IMemoryPool *pmp, IDatum *pdatum, ERangeInclusion eri,
	...
	// print a bound
	IOstream &
	OsPrintBound(IOstream &os, IDatum *pdatum, const CHAR *szInfinity) const;

public:
	// ctor
	CRange(IMDId *pmdid, const IComparator *pcomp, IDatum *pdatumLeft,
	...
	CRange *
	PrngIntersect(IMemoryPool *pmp, CRange *prange);

	// difference between this range and a given range on the left side only
	CRange *
	PrngDifferenceLeft(IMemoryPool *pmp, CRange *prange);

	// difference between this range and a given range on the right side only
	CRange *
	PrngDifferenceRight(IMemoryPool *pmp, CRange *prange);
	...
	CRange *
	PrngExtend(IMemoryPool *pmp, CRange *prange);

	// construct scalar expression
	CExpression *
	PexprScalar(IMemoryPool *pmp, const CColRef *pcr);
