...
public:
	...
	static void
	PrintExpression(CExpression *pexpr);

	// print memo to debug trace
	static void
	PrintMemo(CMemo *pmemo);

	static IOstream &
	OsPrintDrgPcoldesc(IOstream &os, DrgPcoldesc *pdrgpcoldescIncludedCols,
					   ULONG ulLength);
#endif  // GPOS_DEBUG
	...
	static void
	ValidateCTEProducerConsumerLocality(IMemoryPool *pmp, CExpression *pexpr,
										EExecLocalityType edt, HMUlUl *phmulul);

	// check is a comparison between given types or a comparison after casting
	// one side to an another exists
	static BOOL
	FCmpOrCastedCmpExists(IMDId *pmdidLeft, IMDId *pmdidRight,
						  IMDType::ECmpType ecmpt);

	// return the mdid of the given scalar comparison between the two types
	static IMDId *
	PmdidScCmp(IMemoryPool *pmp, CMDAccessor *pmda, IMDId *pmdidLeft,
			   IMDId *pmdidRight, IMDType::ECmpType ecmpt);

	// generate a comparison expression for two column references
	static CExpression *
	PexprScalarCmp(IMemoryPool *pmp, const CColRef *pcrLeft,
	...
	static CExpression *
	PexprScalarCmp(IMemoryPool *pmp, CExpression *pexprLeft,
				   const CColRef *pcrRight, CWStringConst strOp,
				   IMDId *pmdidOp);
	...
	static CExpression *
	PexprScalarArrayCmp(IMemoryPool *pmp,
	...
	static CExpression *
	PexprIDF(IMemoryPool *pmp, CExpression *pexprLeft, CExpression *pexprRight);

	// generate an Is NOT Distinct From expression for two column references
	static CExpression *
	PexprINDF(IMemoryPool *pmp, const CColRef *pcrLeft,
			  const CColRef *pcrRight);

	// generate an Is NOT Distinct From expression for scalar expressions
	static CExpression *
	PexprINDF(IMemoryPool *pmp, CExpression *pexprLeft,
			  CExpression *pexprRight);

	// generate an Is NULL expression
	static CExpression *
	PexprIsNull(IMemoryPool *pmp, CExpression *pexpr);

	// generate an Is NOT NULL expression
	static CExpression *
	PexprIsNotNull(IMemoryPool *pmp, CExpression *pexpr);

	// generate an Is NOT FALSE expression
	static CExpression *
	PexprIsNotFalse(IMemoryPool *pmp, CExpression *pexpr);

	// find if a scalar expression uses a nullable columns from the output of a logical expression
	static BOOL
	FUsesNullableCol(IMemoryPool *pmp, CExpression *pexprScalar,
					 CExpression *pexprLogical);

	// generate a scalar op expression for a column reference and an expression
	static CExpression *
	PexprScalarOp(IMemoryPool *pmp, const CColRef *pcrLeft, CExpression *pexpr,
				  CWStringConst strOp, IMDId *pmdidOp,
				  IMDId *pmdidReturnType = NULL);

	// generate a scalar bool op expression
	static CExpression *
	PexprScalarBoolOp(IMemoryPool *pmp, CScalarBoolOp::EBoolOperator eboolop,
					  DrgPexpr *pdrgpexpr);

	// negate the given expression
	static CExpression *
	PexprNegate(IMemoryPool *pmp, CExpression *pexpr);

	// generate a scalar ident expression
	static CExpression *
	PexprScalarIdent(IMemoryPool *pmp, const CColRef *pcr);

	// generate a scalar project element expression
	static CExpression *
	PexprScalarProjectElement(IMemoryPool *pmp, CColRef *pcr,
							  CExpression *pexpr);
	...
	static CExpression *
	PexprLogicalProjectNulls(IMemoryPool *pmp, DrgPcr *pdrgpcr,
							 CExpression *pexpr, HMUlCr *phmulcr = NULL);

	// construct a project list using the given columns and datums
	// store the mapping in the phmulcr map if not NULL
	static CExpression *
	PexprScalarProjListConst(IMemoryPool *pmp, DrgPcr *pdrgpcr,
							 DrgPdatum *pdrgpdatum, HMUlCr *phmulcr);

	// generate a project expression
	static CExpression *
	PexprAddProjection(IMemoryPool *pmp, CExpression *pexpr,
	...
	// generate an aggregate expression
	static CExpression *
	PexprLogicalGbAggGlobal(IMemoryPool *pmp, DrgPcr *pdrgpcr,
							CExpression *pexpr, CExpression *pexprPrL);

	// generate an aggregate expression
	static CExpression *
	PexprLogicalGbAgg(IMemoryPool *pmp, DrgPcr *pdrgpcr, CExpression *pexpr,
					  CExpression *pexprPrL, COperator::EGbAggType egbaggtype);

	// check if the aggregate is local or global
	static BOOL
	FHasGlobalAggFunc(const CExpression *pexprProjList);

	// generate a bool expression
	static CExpression *
	PexprScalarConstBool(IMemoryPool *pmp, BOOL fVal, BOOL fNull = false);

	// generate an int4 expression
	static CExpression *
	PexprScalarConstInt4(IMemoryPool *pmp, INT iVal);

	// generate an int8 expression
	static CExpression *
	PexprScalarConstInt8(IMemoryPool *pmp, LINT lVal, BOOL fNull = false);

	// generate an oid constant expression
	static CExpression *
	PexprScalarConstOid(IMemoryPool *pmp, OID oidVal);

	// comparison operator type
	static IMDType::ECmpType
	Ecmpt(IMDId *pmdid);

	// comparison operator type
	static IMDType::ECmpType
	Ecmpt(CMDAccessor *pmda, IMDId *pmdid);

	// generate a binary join expression
	template <class T>
	static CExpression *
	PexprLogicalJoin(IMemoryPool *pmp, CExpression *pexprLeft,
					 CExpression *pexprRight, CExpression *pexprPredicate);

	// generate an apply expression
	template <class T>
	static CExpression *
	PexprLogicalApply(IMemoryPool *pmp, CExpression *pexprLeft,
					  CExpression *pexprRight, CExpression *pexprPred = NULL);
	...
	static CExpression *
	PexprLogicalApply(IMemoryPool *pmp, CExpression *pexprLeft,
					  CExpression *pexprRight, DrgPcr *pdrgpcrInner,
					  COperator::EOperatorId eopidOriginSubq,
					  CExpression *pexprPred = NULL);
	...
	template <class T>
	static CExpression *
	PexprLogicalCorrelatedQuantifiedApply(
		IMemoryPool *pmp, CExpression *pexprLeft, CExpression *pexprRight,
		DrgPcr *pdrgpcrInner, COperator::EOperatorId eopidOriginSubq,
		CExpression *pexprPred = NULL);
	...
	static CColRef *
	PcrExtractPartKey(DrgDrgPcr *pdrgpdrgpcr, ULONG ulLevel);
	...
	static DrgPexpr *
	PdrgpexprDedup(IMemoryPool *pmp, DrgPexpr *pdrgpexpr);

	// deep equality of expression trees
	static BOOL
	FEqual(const CExpression *pexprLeft, const CExpression *pexprRight);

	// compare expression against an array of expressions
	static BOOL
	FEqualAny(const CExpression *pexpr, const DrgPexpr *pdrgpexpr);

	// deep equality of expression arrays
	static BOOL
	FEqual(const DrgPexpr *pdrgpexprLeft, const DrgPexpr *pdrgpexprRight);

	// check if first expression array contains all expressions in second array
	static BOOL
	FContains(const DrgPexpr *pdrgpexprFst, const DrgPexpr *pdrgpexprSnd);

	// return the number of occurrences of the given expression in the given
	// array of expressions
	static ULONG
	UlOccurrences(const CExpression *pexpr, DrgPexpr *pdrgpexpr);
	...
	static IOstream &
	OsPrintDrgPcr(IOstream &os, const DrgPcr *, ULONG = ULONG_MAX);
	...
	static DrgPcr *
	PdrgpcrNonSystemCols(IMemoryPool *pmp, CColRefSet *pcrs);

	// create an array of output columns including a key for grouping
	static DrgPcr *
	PdrgpcrGroupingKey(IMemoryPool *pmp, CExpression *pexpr,
					   DrgPcr **ppdrgpcrKey);
	...
	static DrgPcrs *
	PdrgpcrsAddEquivClass(IMemoryPool *pmp, CColRefSet *pcrsNew,
						  DrgPcrs *pdrgpcrs);

	// merge 2 arrays of equivalence classes
	static DrgPcrs *
	PdrgpcrsMergeEquivClasses(IMemoryPool *pmp, DrgPcrs *pdrgpcrsFst,
							  DrgPcrs *pdrgpcrsSnd);

	// intersect 2 arrays of equivalence classes
	static DrgPcrs *
	PdrgpcrsIntersectEquivClasses(IMemoryPool *pmp, DrgPcrs *pdrgpcrsFst,
								  DrgPcrs *pdrgpcrsSnd);

	// return a copy of equivalence classes from all children
	static DrgPcrs *
	PdrgpcrsCopyChildEquivClasses(IMemoryPool *pmp, CExpressionHandle &exprhdl);

	// return a copy of the given array of columns, excluding the columns
	// in the given colrefset
	static DrgPcr *
	PdrgpcrExcludeColumns(IMemoryPool *pmp, DrgPcr *pdrgpcrOriginal,
	...
	static DrgPcr *
	PdrgpcrCopy(IMemoryPool *pmp, DrgPcr *pdrgpcr, BOOL fAllComputed = false,
				HMUlCr *phmulcr = NULL);

	// equality check between two arrays of column refs. Inputs can be NULL
	static BOOL
	FEqual(DrgPcr *pdrgpcrFst, DrgPcr *pdrgpcrSnd);

	// compute hash value for an array of column references
	static ULONG
	UlHashColArray(const DrgPcr *pdrgpcr, const ULONG ulMaxCols = 5);
	...
	static CColRefSet *
	PcrsCTEProducerColumns(IMemoryPool *pmp, CColRefSet *pcrsInput,
	...
	static CPartConstraint *
	PpartcnstrFromMDPartCnstr(IMemoryPool *pmp, CMDAccessor *pmda,
