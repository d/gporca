...
template <class T>
CExpression *
CUtils::PexprLogicalJoin(IMemoryPool *pmp, CExpression *pexprLeft,
						 CExpression *pexprRight, CExpression *pexprPredicate)
{
	...
	return GPOS_NEW(pmp) CExpression(pmp, GPOS_NEW(pmp) T(pmp), pexprLeft,
...
template <class T>
CExpression *
CUtils::PexprLogicalApply(IMemoryPool *pmp, CExpression *pexprLeft,
						  ...
						  CExpression *pexprPred)
{
	...
	return GPOS_NEW(pmp)
		CExpression(pmp, GPOS_NEW(pmp) T(pmp, pdrgpcr, eopidOriginSubq),
						  ...
						  CExpression *pexprPred)
{
	...
	return GPOS_NEW(pmp)
		CExpression(pmp, GPOS_NEW(pmp) T(pmp, pdrgpcrInner, eopidOriginSubq),
					pexprLeft, pexprRight, pexprScalar);
}
	...
	CExpression *pexprPred)
{
	...
	if (COperator::EopLogicalSelect != pexprRight->Pop()->Eopid())
	{
		...
		GPOS_ASSERT(!CUtils::FHasOuterRefs(pexprRight) &&
					"unexpected outer references in inner child of Semi Apply "
					"expression ");
		pexprScalar->Release();
		pexprScalar = PexprScalarConstBool(pmp, true /*fVal*/);
	}
...
void
CUtils::AddRefAppend(CDynamicPtrArray<T, pfnDestroy> *pdrgptOutput,
...
CUtils::FScalarConstInt(CExpression *pexpr)
{
	...
	GPOS_ASSERT(IMDType::EtiInt2 == eti || IMDType::EtiInt4 == eti ||
				IMDType::EtiInt8 == eti);
...
CUtils::FMatchIndex(T *pop1, COperator *pop2)
{
	...
	return pop1->UlOriginOpId() == popIndex->UlOriginOpId() &&
		   ...
		   pop1->Pindexdesc()->Pmdid()->FEquals(
			   popIndex->Pindexdesc()->Pmdid()) &&
