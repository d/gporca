...
public:
	// ctor
	CWindowFrame(IMemoryPool *pmp, EFrameSpec efs, EFrameBoundary efbLeading,
	...
	// matching function
	BOOL
	FMatch(const CWindowFrame *pwf) const;
	...
	virtual CWindowFrame *
	PwfCopyWithRemappedColumns(IMemoryPool *pmp, HMUlCr *phmulcr,
							   BOOL fMustExist);
	...
	static BOOL
	FEqual(const DrgPwf *pdrgpwfFirst, const DrgPwf *pdrgpwfSecond);

	// combine hash values of a maximum number of entries
	static ULONG
	UlHash(const DrgPwf *pdrgpwfFirst, ULONG ulMaxSize);

	// print array of window frame objects
	static IOstream &
	OsPrint(IOstream &os, const DrgPwf *pdrgpwf);

	// check if a given window frame is empty
	static BOOL
	FEmpty(CWindowFrame *pwf)
	{
