...
private:
	...
	void
	ApplyTransformations(IMemoryPool *pmpLocal, CXformSet *pxfs,
						 CGroupExpression *pgexpr);

	// transition a given group to a target state
	void
	TransitionGroup(IMemoryPool *pmpLocal, CGroup *pgroup,
					CGroup::EState estTarget);

	// transition a given group expression to a target state
	void
	TransitionGroupExpression(IMemoryPool *pmpLocal, CGroupExpression *pgexpr,
	...
	CCostContext *
	PccOptimizeChild(CExpressionHandle &exprhdl, CExpressionHandle &exprhdlRel,
					 COptimizationContext *pocOrigin, DrgPdp *pdrgpdp,
					 DrgPstat *pdrgpstatCurrentCtxt, ULONG ulChildIndex,
					 ULONG ulOptReq);

	// optimize child groups of a given group expression
	DrgPoc *
	PdrgpocOptimizeChildren(CExpressionHandle &exprhdl,
							COptimizationContext *pocOrigin, ULONG ulOptReq);

	// optimize group expression under a given context
	void
	OptimizeGroupExpression(CGroupExpression *pgexpr,
							COptimizationContext *poc);

	// optimize group under a given context
	CGroupExpression *
	PgexprOptimize(CGroup *pgroup, COptimizationContext *poc,
				   CGroupExpression *pgexprOrigin);
#endif  // GPOS_DEBUG

	// initialize query logical expression
	void
	InitLogicalExpression(CExpression *pexpr);

	// insert children of the given expression to memo, and
	// copy the resulting groups to the given group array
	void
	InsertExpressionChildren(CExpression *pexpr, DrgPgroup *pdrgpgroupChildren,
							 CXform::EXformId exfidOrigin,
							 CGroupExpression *pgexprOrigin);

	// create and schedule the main optimization job
	void
	ScheduleMainJob(CSchedulerContext *psc, COptimizationContext *poc);

	// build memo using multiple threads
	void
	MultiThreadedOptimize(ULONG ulWorkers = 4);
	...
	void
	PrintActivatedXforms(IOstream &os) const;
	...
	ULLONG
	UllRandomPlanId(ULONG *pulSeed);

	// extract a plan sample and handle exceptions according to enumerator configurations
	BOOL
	FValidPlanSample(CEnumeratorConfig *pec, ULLONG ullPlanId,
					 CExpression **ppexpr);
	...
	BOOL
	FChildrenOptimized(DrgPoc *pdrgpoc);

	// check if ayn of the given property enforcing types prohibits enforcement
	static BOOL
	FProhibited(CEnfdProp::EPropEnforcingType epetOrder,
	...
	static BOOL
	FPossibleDuplicateGroups(CGroup *pgroupFst, CGroup *pgroupSnd);

	// check if optimization is possible under the given property enforcing types
	static BOOL
	FOptimize(CEnfdProp::EPropEnforcingType epetOrder,
			  CEnfdProp::EPropEnforcingType epetDistribution,
			  CEnfdProp::EPropEnforcingType epetRewindability,
			  CEnfdProp::EPropEnforcingType epetPropagation);
	...
	// propagation spec
	static BOOL
	FCheckReqdPartPropagation(CPhysical *pop, CEnfdPartitionPropagation *pepp);

	// unrank the plan with the given 'ullPlanId' from the memo
	CExpression *
	PexprUnrank(ULLONG ullPlanId);
	...
	BOOL
	FSafeToPruneWithDPEStats(CGroupExpression *pgexpr, CReqdPropPlan *prpp,
							 CCostContext *pccChild, ULONG ulChildIndex);

	// print current memory consumption
	IOstream &
	OsPrintMemoryConsumption(IOstream &os, const CHAR *szHeader) const;
...
public:
	// ctor
	explicit CEngine(IMemoryPool *pmp);
	...
	void
	Init(CQueryContext *pqc, DrgPss *pdrgpss);
	...
	CGroup *
	PgroupInsert(CGroup *pgroupTarget, CExpression *pexpr,
				 CXform::EXformId exfidOrigin, CGroupExpression *pgexprOrigin,
				 BOOL fIntermediate);

	// insert a set of xform results into the memo
	void
	InsertXformResult(CGroup *pgroupOrigin, CXformResult *pxfres,
					  CXform::EXformId exfidOrigin,
					  CGroupExpression *pgexprOrigin, ULONG ulXformTime);

	// add enforcers to the memo
	void
	AddEnforcers(CGroupExpression *pgexprChild, DrgPexpr *pdrgpexprEnforcers);
	...
	BOOL
	FCheckReqdProps(CExpressionHandle &exprhdl, CReqdPropPlan *prpp,
					ULONG ulOptReq);

	// check enforceable properties;
	// return false if it's impossible for the operator to satisfy one or more
	BOOL
	FCheckEnfdProps(IMemoryPool *pmp, CGroupExpression *pgexpr,
					COptimizationContext *poc, ULONG ulOptReq, DrgPoc *pdrgpoc);

	// check if the given expression has valid cte and partition properties
	// with respect to the given requirements
	BOOL
	FValidCTEAndPartitionProperties(IMemoryPool *pmp,
	...
	DrgPoc *
	PdrgpocChildren(IMemoryPool *pmp, CExpressionHandle &exprhdl);
	...
	BOOL
	FOptimizeChild(CGroupExpression *pgexprParent,
				   CGroupExpression *pgexprChild,
				   COptimizationContext *pocChild, EOptimizationLevel eol);

	// determine if a plan, rooted by given group expression, can be safely pruned based on cost bounds
	BOOL
	FSafeToPrune(CGroupExpression *pgexpr, CReqdPropPlan *prpp,
				 CCostContext *pccChild, ULONG ulChildIndex,
	...
	static EOptimizationLevel
	EolDamp(EOptimizationLevel eol);

	// derive statistics
	static void
	DeriveStats(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal, CGroup *pgroup,
				CReqdPropRelational *prprel);

	// return the first group expression in a given group
	static CGroupExpression *
	PgexprFirst(CGroup *pgroup);
