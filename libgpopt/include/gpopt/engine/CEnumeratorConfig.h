...
private:
	...
	public:
		// ctor
		SSamplePlan(ULLONG ullPlanId, CCost cost)
			: m_ullPlanId(ullPlanId), m_cost(cost)
		{
		}
	...
	static DOUBLE
	DGaussian(DOUBLE d, DOUBLE dMean, DOUBLE dStd);

public:
	// ctor
	CEnumeratorConfig(IMemoryPool *pmp, ULLONG ullPlanId, ULLONG ullSamples,
	...
	BOOL
	FAddSample(ULLONG ullPlanId, CCost cost);
	...
	CDouble
	DCostDistrX(ULONG ulPos) const;

	// return y-value of cost distribution
	CDouble
	DCostDistrY(ULONG ulPos) const;
	...
	void
	DumpSamples(CWStringDynamic *pstr, ULONG ulSessionId, ULONG ulCommandId);

	// dump fitted cost distribution to an output file
	void
	DumpCostDistr(CWStringDynamic *pstr, ULONG ulSessionId, ULONG ulCommandId);

	// print ids of plans in the generated sample
	void
	PrintPlanSample() const;

	// compute Gaussian kernel density
	static void
	GussianKernelDensity(DOUBLE *pdObervationX, DOUBLE *pdObervationY,
						 ULONG ulObservations, DOUBLE *pdX, DOUBLE *pdY,
						 ULONG ulSize);

	// generate default enumerator configurations
	static CEnumeratorConfig *
	PecDefault(IMemoryPool *pmp)
	{
		return GPOS_NEW(pmp)
			CEnumeratorConfig(pmp, 0 /*ullPlanId*/, 0 /*ullSamples*/);
	}

	// generate enumerator configuration for a given plan id
	static CEnumeratorConfig *
	Pec(IMemoryPool *pmp, ULLONG ullPlanId)
	{
		return GPOS_NEW(pmp)
			CEnumeratorConfig(pmp, ullPlanId, 0 /*ullSamples*/);
	}
