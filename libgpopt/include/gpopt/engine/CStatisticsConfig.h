...
public:
	// ctor
	CStatisticsConfig(IMemoryPool *pmp, CDouble dDampingFactorFilter,
	...
	void
	AddMissingStatsColumn(CMDIdColStats *pmdidCol);

	// collect the missing statistics columns
	void
	CollectMissingStatsColumns(DrgPmdid *pdrgmdid);

	// generate default optimizer configurations
	static CStatisticsConfig *
	PstatsconfDefault(IMemoryPool *pmp)
	{
		return GPOS_NEW(pmp) CStatisticsConfig(
			pmp, 0.75 /* dDampingFactorFilter */, 0.01 /* dDampingFactorJoin */,
