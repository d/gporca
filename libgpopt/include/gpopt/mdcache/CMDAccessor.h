...
private:
	...
	typedef CSyncHashtableIter<SMDAccessorElem, MdidPtr, CSpinlockMDAcc>
		MDHTIter;
	...
	typedef CSyncHashtable<SMDProviderElem, SMDProviderElem, CSpinlockMDAccMDP>
		MDPHT;
	...
	typedef CSyncHashtableIter<SMDProviderElem, SMDProviderElem,
							   CSpinlockMDAccMDP>
		MDPHTIter;
	...
	public:
		...
		static BOOL
		FEqual(const MdidPtr &pmdidLeft, const MdidPtr &pmdidRight);

		// hash function for cost contexts hash table
		static ULONG
		UlHash(const MdidPtr &pmdid);
	};
	...
	public:
		...
		// equality function for hash tables
		static BOOL
		FEqual(const SMDProviderElem &mdpelemLeft,
			   const SMDProviderElem &mdpelemRight);

		// hash function for MD providers hash table
		static ULONG
		UlHash(const SMDProviderElem &mdpelem);
	};
...
private:
	...
	const IMDType *
	Pmdtype(CSystemId sysid, IMDType::ETypeInfo eti);

	// return the generic type corresponding to the given type info
	const IMDType *
	Pmdtype(IMDType::ETypeInfo eti);

	// destroy accessor element when MDAccessor is destroyed
	static void
	DestroyAccessorElement(SMDAccessorElem *pmdaccelem);

	// destroy accessor element when MDAccessor is destroyed
	static void
	DestroyProviderElement(SMDProviderElem *pmdpelem);

	// lookup an MD provider by system id
	IMDProvider *
	Pmdp(CSystemId sysid);

	// initialize hash tables
	void
	InitHashtables(IMemoryPool *pmp);

	// return the column statistics meta data object for a given column of a table
	const IMDColStats *
	Pmdcolstats(IMemoryPool *pmp, IMDId *pmdidRel, ULONG ulPos);

	// record histogram and width information for a given column of a table
	void
	RecordColumnStats(IMemoryPool *pmp, IMDId *pmdidRel, ULONG ulColId,
					  ULONG ulPos, BOOL fSystemCol, BOOL fEmptyTable,
					  HMUlHist *phmulhist, HMUlDouble *phmuldoubleWidth,
					  CStatisticsConfig *pstatsconf);

	// construct a stats histogram from an MD column stats object
	CHistogram *
	Phist(IMemoryPool *pmp, IMDId *pmdidType, const IMDColStats *pmdcolstats);

	// construct a typed bucket from a DXL bucket
	CBucket *
	Pbucket(IMemoryPool *pmp, IMDId *pmdidType, const CDXLBucket *pdxlbucket);

	// construct a typed datum from a DXL bucket
	IDatum *
	Pdatum(IMemoryPool *pmp, IMDId *pmdidType, const CDXLDatum *pdxldatum);

public:
	// ctors
	CMDAccessor(IMemoryPool *pmp, MDCache *pcache);
	CMDAccessor(IMemoryPool *pmp, MDCache *pcache, CSystemId sysid,
				IMDProvider *pmdp);
	CMDAccessor(IMemoryPool *pmp, MDCache *pcache, const DrgPsysid *pdrgpsysid,
	...
	void
	RegisterProvider(CSystemId sysid, IMDProvider *pmdp);

	// register given MD providers
	void
	RegisterProviders(const DrgPsysid *pdrgpsysid, const DrgPmdp *pdrgpmdp);
