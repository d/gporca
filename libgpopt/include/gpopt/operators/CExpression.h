...
private:
	...
	void
	SetPdp(CDrvdProp *pdp, const CDrvdProp::EPropType ept);

#ifdef GPOS_DEBUG

	// assert valid property derivation
	void
	AssertValidPropDerivation(const CDrvdProp::EPropType ept);

	// print expression properties
	void
	PrintProperties(IOstream &os, CPrintPrefix &pfx) const;

#endif  // GPOS_DEBUG

	// check if the expression satisfies partition enforcer condition
	BOOL
	FValidPartEnforcers(CDrvdPropCtxtPlan *pdpctxtplan);

	// check if the distributions of all children are compatible
	BOOL
	FValidChildrenDistribution(CDrvdPropCtxtPlan *pdpctxtplan);

	// copy group properties and stats to expression
	void
	CopyGroupPropsAndStats(IStatistics *pstatsInput);

	// decorate expression tree with required plan properties
	CReqdPropPlan *
	PrppDecorate(IMemoryPool *pmp, CReqdPropPlan *prppInput);
...
public:
	...
	// ctor for leaf nodes
	CExpression(IMemoryPool *pmp, COperator *pop,
				CGroupExpression *pgexpr = NULL);

	// ctor for unary expressions
	CExpression(IMemoryPool *pmp, COperator *pop, CExpression *pexpr);

	// ctor for binary expressions
	CExpression(IMemoryPool *pmp, COperator *pop, CExpression *pexprChildFirst,
	...
	CExpression(IMemoryPool *pmp, COperator *pop, DrgPexpr *pdrgpexpr);

	// ctor for n-ary expression with origin group expression
	CExpression(IMemoryPool *pmp, COperator *pop, CGroupExpression *pgexpr,
				DrgPexpr *pdrgpexpr, IStatistics *pstatsInput,
				CCost cost = GPOPT_INVALID_COST);

	// ctor for expression with derived properties
	CExpression(IMemoryPool *pmp, CDrvdProp *pdprop);
	...
	CDrvdProp *
	Pdp(const CDrvdProp::EPropType ept) const;
	...
	CDrvdProp *
	PdpDerive(CDrvdPropCtxt *pdpctxt = NULL);

	// derive statistics
	IStatistics *
	PstatsDerive(CReqdPropRelational *prprel, DrgPstat *pdrgpstatCtxt);

	// reset a derived property
	void
	ResetDerivedProperty(CDrvdProp::EPropType ept);

	// reset all derived properties
	void
	ResetDerivedProperties();
	...
	CReqdPropPlan *
	PrppCompute(IMemoryPool *pmp, CReqdPropPlan *prppInput);
	...
	// print driver
	IOstream &
	OsPrint(IOstream &os, const CPrintPrefix * = NULL, BOOL fLast = true) const;

	// match with group expression
	BOOL
	FMatchPattern(CGroupExpression *pgexpr) const;

	// return a copy of the expression with remapped columns
	CExpression *
	PexprCopyWithRemappedColumns(IMemoryPool *pmp, HMUlCr *phmulcr,
								 BOOL fMustExist) const;

	// compare entire expression rooted here
	BOOL
	FMatch(CExpression *pexpr) const;

#ifdef GPOS_DEBUG
	// match against given pattern
	BOOL
	FMatchPattern(CExpression *pexpr) const;

	// match children against given pattern
	BOOL
	FMatchPatternChildren(CExpression *pexpr) const;

	// compare entire expression rooted here
	BOOL
	FMatchDebug(CExpression *pexpr) const;
	...
	BOOL
	FValidPlan(const CReqdPropPlan *prpp, CDrvdPropCtxtPlan *pdpctxtplan);

	// static hash function
	static ULONG
	UlHash(const CExpression *pexpr);

	// static hash function
	static ULONG
	UlHashDedup(const CExpression *pexpr);

	// rehydrate expression from a given cost context and child expressions
	static CExpression *
	PexprRehydrate(IMemoryPool *pmp, CCostContext *pcc, DrgPexpr *pdrgpexpr,
...
// map iterator
typedef CHashMapIter<ULONG, CExpression, gpos::UlHash<ULONG>,
