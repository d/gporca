...
private:
	...
	static void
	AddFactor(IMemoryPool *pmp, CExpression *pexpr, DrgPexpr *pdrgpexprFactors,
			  DrgPexpr *pdrgpexprResidual, ExprMap *pexprmap,
			  ULONG ulDisjuncts);

	// helper for building a factors map
	static ExprMap *
	PexprmapFactors(IMemoryPool *pmp, CExpression *pexpr);

	// factorize common expressions in Or tree
	static CExpression *
	PexprFactorizeDisj(IMemoryPool *pmp, CExpression *pexpr,
					   CExpression *  // pexprLowestLogicalAncestor
	);

	// visitor-like function to process descendents that are OR operators
	static CExpression *
	PexprProcessDisjDescendents(IMemoryPool *pmp, CExpression *pexpr,
								CExpression *pexprLowestLogicalAncestor,
								PexprProcessDisj pexprorfun);

	// discover common factors in scalar expression
	static CExpression *
	PexprDiscoverFactors(IMemoryPool *pmp, CExpression *pexpr);
	...
	static DrgPdrgPexpr *
	PdrgPdrgpexprDisjunctArrayForSourceId(IMemoryPool *pmp,
	...
	// or construct a new array and add it to the map
	static DrgPdrgPexpr *
	PdrgPdrgpexprDisjunctArrayForColumn(IMemoryPool *pmp,
										ColumnToArrayPosMap *pcol2array,
										BOOL fAllowNewSources, CColRef *pcr);

	// if the given expression is a table column to constant comparison,
	// record it in the map
	static void
	StoreBaseOpToColumnExpr(IMemoryPool *pmp, CExpression *pexpr,
							SourceToArrayPosMap *psrc2array,
							ColumnToArrayPosMap *pcol2array,
	...
	static void
	AddInferredFiltersFromArray(IMemoryPool *pmp,
	...
	// of the given maps
	static CExpression *
	PexprAddInferredFilters(IMemoryPool *pmp, CExpression *pexpr,
							SourceToArrayPosMap *psrc2array,
							ColumnToArrayPosMap *pcol2array);
	...
	//	expression
	static CColRefSet *
	PcrsColumnsProducedByChildren(IMemoryPool *pmp, CExpression *pexpr);
...
public:
	...
	static CExpression *
	PexprFactorize(IMemoryPool *pmp, CExpression *pexpr);

	// compute disjunctive pre-filters that can be pushed to the column creators
	static CExpression *
	PexprExtractInferredFilters(IMemoryPool *pmp, CExpression *pexpr);
};
}  // namespace gpopt
