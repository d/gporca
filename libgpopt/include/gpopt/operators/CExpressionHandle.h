...
private:
	...
	void
	DerivePlanProps(CDrvdPropCtxtPlan *pdpctxtplan);
	...
	BOOL
	FAttachedToLeafPattern() const;

	// stat derivation at root operator where handle is attached
	void
	DeriveRootStats(DrgPstat *pdrgpstatCtxt);

public:
	// ctor
	explicit CExpressionHandle(IMemoryPool *pmp);
	...
	void
	Attach(CExpression *pexpr);

	// attach handle to a given group expression
	void
	Attach(CGroupExpression *pgexpr);

	// attach handle to a given cost context
	void
	Attach(CCostContext *pcc);

	// recursive property derivation,
	void
	DeriveProps(CDrvdPropCtxt *pdpctxt);

	// recursive stats derivation
	void
	DeriveStats(DrgPstat *pdrgpstatCtxt, BOOL fComputeRootStats = true);

	// stats derivation for attached cost context
	void
	DeriveCostContextStats();

	// stats derivation using given properties and context
	void
	DeriveStats(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal,
	...
	void
	InitReqdProps(CReqdProp *prpInput);

	// compute required properties of the n-th child
	void
	ComputeChildReqdProps(ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt,
						  ULONG ulOptReq);

	// copy required properties of the n-th child
	void
	CopyChildReqdProps(ULONG ulChildIndex, CReqdProp *prp);

	// compute required columns of the n-th child
	void
	ComputeChildReqdCols(ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt);

	// required properties computation of all children
	void
	ComputeReqdProps(CReqdProp *prpInput, ULONG ulOptReq);

	// derived relational props of n-th child
	CDrvdPropRelational *
	Pdprel(ULONG ulChildIndex) const;

	// derived stats of n-th child
	IStatistics *
	Pstats(ULONG ulChildIndex) const;

	// derived plan props of n-th child
	CDrvdPropPlan *
	Pdpplan(ULONG ulChildIndex) const;

	// derived scalar props of n-th child
	CDrvdPropScalar *
	Pdpscalar(ULONG ulChildIndex) const;
	...
	BOOL
	FScalarChild(ULONG ulChildIndex) const;

	// required relational props of n-th child
	CReqdPropRelational *
	Prprel(ULONG ulChildIndex) const;

	// required plan props of n-th child
	CReqdPropPlan *
	Prpp(ULONG ulChildIndex) const;
	...
	ULONG
	UlLastNonScalarChild() const;

	// index of the first non-scalar child
	ULONG
	UlFirstNonScalarChild() const;

	// number of non-scalar children
	ULONG
	UlNonScalarChildren() const;
	...
	BOOL
	FHasOuterRefs(ULONG ulChildIndex) const
	{
