...
private:
	...
	static void
	UnnestChild(IMemoryPool *pmp, CExpression *pexpr, ULONG UlChildIndex,
				BOOL fAnd, BOOL fOr, BOOL fNotChildren, DrgPexpr *pdrgpexpr);

	// append the unnested children of given expression to given array
	static void
	AppendChildren(IMemoryPool *pmp, CExpression *pexpr, DrgPexpr *pdrgpexpr);

	// return an array of expression children after being unnested
	static DrgPexpr *
	PdrgpexprUnnestChildren(IMemoryPool *pmp, CExpression *pexpr);

	// push not expression one level down the given expression
	static CExpression *
	PexprPushNotOneLevel(IMemoryPool *pmp, CExpression *pexpr);

public:
	// remove duplicate AND/OR children
	static CExpression *
	PexprDedupChildren(IMemoryPool *pmp, CExpression *pexpr);

	// unnest AND/OR/NOT predicates
	static CExpression *
	PexprUnnest(IMemoryPool *pmp, CExpression *pexpr);
};
}  // namespace gpopt
