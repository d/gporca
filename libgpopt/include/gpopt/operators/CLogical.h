...
protected:
	...
	static CPropConstraint *
	PpcDeriveConstraintRestrict(IMemoryPool *pmp, CExpressionHandle &exprhdl,
								CColRefSet *pcrsOutput);

	// default max card for join and apply operators
	static CMaxCard
	MaxcardDef(CExpressionHandle &exprhdl);

	// compute max card given scalar child and constraint property
	static CMaxCard
	Maxcard(CExpressionHandle &exprhdl, ULONG ulScalarIndex, CMaxCard maxcard);

	// compute order spec based on an index
	static COrderSpec *
	PosFromIndex(IMemoryPool *pmp, const IMDIndex *pmdindex, DrgPcr *pdrgpcr,
				 const CTableDescriptor *ptabdesc);

	// derive function properties using data access property of scalar child
	static CFunctionProp *
	PfpDeriveFromScalar(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						ULONG ulScalarIndex);

	// derive outer references
	static CColRefSet *
	PcrsDeriveOuter(IMemoryPool *pmp, CExpressionHandle &exprhdl,
					CColRefSet *pcrsUsedAdditional);

public:
	// ctor
	explicit CLogical(IMemoryPool *pmp);
	...
	virtual CColRefSet *
	PcrsDeriveOutput(IMemoryPool *pmp, CExpressionHandle &exprhdl) = 0;
	...
	virtual CColRefSet *
	PcrsDeriveOuterIndexGet(IMemoryPool *pmp, CExpressionHandle &exprhdl);
	...
	virtual CColRefSet *
	PcrsDeriveCorrelatedApply(IMemoryPool *pmp,
	...
	// derive join depth
	virtual ULONG
	UlJoinDepth(IMemoryPool *pmp, CExpressionHandle &exprhdl) const;

	// derive partition information
	virtual CPartInfo *
	PpartinfoDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl) const = 0;

	// derive constraint property
	virtual CPropConstraint *
	PpcDeriveConstraint(IMemoryPool *pmp, CExpressionHandle &exprhdl) const = 0;
	...
	PstatsDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				 DrgPstat *pdrgpstatCtxt) const = 0;

	// promise level for stat derivation
	virtual EStatPromise
	Esp(CExpressionHandle &) const = 0;
	...
	PcrsStat(IMemoryPool *pmp, CExpressionHandle &exprhdl,
			 CColRefSet *pcrsInput, ULONG ulChildIndex) const = 0;
	...
	virtual CXformSet *
	PxfsCandidates(IMemoryPool *pmp) const = 0;
	...
	virtual BOOL FCanPullProjectionsUp(ULONG  //ulChildIndex
									   ) const
	{
		return true;
	}
	...
	static IStatistics *
	PstatsBaseTable(IMemoryPool *pmp, CExpressionHandle &exprhdl,
