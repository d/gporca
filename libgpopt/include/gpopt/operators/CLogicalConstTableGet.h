...
private:
	...
	DrgPcoldesc *
	PdrgpcoldescMapping(IMemoryPool *pmp, DrgPcr *pdrgpcr) const;

public:
	// ctors
	explicit CLogicalConstTableGet(IMemoryPool *pmp);

	CLogicalConstTableGet(IMemoryPool *pmp, DrgPcoldesc *pdrgpcoldesc,
						  DrgPdrgPdatum *pdrgpdrgpdatum);

	CLogicalConstTableGet(IMemoryPool *pmp, DrgPcr *pdrgpcrOutput,
						  DrgPdrgPdatum *pdrgpdrgpdatum);
						...
						) const
	{
		...
		return GPOS_NEW(pmp)
			CPropConstraint(pmp, GPOS_NEW(pmp) DrgPcrs(pmp), NULL /*pcnstr*/);
	}
