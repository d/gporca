...
protected:
	...
	void
	ExtractColIdsAttno(IMemoryPool *pmp, CColRefSet *pcrs,
					   DrgPul *pdrgpulColIds, DrgPul *pdrgpulPos) const;

	// derive stats from base table using filters on partition and/or index columns
	IStatistics *
	PstatsDeriveFilter(IMemoryPool *pmp, CExpressionHandle &exprhdl,
					   CExpression *pexprFilter) const;

public:
	// ctors
	explicit CLogicalDynamicGetBase(IMemoryPool *pmp);

	CLogicalDynamicGetBase(IMemoryPool *pmp, const CName *pnameAlias,
						   CTableDescriptor *ptabdesc, ULONG ulScanId,
						   DrgPcr *pdrgpcr, DrgDrgPcr *pdrgpdrgpcrPart,
						   ULONG ulSecondaryScanId, BOOL fPartial,
	...
	// set part constraint
	virtual void
	SetPartConstraint(CPartConstraint *ppartcnstr);

	// set secondary scan id
	virtual void
	SetSecondaryScanId(ULONG ulScanId);
