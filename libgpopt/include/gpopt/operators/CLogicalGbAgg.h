...
protected:
	...
	CColRefSet *
	PcrsStatGbAgg(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				  CColRefSet *pcrsInput, ULONG ulChildIndex,
				  DrgPcr *pdrgpcrGrp) const;

public:
	// ctor
	explicit CLogicalGbAgg(IMemoryPool *pmp);

	// ctor
	CLogicalGbAgg(IMemoryPool *pmp, DrgPcr *pdrgpcr,
	...
	// ctor
	CLogicalGbAgg(IMemoryPool *pmp, DrgPcr *pdrgpcr, DrgPcr *pdrgpcrMinimal,
				  COperator::EGbAggType egbaggtype);
	...
	// derive statistics
	static IStatistics *
	PstatsDerive(IMemoryPool *pmp, IStatistics *pstatsChild,
				 DrgPcr *pdrgpcrGroupingCols, DrgPul *pdrgpulComputedCols,
				 CBitSet *pbsKeys);

	// print group by aggregate type
	static IOstream &
	OsPrintGbAggType(IOstream &os, COperator::EGbAggType egbaggtype);
