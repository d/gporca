...
private:
	...
	void
	SetHasOrderSpecs(IMemoryPool *pmp);

	// set the flag indicating that SeqPrj has specified frame specs
	void
	SetHasFrameSpecs(IMemoryPool *pmp);
...
public:
	// ctor
	CLogicalSequenceProject(IMemoryPool *pmp, CDistributionSpec *pds,
							DrgPos *pdrgpos, DrgPwf *pdrgpwf);

	// ctor for pattern
	explicit CLogicalSequenceProject(IMemoryPool *pmp);
	...
	CLogicalSequenceProject *
	PopRemoveLocalOuterRefs(IMemoryPool *pmp, CExpressionHandle &exprhdl);

	// return true if outer references are included in Partition/Order, or window frame edges
	BOOL
	FHasLocalOuterRefs(CExpressionHandle &exprhdl) const;
