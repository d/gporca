...
protected:
	...
	void
	BuildColumnSets(IMemoryPool *pmp);

	// output equivalence classes
	DrgPcrs *
	PdrgpcrsOutputEquivClasses(IMemoryPool *pmp, CExpressionHandle &exprhdl,
							   BOOL fIntersect) const;

	// equivalence classes from one input child, mapped to output columns
	DrgPcrs *
	PdrgpcrsInputMapped(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						ULONG ulChild) const;

	// constraints for a given output column from all children
	DrgPcnstr *
	PdrgpcnstrColumn(IMemoryPool *pmp, CExpressionHandle &exprhdl,
					 ULONG ulColIndex, ULONG ulStart) const;

	// get constraint for a given output column from a given children
	CConstraint *
	PcnstrColumn(IMemoryPool *pmp, CExpressionHandle &exprhdl, ULONG ulColIndex,
				 ULONG ulChild) const;

	// derive constraint property for intersect and union operators
	CPropConstraint *
	PpcDeriveConstraintIntersectUnion(IMemoryPool *pmp,
									  CExpressionHandle &exprhdl,
									  BOOL fIntersect) const;

public:
	// ctor
	explicit CLogicalSetOp(IMemoryPool *pmp);

	CLogicalSetOp(IMemoryPool *pmp, DrgPcr *pdrgOutput, DrgPcr *pdrgpcrLeft,
				  DrgPcr *pdrgpcrRight);

	CLogicalSetOp(IMemoryPool *pmp, DrgPcr *pdrgpcrOutput,
