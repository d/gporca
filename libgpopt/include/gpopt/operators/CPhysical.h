...
#define GPOPT_PLAN_PROPS \
	4  // number of plan properties requested during optimization, \
...
private:
	...
	public:
		// ctor
		CReqdColsRequest(CColRefSet *pcrsRequired, ULONG ulChildIndex,
		...
		// hash function
		static ULONG
		UlHash(const CReqdColsRequest *prcr);

		// equality function
		static BOOL
		FEqual(const CReqdColsRequest *prcrFst,
			   const CReqdColsRequest *prcrSnd);

	};  // class CReqdColsRequest

	// map of incoming required columns request to computed column sets
	typedef CHashMap<CReqdColsRequest, CColRefSet, CReqdColsRequest::UlHash,
					 CReqdColsRequest::FEqual, CleanupRelease<CReqdColsRequest>,
	...
	void
	UpdateOptRequests(ULONG ulPropIndex, ULONG ulRequests);
	...
	static BOOL
	FCanPushPartReqToChild(CBitSet *pbsPartConsumer, ULONG ulChildIndex);
...
protected:
	...
	CCTEReq *
	PcterNAry(IMemoryPool *pmp, CExpressionHandle &exprhdl, CCTEReq *pcter,
			  ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt) const;
	...
	CColRefSet *
	PcrsChildReqd(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				  CColRefSet *pcrsInput, ULONG ulChildIndex,
				  ULONG ulScalarIndex);

	// compute distribution spec from the table descriptor
	static CDistributionSpec *
	PdsCompute(IMemoryPool *pmp, const CTableDescriptor *ptabdesc,
			   DrgPcr *pdrgpcrOutput);

	// helper for a simple case of computing child's required sort order
	static COrderSpec *
	PosPassThru(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				COrderSpec *posInput, ULONG ulChildIndex);

	// helper for a simple case of computing child's required distribution
	static CDistributionSpec *
	PdsPassThru(IMemoryPool *pmp, CExpressionHandle &exprhdl,
	...
	static CDistributionSpec *
	PdsMasterOnlyOrReplicated(IMemoryPool *pmp, CExpressionHandle &exprhdl,
							  CDistributionSpec *pdsInput, ULONG ulChildIndex,
							  ULONG ulOptReq);
	...
	static CDistributionSpec *
	PdsUnary(IMemoryPool *pmp, CExpressionHandle &exprhdl,
			 CDistributionSpec *pdsInput, ULONG ulChildIndex, ULONG ulOptReq);

	// helper for a simple case of computing child's required rewindability
	static CRewindabilitySpec *
	PrsPassThru(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				CRewindabilitySpec *prsRequired, ULONG ulChildIndex);

	// pass partition propagation requirement to the child
	static CPartitionPropagationSpec *
	PppsRequiredPushThru(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						 CPartitionPropagationSpec *pppsRequired,
						 ULONG ulChildIndex);

	// pass partition propagation requirement to the children of an n-ary operator
	static CPartitionPropagationSpec *
	PppsRequiredPushThruNAry(IMemoryPool *pmp, CExpressionHandle &exprhdl,
							 CPartitionPropagationSpec *pppsRequired,
							 ULONG ulChildIndex);
	...
	// operators
	static CPartitionPropagationSpec *
	PppsRequiredPushThruUnresolvedUnary(IMemoryPool *pmp,
...
public:
	...
	PcrsRequired(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				 ...
				 DrgPdp *pdrgpdpCtxt, ULONG ulOptReq) = 0;
	...
	PosRequired(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				...
				DrgPdp *pdrgpdpCtxt, ULONG ulOptReq) const = 0;
	...
	virtual BOOL
	FProvidesReqdCTEs(CExpressionHandle &exprhdl, const CCTEReq *pcter) const;
	...
	// derive sort order
	virtual COrderSpec *
	PosDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl) const = 0;

	// dderive distribution
	virtual CDistributionSpec *
	PdsDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl) const = 0;

	// derived properties: derive rewindability
	virtual CRewindabilitySpec *
	PrsDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl) const = 0;
	...
	PpfmDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl) const = 0;

	// derive cte map
	virtual CCTEMap *
	PcmDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl) const;
	...
	EpetOrder(CExpressionHandle &exprhdl, const CEnfdOrder *peo) const = 0;
	...
	Edm(CReqdPropPlan *prppInput, ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt,
		ULONG ulOptReq);

	// order matching type
	virtual CEnfdOrder::EOrderMatching
	Eom(CReqdPropPlan *prppInput, ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt,
		ULONG ulOptReq);

	// rewindability matching type
	virtual CEnfdRewindability::ERewindabilityMatching
	Erm(CReqdPropPlan *prppInput, ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt,
		ULONG ulOptReq);

	// check if optimization contexts is valid
	virtual BOOL
	FValidContext(IMemoryPool *,		   // pmp
	...
	virtual BOOL
	FPassThruStats() const = 0;

	// true iff the delivered distributions of the children are compatible among themselves
	virtual BOOL
	FCompatibleChildrenDistributions(const CExpressionHandle &exprhdl) const;
	...
	PdssMatching(IMemoryPool *pmp, CDistributionSpecSingleton *pdss);
