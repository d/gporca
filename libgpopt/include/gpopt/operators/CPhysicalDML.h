...
private:
	...
	BOOL
	FInsertSortOnParquet();

	// do we need to sort on insert
	BOOL
	FInsertSortOnRows(COptimizerConfig *poconf);

	// compute required order spec
	COrderSpec *
	PosComputeRequired(IMemoryPool *pmp, CTableDescriptor *ptabdesc);

	// compute local required columns
	void
	ComputeRequiredLocalColumns(IMemoryPool *pmp);
...
public:
	// ctor
	CPhysicalDML(IMemoryPool *pmp, CLogicalDML::EDMLOperator edmlop,
