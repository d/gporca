...
private:
	...
	CDistributionSpec *
	PdsRequiredReplicate(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						 CDistributionSpec *pdsInput, ULONG ulChildIndex,
						 DrgPdp *pdrgpdpCtxt, ULONG ulOptReq) const;

	// create (singleton, singleton) optimization request
	CDistributionSpec *
	PdsRequiredSingleton(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						 CDistributionSpec *pdsInput, ULONG ulChildIndex,
	...
	CDistributionSpecHashed *
	PdshashedPassThru(IMemoryPool *pmp, CExpressionHandle &exprhdl,
	...
	BOOL
	FNullableHashKey(ULONG ulKey, CColRefSet *pcrsNotNullInner,
					 BOOL fInner) const;

protected:
	// helper for computing a hashed distribution matching the given distribution
	CDistributionSpecHashed *
	PdshashedMatching(IMemoryPool *pmp, CDistributionSpecHashed *pdshashed,
					  ULONG ulSourceChild) const;

	// check whether the hash keys from one child are nullable
	BOOL
	FNullableHashKeys(CColRefSet *pcrsNotNullInner, BOOL fInner) const;

public:
	// ctor
	CPhysicalHashJoin(IMemoryPool *pmp, DrgPexpr *pdrgpexprOuterKeys,
	...
	PosRequired(IMemoryPool *pmp, CExpressionHandle &exprhdl,
				COrderSpec *posInput, ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt,
				ULONG ulOptReq) const;
