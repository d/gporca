...
private:
	...
	static BOOL
	FProcessingChildWithPartConsumer(BOOL fOuterPartConsumerTest,
...
protected:
	...
	public:
		// ctor
		CPartPropReq(CPartitionPropagationSpec *pppsRequired,
					 ULONG ulChildIndex, ULONG ulOuterChild, ULONG ulInnerChild,
					 ULONG ulScalarChild)
		...
		// hash function
		static ULONG
		UlHash(const CPartPropReq *pppr);

		// equality function
		static BOOL
		FEqual(const CPartPropReq *ppprFst, const CPartPropReq *ppprSnd);

	};  // class CPartPropReq

	// map partition propagation request to partition propagation spec
	typedef CHashMap<CPartPropReq, CPartitionPropagationSpec,
					 CPartPropReq::UlHash, CPartPropReq::FEqual,
	...
	// ctor
	explicit CPhysicalJoin(IMemoryPool *pmp);
	...
	BOOL
	FFirstChildToOptimize(ULONG ulChildIndex) const;

	// helper to compute required distribution of correlated join's children
	CDistributionSpec *
	PdsRequiredCorrelatedJoin(IMemoryPool *pmp, CExpressionHandle &exprhdl,
	...
	CRewindabilitySpec *
	PrsRequiredCorrelatedJoin(IMemoryPool *pmp, CExpressionHandle &exprhdl,
	...
	CPartPropReq *
	PpprCreate(IMemoryPool *pmp, CExpressionHandle &exprhdl,
			   CPartitionPropagationSpec *pppsRequired, ULONG ulChildIndex);

	// compute required partition propagation of the n-th child
	CPartitionPropagationSpec *
	PppsRequiredCompute(IMemoryPool *pmp, CExpressionHandle &exprhdl,
	...
	CPartitionPropagationSpec *
	PppsRequiredJoinChild(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						  CPartitionPropagationSpec *pppsRequired,
						  ULONG ulChildIndex, DrgPdp *pdrgpdpCtxt,
						  BOOL fNLJoin);
	...
	static COrderSpec *
	PosPropagateToOuter(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						COrderSpec *posRequired);

	// helper for checking if required sort columns come from outer child
	static BOOL
	FSortColsInOuterChild(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						  COrderSpec *pos);

	// helper for checking if the outer input of a binary join operator
	// includes the required columns
	static BOOL
	FOuterProvidesReqdCols(CExpressionHandle &exprhdl,
						   CColRefSet *pcrsRequired);

	// helper for adding a pair of hash join keys to given arrays
	static void
	AddHashKeys(CExpression *pexprPred, CExpression *pexprOuter,
				CExpression *pexprInner, DrgPexpr *pdrgpexprOuter,
				DrgPexpr *pdrgpexprInner);

	// helper to add filter on part key
	static void
	AddFilterOnPartKey(IMemoryPool *pmp, BOOL fNLJoin, CExpression *pexprScalar,
					   CPartIndexMap *ppimSource, CPartFilterMap *ppfmSource,
					   ULONG ulChildIndex, ULONG ulPartIndexId,
					   BOOL fOuterPartConsumer, CPartIndexMap *ppimResult,
	...
	static CExpression *
	PexprJoinPredOnPartKeys(IMemoryPool *pmp, CExpression *pexprScalar,
							CPartIndexMap *ppimSource, ULONG ulPartIndexId,
							CColRefSet *pcrsAllowedRefs);

	// are the given predicate parts hash-join compatible?
	static BOOL
	FHashJoinCompatible(CExpression *pexprOuter, CExpression *pexprInner,
...
public:
	...
	static BOOL
	FHashJoinCompatible(CExpression *pexprPred, CExpression *pexprOuter,
						CExpression *pexprInner);

	// extract expressions that can be used in hash-join from the given predicate
	static void
	ExtractHashJoinExpressions(CExpression *pexprPred, CExpression **ppexprLeft,
	...
	FHashJoinPossible(
		...
		DrgPexpr *pdrgpexprInner,
		CExpression **
			ppexprResult  // output: join expression to be tarnsformed to hash join
	);

	// return number of distribution requests for correlated join
	static ULONG
	UlDistrRequestsForCorrelatedJoin();
