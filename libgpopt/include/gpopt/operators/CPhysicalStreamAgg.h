...
private:
	...
	COrderSpec *
	PosCovering(IMemoryPool *pmp, COrderSpec *posRequired,
				DrgPcr *pdrgpcrGrp) const;

protected:
	// compute required sort columns of the n-th child
	COrderSpec *
	PosRequiredStreamAgg(IMemoryPool *pmp, CExpressionHandle &exprhdl,
						 COrderSpec *posRequired, ULONG ulChildIndex,
						 DrgPcr *pdrgpcrGrp) const;

	// initialize the order spec using the given array of columns
	void
	InitOrderSpec(IMemoryPool *pmp, DrgPcr *pdrgpcrOrder);
