...
private:
	...
	static void
	CollectConjuncts(CExpression *pexpr, DrgPexpr *pdrgpexpr);

	// collect disjuncts recursively
	static void
	CollectDisjuncts(CExpression *pexpr, DrgPexpr *pdrgpexpr);

	// check if a conjunct/disjunct can be skipped
	static BOOL
	FSkippable(CExpression *pexpr, BOOL fConjunction);

	// check if a conjunction/disjunction can be reduced to a constant True/False
	static BOOL
	FReducible(CExpression *pexpr, BOOL fConjunction);

	// check if given expression is a comparison over the given column
	static BOOL
	FComparison(CExpression *pexpr, CColRef *pcr, CColRefSet *pcrsAllowedRefs);
	...
	static BOOL
	FValidRefsOnly(CExpression *pexprScalar, CColRefSet *pcrsAllowedRefs);

	// determine which predicates we should test implication for
	static BOOL
	FCheckPredicateImplication(CExpression *pexprPred);

	// check if predicate is implied by given equivalence classes
	static BOOL
	FImpliedPredicate(CExpression *pexprPred, DrgPcrs *pdrgpcrsEquivClasses);

	// helper to create index lookup comparison predicate with index key on left side
	static CExpression *
	PexprIndexLookupKeyOnLeft(IMemoryPool *pmp, CMDAccessor *pmda,
							  CExpression *pexprScalar,
							  const IMDIndex *pmdindex, DrgPcr *pdrgpcrIndex,
							  CColRefSet *pcrsOuterRefs);

	// helper to create index lookup comparison predicate with index key on right side
	static CExpression *
	PexprIndexLookupKeyOnRight(IMemoryPool *pmp, CMDAccessor *pmda,
							   CExpression *pexprScalar,
							   const IMDIndex *pmdindex, DrgPcr *pdrgpcrIndex,
							   CColRefSet *pcrsOuterRefs);
	...
	static CExpression *
	PexprReplaceColsWithNulls(IMemoryPool *pmp, CExpression *pexprScalar,
							  CColRefSet *pcrs);
...
public:
	...
	// does the given expression have any NOT children?
	static BOOL
	FHasNegatedChild(CExpression *pexpr);

	// is the given expression an inner join
	static BOOL
	FInnerJoin(CExpression *pexpr)
	{
	...
	static void
	CollectChildren(CExpression *pexpr, DrgPexpr *pdrgpexprLogical,
	...
	static BOOL
	FCollapsibleChildUnionUnionAll(CExpression *pexprParent,
								   ULONG ulChildIndex);
	...
	static void
	CollectGrandChildrenUnionUnionAll(IMemoryPool *pmp, CExpression *pexpr,
									  ULONG ulChildIndex,
	...
	static DrgPexpr *
	PdrgpexprAppendConjunctsDedup(IMemoryPool *pmp, DrgPexpr *pdrgpexpr,
								  CExpression *pexpr);

	// extract interesting conditions involving the partitioning keys
	static CExpression *
	PexprExtractPredicatesOnPartKeys(IMemoryPool *pmp, CExpression *pexprScalar,
	...
	// scalar expression
	static CExpression *
	PexprPredicateCol(IMemoryPool *pmp, CConstraint *pcnstr, CColRef *pcr,
					  BOOL fUseConstraints);

	// extract components of a comparison expression on the given key
	static void
	ExtractComponents(CExpression *pexprScCmp, CColRef *pcrKey,
					  CExpression **ppexprKey, CExpression **ppexprOther,
					  IMDType::ECmpType *pecmpt);

	// Expression is a comparison with a simple identifer on at least one side
	static BOOL
	FIdentCompare(CExpression *pexpr, IMDType::ECmpType pecmpt, CColRef *pcr);

	// check if given column is a constant
	static BOOL
	FConstColumn(CConstraint *pcnstr, const CColRef *pcr);

	// check if given column is a disjunction of constants
	static BOOL
	FColumnDisjunctionOfConst(CConstraint *pcnstr, const CColRef *pcr);

	// check if given column is a disjunction of constants
	static BOOL
	FColumnDisjunctionOfConst(CConstraintInterval *pcnstr, const CColRef *pcr);

	// split predicates into those that refer to an index key, and those that don't
	static void
	ExtractIndexPredicates(
	...
	static CExpression *
	PexprInverseComparison(IMemoryPool *pmp, CExpression *pexprCmp);

	// prune unnecessary equality operations
	static CExpression *
	PexprPruneSuperfluosEquality(IMemoryPool *pmp, CExpression *pexpr);

	// remove conjuncts that are implied based on child columns equivalence classes
	static CExpression *
	PexprRemoveImpliedConjuncts(IMemoryPool *pmp, CExpression *pexprScalar,
								CExpressionHandle &exprhdl);

	//	check if given correlations are valid for semi join operator;
	static BOOL
	FValidSemiJoinCorrelations(IMemoryPool *pmp, CExpression *pexprOuter,
	...
	static BOOL
	FSimpleEqualityUsingCols(IMemoryPool *pmp, CExpression *pexprScalar,
							 CColRefSet *pcrs);

	// check if given expression is a valid index lookup predicate
	// return (modified) predicate suited for index lookup
	static CExpression *
	PexprIndexLookup(IMemoryPool *pmp, CMDAccessor *pmda, CExpression *pexpPred,
					 const IMDIndex *pmdindex, DrgPcr *pdrgpcrIndex,
					 CColRefSet *pcrsOuterRefs);

	// split given scalar expression into two conjunctions; without and with outer references
	static void
	SeparateOuterRefs(IMemoryPool *pmp, CExpression *pexprScalar,
					  CColRefSet *pcrsOuter, CExpression **ppexprLocal,
					  CExpression **ppexprOuterRef);

	// is the condition a LIKE predicate
	static BOOL
	FLikePredicate(IMDId *pmdid);

	// is the condition a LIKE predicate
	static BOOL
	FLikePredicate(CExpression *pexprPred);

	// extract the components of a LIKE predicate
	static void
	ExtractLikePredComponents(CExpression *pexprPred,
	...
	static BOOL
	FNullRejecting(IMemoryPool *pmp, CExpression *pexprScalar,
				   CColRefSet *pcrs);

	// returns true iff the given predicate pexprPred is compatible with the given index pmdindex
	static BOOL
	FCompatibleIndexPredicate(CExpression *pexprPred, const IMDIndex *pmdindex,
	...
	static BOOL
	FCompatiblePredicates(DrgPexpr *pdrgpexprPred, const IMDIndex *pmdindex,
						  DrgPcr *pdrgpcrIndex, CMDAccessor *pmda);

	// check if the expensive CNF conversion is beneficial in finding predicate for hash join
	static BOOL
	FConvertToCNF(CExpression *pexpr, CExpression *pexprOuter,
				  CExpression *pexprInner);
