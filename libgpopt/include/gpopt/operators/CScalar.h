...
private:
	...
	static CPartInfo *
	PpartinfoDeriveCombineScalar(IMemoryPool *pmp, CExpressionHandle &exprhdl);

protected:
	// perform conjunction of child boolean evaluation results
	static EBoolEvalResult
	EberConjunction(DrgPul *pdrgpulChildren);

	// perform disjunction of child boolean evaluation results
	static EBoolEvalResult
	EberDisjunction(DrgPul *pdrgpulChildren);

	// return Null if any child is Null
	static EBoolEvalResult
	EberNullOnAnyNullChild(DrgPul *pdrgpulChildren);

	// return Null if all children are Null
	static EBoolEvalResult
	EberNullOnAllNullChildren(DrgPul *pdrgpulChildren);

public:
	// ctor
	explicit CScalar(IMemoryPool *pmp) : COperator(pmp)
	{
	}
	...
	virtual CColRefSet *
	PcrsSetReturningFunction(IMemoryPool *pmp,
							 CExpressionHandle &  // exprhdl
	)
	{
		// return an empty set of column refs
	...
	{
		return PfpDeriveFromChildren(pmp, exprhdl,
	...
	virtual BOOL
	FHasSubquery(CExpressionHandle &exprhdl);

	// derive non-scalar function existence
	virtual BOOL
	FHasNonScalarFunction(CExpressionHandle &exprhdl);

	// boolean expression evaluation
	virtual EBoolEvalResult
	Eber(DrgPul *  // pdrgpulChildren
		 ) const
	{
	...
	static EBoolEvalResult
	EberEvaluate(IMemoryPool *pmp, CExpression *pexprScalar);
