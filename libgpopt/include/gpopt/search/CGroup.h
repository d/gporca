...
private:
	...
	public:
		// ctor
		SContextLink(CCostContext *pccParent, ULONG ulChildIndex,
		...
		// hash function
		static ULONG
		UlHash(const SContextLink *pclink);

		// equality function
		static BOOL
		FEqual(const SContextLink *pclink1, const SContextLink *pclink2);

	};  // struct SContextLink

	// map of processed links in TreeMap structure
	typedef CHashMap<SContextLink, BOOL, SContextLink::UlHash,
					 SContextLink::FEqual, CleanupDelete<SContextLink>,
					 CleanupDelete<BOOL> >
		LinkMap;
	...
	// set hash join keys
	void
	SetHashJoinKeys(DrgPexpr *pdrgpexprOuter, DrgPexpr *pdrgpexprInner);

	// insert new group expression
	void
	Insert(CGroupExpression *pgexpr);

	// move duplicate group expression to duplicates list
	void
	MoveDuplicateGExpr(CGroupExpression *pgexpr);

	// initialize group's properties
	void
	InitProperties(CDrvdProp *pdp);

	// initialize group's stats
	void
	InitStats(IStatistics *pstats);
	...
	CGroupExpression *
	PgexprNext(CGroupExpression *pgexpr);

	// return true if first promise is better than second promise
	BOOL
	FBetterPromise(IMemoryPool *pmp, CLogical::EStatPromise espFst,
				   CGroupExpression *pgexprFst, CLogical::EStatPromise espSnd,
				   CGroupExpression *pgexprSnd) const;

	// derive stats recursively on child groups
	CLogical::EStatPromise
	EspDerive(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal,
	...
	RecursiveBuildTreeMap(
		...
		CTreeMap<CCostContext, CExpression, CDrvdPropCtxtPlan,
				 CCostContext::UlHash, CCostContext::FEqual> *ptmap);

	// print scalar group properties
	IOstream &
	OsPrintGrpScalarProps(IOstream &os, const CHAR *szPrefix);

	// print group properties
	IOstream &
	OsPrintGrpProps(IOstream &os, const CHAR *szPrefix);

	// print group optimization contexts
	IOstream &
	OsPrintGrpOptCtxts(IOstream &os, const CHAR *szPrefix);

	// initialize and return empty stats for this group
	IStatistics *
	PstatsInitEmpty(IMemoryPool *pmpGlobal);

	// find the group expression having the best stats promise
	CGroupExpression *
	PgexprBestPromise(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal,
...
public:
	...
	DrgPexpr *
	PdrgpexprHashJoinKeysOuter() const
	{
	...
	DrgPexpr *
	PdrgpexprHashJoinKeysInner() const
	{
	...
	BOOL
	FStatsDerivable(IMemoryPool *pmp);
	...
	PocLookup(IMemoryPool *pmp, CReqdPropPlan *prpp, ULONG ulSearchStageIndex);

	// lookup the best context across all stages for the given required properties
	COptimizationContext *
	PocLookupBest(IMemoryPool *pmp, ULONG ulSearchStages, CReqdPropPlan *prpp);

	// insert given context into contexts hash table
	COptimizationContext *
	PocInsert(COptimizationContext *poc);

	// update the best group cost under the given optimization context
	void
	UpdateBestCost(COptimizationContext *poc, CCostContext *pcc);

	// lookup best expression under given optimization context
	CGroupExpression *
	PgexprBest(COptimizationContext *poc);
	...
	IStatistics *
	PstatsRecursiveDerive(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal,
						  CReqdPropRelational *prprel, DrgPstat *pdrgpstatCtxt);

	// find group expression with best stats promise and the same given children
	CGroupExpression *
	PgexprBestPromise(IMemoryPool *pmp, CGroupExpression *pgexprToMatch);

	// link parent group expression to group members
	void
	BuildTreeMap(IMemoryPool *pmp, COptimizationContext *poc,
				 CCostContext *pccParent, ULONG ulChildIndex,
				 CTreeMap<CCostContext, CExpression, CDrvdPropCtxtPlan,
						  CCostContext::UlHash, CCostContext::FEqual> *ptmap);
	...
	CGroupExpression *
	PgexprAnyCTEConsumer();

	// compute stats during costing
	IStatistics *
	PstatsCompute(COptimizationContext *poc, CExpressionHandle &exprhdl,
				  CGroupExpression *pgexpr);

	// compute cost lower bound for the plan satisfying given required properties
	CCost
	CostLowerBound(IMemoryPool *pmp, CReqdPropPlan *prppInput);

	// matching of pairs of arrays of groups
	static BOOL
	FMatchGroups(DrgPgroup *pdrgpgroupFst, DrgPgroup *pdrgpgroupSnd);

	// matching of pairs of arrays of groups while skipping scalar groups
	static BOOL
	FMatchNonScalarGroups(DrgPgroup *pdrgpgroupFst, DrgPgroup *pdrgpgroupSnd);

	// determine if a pair of groups are duplicates
	static BOOL
	FDuplicateGroups(CGroup *pgroupFst, CGroup *pgroupSnd);
