...
private:
	...
	typedef CHashMap<CPartialPlan, CCost, CPartialPlan::UlHash,
					 CPartialPlan::FEqual, CleanupRelease<CPartialPlan>,
					 CleanupDelete<CCost> >
		PartialPlanCostMap;
	...
	void
	PrintXform(IMemoryPool *pmp, CXform *pxform, CExpression *pexpr,
			   CXformResult *pxfres);

	// preprocessing before applying transformation
	void
	PreprocessTransform(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal,
						CXform *pxform);

	// postprocessing after applying transformation
	void
	PostprocessTransform(IMemoryPool *pmpLocal, IMemoryPool *pmpGlobal,
						 CXform *pxform);

	// costing scheme
	CCost
	CostCompute(IMemoryPool *pmp, CCostContext *pcc) const;
	...
	BOOL
	FValidContext(IMemoryPool *pmp, COptimizationContext *poc,
				  DrgPoc *pdrgpocChild);

	// remove cost context in hash table
	CCostContext *
	PccRemove(COptimizationContext *poc, ULONG ulOptReq);
	...
	CCostContext *
	PccInsertBest(CCostContext *pcc);

	// print group expression cost contexts
	IOstream &
	OsPrintCostContexts(IOstream &os, const CHAR *szPrefix);
...
public:
	// ctor
	CGroupExpression(IMemoryPool *pmp, COperator *pop, DrgPgroup *pdrgpgroup,
	...
	BOOL
	FCostContextExists(COptimizationContext *poc, DrgPoc *pdrgpoc);

	// compute and store expression's cost under a given context
	CCostContext *
	PccComputeCost(IMemoryPool *pmp, COptimizationContext *poc, ULONG ulOptReq,
				   DrgPoc *pdrgpoc, BOOL fPruned, CCost costLowerBound);
	...
	void
	Init(CGroup *pgroup, ULONG ulId);

	// reset group expression
	void
	Reset(CGroup *pgroup, ULONG ulId)
	{
		m_pgroup = pgroup;
		m_ulId = ulId;
	}
	...
	static BOOL
	FEqual(const CGroupExpression &gexprLeft,
	...
	BOOL
	FMatch(const CGroupExpression *) const;

	// match non-scalar children of group expression against given children of passed expression
	BOOL
	FMatchNonScalarChildren(const CGroupExpression *pgexpr) const;
	...
	static ULONG
	UlHash(COperator *pop, DrgPgroup *drgpgroup);

	// static hash function for group expression
	static ULONG
	UlHash(const CGroupExpression &);

	// transform group expression
	void
	Transform(IMemoryPool *pmp, IMemoryPool *pmpLocal, CXform *pxform,
	...
	BOOL
	FTransitioned(EState estate) const;
	...
	CCostContext *
	PccLookup(COptimizationContext *poc, ULONG ulOptReq);

	// lookup all cost contexts matching given optimization context
	DrgPcc *
	PdrgpccLookupAll(IMemoryPool *pmp, COptimizationContext *poc);

	// insert a cost context in hash table
	CCostContext *
	PccInsert(CCostContext *pcc);
	...
	// print driver
	IOstream &
	OsPrint(IOstream &os, const CHAR * = "");
