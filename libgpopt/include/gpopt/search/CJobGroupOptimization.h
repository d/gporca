...
private:
	...
	static EEvent
	EevtStartOptimization(CSchedulerContext *psc, CJob *pj);
	...
	static EEvent
	EevtCompleteOptimization(CSchedulerContext *psc, CJob *pj);
...
public:
	...
	// initialize job
	void
	Init(CGroup *pgroup, CGroupExpression *pgexprOrigin,
	...
	virtual BOOL
	FScheduleGroupExpressions(CSchedulerContext *psc);

	// schedule a new group optimization job
	static void
	ScheduleJob(CSchedulerContext *psc, CGroup *pgroup,
				CGroupExpression *pgexprOrigin, COptimizationContext *poc,
				CJob *pjParent);
