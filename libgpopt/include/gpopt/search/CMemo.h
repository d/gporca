...
typedef CTreeMap<CCostContext, CExpression, CDrvdPropCtxtPlan,
				 CCostContext::UlHash, CCostContext::FEqual>
	MemoTreeMap;
...
private:
	...
	// add new group
	void
	Add(CGroup *pgroup, CExpression *pexprOrigin);
	...
	CGroup *
	PgroupInsert(CGroup *pgroupTarget, CGroupExpression *pgexpr,
				 CExpression *pexprOrigin, BOOL fNewGroup);

	// helper to check if a new group needs to be created
	BOOL
	FNewGroup(CGroup **ppgroupTarget, CGroupExpression *pgexpr, BOOL fScalar);
...
public:
	// ctor
	explicit CMemo(IMemoryPool *pmp);
	...
	void
	MarkDuplicates(CGroup *pgroupFst, CGroup *pgroupSnd);
	...
	// set root group
	void
	SetRoot(CGroup *pgroup);

	// insert group expression into hash table
	CGroup *
	PgroupInsert(CGroup *pgroupTarget, CExpression *pexprOrigin,
				 CGroupExpression *pgexpr);

	// extract a plan that delivers the given required properties
	CExpression *
	PexprExtractPlan(IMemoryPool *pmp, CGroup *pgroupRoot,
	...
	void
	DeriveStatsIfAbsent(IMemoryPool *pmp);

	// build tree map
	void
	BuildTreeMap(COptimizationContext *poc);
