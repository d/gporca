...
private:
	...
	void
	ProcessJobs(CSchedulerContext *psc);

	// keep executing waiting jobs (if any)
	void
	ExecuteJobs(CSchedulerContext *psc);

	// process job execution results
	void
	ProcessJobResult(CJob *pj, CSchedulerContext *psc, BOOL fCompleted);
	...
	// execute job
	BOOL
	FExecute(CJob *pj, CSchedulerContext *psc);

	// process job execution outcome
	EJobResult
	EjrPostExecute(CJob *pj, BOOL fCompleted);

	// resume parent job
	void
	ResumeParent(CJob *pj);
	...
	BOOL
	FIncreaseWorkers() const
	{
		GPOS_ASSERT(m_ulpTasksMax >= m_ulpRunning);
		return (m_ulpTasksMax > m_ulpTasksActive &&
				(OPT_SCHED_QUEUED_RUNNING_RATIO <
				 m_ulpQueued / (m_ulpTasksActive + 1)));
	}
...
public:
	// ctor
	CScheduler(IMemoryPool *pmp, ULONG ulJobs, ULONG_PTR ulpTasks
#ifdef GPOS_DEBUG
			   ,
