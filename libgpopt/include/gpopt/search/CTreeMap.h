...
//
//---------------------------------------------------------------------------
template <class T, class R, class U, ULONG (*pfnHash)(const T *),
...
private:
	...
	public:
		// ctor
		STreeLink(const T *ptParent, ULONG ulChildIndex, const T *ptChild)
			: m_ptParent(ptParent),
		...
		// hash function
		static ULONG
		UlHash(const STreeLink *ptlink)
		{
			ULONG ulHashParent = pfnHash(ptlink->m_ptParent);
			ULONG ulHashChild = pfnHash(ptlink->m_ptChild);
			ULONG ulHashChildIndex =
				gpos::UlHash<ULONG>(&ptlink->m_ulChildIndex);

			return UlCombineHashes(
				ulHashParent, UlCombineHashes(ulHashChild, ulHashChildIndex));
		}

		// equality function
		static BOOL
		FEqual(const STreeLink *ptlink1, const STreeLink *ptlink2)
		{
	...
	private:
		...
		// rehydrate tree
		R *
		PrUnrank(IMemoryPool *pmp, PrFn prfn, U *pU, ULONG ulChild,
				 ULLONG ullRank)
		{
			GPOS_CHECK_STACK_SIZE;
	...
	public:
		...
		R *
		PrUnrank(IMemoryPool *pmp, PrFn prfn, U *pU, ULLONG ullRank)
		{
			GPOS_CHECK_STACK_SIZE;
			...
			else
			{
				...
				for (ULONG ulChild = 0; ulChild < ulChildren; ulChild++)
				{
					...
					pdrg->Append(
						PrUnrank(pmp, prfn, pU, ulChild, ullLocalRank));
	...
	// map of all nodes
	typedef gpos::CHashMap<T, CTreeNode, pfnHash, pfnEq, CleanupNULL,
						   CleanupDelete<CTreeNode> >
		TMap;
	typedef gpos::CHashMapIter<T, CTreeNode, pfnHash, pfnEq, CleanupNULL,
							   CleanupDelete<CTreeNode> >
		TMapIter;
...
public:
		  ...
		  m_plinkmap(NULL)
	{
		...
		m_ptnRoot =
			GPOS_NEW(pmp) CTreeNode(pmp, 0 /* ulCounter */, NULL /* pt */);
	}
	...
	R *
	PrUnrank(IMemoryPool *pmp, U *pU, ULLONG ullRank) const
	{
