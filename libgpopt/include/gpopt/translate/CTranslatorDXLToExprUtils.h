...
public:
	...
	static CExpression *
	PexprConstInt8(IMemoryPool *pmp, CMDAccessor *pmda, CSystemId sysid,
				   LINT liValue);

	// create a scalar const operator from a DXL scalar const operator
	static CScalarConst *
	PopConst(IMemoryPool *pmp, CMDAccessor *pmda,
			 const CDXLScalarConstValue *pdxlop);

	// create a datum from a DXL scalar const operator
	static IDatum *
	Pdatum(CMDAccessor *pmda, const CDXLScalarConstValue *pdxlop);

	// create a datum array from a dxl datum array
	static DrgPdatum *
	Pdrgpdatum(IMemoryPool *pmp, CMDAccessor *pmda,
			   const DrgPdxldatum *pdrgpdatum);

	// update table descriptor's key sets info from the MD cache object
	static void
	AddKeySets(IMemoryPool *pmp, CTableDescriptor *ptabdesc,
			   const IMDRelation *pmdrel, HMUlUl *phmululColMapping);

	// check if a dxl node is a boolean expression of the given type
	static BOOL
	FScalarBool(const CDXLNode *pdxln, EdxlBoolExprType edxlboolexprtype);

	// returns the equivalent bool expression type in the optimizer for
	// a given DXL bool expression type
	static CScalarBoolOp::EBoolOperator
	EBoolOperator(EdxlBoolExprType edxlbooltype);

	// construct a dynamic array of col refs corresponding to the given col ids
	static DrgPcr *
	Pdrgpcr(IMemoryPool *pmp, HMUlCr *phmulcr, const DrgPul *pdrgpulColIds);

	// is the given expression is a scalar function that casts
	static BOOL
	FCastFunc(CMDAccessor *pmda, const CDXLNode *pdxln, IMDId *pmdidInput);
};
}  // namespace gpopt
