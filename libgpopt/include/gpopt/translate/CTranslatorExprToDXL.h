...
typedef CHashMap<CColRef, CDXLNode, gpos::UlHash<CColRef>,
				 gpos::FEqual<CColRef>, CleanupNULL<CColRef>,
...
private:
	...
	typedef CDXLNode *(CTranslatorExprToDXL::*PfPdxlnPhysical)(
		CExpression *pexpr, DrgPcr *pdrgpcr,
		DrgPds *
	...
	EdxlBoolExprType
	Edxlbooltype(const CScalarBoolOp::EBoolOperator eboolop) const;

	// return the EdxlDmlType for a given DML op type
	EdxlDmlType
	Edxldmloptype(const CLogicalDML::EDMLOperator edmlop) const;

	// return outer refs in correlated join inner child
	CColRefSet *
	PcrsOuterRefsForCorrelatedNLJoin(CExpression *pexpr) const;

	// functions translating different optimizer expressions into their
	// DXL counterparts

	CDXLNode *
	PdxlnTblScan(CExpression *pexprTblScan, CColRefSet *pcrsOutput,
	...
	CDXLNode *
	PdxlnIndexScanWithInlinedCondition(CExpression *pexprIndexScan,
									   CExpression *pexprScalarCond,
									   CDXLPhysicalProperties *pdxlprop,
									   DrgPcr *pdrgpcr,
	...
	CDXLNode *
	PdxlnIndexScan(CExpression *pexprIndexScan, DrgPcr *pdrgpcr,
				   CDXLPhysicalProperties *pdxlprop, CReqdPropPlan *prpp);

	// translate index scan
	CDXLNode *
	PdxlnIndexScan(CExpression *pexprIndexScan, DrgPcr *pdrgpcr,
				   DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				   BOOL *pfDML);

	// translate a bitmap index probe expression to DXL
	CDXLNode *
	PdxlnBitmapIndexProbe(CExpression *pexprBitmapIndexProbe);

	// translate a bitmap bool op expression to DXL
	CDXLNode *
	PdxlnBitmapBoolOp(CExpression *pexprBitmapBoolOp);

	// translate a bitmap table scan expression to DXL
	CDXLNode *
	PdxlnBitmapTableScan(CExpression *pexprBitmapTableScan, DrgPcr *pdrgpcr,
						 DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
						 BOOL *pfDML);

	// translate a bitmap table scan expression to DXL
	CDXLNode *
	PdxlnBitmapTableScan(CExpression *pexprBitmapTableScan,
						 CColRefSet *pcrsOutput, DrgPcr *pdrgpcr,
						 DrgPds *pdrgpdsBaseTables, CExpression *pexprScalar,
						 CDXLPhysicalProperties *pdxlprop);

	// translate a partition selector into DXL while inlining the given condition in the child
	CDXLNode *
	PdxlnPartitionSelectorWithInlinedCondition(CExpression *pexprFilter,
											   DrgPcr *pdrgpcr,
											   DrgPds *pdrgpdsBaseTables,
	...
	CDXLNode *
	PdxlnResultFromFilter(CExpression *pexprFilter, DrgPcr *pdrgpcr,
						  DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
						  BOOL *pfDML);

	CDXLNode *
	PdxlnResult(CExpression *pexprFilter, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				BOOL *pfDML);

	// create a DXL result node for the given project list
	CDXLNode *
	PdxlnResult(CDXLNode *pdxlnProjList, CExpression *pexprFilter,
				DrgPcr *pdrgpcr, DrgPds *pdrgpdsBaseTables,
	...
	CDXLNode *
	PdxlnAddScalarFilterOnRelationalChild(CDXLNode *pdxlnRelationalChild,
	...
	PdxlnResult(CExpression *pexprFilter, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				BOOL *pfDML, CDXLPhysicalProperties *pdxlprop);

	CDXLNode *
	PdxlnResult(CExpression *pexprRelational, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
	...
	CDXLNode *
	PdxlnComputeScalar(CExpression *pexprComputeScalar, DrgPcr *pdrgpcr,
					   DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
					   BOOL *pfDML);

	CDXLNode *
	PdxlnAggregate(CExpression *pexprHashAgg, DrgPcr *pdrgpcr,
				   DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				   BOOL *pfDML);

	CDXLNode *
	PdxlnAggregateDedup(CExpression *pexprAgg, DrgPcr *pdrgpcr,
						DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
						BOOL *pfDML);

	CDXLNode *
	PdxlnAggregate(CExpression *pexprAgg, DrgPcr *pdrgpcr,
				   DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				   BOOL *pfDML, EdxlAggStrategy edxlaggstr,
				   const DrgPcr *pdrgpcrGroupingCols, CColRefSet *pcrsKeys);

	CDXLNode *
	PdxlnSort(CExpression *pexprSort, DrgPcr *pdrgpcr,
			  DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
			  BOOL *pfDML);

	CDXLNode *
	PdxlnLimit(CExpression *pexprLimit, DrgPcr *pdrgpcr,
			   DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
			   BOOL *pfDML);

	CDXLNode *
	PdxlnWindow(CExpression *pexprSeqPrj, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				BOOL *pfDML);

	CDXLNode *
	PdxlnNLJoin(CExpression *pexprNLJ, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				BOOL *pfDML);

	CDXLNode *
	PdxlnHashJoin(CExpression *pexprHJ, DrgPcr *pdrgpcr,
				  DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				  BOOL *pfDML);

	CDXLNode *
	PdxlnCorrelatedNLJoin(CExpression *pexprNLJ, DrgPcr *pdrgpcr,
						  DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
						  BOOL *pfDML);

	CDXLNode *
	PdxlnCTEProducer(CExpression *pexprCTEProducer, DrgPcr *pdrgpcr,
					 DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
					 BOOL *pfDML);

	CDXLNode *
	PdxlnCTEConsumer(CExpression *pexprCTEConsumer, DrgPcr *pdrgpcr,
					 DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
					 BOOL *pfDML);

	// store outer references in index NLJ inner child into global map
	void
	StoreIndexNLJOuterRefs(CPhysical *pop);

	// build a scalar DXL subplan node
	void
	BuildDxlnSubPlan(CDXLNode *pdxlnRelChild, const CColRef *pcr,
					 DrgPdxlcr *pdrgdxlcr);

	// build a boolean scalar dxl node with a subplan as its child
	CDXLNode *
	PdxlnBooleanScalarWithSubPlan(CDXLNode *pdxlnRelChild,
								  DrgPdxlcr *pdrgdxlcr);

	CDXLNode *
	PdxlnScBoolExpr(EdxlBoolExprType boolexptype, CDXLNode *pdxlnLeft,
					CDXLNode *pdxlnRight);

	CDXLNode *
	PdxlnTblScanFromNLJoinOuter(CExpression *pexprRelational,
								CDXLNode *pdxlnScalar, DrgPcr *pdrgpcr,
								DrgPds *pdrgpdsBaseTables,
								ULONG *pulNonGatherMotions,
								CDXLPhysicalProperties *pdxlprop);

	CDXLNode *
	PdxlnResultFromNLJoinOuter(CExpression *pexprRelational,
							   CDXLNode *pdxlnScalar, DrgPcr *pdrgpcr,
							   DrgPds *pdrgpdsBaseTables,
							   ULONG *pulNonGatherMotions, BOOL *pfDML,
							   CDXLPhysicalProperties *pdxlprop);
	...
	CDXLNode *
	PdxlnMotion(CExpression *pexprMotion, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				BOOL *pfDML);

	CDXLNode *
	PdxlnMaterialize(CExpression *pexprSpool, DrgPcr *pdrgpcr,
					 DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
					 BOOL *pfDML);

	// translate a sequence expression
	CDXLNode *
	PdxlnSequence(CExpression *pexprSequence, DrgPcr *pdrgpcr,
				  DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				  BOOL *pfDML);

	// translate a dynamic table scan
	CDXLNode *
	PdxlnDynamicTableScan(CExpression *pexprDTS, DrgPcr *pdrgpcr,
						  DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
						  BOOL *pfDML);
	...
	CDXLNode *
	PdxlnDynamicBitmapTableScan(CExpression *pexprDynamicBitmapTableScan,
								DrgPcr *pdrgpcr, DrgPds *pdrgpdsBaseTables,
								ULONG *pulNonGatherMotions, BOOL *pfDML);
	...
	CDXLNode *
	PdxlnDynamicIndexScan(CExpression *pexprDIS, DrgPcr *pdrgpcr,
						  CDXLPhysicalProperties *pdxlprop,
						  CReqdPropPlan *prpp);
	...
	CDXLNode *
	PdxlnResultFromConstTableGet(CExpression *pexprCTG, DrgPcr *pdrgpcr,
								 DrgPds *pdrgpdsBaseTables,
								 ULONG *pulNonGatherMotions, BOOL *pfDML);
	...
	CDXLNode *
	PdxlnTVF(CExpression *pexprTVF, DrgPcr *pdrgpcr, DrgPds *pdrgpdsBaseTables,
			 ULONG *pulNonGatherMotions, BOOL *pfDML);

	// translate an union all op
	CDXLNode *
	PdxlnAppend(CExpression *pexprUnionAll, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				BOOL *pfDML);

	// translate a partition selector
	CDXLNode *
	PdxlnPartitionSelector(CExpression *pexpr, DrgPcr *pdrgpcr,
						   DrgPds *pdrgpdsBaseTables,
						   ULONG *pulNonGatherMotions, BOOL *pfDML);
	...
	CDXLNode *
	PdxlnPartitionSelectorDML(CExpression *pexpr, DrgPcr *pdrgpc,
							  DrgPds *pdrgpdsBaseTables,
							  ULONG *pulNonGatherMotions, BOOL *pfDMLr);

	// translate an expansion-based partition selector with a scalar condition to inline
	CDXLNode *
	PdxlnPartitionSelectorExpand(CExpression *pexpr, DrgPcr *pdrgpcr,
								 DrgPds *pdrgpdsBaseTables,
								 ULONG *pulNonGatherMotions, BOOL *pfDML,
								 CExpression *pexprScalarCond,
								 CDXLPhysicalProperties *pdxlprop);
	...
	CDXLNode *
	PdxlnPartFilterList(CExpression *pexpr, BOOL fEqFilters);
	...
	BOOL
	FEqPartFiltersAllLevels(CExpression *pexpr, BOOL fCheckGeneralFilters);

	// translate a filter-based partition selector
	CDXLNode *
	PdxlnPartitionSelectorFilter(CExpression *pexpr, DrgPcr *pdrgpcr,
								 DrgPds *pdrgpdsBaseTables,
								 ULONG *pulNonGatherMotions, BOOL *pfDML,
								 CExpression *pexprScalarCond,
								 CDXLPhysicalProperties *pdxlprop);
	...
	void
	TranslatePartitionFilters(CExpression *pexprPartSelector, BOOL fPassThrough,
	...
	void
	ConstructLevelFilters4PartitionSelector(CExpression *pexprPartSelector,
											CDXLNode **ppdxlnEqFilters,
											CDXLNode **ppdxlnFilters);
	...
	CDXLNode *
	PdxlnPredOnPartKey(CExpression *pexprPred, CColRef *pcrPartKey,
	...
	// comparison type flags accordingly
	CDXLNode *
	PdxlnConjDisjOnPartKey(CExpression *pexprPred, CColRef *pcrPartKey,
						   IMDId *pmdidTypePartKey, ULONG ulPartLevel,
						   BOOL fRangePart, BOOL *pfLTComparison,
						   BOOL *pfGTComparison, BOOL *pfEQComparison);
	...
	// comparison type flags accordingly
	CDXLNode *
	PdxlnScCmpPartKey(CExpression *pexprScCmp, CColRef *pcrPartKey,
					  IMDId *pmdidTypePartKey, ULONG ulPartLevel,
					  BOOL fRangePart, BOOL *pfLTComparison,
					  BOOL *pfGTComparison, BOOL *pfEQComparison);
	...
	CDXLNode *
	PdxlnScNullTestPartKey(IMDId *pmdidTypePartKey, ULONG ulPartLevel,
						   BOOL fRangePart, BOOL fIsNull);

	// translate the child of a partition selector expression, pushing the given
	// scalar predicate if available
	CDXLNode *
	PdxlnPartitionSelectorChild(CExpression *pexprChild,
								CExpression *pexprScalarCond,
								CDXLPhysicalProperties *pdxlprop,
								DrgPcr *pdrgpcr, DrgPds *pdrgpdsBaseTables,
								ULONG *pulNonGatherMotions, BOOL *pfDML);
	...
	CDXLNode *
	PdxlArrayExprOnPartKey(CExpression *pexprPred, CColRef *pcrPartKey,
						   IMDId *pmdidTypePartKey, ULONG ulPartLevel,
						   BOOL fRangePart,
	...
	CDXLNode *
	PdxlnDML(CExpression *pexpr, DrgPcr *pdrgpcr, DrgPds *pdrgpdsBaseTables,
			 ULONG *pulNonGatherMotions, BOOL *pfDML);

	// translate a CTAS operator
	CDXLNode *
	PdxlnCTAS(CExpression *pexpr, DrgPds *pdrgpdsBaseTables,
			  ULONG *pulNonGatherMotions, BOOL *pfDML);

	// translate a split operator
	CDXLNode *
	PdxlnSplit(CExpression *pexpr, DrgPcr *pdrgpcr, DrgPds *pdrgpdsBaseTables,
			   ULONG *pulNonGatherMotions, BOOL *pfDML);

	// translate an assert operator
	CDXLNode *
	PdxlnAssert(CExpression *pexprAssert, DrgPcr *pdrgpcr,
				DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
				BOOL *pfDML);

	// translate a row trigger operator
	CDXLNode *
	PdxlnRowTrigger(CExpression *pexpr, DrgPcr *pdrgpcr,
					DrgPds *pdrgpdsBaseTables, ULONG *pulNonGatherMotions,
					BOOL *pfDML);

	// translate a scalar If statement
	CDXLNode *
	PdxlnScIfStmt(CExpression *pexprScIf);

	// translate a scalar switch
	CDXLNode *
	PdxlnScSwitch(CExpression *pexprScSwitch);

	// translate a scalar switch case
	CDXLNode *
	PdxlnScSwitchCase(CExpression *pexprScSwitchCase);

	// translate a scalar case test
	CDXLNode *
	PdxlnScCaseTest(CExpression *pexprScCaseTest);

	// translate a scalar comparison
	CDXLNode *
	PdxlnScCmp(CExpression *pexprScCmp);

	// translate a scalar distinct comparison
	CDXLNode *
	PdxlnScDistinctCmp(CExpression *pexprScIsDistFrom);

	// translate a scalar op node
	CDXLNode *
	PdxlnScOp(CExpression *pexprScOp);

	// translate a scalar constant
	CDXLNode *
	PdxlnScConst(CExpression *pexprScConst);

	// translate a scalar coalesce
	CDXLNode *
	PdxlnScCoalesce(CExpression *pexprScCoalesce);

	// translate a scalar MinMax
	CDXLNode *
	PdxlnScMinMax(CExpression *pexprScMinMax);

	// translate a scalar boolean expression
	CDXLNode *
	PdxlnScBoolExpr(CExpression *pexprScBoolExpr);

	// translate a scalar identifier
	CDXLNode *
	PdxlnScId(CExpression *pexprScId);

	// translate a scalar function expression
	CDXLNode *
	PdxlnScFuncExpr(CExpression *pexprScFunc);

	// translate a scalar window function expression
	CDXLNode *
	PdxlnScWindowFuncExpr(CExpression *pexprScFunc);

	// get the DXL representation of the window stage
	EdxlWinStage
	Ews(CScalarWindowFunc::EWinStage ews) const;

	// translate a scalar aggref
	CDXLNode *
	PdxlnScAggref(CExpression *pexprScAggFunc);

	// translate a scalar nullif
	CDXLNode *
	PdxlnScNullIf(CExpression *pexprScNullIf);

	// translate a scalar null test
	CDXLNode *
	PdxlnScNullTest(CExpression *pexprScNullTest);

	// translate a scalar boolean test
	CDXLNode *
	PdxlnScBooleanTest(CExpression *pexprScBooleanTest);

	// translate a scalar cast
	CDXLNode *
	PdxlnScCast(CExpression *pexprScCast);

	// translate a scalar coerce
	CDXLNode *
	PdxlnScCoerceToDomain(CExpression *pexprScCoerce);

	// translate a scalar coerce using I/O functions
	CDXLNode *
	PdxlnScCoerceViaIO(CExpression *pexprScCoerce);

	// translate a scalar array coerce expr with element coerce function
	CDXLNode *
	PdxlnScArrayCoerceExpr(CExpression *pexprScArrayCoerceExpr);

	// translate an array
	CDXLNode *
	PdxlnArray(CExpression *pexpr);

	// translate an arrayref
	CDXLNode *
	PdxlnArrayRef(CExpression *pexpr);

	// translate an arrayref index list
	CDXLNode *
	PdxlnArrayRefIndexList(CExpression *pexpr);

	// translate the arrayref index list bound
	CDXLScalarArrayRefIndexList::EIndexListBound
	Eilb(const CScalarArrayRefIndexList::EIndexListType eilt);

	// translate an array compare
	CDXLNode *
	PdxlnArrayCmp(CExpression *pexpr);

	// translate an assert predicate
	CDXLNode *
	PdxlnAssertPredicate(CExpression *pexpr);

	// translate an assert constraint
	CDXLNode *
	PdxlnAssertConstraint(CExpression *pexpr);

	// translate a DML action expression
	CDXLNode *
	PdxlnDMLAction(CExpression *pexpr);

	// translate a window frame
	CDXLWindowFrame *
	Pdxlwf(CWindowFrame *pwf);

	CDXLTableDescr *
	Pdxltabdesc(const CTableDescriptor *ptabdesc, const DrgPcr *pdrgpcrOutput);

	// compute physical properties like operator cost from the expression
	CDXLPhysicalProperties *
	Pdxlprop(const CExpression *pexpr);

	// translate a colref set of output col into a dxl proj list
	CDXLNode *
	PdxlnProjList(const CColRefSet *pcrsOutput, DrgPcr *pdrgpcr);

	// translate a project list expression into a DXL proj list node
	// according to the order specified in the dynamic array
	CDXLNode *
	PdxlnProjList(const CExpression *pexprProjList,
				  const CColRefSet *pcrsOutput, DrgPcr *pdrgpcr);
	...
	void
	TranslateScalarChildren(CExpression *pexpr, CDXLNode *pdxln);

	// add a result node, if required a materialize node is added below result node to avoid deadlock hazard
	CDXLNode *
	PdxlnResult(CDXLPhysicalProperties *pdxlprop, CDXLNode *pdxlnPrL,
				CDXLNode *pdxlnChild);

	// add a materialize node
	CDXLNode *
	PdxlnMaterialize(CDXLNode *pdxln);

	// add result node if necessary
	CDXLNode *
	PdxlnRemapOutputColumns(CExpression *pexpr, CDXLNode *pdxln,
							DrgPcr *pdrgpcrRequired, DrgPcr *pdrgpcrOrder);

	// combines the ordered columns and required columns into a single list
	DrgPcr *
	PdrgpcrMerge(IMemoryPool *pmp, DrgPcr *pdrgpcrOrder,
				 DrgPcr *pdrgpcrRequired);

	// helper to add a project of bool constant
	CDXLNode *
	PdxlnProjectBoolConst(CDXLNode *pdxln, BOOL fVal);

	// helper to build a Result expression with project list restricted to required column
	CDXLNode *
	PdxlnRestrictResult(CDXLNode *pdxln, CColRef *pcr);
	...
	// in subplan map
	void
	BuildScalarSubplans(DrgPcr *pdrgpcrInner, CExpression *pexprInner,
						DrgPdxlcr *pdrgdxlcr, DrgPds *pdrgpdsBaseTables,
						ULONG *pulNonGatherMotions, BOOL *pfDML);

	// helper to build subplans for quantified (ANY/ALL) subqueries
	CDXLNode *
	PdxlnQuantifiedSubplan(DrgPcr *pdrgpcrInner,
	...
	CDXLNode *
	PdxlnExistentialSubplan(DrgPcr *pdrgpcrInner,
							CExpression *pexprCorrelatedNLJoin,
							DrgPdxlcr *pdrgdxlcr, DrgPds *pdrgpdsBaseTables,
							ULONG *pulNonGatherMotions, BOOL *pfDML);
...
public:
	// ctor
	CTranslatorExprToDXL(IMemoryPool *pmp, CMDAccessor *pmda,
						 DrgPi *pdrgpiSegments, BOOL fInitColumnFactory = true);
	...
	// main driver
	CDXLNode *
	PdxlnTranslate(CExpression *pexpr, DrgPcr *pdrgpcr,
