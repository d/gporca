...
private:
	...
	static CDXLNode *
	PdxlnCmp(IMemoryPool *pmp, CMDAccessor *pmda, ULONG ulPartLevel,
			 BOOL fLowerBound, CDXLNode *pdxlnScalar, IMDType::ECmpType ecmpt,
			 IMDId *pmdidTypePartKey, IMDId *pmdidTypeExpr,
			 IMDId *pmdidTypeCastExpr, IMDId *pmdidCastFunc);


	// create a column reference
	static CColRef *
	PcrCreate(IMemoryPool *pmp, CMDAccessor *pmda, CColumnFactory *pcf,
			  IMDId *pmdid, INT iTypeModifier, const WCHAR *wszName);

	// find the partitioning level of the given part key, given the whole
	// array of part keys
	static ULONG
	UlPartKeyLevel(const CColRef *pcr, DrgDrgPcr *pdrgpdrgpcr);

	// construct a test for a partial scan given a part constraint
	static CDXLNode *
	PdxlnPartialScanTest(IMemoryPool *pmp, CMDAccessor *pmda,
						 CColumnFactory *pcf, CConstraint *pcnstr,
						 DrgDrgPcr *pdrgpdrgpcrPartKeys, BOOL fRangePart);

	// construct a test for a conjunction or disjunction-based part constraint
	static CDXLNode *
	PdxlnPartialScanTestConjDisj(IMemoryPool *pmp, CMDAccessor *pmda,
								 CColumnFactory *pcf, DrgPcnstr *pdrgpcnstr,
								 BOOL fConjunction,
	...
	static CDXLNode *
	PdxlnPartialScanTestConjunction(IMemoryPool *pmp, CMDAccessor *pmda,
									CColumnFactory *pcf, CConstraint *pcnstr,
									DrgDrgPcr *pdrgpdrgpcrPartKeys,
									BOOL fRangePart);
	...
	static CDXLNode *
	PdxlnPartialScanTestDisjunction(IMemoryPool *pmp, CMDAccessor *pmda,
									CColumnFactory *pcf, CConstraint *pcnstr,
									DrgDrgPcr *pdrgpdrgpcrPartKeys,
									BOOL fRangePart);
	...
	static CDXLNode *
	PdxlnPartialScanTestNegation(IMemoryPool *pmp, CMDAccessor *pmda,
								 CColumnFactory *pcf, CConstraint *pcnstr,
								 DrgDrgPcr *pdrgpdrgpcrPartKeys,
								 BOOL fRangePart);
	...
	static CDXLNode *
	PdxlnPartialScanTestInterval(IMemoryPool *pmp, CMDAccessor *pmda,
								 CConstraint *pcnstr,
								 DrgDrgPcr *pdrgpdrgpcrPartKeys,
								 BOOL fRangePart);
	...
	static CDXLNode *
	PdxlnPartialScanTestRange(IMemoryPool *pmp, CMDAccessor *pmda, CRange *prng,
							  IMDId *pmdidPartKeyType, ULONG ulPartLevel,
							  BOOL fRangePart);

	// construct a test for testing range containment with respect to the
	// start of the range
	static CDXLNode *
	PdxlnRangeStartPredicate(IMemoryPool *pmp, CMDAccessor *pmda,
							 IDatum *pdatum, CRange::ERangeInclusion eri,
							 IMDId *pmdidPartKeyType, ULONG ulPartLevel);
	...
	// end of the range
	static CDXLNode *
	PdxlnRangeEndPredicate(IMemoryPool *pmp, CMDAccessor *pmda, IDatum *pdatum,
	...
	static CDXLNode *
	PdxlnRangePointPredicate(IMemoryPool *pmp, CMDAccessor *pmda,
							 IDatum *pdatum, CRange::ERangeInclusion eri,
							 IMDId *pmdidPartKeyType, IMDId *pmdidCmpExcl,
							 IMDId *pmdidCmpIncl, ULONG ulPartLevel,
							 BOOL fLower);

	// construct a test for the default partition
	static CDXLNode *
	PdxlnDefaultPartitionTest(IMemoryPool *pmp, ULONG ulPartLevel);

	// compute a DXL datum from a point constraint
	static CDXLDatum *
	PdxldatumFromPointConstraint(IMemoryPool *pmp, CMDAccessor *pmda,
	...
	static DrgPdrgPdxldatum *
	PdrgpdrgpdxldatumFromDisjPointConstraint(IMemoryPool *pmp,
											 CMDAccessor *pmda,
											 const CColRef *pcrDistrCol,
											 CConstraint *pcnstrDistrCol);
	...
	static CDXLDirectDispatchInfo *
	Pdxlddinfo(IMemoryPool *pmp, CMDAccessor *pmda, DrgPexpr *pdrgpexprHashed,
			   CConstraint *pcnstr);

	// compute the direct dispatch info for a single distribution key from the constraints
	// on the distribution key
	static CDXLDirectDispatchInfo *
	PdxlddinfoSingleDistrKey(IMemoryPool *pmp, CMDAccessor *pmda,
	...
	static BOOL
	FDirectDispatchable(const CColRef *pcrDistrCol, const CDXLDatum *pdxldatum);

public:
	// construct a default properties container
	static CDXLPhysicalProperties *
	Pdxlprop(IMemoryPool *pmp);

	// create a scalar const value expression for the given bool value
	static CDXLNode *
	PdxlnBoolConst(IMemoryPool *pmp, CMDAccessor *pmda, BOOL fVal);

	// create a scalar const value expression for the given int4 value
	static CDXLNode *
	PdxlnInt4Const(IMemoryPool *pmp, CMDAccessor *pmda, INT iVal);

	// construct a filter node for a list partition predicate
	static CDXLNode *
	PdxlnListFilterScCmp(IMemoryPool *pmp, CMDAccessor *pmda,
						 CDXLNode *pdxlnPartKey, CDXLNode *pdxlnScalar,
						 IMDId *pmdidTypePartKey, IMDId *pmdidTypeOther,
						 IMDType::ECmpType ecmpt, ULONG ulPartLevel,
						 BOOL fHasDefaultPart);

	// construct a DXL node for the part key portion of the list partition filter
	static CDXLNode *
	PdxlnListFilterPartKey(IMemoryPool *pmp, CMDAccessor *pmda,
						   CExpression *pexprPartKey, IMDId *pmdidTypePartKey,
						   ULONG ulPartLevel);

	// construct a filter node for a range predicate
	static CDXLNode *
	PdxlnRangeFilterScCmp(IMemoryPool *pmp, CMDAccessor *pmda,
						  CDXLNode *pdxlnScalar, IMDId *pmdidTypePartKey,
						  IMDId *pmdidTypeOther, IMDId *pmdidTypeCastExpr,
						  IMDId *pmdidCastFunc, IMDType::ECmpType ecmpt,
						  ULONG ulPartLevel);

	// construct a range filter for an equality comparison
	static CDXLNode *
	PdxlnRangeFilterEqCmp(IMemoryPool *pmp, CMDAccessor *pmda,
						  CDXLNode *pdxlnScalar, IMDId *pmdidTypePartKey,
						  IMDId *pmdidTypeOther, IMDId *pmdidTypeCastExpr,
	...
	static CDXLNode *
	PdxlnRangeFilterPartBound(IMemoryPool *pmp, CMDAccessor *pmda,
							  CDXLNode *pdxlnScalar, IMDId *pmdidTypePartKey,
							  IMDId *pmdidTypeOther, IMDId *pmdidTypeCastExpr,
							  IMDId *pmdidCastFunc, ULONG ulPartLevel,
							  ULONG fLowerBound, IMDType::ECmpType ecmpt);

	// construct predicates to cover the cases of default partition and
	// open-ended partitions if necessary
	static CDXLNode *
	PdxlnRangeFilterDefaultAndOpenEnded(IMemoryPool *pmp, ULONG ulPartLevel,
										BOOL fLTComparison, BOOL fGTComparison,
										BOOL fEQComparison, BOOL fDefaultPart);

	// construct a test for partial scan in the partial partition propagator
	static CDXLNode *
	PdxlnPartialScanTest(IMemoryPool *pmp, CMDAccessor *pmda,
						 CColumnFactory *pcf, const CPartConstraint *ppartcnstr,
						 DrgDrgPcr *pdrgpdrgpcrPartKeys,
	...
	static CDXLNode *
	PdxlnPropagationExpressionForPartConstraints(IMemoryPool *pmp,
												 CMDAccessor *pmda,
												 CColumnFactory *pcf,
	...
	static BOOL
	FScalarConstTrue(CMDAccessor *pmda, CDXLNode *pdxln);

	// check if the DXL Node is a scalar const false
	static BOOL
	FScalarConstFalse(CMDAccessor *pmda, CDXLNode *pdxln);

	// check whether a project list has the same columns in the given array
	// and in the same order
	static BOOL
	FProjectListMatch(CDXLNode *pdxlnPrL, DrgPcr *pdrgpcr);

	// create a project list by creating references to the columns of the given
	// project list of the child node
	static CDXLNode *
	PdxlnProjListFromChildProjList(IMemoryPool *pmp, CColumnFactory *pcf,
								   HMCrDxln *phmcrdxln,
								   const CDXLNode *pdxlnProjListChild);

	// construct the project list of a partition selector
	static CDXLNode *
	PdxlnPrLPartitionSelector(IMemoryPool *pmp, CMDAccessor *pmda,
							  CColumnFactory *pcf, HMCrDxln *phmcrdxln,
							  BOOL fUseChildProjList, CDXLNode *pdxlnPrLChild,
							  CColRef *pcrOid, ULONG ulPartLevels,
							  BOOL fGeneratePartOid);

	// construct the propagation expression for a partition selector
	static CDXLNode *
	PdxlnPropExprPartitionSelector(IMemoryPool *pmp, CMDAccessor *pmda,
								   CColumnFactory *pcf, BOOL fConditional,
								   PartCnstrMap *ppartcnstrmap,
								   DrgDrgPcr *pdrgpdrgpcrKeys, ULONG ulScanId,
								   DrgPsz *pdrgszPartTypes);

	// create a DXL project elem node from as a scalar identifier for the
	// child project element node
	static CDXLNode *
	PdxlnProjElem(IMemoryPool *pmp, CColumnFactory *pcf, HMCrDxln *phmcrdxln,
				  const CDXLNode *pdxlnProjElemChild);

	// create a scalar identifier node for the given column reference
	static CDXLNode *
	PdxlnIdent(IMemoryPool *pmp, HMCrDxln *phmcrdxlnSubplans,
			   HMCrDxln *phmcrdxlnIndexLookup, const CColRef *pcr);

	// replace subplan entry in the given map with a dxl column reference
	static void
	ReplaceSubplan(IMemoryPool *pmp, HMCrDxln *phmcrdxlnSubplans,
				   const CColRef *pcr, CDXLScalarProjElem *pdxlopPrEl);

	// create a project elem from a given col ref
	static CDXLNode *
	PdxlnProjElem(IMemoryPool *pmp, HMCrDxln *phmcrdxlnSubplans,
				  const CColRef *pcr);

	// construct an array of NULL datums for a given array of columns
	static DrgPdatum *
	PdrgpdatumNulls(IMemoryPool *pmp, DrgPcr *pdrgpcr);

	// map an array of columns to a new array of columns
	static DrgPcr *
	PdrgpcrMapColumns(IMemoryPool *pmp, DrgPcr *pdrgpcrInput, HMCrUl *phmcrul,
					  DrgPcr *pdrgpcrMapDest);

	// combine two boolean expressions using the given boolean operator
	static CDXLNode *
	PdxlnCombineBoolean(IMemoryPool *pmp, CDXLNode *pdxlnFst,
						CDXLNode *pdxlnSnd, EdxlBoolExprType boolexptype);

	// construct a partition selector node
	static CDXLNode *
	PdxlnPartitionSelector(IMemoryPool *pmp, IMDId *pmdid, ULONG ulPartLevels,
						   ULONG ulScanId, CDXLPhysicalProperties *pdxlprop,
						   CDXLNode *pdxlnPrL, CDXLNode *pdxlnEqFilters,
						   CDXLNode *pdxlnFilters, CDXLNode *pdxlnResidual,
						   CDXLNode *pdxlnPropagation, CDXLNode *pdxlnPrintable,
						   CDXLNode *pdxlnChild = NULL);

	// create a DXL result node
	static CDXLNode *
	PdxlnResult(IMemoryPool *pmp, CDXLPhysicalProperties *pdxlprop,
				CDXLNode *pdxlnPrL, CDXLNode *pdxlnFilter,
				CDXLNode *pdxlnOneTimeFilter, CDXLNode *pdxlnChild);

	// create a DXL ValuesScan node
	static CDXLNode *
	PdxlnValuesScan(IMemoryPool *pmp, CDXLPhysicalProperties *pdxlprop,
					CDXLNode *pdxlnPrL, DrgPdrgPdatum *pdrgpdrgdatum);

	// build hashmap based on a column array, where the key is the column
	// and the value is the index of that column in the array
	static HMCrUl *
	PhmcrulColIndex(IMemoryPool *pmp, DrgPcr *pdrgpcr);

	// set statistics of the operator
	static void
	SetStats(IMemoryPool *pmp, CMDAccessor *pmda, CDXLNode *pdxln,
			 const IStatistics *pstats, BOOL fRoot);

	// set direct dispatch info of the operator
	static void
	SetDirectDispatchInfo(IMemoryPool *pmp, CMDAccessor *pmda, CDXLNode *pdxln,
						  CDrvdPropRelational *pdpRel,
						  DrgPds *pdrgpdsBaseTables);

	// is the aggregate a local hash aggregate that is safe to stream
	static BOOL
	FLocalHashAggStreamSafe(CExpression *pexprAgg);

	// if operator is a scalar cast, extract cast type and function
	static void
	ExtractCastMdids(COperator *pop, IMDId **ppmdidType,
					 IMDId **ppmdidCastFunc);

	// produce DXL representation of a datum
	static CDXLDatum *
	Pdxldatum(IMemoryPool *pmp, CMDAccessor *pmda, IDatum *pdatum)
	{
