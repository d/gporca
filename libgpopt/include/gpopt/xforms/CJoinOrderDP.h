...
private:
	...
	struct SComponentPair : public CRefCount
	{
		...
		// hashing function
		static ULONG
		UlHash(const SComponentPair *pcomppair);

		// equality function
		static BOOL
		FEqual(const SComponentPair *pcomppairFst,
	...
	typedef CHashMap<SComponentPair, CExpression, SComponentPair::UlHash,
					 SComponentPair::FEqual, CleanupRelease<SComponentPair>,
	...
	CExpression *
	PexprBuildPred(CBitSet *pbsFst, CBitSet *pbsSnd);

	// lookup best join order for given set
	CExpression *
	PexprLookup(CBitSet *pbs);

	// extract predicate joining the two given sets
	CExpression *
	PexprPred(CBitSet *pbsFst, CBitSet *pbsSnd);

	// join expressions in the given two sets
	CExpression *
	PexprJoin(CBitSet *pbsFst, CBitSet *pbsSnd);

	// join expressions in the given set
	CExpression *
	PexprJoin(CBitSet *pbs);

	// find best join order for given component using dynamic programming
	CExpression *
	PexprBestJoinOrderDP(CBitSet *pbs);

	// find best join order for given component
	CExpression *
	PexprBestJoinOrder(CBitSet *pbs);

	// generate cross product for the given components
	CExpression *
	PexprCross(CBitSet *pbs);

	// join a covered subset with uncovered subset
	CExpression *
	PexprJoinCoveredSubsetWithUncoveredSubset(CBitSet *pbs, CBitSet *pbsCovered,
											  CBitSet *pbsUncovered);

	// return a subset of the given set covered by one or more edges
	CBitSet *
	PbsCovered(CBitSet *pbsInput);

	// add given join order to best results
	void
	AddJoinOrder(CExpression *pexprJoin, CDouble dCost);

	// compute cost of given join expression
	CDouble
	DCost(CExpression *pexpr);

	// derive stats on given expression
	void
	DeriveStats(CExpression *pexpr);

	// add expression to cost map
	void
	InsertExpressionCost(CExpression *pexpr, CDouble dCost,
						 BOOL fValidateInsert);

	// generate all subsets of the given array of elements
	static void
	GenerateSubsets(IMemoryPool *pmp, CBitSet *pbsCurrent, ULONG *pulElems,
					ULONG ulSize, ULONG ulIndex, DrgPbs *pdrgpbsSubsets);

	// driver of subset generation
	static DrgPbs *
	PdrgpbsSubsets(IMemoryPool *pmp, CBitSet *pbs);

public:
	// ctor
	CJoinOrderDP(IMemoryPool *pmp, DrgPexpr *pdrgpexprComponents,
