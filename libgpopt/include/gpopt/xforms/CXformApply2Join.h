...
private:
	...
	static BOOL
	FCanCreateCorrelatedApply(IMemoryPool *, CExpression *pexprApply)
	{
		...
		return COperator::EopLogicalInnerApply == eopid ||
	...
	static void
	CreateCorrelatedApply(IMemoryPool *pmp, CExpression *pexprApply,
						  CXformResult *pxfres)
	{
		...
		switch (eopid)
		{
			case COperator::EopLogicalInnerApply:
				pexprResult =
					CUtils::PexprLogicalApply<CLogicalInnerCorrelatedApply>(
						pmp, pexprOuter, pexprInner, pdrgpcr, eopidSubq,
						pexprScalar);
				break;
			...
			case COperator::EopLogicalLeftSemiApplyIn:
				pexprResult = CUtils::PexprLogicalCorrelatedQuantifiedApply<
					CLogicalLeftSemiCorrelatedApplyIn>(pmp, pexprOuter,
													   pexprInner, pdrgpcr,
...
protected:
	...
	static BOOL
	FDecorrelate(IMemoryPool *pmp, CExpression *pexprApply,
				 CExpression **ppexprInner, DrgPexpr **ppdrgpexpr)
	{
		GPOS_ASSERT(NULL != pexprApply);
		...
		*ppdrgpexpr = GPOS_NEW(pmp) DrgPexpr(pmp);
		...
		if (!CDecorrelator::FProcess(pmp, (*pexprApply)[1],
									 false /*fEqualityOnly*/, ppexprInner,
									 *ppdrgpexpr))
		{
			// decorrelation filed
			(*ppdrgpexpr)->Release();
		...
		if ((COperator::EopLogicalLeftSemiJoin == pexprApply->Pop()->Eopid() ||
			...
			!CPredicateUtils::FValidSemiJoinCorrelations(
				pmp, (*pexprApply)[0], (*ppexprInner), (*ppdrgpexpr)))
		{
			(*ppdrgpexpr)->Release();
						  ...
						  CExpression *pexprApply)
	{
		...
		GPOS_ASSERT_IMP(
			CUtils::FHasOuterRefs((*pexprApply)[1]),
			!exprhdl.Pdprel(1)->PcrsOuter()->FSubset(
...
public:
	...
	explicit CXformApply2Join<TApply, TJoin>(IMemoryPool *pmp, BOOL)
		:  // pattern
		  CXformExploration(GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) TApply(pmp),
	...
	explicit CXformApply2Join<TApply, TJoin>(IMemoryPool *pmp)
		:  // pattern
		  CXformExploration(GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) TApply(pmp),
			  GPOS_NEW(pmp) CExpression(
