...
private:
	...
	static CExpression *
	PexprMDQAs2Join(IMemoryPool *pmp, CExpression *pexpr);

	// expand GbAgg with multiple distinct aggregates into a join of single distinct aggregates
	static CExpression *
	PexprExpandMDQAs(IMemoryPool *pmp, CExpression *pexpr);

	// main transformation function driver
	static CExpression *
	PexprTransform(IMemoryPool *pmp, CExpression *pexpr);

public:
	// ctor
	explicit CXformGbAggWithMDQA2Join(IMemoryPool *pmp);
