...
private:
	...
	CreateAlternativesForBtreeIndex(
		IMemoryPool *pmp, ULONG ulOriginOpId, CExpression *pexprOuter,
		CExpression *pexprInner, CMDAccessor *pmda,
		DrgPexpr *pdrgpexprConjuncts, CColRefSet *pcrsScalarExpr,
	...
	void
	ComputeColumnSets(IMemoryPool *pmp, CExpression *pexprInner,
					  CExpression *pexprScalar, CColRefSet **ppcrsScalarExpr,
	...
	PexprIndexApplyOverCTEConsumer(
		...
		DrgPul *pdrgpulIndexesOfRefsInScan) const;

	// create a union-all with the given children
	CExpression *
	PexprConstructUnionAll(IMemoryPool *pmp, DrgPcr *pdrgpcrLeftSchema,
						   DrgPcr *pdrgpcrRightSchema,
						   CExpression *pexprLeftChild,
						   CExpression *pexprRightChild, ULONG ulScanId) const;

	//	construct a CTE Anchor over the given UnionAll and adds it to the given
	//	Xform result
	void
	AddUnionPlanForPartialIndexes(IMemoryPool *pmp,
	...
	BOOL
	FMatchDistKeyAndIndexKey(const IMDRelation *pmdrel,
...
protected:
	...
	// for partial indexes
	virtual void
	CreatePartialIndexApplyAlternatives(IMemoryPool *pmp, ULONG ulOriginOpId,
	...
	PopLogicalJoin(IMemoryPool *pmp) const = 0;
	...
	PopLogicalApply(IMemoryPool *pmp, DrgPcr *pdrgpcrOuterRefs) const = 0;
