...
template <class TJoin, class TApply, class TGet, BOOL fWithSelect,
		  BOOL fPartial, IMDIndex::EmdindexType eidxtype>
...
private:
	...
	BOOL
	FCanLeftOuterIndexApply(IMemoryPool *pmp, CExpression *pexprInner,
							CExpression *pexprScalar) const
	{
		...
		IMDRelation::Ereldistrpolicy ereldist =
			popGet->Ptabdesc()->Ereldistribution();
		...
		CColRefSet *pcrsInnerRefs =
			GPOS_NEW(pmp) CColRefSet(pmp, *pcrsScalarExpr);
		...
		if (fCanOuterIndexApply)
		{
			...
			for (ULONG ul = 0; ul < pdrgpexpr->UlLength(); ul++)
			{
				...
				CColRefSet *pcrsPred =
					CDrvdPropScalar::Pdpscalar(pexprPred->PdpDerive())
						->PcrsUsed();
			...
			fCanOuterIndexApply =
				pcrsEquivPredInner->FSubset(popGet->PcrsDist());
...
protected:
	...
	PopLogicalJoin(IMemoryPool *pmp) const
	{
	...
	PopLogicalApply(IMemoryPool *pmp, DrgPcr *pdrgpcr) const
	{
...
public:
	...
	explicit CXformJoin2IndexApplyBase<TJoin, TApply, TGet, fWithSelect,
									   fPartial,
									   eidxtype>(IMemoryPool *pmp)
		:  // pattern
		  CXformJoin2IndexApply(GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) TJoin(pmp),
			  GPOS_NEW(pmp) CExpression(
				  pmp, GPOS_NEW(pmp) CPatternLeaf(pmp)),  // outer child
			  fWithSelect ? GPOS_NEW(pmp)
								CExpression  // inner child with Select operator
				  (pmp, GPOS_NEW(pmp) CLogicalSelect(pmp),
				   GPOS_NEW(pmp) CExpression(
					   pmp, GPOS_NEW(pmp) TGet(pmp)),  // Get below Select
				   GPOS_NEW(pmp) CExpression(
					   pmp, GPOS_NEW(pmp) CPatternTree(pmp))  // predicate
						  ...
						  : GPOS_NEW(pmp) CExpression(
								pmp,
								GPOS_NEW(pmp) TGet(
									pmp)),  // inner child with Get operator,
			  GPOS_NEW(pmp) CExpression(
