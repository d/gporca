...
private:
	...
	static BOOL
	FSplitCorrelations(IMemoryPool *pmp, CExpression *pexprOuter,
					   CExpression *pexprInner, DrgPexpr *pdrgpexprAllCorr,
	...
	static BOOL
	FDecorrelate(IMemoryPool *pmp, CExpression *pexpr,
				 CExpression **ppexprInnerNew, DrgPexpr **ppdrgpexprCorr);

	// decorrelate semi apply with external correlations
	static CExpression *
	PexprDecorrelate(IMemoryPool *pmp, CExpression *pexpr);

public:
	// ctor
	explicit CXformLeftSemiApplyWithExternalCorrs2InnerJoin(IMemoryPool *pmp)
	...
	CXformLeftSemiApplyWithExternalCorrs2InnerJoin(IMemoryPool *pmp,
