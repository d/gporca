...
private:
	...
	CColRefSet *
	Pcrs(IMemoryPool *pmp, CLogicalGet *popGet, const IMDIndex *pmdindex,
		 BOOL fIncludedColumns) const;

	// create an index get plan when applicable
	void
	CreatePartialIndexGetPlan(IMemoryPool *pmp, CExpression *pexpr,
	...
	static CExpression *
	PexprSelectOverDynamicGet(IMemoryPool *pmp, CLogicalDynamicGet *popGet,
							  CExpression *pexprScalar, DrgPcr *pdrgpcrDGet,
							  CPartConstraint *ppartcnstr);

public:
	// ctor
	explicit CXformSelect2PartialDynamicIndexGet(IMemoryPool *pmp);
