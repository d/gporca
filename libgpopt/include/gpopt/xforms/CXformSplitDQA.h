...
private:
	...
	static CExpression *
	PexprSplitHelper(IMemoryPool *pmp, CColumnFactory *pcf, CMDAccessor *pmda,
	...
	// aggregation
	static void
	PopulatePrLMultiPhaseAgg(IMemoryPool *pmp, CColumnFactory *pcf,
							 CMDAccessor *pmda, CExpression *pexprPrEl,
							 DrgPexpr *pdrgpexprPrElFirstStage,
							 DrgPexpr *pdrgpexprPrElSecondStage,
	...
	static CExpression *
	PexprPrElAgg(IMemoryPool *pmp, CExpression *pexprAggFunc,
				 EAggfuncStage eaggfuncstage, CColRef *pcrPreviousStage,
				 CColRef *pcrGlobal);

	// extract arguments of distinct aggs
	static void
	ExtractDistinctCols(IMemoryPool *pmp, CColumnFactory *pcf,
						CMDAccessor *pmda, CExpression *pexpr,
						DrgPexpr *pdrgpexprChildPrEl, HMExprCr *phmexprcr,
						DrgPcr **ppdrgpcrArgDQA);

	// return the column reference of the argument to the aggregate function
	static CColRef *
	PcrAggFuncArgument(IMemoryPool *pmp, CMDAccessor *pmda, CColumnFactory *pcf,
					   CExpression *pexprArg, DrgPexpr *pdrgpexprChildPrEl);

public:
	// ctor
	explicit CXformSplitDQA(IMemoryPool *pmp);
	...
	// actual transform
	void
	Transform(CXformContext *, CXformResult *, CExpression *) const;
