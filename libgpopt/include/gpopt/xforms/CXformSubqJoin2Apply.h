...
private:
	...
	typedef CHashMap<CExpression, CColRef, UlHashPtr<CExpression>,
					 FEqualPtr<CExpression>, CleanupRelease<CExpression>,
					 CleanupNULL<CColRef> >
		HMExprCr;
	...
	void
	Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr,
			  BOOL fEnforceCorrelatedApply) const;

	// collect subqueries that exclusively use outer/inner child
	static void
	CollectSubqueries(IMemoryPool *pmp, CExpression *pexpr, DrgPcrs *pdrgpcrs,
					  DrgPdrgPexpr *pdrgpdrgpexprSubqs);

	// replace subqueries with scalar identifier based on given map
	static CExpression *
	PexprReplaceSubqueries(IMemoryPool *pmp, CExpression *pexprScalar,
						   HMExprCr *phmexprcr);

	// push down subquery below join
	static CExpression *
	PexprSubqueryPushDown(IMemoryPool *pmp, CExpression *pexpr,
						  BOOL fEnforceCorrelatedApply);

public:
	// ctor
	explicit CXformSubqJoin2Apply(IMemoryPool *pmp);
