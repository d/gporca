...
private:
	...
	static DrgPcr *
	PdrgpcrSubqueries(IMemoryPool *pmp, CExpression *pexprCTEProducer,
					  CExpression *pexprCTEConsumer);

	//	collect subquery arguments and return a Project expression
	static CExpression *
	PexprProjectSubqueries(IMemoryPool *pmp, CExpression *pexprTVF);

public:
	// ctor
	explicit CXformUnnestTVF(IMemoryPool *pmp);
