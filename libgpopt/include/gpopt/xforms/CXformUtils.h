...
typedef CHashMap<CExpression, DrgPexpr, CExpression::UlHash, CUtils::FEqual,
				 CleanupRelease<CExpression>, CleanupRelease<DrgPexpr> >
	HMExprDrgPexpr;
...
private:
	...
	typedef CLogical *(*PDynamicIndexOpConstructor)(
		IMemoryPool *pmp, const IMDIndex *pmdindex, CTableDescriptor *ptabdesc,
		ULONG ulOriginOpId, CName *pname, ULONG ulPartIndex,
		DrgPcr *pdrgpcrOutput, DrgDrgPcr *pdrgpdrgpcrPart,
		ULONG ulSecondaryPartIndexId, CPartConstraint *ppartcnstr,
		CPartConstraint *ppartcnstrRel);

	typedef CLogical *(*PStaticIndexOpConstructor)(
		IMemoryPool *pmp, const IMDIndex *pmdindex, CTableDescriptor *ptabdesc,
		ULONG ulOriginOpId, CName *pname, DrgPcr *pdrgpcrOutput);

	typedef CExpression *(PRewrittenIndexPath)(IMemoryPool *pmp,
	...
	// in the index included / key columns
	static DrgPcr *
	PdrgpcrIndexColumns(IMemoryPool *pmp, DrgPcr *pdrgpcr,
						const IMDIndex *pmdindex, const IMDRelation *pmdrel,
						EIndexCols eic);

	// lookup hash join keys in scalar child group
	static void
	LookupHashJoinKeys(IMemoryPool *pmp, CExpression *pexpr,
					   DrgPexpr **ppdrgpexprOuter, DrgPexpr **ppdrgpexprInner);

	// cache hash join keys on scalar child group
	static void
	CacheHashJoinKeys(CExpression *pexpr, DrgPexpr *pdrgpexprOuter,
	...
	static BOOL
	FXformInArray(CXform::EXformId exfid, CXform::EXformId rgXforms[],
				  ULONG ulXforms);

#ifdef GPOS_DEBUG
	// check whether the given join type is swapable
	static BOOL
	FSwapableJoinType(COperator::EOperatorId eopid);
#endif  // GPOS_DEBUG

	// helper function for adding hash join alternative
	template <class T>
	static void
	AddHashJoinAlternative(IMemoryPool *pmp, CExpression *pexprJoin,
						   DrgPexpr *pdrgpexprOuter, DrgPexpr *pdrgpexprInner,
						   CXformResult *pxfres);
	...
	PopGbAggPushableBelowJoin(IMemoryPool *pmp, CLogicalGbAgg *popGbAggOld,
							  CColRefSet *prcsOutput, CColRefSet *pcrsGrpCols);

	// check if the preconditions for pushing down Group by through join are satisfied
	static BOOL
	FCanPushGbAggBelowJoin(CColRefSet *pcrsGrpCols,
	...
	PexprBuildIndexPlan(
		...
		const IMDIndex *pmdindex, const IMDRelation *pmdrel,
		BOOL fAllowPartialIndex, CPartConstraint *ppcForPartialIndexes,
	...
	static CLogical *
	PopStaticBtreeIndexOpConstructor(IMemoryPool *pmp, const IMDIndex *pmdindex,
									 CTableDescriptor *ptabdesc,
									 ULONG ulOriginOpId, CName *pname,
									 DrgPcr *pdrgpcrOutput)
	{
		return GPOS_NEW(pmp) CLogicalIndexGet(
			pmp, pmdindex, ptabdesc, ulOriginOpId, pname, pdrgpcrOutput);
	}

	//	produce an expression representing a new btree index path
	static CExpression *
	PexprRewrittenBtreeIndexPath(IMemoryPool *pmp, CExpression *pexprIndexCond,
								 CExpression *pexprResidualCond,
								 const IMDIndex *,	// pmdindex
								 CTableDescriptor *,  // ptabdesc
								 COperator *popLogical)
	{
		...
		return CUtils::PexprSafeSelect(
			pmp, GPOS_NEW(pmp) CExpression(pmp, popLogical, pexprIndexCond),
			pexprResidualCond);
	}

	// create a candidate dynamic get scan to suplement the partial index scans
	static SPartDynamicIndexGetInfo *
	PpartdigDynamicGet(IMemoryPool *pmp, DrgPexpr *pdrgpexprScalar,
	...
	PexprBitmapFromChildren(IMemoryPool *pmp, CMDAccessor *pmda,
							...
							BOOL fConjunction, CExpression **ppexprRecheck,
	...
	static CExpression *
	PexprBitmapCondToUse(IMemoryPool *pmp, CMDAccessor *pmda,
						 CExpression *pexprPred, BOOL fBoolColumn,
						 BOOL fNegatedBoolColumn, CColRefSet *pcrsScalar);

	// compute the residual predicate for a bitmap table scan
	static void
	ComputeBitmapTableScanResidualPredicate(IMemoryPool *pmp, BOOL fConjunction,
											CExpression *pexprOriginalPred,
											CExpression **ppexprResidual,
	...
	PpartcnstrDisjunction(IMemoryPool *pmp, CPartConstraint *ppartcnstrOld,
	...
	static CExpression *
	PexprBitmapForSelectCondition(IMemoryPool *pmp, CMDAccessor *pmda,
								  CExpression *pexprPred,
								  CTableDescriptor *ptabdesc,
								  const IMDRelation *pmdrel,
								  DrgPcr *pdrgpcrOutput, CColRefSet *pcrsReqd,
								  CExpression **ppexprRecheck, BOOL fBoolColumn,
	...
	// an index lookup
	static CExpression *
	PexprBitmapForIndexLookup(IMemoryPool *pmp, CMDAccessor *pmda,
							  CExpression *pexprPred,
							  CTableDescriptor *ptabdesc,
							  const IMDRelation *pmdrel, DrgPcr *pdrgpcrOutput,
							  CColRefSet *pcrsOuterRefs, CColRefSet *pcrsReqd,
	...
	static BOOL
	FMergeWithPreviousBitmapIndexProbe(IMemoryPool *pmp,
	...
	static DrgPdrgPexpr *
	PdrgpdrgpexprSortedPrjElemsArray(IMemoryPool *pmp,
	...
	static INT
	ICmpPrjElemsArr(const void *pvFst, const void *pvSnd);
...
public:
	...
	template <class T>
	static void
	TransformImplementBinaryOp(CXformContext *pxfctxt, CXformResult *pxfres,
							   CExpression *pexpr);

	// helper function for implementation of hash joins
	template <class T>
	static void
	ImplementHashJoin(CXformContext *pxfctxt, CXformResult *pxfres,
					  CExpression *pexpr, BOOL fAntiSemiJoin = false);

	// helper function for implementation of nested loops joins
	template <class T>
	static void
	ImplementNLJoin(CXformContext *pxfctxt, CXformResult *pxfres,
					CExpression *pexpr);

	// helper for removing IsNotFalse join predicate for GPDB anti-semi hash join
	static BOOL
	FProcessGPDBAntiSemiHashJoin(IMemoryPool *pmp, CExpression *pexpr,
								 CExpression **ppexprResult);

	// check the applicability of logical join to physical join xform
	static CXform::EXformPromise
	ExfpLogicalJoin2PhysicalJoin(CExpressionHandle &exprhdl);

	// check the applicability of semi join to cross product
	static CXform::EXformPromise
	ExfpSemiJoin2CrossProduct(CExpressionHandle &exprhdl);

	// check the applicability of N-ary join expansion
	static CXform::EXformPromise
	ExfpExpandJoinOrder(CExpressionHandle &exprhdl);

	// extract foreign key
	static CColRefSet *
	PcrsFKey(IMemoryPool *pmp, CExpression *pexprOuter, CExpression *pexprInner,
			 CExpression *pexprScalar);

	// compute a swap of the two given joins
	static CExpression *
	PexprSwapJoins(IMemoryPool *pmp, CExpression *pexprTopJoin,
				   CExpression *pexprBottomJoin);

	// push a Gb, optionally with a having clause below the child join
	static CExpression *
	PexprPushGbBelowJoin(IMemoryPool *pmp, CExpression *pexpr);

	// check if the the array of aligned input columns are of the same type
	static BOOL
	FSameDatatype(DrgDrgPcr *pdrgpdrgpcrInput);

	// helper function to separate subquery predicates in a top Select node
	static CExpression *
	PexprSeparateSubqueryPreds(IMemoryPool *pmp, CExpression *pexpr);

	// helper for creating inverse predicate for unnesting subquery ALL
	static CExpression *
	PexprInversePred(IMemoryPool *pmp, CExpression *pexprSubquery);

	// helper for creating a null indicator expression
	static CExpression *
	PexprNullIndicator(IMemoryPool *pmp, CExpression *pexpr);

	// helper for creating a logical DML on top of a project
	static CExpression *
	PexprLogicalDMLOverProject(IMemoryPool *pmp, CExpression *pexprChild,
	...
	static BOOL
	FTriggersExist(CLogicalDML::EDMLOperator edmlop, CTableDescriptor *ptabdesc,
				   BOOL fBefore);

	// does the given trigger type match the given logical DML type
	static BOOL
	FTriggerApplies(CLogicalDML::EDMLOperator edmlop,
					const IMDTrigger *pmdtrigger);

	// construct a trigger expression on top of the given expression
	static CExpression *
	PexprRowTrigger(IMemoryPool *pmp, CExpression *pexprChild,
	...
	PexprRowTrigger(IMemoryPool *pmp, CExpression *pexprChild,
					CLogicalDML::EDMLOperator edmlop, IMDId *pmdidRel,
					BOOL fBefore, DrgPcr *pdrgpcrOld, DrgPcr *pdrgpcrNew);
	...
	static CExpression *
	PexprLogicalPartitionSelector(IMemoryPool *pmp, CTableDescriptor *ptabdesc,
								  DrgPcr *pdrgpcr, CExpression *pexprChild);

	// return partition filter expressions given a table
	// descriptor and the given column references
	static DrgPexpr *
	PdrgpexprPartEqFilters(IMemoryPool *pmp, CTableDescriptor *ptabdesc,
						   DrgPcr *pdrgpcrSource);

	// helper for creating Agg expression equivalent to quantified subquery
	static void
	QuantifiedToAgg(IMemoryPool *pmp, CExpression *pexprSubquery,
	...
	static void
	ExistentialToAgg(IMemoryPool *pmp, CExpression *pexprSubquery,
					 CExpression **ppexprNewSubquery,
					 CExpression **ppexprNewScalar);

	// create a logical assert for the check constraints on the given table
	static CExpression *
	PexprAssertConstraints(IMemoryPool *pmp, CExpression *pexprChild,
						   CTableDescriptor *ptabdesc, DrgPcr *pdrgpcr);

	// create a logical assert for checking cardinality of update values
	static CExpression *
	PexprAssertUpdateCardinality(IMemoryPool *pmp, CExpression *pexprDMLChild,
								 CExpression *pexprDML, CColRef *pcrCtid,
								 CColRef *pcrSegmentId);

	// return true if stats derivation is needed for this xform
	static BOOL
	FDeriveStatsBeforeXform(CXform *pxform);

	// return true if xform is a subquery decorrelation xform
	static BOOL
	FSubqueryDecorrelation(CXform *pxform);

	// return true if xform is a subquery unnesting xform
	static BOOL
	FSubqueryUnnesting(CXform *pxform);

	// return true if xform should be applied to the next binding
	static BOOL
	FApplyToNextBinding(CXform *pxform, CExpression *pexprLastBinding);

	// return true if xform should be applied only once
	static BOOL
	FApplyOnce(CXform::EXformId exfid);

	// return a formatted error message for the given exception
	static CWStringConst *
	PstrErrorMessage(IMemoryPool *pmp, ULONG ulMajor, ULONG ulMinor, ...);

	// return the array of key columns from the given array of columns which appear
	// in the index key columns
	static DrgPcr *
	PdrgpcrIndexKeys(IMemoryPool *pmp, DrgPcr *pdrgpcr,
					 const IMDIndex *pmdindex, const IMDRelation *pmdrel);

	// return the set of key columns from the given array of columns which appear
	// in the index key columns
	static CColRefSet *
	PcrsIndexKeys(IMemoryPool *pmp, DrgPcr *pdrgpcr, const IMDIndex *pmdindex,
				  const IMDRelation *pmdrel);

	// return the set of key columns from the given array of columns which appear
	// in the index included columns
	static CColRefSet *
	PcrsIndexIncludedCols(IMemoryPool *pmp, DrgPcr *pdrgpcr,
						  const IMDIndex *pmdindex, const IMDRelation *pmdrel);

	// check if an index is applicable given the required, output and scalar
	// expression columns
	static BOOL
	FIndexApplicable(IMemoryPool *pmp, const IMDIndex *pmdindex,
					 const IMDRelation *pmdrel, DrgPcr *pdrgpcrOutput,
	...
	static CExpression *
	PexprLogicalIndexGet(IMemoryPool *pmp, CMDAccessor *pmda,
						 CExpression *pexprGet, ULONG ulOriginOpId,
						 DrgPexpr *pdrgpexprConds, CColRefSet *pcrsReqd,
						 CColRefSet *pcrsScalarExpr, CColRefSet *pcrsOuterRefs,
						 const IMDIndex *pmdindex, const IMDRelation *pmdrel,
						 ...
						 CPartConstraint *ppcartcnstrIndex)
	{
		return PexprBuildIndexPlan(
			pmp, pmda, pexprGet, ulOriginOpId, pdrgpexprConds, pcrsReqd,
			pcrsScalarExpr, pcrsOuterRefs, pmdindex, pmdrel, fAllowPartialIndex,
	...
	static CExpression *
	PexprScalarBitmapBoolOp(IMemoryPool *pmp, CMDAccessor *pmda,
							CExpression *pexprOriginalPred, DrgPexpr *pdrgpexpr,
							CTableDescriptor *ptabdesc,
							const IMDRelation *pmdrel, DrgPcr *pdrgpcrOutput,
							CColRefSet *pcrsOuterRefs, CColRefSet *pcrsReqd,
							BOOL fConjunction, CExpression **ppexprRecheck,
	...
	// path expressions
	static CExpression *
	PexprBitmapBoolOp(IMemoryPool *pmp, IMDId *pmdidBitmapType,
					  CExpression *pexprLeft, CExpression *pexprRight,
					  BOOL fConjunction);

	// construct a bitmap index path expression for the given predicate
	static CExpression *
	PexprBitmap(IMemoryPool *pmp, CMDAccessor *pmda,
	...
	static void
	CreateBitmapIndexProbeOps(IMemoryPool *pmp, CMDAccessor *pmda,
							  CExpression *pexprOriginalPred,
							  DrgPexpr *pdrgpexpr, CTableDescriptor *ptabdesc,
							  const IMDRelation *pmdrel, DrgPcr *pdrgpcrOutput,
							  CColRefSet *pcrsOuterRefs, CColRefSet *pcrsReqd,
							  BOOL fConjunction, DrgPexpr *pdrgpexprBitmap,
							  DrgPexpr *pdrgpexprRecheck,
							  DrgPexpr *pdrgpexprResidual);

	// check if expression has any scalar node with ambiguous return type
	static BOOL
	FHasAmbiguousType(CExpression *pexpr, CMDAccessor *pmda);

	// construct a Bitmap(Dynamic)TableGet over BitmapBoolOp for the given
	// logical operator if bitmap indexes exist
	static CExpression *
	PexprBitmapTableGet(IMemoryPool *pmp, CLogical *popGet, ULONG ulOriginOpId,
	...
	// over bitmap bool op
	static CExpression *
	PexprSelect2BitmapBoolOp(IMemoryPool *pmp, CExpression *pexpr);

	// find a set of partial index combinations
	static DrgPdrgPpartdig *
	PdrgpdrgppartdigCandidates(
		IMemoryPool *pmp, CMDAccessor *pmda, DrgPexpr *pdrgpexprScalar,
		DrgDrgPcr *pdrgpdrgpcrPartKey, const IMDRelation *pmdrel,
		CPartConstraint *ppartcnstrRel, DrgPcr *pdrgpcrOutput,
	...
	static CPartConstraint *
	PpartcnstrUpdateCovered(IMemoryPool *pmp, CMDAccessor *pmda,
							DrgPexpr *pdrgpexprScalar,
							CPartConstraint *ppartcnstrCovered,
							CPartConstraint *ppartcnstr, DrgPcr *pdrgpcrOutput,
							DrgPexpr *pdrgpexprIndex,
							DrgPexpr *pdrgpexprResidual,
	...
	static CExpression *
	PexprRemapColumns(IMemoryPool *pmp, CExpression *pexprScalar,
					  DrgPcr *pdrgpcrA, DrgPcr *pdrgpcrRemappedA,
					  DrgPcr *pdrgpcrB, DrgPcr *pdrgpcrRemappedB);

	// construct a partial dynamic index get
	static CExpression *
	PexprPartialDynamicIndexGet(
		IMemoryPool *pmp, CLogicalDynamicGet *popGet, ULONG ulOriginOpId,
		DrgPexpr *pdrgpexprIndex, DrgPexpr *pdrgpexprResidual,
		DrgPcr *pdrgpcrDIG, const IMDIndex *pmdindex, const IMDRelation *pmdrel,
		CPartConstraint *ppartcnstr,
		CColRefSet *
			pcrsAcceptedOuterRefs,  // set of columns to be considered for index apply
	...
	static CExpression *
	PexprCTEConsumer(IMemoryPool *pmp, ULONG ulCTEId, DrgPcr *pdrgpcrConsumer);
	...
	static void
	MapPrjElemsWithDistinctAggs(IMemoryPool *pmp, CExpression *pexprPrjList,
								HMExprDrgPexpr **pphmexprdrgpexpr,
								ULONG *pulDifferentDQAs);

	// convert GbAgg with distinct aggregates to a join
	static CExpression *
	PexprGbAggOnCTEConsumer2Join(IMemoryPool *pmp, CExpression *pexprGbAgg);
