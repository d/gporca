										...
										CExpression *pexpr)
{
	...
	CExpression *pexprBinary = GPOS_NEW(pmp) CExpression(
		pmp, GPOS_NEW(pmp) T(pmp), pexprLeft, pexprRight, pexprScalar);

#ifdef GPOS_DEBUG
	COperator::EOperatorId eopid = pexprBinary->Pop()->Eopid();
#endif  // GPOS_DEBUG
	GPOS_ASSERT(COperator::EopPhysicalInnerNLJoin == eopid ||
...
template <class T>
void
CXformUtils::AddHashJoinAlternative(IMemoryPool *pmp, CExpression *pexprJoin,
									...
									CXformResult *pxfres)
{
	...
	CExpression *pexprResult = GPOS_NEW(pmp)
		CExpression(pmp, GPOS_NEW(pmp) T(pmp, pdrgpexprOuter, pdrgpexprInner),
...
void
CXformUtils::ImplementHashJoin(
	CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr,
...
)
{
	...
	if (NULL != pdrgpexprOuter)
	{
		...
		else
		{
			...
			AddHashJoinAlternative<T>(pmp, pexpr, pdrgpexprOuter,
	...
	BOOL fHashJoinPossible = CPhysicalJoin::FHashJoinPossible(
		pmp, pexpr, pdrgpexprOuter, pdrgpexprInner, &pexprInnerJoin);
	...
	{
		AddHashJoinAlternative<T>(pmp, pexprInnerJoin, pdrgpexprOuter,
								  pdrgpexprInner, pxfres);
	}
	else
	{
	...
	if (!fHashJoinPossible && fAntiSemiJoin)
	{
		...
		if (FProcessGPDBAntiSemiHashJoin(pmp, pexpr, &pexprProcessed))
		{
			...
			ImplementHashJoin<T>(pxfctxt, pxfres, pexprProcessed,
								 false /*fAntiSemiJoin*/);
