...
CDistributionSpecHashedNoOp::CDistributionSpecHashedNoOp(DrgPexpr *pdrgpexpr)
...
BOOL
CDistributionSpecHashedNoOp::FMatch(const CDistributionSpec *pds) const
{
	return pds->Edt() == Edt();
}

void
CDistributionSpecHashedNoOp::AppendEnforcers(IMemoryPool *pmp,
											 ...
											 CExpression *pexpr)
{
	...
	CDistributionSpecHashedNoOp *pdsNoOp = GPOS_NEW(pmp)
		CDistributionSpecHashedNoOp(pdrgpexprNoOpRedistributionColumns);
	pexpr->AddRef();
	CExpression *pexprMotion = GPOS_NEW(pmp) CExpression(
		pmp, GPOS_NEW(pmp) CPhysicalMotionHashDistribute(pmp, pdsNoOp), pexpr);
