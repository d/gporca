...
//---------------------------------------------------------------------------
void
CDrvdPropRelational::Derive(IMemoryPool *pmp, CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
DrgPfd *
CDrvdPropRelational::PdrgpfdChild(IMemoryPool *pmp, ULONG ulChildIndex,
	...
	for (ULONG ul = 0; ul < ulSize; ul++)
	{
		...
		if (pcrsOutput->FSubset(pfd->PcrsKey()))
		{
			...
			if (0 < pcrsDetermined->CElements())
			{
				...
				CFunctionalDependency *pfdNew = GPOS_NEW(pmp)
...
//---------------------------------------------------------------------------
DrgPfd *
CDrvdPropRelational::PdrgpfdLocal(IMemoryPool *pmp, CExpressionHandle &exprhdl)
{
	DrgPfd *pdrgpfd = GPOS_NEW(pmp) DrgPfd(pmp);
	...
	for (ULONG ul = 0; ul < ulKeys; ul++)
	{
		...
		if (0 < pcrsDetermined->CElements())
		{
			...
			CFunctionalDependency *pfdLocal =
				GPOS_NEW(pmp) CFunctionalDependency(pcrsKey, pcrsDetermined);
...
//---------------------------------------------------------------------------
DrgPfd *
CDrvdPropRelational::Pdrgpfd(IMemoryPool *pmp, CExpressionHandle &exprhdl)
{
	...
	CUtils::AddRefAppend<CFunctionalDependency, CleanupRelease>(pdrgpfd,
																pdrgpfdLocal);
