...
//
//---------------------------------------------------------------------------
CEnumeratorConfig::CEnumeratorConfig(IMemoryPool *pmp, ULLONG ullPlanId,
									 ULLONG ullSamples, CDouble dCostThreshold)
	: m_pmp(pmp),
...
//---------------------------------------------------------------------------
BOOL
CEnumeratorConfig::FAddSample(ULLONG ullPlanId, CCost cost)
{
...
CEnumeratorConfig::FitCostDistribution()
{
	...
	{
		pdObervationX[ul] =
			log2(CDouble((CostPlanSample(ul) / CostBest())).DVal());
...
//---------------------------------------------------------------------------
void
CEnumeratorConfig::DumpSamples(CWStringDynamic *pstr,  // samples dump
							   ULONG ulSessionId, ULONG ulCommandId)
{
	...
	CUtils::GenerateFileName(szFileName, "SamplePlans", "xml",
	...
	ULONG ulSessionId, ULONG ulCommandId)
{
	...
	CHAR szFileName[GPOS_FILE_NAME_BUF_SIZE];
	CUtils::GenerateFileName(szFileName, "CostDistr", "xml",
							 GPOS_FILE_NAME_BUF_SIZE, ulSessionId, ulCommandId);
	CHAR *sz = CUtils::SzFromWsz(m_pmp, const_cast<WCHAR *>(pstr->Wsz()));
	CIOUtils::Dump(szFileName, sz);
	GPOS_DELETE_ARRAY(sz);
