...
//---------------------------------------------------------------------------
CDXLMinidump *
CMinidumperUtils::PdxlmdLoad(IMemoryPool *pmp, const CHAR *szFileName)
{
	...
	CParseHandlerDXL *pphdxl =
		CDXLUtils::PphdxlParseDXLFile(pmp, szFileName, NULL /*szXSDPath*/);
	...
	return GPOS_NEW(pmp) CDXLMinidump(
		pbs, poconf, pdxlnQuery, pdrgpdxlnQueryOutput, pdrgpdxlnCTE, pdxlnPlan,
		pdrgpmdobj, pdrgpsysid, ullPlanId, ullPlanSpaceSize);
}
...
void
CMinidumperUtils::GenerateMinidumpFileName(
	CHAR *szBuf, ULONG ulLength, ULONG ulSessionId, ULONG ulCmdId,
...
)
{
	...
	if (NULL == szMinidumpFileName)
	{
		...
		CUtils::GenerateFileName(szBuf, "minidumps/Minidump", "mdp", ulLength,
								 ulSessionId, ulCmdId);
	}
	else
	{
		...
		clib::SzStrNCpy(szBuf + ulPrefixLength,
						szMinidumpFileName + ulNameStart, ulNameLength);
	}
}
...
//---------------------------------------------------------------------------
CDXLNode *
CMinidumperUtils::PdxlnExecuteMinidump(IMemoryPool *pmp, const CHAR *szFileName,
									   ULONG ulSegments, ULONG ulSessionId,
									   ULONG ulCmdId, COptimizerConfig *poconf,
									   IConstExprEvaluator *pceeval)
{
	...
	CDXLNode *pdxlnPlan =
		PdxlnExecuteMinidump(pmp, pdxlmd, szFileName, ulSegments, ulSessionId,
							 ulCmdId, poconf, pceeval);
...
//---------------------------------------------------------------------------
CDXLNode *
CMinidumperUtils::PdxlnExecuteMinidump(IMemoryPool *pmp, CDXLMinidump *pdxlmd,
									   const CHAR *szFileName, ULONG ulSegments,
									   ...
									   IConstExprEvaluator *pceeval)
{
	...
	CDXLNode *result = CMinidumperUtils::PdxlnExecuteMinidump(
		pmp, factory.Pmda(), pdxlmd, szFileName, ulSegments, ulSessionId,
		ulCmdId, poconf, pceeval);
...
//---------------------------------------------------------------------------
CDXLNode *
CMinidumperUtils::PdxlnExecuteMinidump(IMemoryPool *pmp, CMDAccessor *pmda,
									   CDXLMinidump *pdxlmd,
									   const CHAR *szFileName, ULONG ulSegments,
									   ULONG ulSessionId, ULONG ulCmdId,
									   COptimizerConfig *poconf,
									   IConstExprEvaluator *pceeval)
{
	...
	GPOS_ASSERT(NULL != pdxlmd->PdxlnQuery() &&
				...
				NULL != pdxlmd->PdrgpdxlnCTE() && "No query found in Minidump");
	GPOS_ASSERT(NULL != pdxlmd->Pdrgpmdobj() && NULL != pdxlmd->Pdrgpsysid() &&
	...
	GPOS_TRY_HDL(&errhdl)
	{
		pdxlnPlan = COptimizer::PdxlnOptimize(
			pmp, pmda, pdxlmd->PdxlnQuery(), pdxlmd->PdrgpdxlnQueryOutput(),
			pdxlmd->PdrgpdxlnCTE(), pceeval, ulSegments, ulSessionId, ulCmdId,
			NULL,  // pdrgpss
			poconf, szFileName);
	}
	GPOS_CATCH_EX(ex)
