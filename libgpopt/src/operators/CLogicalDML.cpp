...
//
//---------------------------------------------------------------------------
CLogicalDML::CLogicalDML(IMemoryPool *pmp)
	: CLogical(pmp),
	  m_ptabdesc(NULL),
...
//
//---------------------------------------------------------------------------
CLogicalDML::CLogicalDML(IMemoryPool *pmp, EDMLOperator edmlop,
...
//---------------------------------------------------------------------------
BOOL
CLogicalDML::FMatch(COperator *pop) const
{
	if (pop->Eopid() != Eopid())
	{
		return false;
...
//---------------------------------------------------------------------------
COperator *
CLogicalDML::PopCopyWithRemappedColumns(IMemoryPool *pmp, HMUlCr *phmulcr,
										BOOL fMustExist)
{
	DrgPcr *pdrgpcr =
		CUtils::PdrgpcrRemap(pmp, m_pdrgpcrSource, phmulcr, fMustExist);
	...
	return GPOS_NEW(pmp)
		CLogicalDML(pmp, m_edmlop, m_ptabdesc, pdrgpcr, m_pbsModified,
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalDML::PcrsDeriveOutput(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
CPropConstraint *
CLogicalDML::PpcDeriveConstraint(IMemoryPool *pmp,
								 CExpressionHandle &exprhdl) const
{
	CColRefSet *pcrsOutput = GPOS_NEW(pmp) CColRefSet(pmp);
	pcrsOutput->Include(m_pdrgpcrSource);
	CPropConstraint *ppc =
		PpcDeriveConstraintRestrict(pmp, exprhdl, pcrsOutput);
	pcrsOutput->Release();
...
//---------------------------------------------------------------------------
CKeyCollection *
CLogicalDML::PkcDeriveKeys(IMemoryPool *,  // pmp
						   CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
CMaxCard
CLogicalDML::Maxcard(IMemoryPool *,  // pmp
					 CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
CXformSet *
CLogicalDML::PxfsCandidates(IMemoryPool *pmp) const
{
...
//---------------------------------------------------------------------------
IStatistics *
CLogicalDML::PstatsDerive(IMemoryPool *,  // pmp,
						  CExpressionHandle &exprhdl,
						  DrgPstat *  // not used
						  ) const
{
