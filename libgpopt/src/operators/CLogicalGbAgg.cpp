...
//
//---------------------------------------------------------------------------
CLogicalGbAgg::CLogicalGbAgg(IMemoryPool *pmp)
	: CLogicalUnary(pmp),
	  m_pdrgpcr(NULL),
...
//
//---------------------------------------------------------------------------
CLogicalGbAgg::CLogicalGbAgg(IMemoryPool *pmp, DrgPcr *pdrgpcr,
...
CLogicalGbAgg::CLogicalGbAgg(IMemoryPool *pmp, DrgPcr *pdrgpcr,
							 COperator::EGbAggType egbaggtype,
							 BOOL fGeneratesDuplicates, DrgPcr *pdrgpcrArgDQA)
	: CLogicalUnary(pmp),
	  m_pdrgpcr(pdrgpcr),
	  m_pdrgpcrMinimal(NULL),
	  m_egbaggtype(egbaggtype),
	  ...
	  m_pdrgpcrArgDQA(NULL)
{
	...
	GPOS_ASSERT_IMP(NULL != pdrgpcrMinimal,
					pdrgpcrMinimal->UlLength() <= pdrgpcr->UlLength());
...
//---------------------------------------------------------------------------
COperator *
CLogicalGbAgg::PopCopyWithRemappedColumns(IMemoryPool *pmp, HMUlCr *phmulcr,
										  BOOL fMustExist)
{
	...
	{
		pdrgpcrMinimal =
			CUtils::PdrgpcrRemap(pmp, m_pdrgpcrMinimal, phmulcr, fMustExist);
	}
	...
	{
		pdrgpcrArgDQA =
			CUtils::PdrgpcrRemap(pmp, m_pdrgpcrArgDQA, phmulcr, fMustExist);
	}
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalGbAgg::PcrsDeriveOutput(IMemoryPool *pmp, CExpressionHandle &exprhdl)
{
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalGbAgg::PcrsDeriveOuter(IMemoryPool *pmp, CExpressionHandle &exprhdl)
{
...
//---------------------------------------------------------------------------
CPropConstraint *
CLogicalGbAgg::PpcDeriveConstraint(IMemoryPool *pmp,
								   CExpressionHandle &exprhdl) const
{
	...
	CPropConstraint *ppc =
		PpcDeriveConstraintRestrict(pmp, exprhdl, pcrsGrouping);
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalGbAgg::PcrsStat(IMemoryPool *pmp, CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalGbAgg::PcrsStatGbAgg(IMemoryPool *pmp, CExpressionHandle &exprhdl,
							 CColRefSet *pcrsInput, ULONG ulChildIndex,
							 DrgPcr *pdrgpcrGrp) const
{
	...
	CColRefSet *pcrsRequired =
		PcrsReqdChildStats(pmp, exprhdl, pcrsInput, pcrs, ulChildIndex);
	pcrs->Release();
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalGbAgg::PcrsDeriveNotNull(IMemoryPool *pmp,
								 CExpressionHandle &exprhdl) const
{
	GPOS_ASSERT(2 == exprhdl.UlArity());
...
CLogicalGbAgg::UlHash() const
{
	...
	return gpos::UlCombineHashes(ulHash,
								 gpos::UlHash<BOOL>(&m_fGeneratesDuplicates));
}
...
//---------------------------------------------------------------------------
CKeyCollection *
CLogicalGbAgg::PkcDeriveKeys(IMemoryPool *pmp, CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
CMaxCard
CLogicalGbAgg::Maxcard(IMemoryPool *,  //pmp
					   CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
BOOL
CLogicalGbAgg::FMatch(COperator *pop) const
{
	if (pop->Eopid() != Eopid())
	{
		return false;
...
//---------------------------------------------------------------------------
CXformSet *
CLogicalGbAgg::PxfsCandidates(IMemoryPool *pmp) const
{
...
//---------------------------------------------------------------------------
IStatistics *
CLogicalGbAgg::PstatsDerive(IMemoryPool *pmp, IStatistics *pstatsChild,
							DrgPcr *pdrgpcrGroupingCols,
							DrgPul *pdrgpulComputedCols, CBitSet *pbsKeys)
{
	...
	IStatistics *pstats = CGroupByStatsProcessor::PstatsGroupBy(
		pmp, dynamic_cast<CStatistics *>(pstatsChild), pdrgpulGroupingCols,
		pdrgpulComputedCols, pbsKeys);
...
//---------------------------------------------------------------------------
IStatistics *
CLogicalGbAgg::PstatsDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl,
							DrgPstat *  // not used
							) const
{
	...
	exprhdl.Pdpscalar(1 /*ulChildIndex*/)
		->PcrsDefined()
		->ExtractColIds(pmp, pdrgpulComputedCols);

	IStatistics *pstats = PstatsDerive(pmp, pstatsChild, Pdrgpcr(),
									   pdrgpulComputedCols, NULL /*pbsKeys*/);
