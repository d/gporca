...
//
//---------------------------------------------------------------------------
CLogicalSplit::CLogicalSplit(IMemoryPool *pmp)
	: CLogical(pmp),
...
//
//---------------------------------------------------------------------------
CLogicalSplit::CLogicalSplit(IMemoryPool *pmp, DrgPcr *pdrgpcrDelete,
							 DrgPcr *pdrgpcrInsert, CColRef *pcrCtid,
...
//---------------------------------------------------------------------------
BOOL
CLogicalSplit::FMatch(COperator *pop) const
{
	if (pop->Eopid() == Eopid())
	{
...
CLogicalSplit::UlHash() const
{
	ULONG ulHash = gpos::UlCombineHashes(
		COperator::UlHash(), CUtils::UlHashColArray(m_pdrgpcrInsert));
...
//---------------------------------------------------------------------------
COperator *
CLogicalSplit::PopCopyWithRemappedColumns(IMemoryPool *pmp, HMUlCr *phmulcr,
										  BOOL fMustExist)
{
	...
	return GPOS_NEW(pmp)
		CLogicalSplit(pmp, pdrgpcrDelete, pdrgpcrInsert, pcrCtid, pcrSegmentId,
					  pcrAction, pcrTupleOid);
}
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalSplit::PcrsDeriveOutput(IMemoryPool *pmp, CExpressionHandle &exprhdl)
{
...
//---------------------------------------------------------------------------
CKeyCollection *
CLogicalSplit::PkcDeriveKeys(IMemoryPool *,  // pmp
...
//---------------------------------------------------------------------------
CMaxCard
CLogicalSplit::Maxcard(IMemoryPool *,  // pmp
					   CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
CXformSet *
CLogicalSplit::PxfsCandidates(IMemoryPool *pmp) const
{
...
//---------------------------------------------------------------------------
IStatistics *
CLogicalSplit::PstatsDerive(IMemoryPool *pmp, CExpressionHandle &exprhdl,
							DrgPstat *  // not used
							) const
{
