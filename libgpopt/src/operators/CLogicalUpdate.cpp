...
//
//---------------------------------------------------------------------------
CLogicalUpdate::CLogicalUpdate(IMemoryPool *pmp)
	: CLogical(pmp),
	  m_ptabdesc(NULL),
...
//
//---------------------------------------------------------------------------
CLogicalUpdate::CLogicalUpdate(IMemoryPool *pmp, CTableDescriptor *ptabdesc,
							   DrgPcr *pdrgpcrDelete, DrgPcr *pdrgpcrInsert,
...
//---------------------------------------------------------------------------
BOOL
CLogicalUpdate::FMatch(COperator *pop) const
{
	if (pop->Eopid() != Eopid())
	{
		return false;
...
CLogicalUpdate::UlHash() const
{
	...
	ulHash =
		gpos::UlCombineHashes(ulHash, CUtils::UlHashColArray(m_pdrgpcrDelete));
	ulHash =
		gpos::UlCombineHashes(ulHash, CUtils::UlHashColArray(m_pdrgpcrInsert));
	ulHash = gpos::UlCombineHashes(ulHash, gpos::UlHashPtr<CColRef>(m_pcrCtid));
	ulHash =
		gpos::UlCombineHashes(ulHash, gpos::UlHashPtr<CColRef>(m_pcrSegmentId));
...
//---------------------------------------------------------------------------
COperator *
CLogicalUpdate::PopCopyWithRemappedColumns(IMemoryPool *pmp, HMUlCr *phmulcr,
										   BOOL fMustExist)
{
	DrgPcr *pdrgpcrDelete =
		CUtils::PdrgpcrRemap(pmp, m_pdrgpcrDelete, phmulcr, fMustExist);
	DrgPcr *pdrgpcrInsert =
		CUtils::PdrgpcrRemap(pmp, m_pdrgpcrInsert, phmulcr, fMustExist);
	CColRef *pcrCtid = CUtils::PcrRemap(m_pcrCtid, phmulcr, fMustExist);
	CColRef *pcrSegmentId =
		CUtils::PcrRemap(m_pcrSegmentId, phmulcr, fMustExist);
	m_ptabdesc->AddRef();
	...
	return GPOS_NEW(pmp)
		CLogicalUpdate(pmp, m_ptabdesc, pdrgpcrDelete, pdrgpcrInsert, pcrCtid,
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalUpdate::PcrsDeriveOutput(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
CKeyCollection *
CLogicalUpdate::PkcDeriveKeys(IMemoryPool *,  // pmp
...
//---------------------------------------------------------------------------
CMaxCard
CLogicalUpdate::Maxcard(IMemoryPool *,  // pmp
						CExpressionHandle &exprhdl) const
{
...
//---------------------------------------------------------------------------
CXformSet *
CLogicalUpdate::PxfsCandidates(IMemoryPool *pmp) const
{
...
//---------------------------------------------------------------------------
IStatistics *
CLogicalUpdate::PstatsDerive(IMemoryPool *,  // pmp,
							 CExpressionHandle &exprhdl,
							 DrgPstat *  // not used
							 ) const
{
