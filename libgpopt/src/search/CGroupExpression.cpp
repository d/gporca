...
//
//---------------------------------------------------------------------------
CGroupExpression::CGroupExpression(IMemoryPool *pmp, COperator *pop,
	  ...
	  m_ppartialplancostmap(NULL)
{
	...
	{
		m_pdrgpgroupSorted =
			GPOS_NEW(pmp) DrgPgroup(pmp, pdrgpgroup->UlLength());
	...
	m_sht.Init(
		pmp, GPOPT_COSTCTXT_HT_BUCKETS, GPOS_OFFSET(CCostContext, m_link),
		GPOS_OFFSET(CCostContext, m_poc), &(COptimizationContext::m_pocInvalid),
		COptimizationContext::UlHash, COptimizationContext::FEqual);
}
...
//---------------------------------------------------------------------------
void
CGroupExpression::Init(CGroup *pgroup, ULONG ulId)
{
	SetGroup(pgroup);
	SetId(ulId);
...
//---------------------------------------------------------------------------
BOOL
CGroupExpression::FValidContext(IMemoryPool *pmp, COptimizationContext *poc,
								DrgPoc *pdrgpocChild)
{
...
//---------------------------------------------------------------------------
void
CGroupExpression::SetId(ULONG ulId)
{
...
//---------------------------------------------------------------------------
BOOL
CGroupExpression::FCostContextExists(COptimizationContext *poc, DrgPoc *pdrgpoc)
{
...
CCostContext *
CGroupExpression::PccComputeCost(
	IMemoryPool *pmp, COptimizationContext *poc, ULONG ulOptReq,
...
//---------------------------------------------------------------------------
CCost
CGroupExpression::CostLowerBound(IMemoryPool *pmp, CReqdPropPlan *prppInput,
								 CCostContext *pccChild, ULONG ulChildIndex)
{
	...
	CPartialPlan *ppp =
		GPOS_NEW(pmp) CPartialPlan(this, prppInput, pccChild, ulChildIndex);
...
//---------------------------------------------------------------------------
CCost
CGroupExpression::CostCompute(IMemoryPool *pmp, CCostContext *pcc) const
{
	GPOS_ASSERT(NULL != pcc);
	...
	for (ULONG ul = 0; ul < ulLen; ul++)
	{
		...
		pdrgpcostChildren->Append(GPOS_NEW(pmp)
...
//---------------------------------------------------------------------------
DrgPcc *
CGroupExpression::PdrgpccLookupAll(IMemoryPool *pmp, COptimizationContext *poc)
{
	GPOS_ASSERT(NULL != poc);
...
//---------------------------------------------------------------------------
void
CGroupExpression::PreprocessTransform(IMemoryPool *pmpLocal,
									  IMemoryPool *pmpGlobal, CXform *pxform)
{
...
//---------------------------------------------------------------------------
void
CGroupExpression::PostprocessTransform(IMemoryPool *,  // pmpLocal
...
void
CGroupExpression::Transform(
	IMemoryPool *pmp, IMemoryPool *pmpLocal, CXform *pxform,
	CXformResult *pxfres,
...
//---------------------------------------------------------------------------
BOOL
CGroupExpression::FMatch(const CGroupExpression *pgexpr) const
{
	GPOS_ASSERT(NULL != pgexpr);
...
//---------------------------------------------------------------------------
ULONG
CGroupExpression::UlHash(COperator *pop, DrgPgroup *pdrgpgroup)
{
	GPOS_ASSERT(NULL != pop);
	GPOS_ASSERT(NULL != pdrgpgroup);
...
//---------------------------------------------------------------------------
ULONG
CGroupExpression::UlHash(const CGroupExpression &gexpr)
{
	return gexpr.UlHash();
}
...
//---------------------------------------------------------------------------
IStatistics *
CGroupExpression::PstatsRecursiveDerive(IMemoryPool *,  // pmpLocal
										IMemoryPool *pmpGlobal,
...
//---------------------------------------------------------------------------
void
CGroupExpression::PrintXform(IMemoryPool *pmp, CXform *pxform,
...
//---------------------------------------------------------------------------
IOstream &
CGroupExpression::OsPrintCostContexts(IOstream &os, const CHAR *szPrefix)
{
...
//---------------------------------------------------------------------------
IOstream &
CGroupExpression::OsPrint(IOstream &os, const CHAR *szPrefix)
{
	os << szPrefix << m_ulId << ": ";
	...
	{
		os << " Dup. of GrpExpr " << m_pgexprDuplicate->UlId() << " in Grp "
		   << m_pgexprDuplicate->Pgroup()->UlId();
	}
	...
	if (GPOS_FTRACE(EopttracePrintXform) && ExfidOrigin() != CXform::ExfInvalid)
	{
		...
		os << ", Grp: " << m_pgexprOrigin->Pgroup()->UlId()
		   << ", GrpExpr: " << m_pgexprOrigin->UlId() << ")";
	}
	os << std::endl;
