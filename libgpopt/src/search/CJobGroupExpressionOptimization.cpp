	...
	CSchedulerContext *psc)
{
	...
	m_pexprhdlPlan =
		GPOS_NEW(psc->PmpGlobal()) CExpressionHandle(psc->PmpGlobal());
	...
	CGroupExpression *pgexprForStats =
		m_pgexpr->Pgroup()->PgexprBestPromise(psc->PmpGlobal(), m_pgexpr);
	...
	m_pdrgpstatCurrentCtxt =
		GPOS_NEW(psc->PmpGlobal()) DrgPstat(psc->PmpGlobal());
												...
												CJob *pjOwner)
{
	...
	if (psc->Peng()->FSafeToPrune(pjgeo->m_pgexpr, pjgeo->m_poc->Prpp(),
								  NULL /*pccChild*/, ULONG_MAX /*ulChildIndex*/,
								  &costLowerBound))
	{
		(void) pjgeo->m_pgexpr->PccComputeCost(
			psc->PmpGlobal(), pjgeo->m_poc, pjgeo->m_ulOptReq, NULL /*pdrgpoc*/,
...
CJobGroupExpressionOptimization::DerivePrevChildProps(CSchedulerContext *psc)
{
	...
	COptimizationContext *pocChild = pgroupChild->PocLookupBest(
		psc->PmpGlobal(), psc->Peng()->UlSearchStages(),
								  ...
								  ulPrevChildIndex, &costLowerBound))
	{
		...
		(void) m_pgexpr->PccComputeCost(psc->PmpGlobal(), m_poc, m_ulOptReq,
...
CJobGroupExpressionOptimization::ScheduleChildGroupsJobs(CSchedulerContext *psc)
{
	...
	COptimizationContext *pocChild = GPOS_NEW(psc->PmpGlobal())
		COptimizationContext(psc->PmpGlobal(), pgroupChild,
							 m_pexprhdlPlan->Prpp(m_ulChildIndex), prprel,
							 pdrgpstatCtxt, psc->Peng()->UlCurrSearchStage());
												  ...
												  CJob *pjOwner)
{
	...
	pjgeo->m_pdrgpoc =
		psc->Peng()->PdrgpocChildren(psc->PmpGlobal(), *pjgeo->m_pexprhdlPlan);

	// enforce physical properties
	BOOL fCheckEnfdProps = psc->Peng()->FCheckEnfdProps(
		psc->PmpGlobal(), pjgeo->m_pgexpr, pjgeo->m_poc, pjgeo->m_ulOptReq,
		pjgeo->m_pdrgpoc);
	if (fCheckEnfdProps)
	{
												  ...
												  CJob *pjOwner)
{
	...
	CCostContext *pcc =
		pgexpr->PccComputeCost(psc->PmpGlobal(), poc, ulOptReq, pdrgpoc,
	...
	ULONG ulOptReq, CJob *pjParent)
{
	...
	CReqdPropPlan *prppCTEProducer = COptimizationContext::PrppCTEProducer(
		psc->PmpGlobal(), poc, psc->Peng()->UlSearchStages());
	if (NULL == prppCTEProducer)
	{
