...
//
//---------------------------------------------------------------------------
CMemo::CMemo(IMemoryPool *pmp)
	: m_pmp(pmp), m_pgroupRoot(NULL), m_ulpGrps(0), m_pmemotmap(NULL)
{
	GPOS_ASSERT(NULL != pmp);

	m_sht.Init(
		pmp, GPOPT_MEMO_HT_BUCKETS, GPOS_OFFSET(CGroupExpression, m_linkMemo),
		0, /*cKeyOffset (0 because we use CGroupExpression class as key)*/
		&(CGroupExpression::m_gexprInvalid), CGroupExpression::UlHash,
...
//---------------------------------------------------------------------------
CExpression *
CMemo::PexprExtractPlan(IMemoryPool *pmp, CGroup *pgroupRoot,
						CReqdPropPlan *prppInput, ULONG ulSearchStages)
{
	...
	for (ULONG i = 0; i < ulArity; i++)
	{
		...
		CExpression *pexprChild =
			PexprExtractPlan(pmp, pgroupChild, prpp, ulSearchStages);
		pdrgpexpr->Append(pexprChild);
	}

	pgexprBest->Pop()->AddRef();
	CExpression *pexpr = GPOS_NEW(pmp) CExpression(
		pmp, pgexprBest->Pop(), pgexprBest, pdrgpexpr, pstats, cost);
...
CMemo::BuildTreeMap(COptimizationContext *poc)
{
	...
	m_pmemotmap =
		GPOS_NEW(m_pmp) MemoTreeMap(m_pmp, CExpression::PexprRehydrate);
	m_pgroupRoot->BuildTreeMap(m_pmp, poc, NULL /*pccParent*/,
							   ULONG_MAX /*ulChildIndex*/, m_pmemotmap);
}
