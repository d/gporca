...
//
//---------------------------------------------------------------------------
CScheduler::CScheduler(IMemoryPool *pmp, ULONG ulJobs, ULONG_PTR ulpTasks
#ifdef GPOS_DEBUG
					   ,
					   BOOL fTrackingJobs
#endif  // GPOS_DEBUG
...
CScheduler::~CScheduler()
{
	GPOS_ASSERT_IMP(!ITask::PtskSelf()->FPendingExc(), 0 == m_ulpTotal);
...
//---------------------------------------------------------------------------
void
CScheduler::ProcessJobs(CSchedulerContext *psc)
{
	while (true)
	{
		IncTasksActive();
