...
//
//---------------------------------------------------------------------------
CXformCTEAnchor2Sequence::CXformCTEAnchor2Sequence(IMemoryPool *pmp)
	: CXformExploration(
		  // pattern
...
CXformCTEAnchor2Sequence::Exfp(CExpressionHandle &exprhdl) const
{
	...
	const ULONG ulConsumers =
		COptCtxt::PoctxtFromTLS()->Pcteinfo()->UlConsumers(ulId);
									...
									CExpression *pexpr) const
{
	...
	CExpression *pexprProducer =
		COptCtxt::PoctxtFromTLS()->Pcteinfo()->PexprCTEProducer(ulId);
	...
	CExpression *pexprSequence = GPOS_NEW(pmp) CExpression(
		pmp, GPOS_NEW(pmp) CLogicalSequence(pmp), pexprProducer, pexprChild);
