...
//
//---------------------------------------------------------------------------
CXformCollapseGbAgg::CXformCollapseGbAgg(IMemoryPool *pmp)
	: CXformExploration(
		  // pattern
							   ...
							   CExpression *pexpr) const
{
	...
	if (0 < pexprTopProjectList->UlArity() ||
		0 < pexprBottomProjectList->UlArity())
	{
	...
	CColRefSet *pcrsTopGrpCols =
		GPOS_NEW(pmp) CColRefSet(pmp, popTopGbAgg->Pdrgpcr());
	CColRefSet *pcrsBottomGrpCols =
		GPOS_NEW(pmp) CColRefSet(pmp, popBottomGbAgg->Pdrgpcr());
	...
	CExpression *pexprGbAggNew = GPOS_NEW(pmp)
		CExpression(pmp, popTopGbAgg, pexprSelect, pexprTopProjectList);
