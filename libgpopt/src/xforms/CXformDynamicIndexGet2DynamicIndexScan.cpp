...
CXformDynamicIndexGet2DynamicIndexScan::CXformDynamicIndexGet2DynamicIndexScan(
	...
	: CXformImplementation(
		  ...
		  GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) CLogicalDynamicIndexGet(pmp),
			  GPOS_NEW(pmp) CExpression(
				  pmp,
				  GPOS_NEW(pmp) CPatternLeaf(pmp))  // index lookup predicate
			  ))
{
}
												  ...
												  CExpression *pexpr) const
{
	...
	CExpression *pexprAlt = GPOS_NEW(pmp) CExpression(
		...
		GPOS_NEW(pmp) CPhysicalDynamicIndexScan(
			pmp, popIndexGet->FPartial(), pindexdesc, ptabdesc,
			pexpr->Pop()->UlOpId(), pname, pdrgpcrOutput,
			popIndexGet->UlScanId(), pdrgpdrgpcrPart,
			popIndexGet->UlSecondaryScanId(), ppartcnstr, ppartcnstrRel, pos),
		pexprIndexCond);
