...
//
//---------------------------------------------------------------------------
CXformImplementLimit::CXformImplementLimit(IMemoryPool *pmp)
	:  // pattern
	  CXformImplementation(GPOS_NEW(pmp) CExpression(
		  ...
		  GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) CPatternLeaf(pmp)),  // scalar child for offset
		  GPOS_NEW(pmp)
			  CExpression(pmp,
								...
								CExpression *pexpr) const
{
	...
	CExpression *pexprLimit = GPOS_NEW(pmp)
		CExpression(pmp,
					GPOS_NEW(pmp) CPhysicalLimit(pmp, pos, popLimit->FGlobal(),
												 popLimit->FHasCount(),
