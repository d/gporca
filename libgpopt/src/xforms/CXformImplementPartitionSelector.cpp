...
CXformImplementPartitionSelector::CXformImplementPartitionSelector(
	...
	:  // pattern
	  CXformImplementation(GPOS_NEW(pmp) CExpression(
		  pmp, GPOS_NEW(pmp) CLogicalPartitionSelector(pmp),
											...
											CExpression *pexpr) const
{
	...
	CPhysicalPartitionSelectorDML *popPhysicalPartitionSelector =
		GPOS_NEW(pmp) CPhysicalPartitionSelectorDML(pmp, pmdid, phmulexprFilter,
													popSelector->PcrOid());

	CExpression *pexprPartitionSelector = GPOS_NEW(pmp)
		CExpression(pmp, popPhysicalPartitionSelector, pexprRelational);
