...
//
//---------------------------------------------------------------------------
CXformIntersect2Join::CXformIntersect2Join(IMemoryPool *pmp)
	: CXformExploration(
		  // pattern
		  GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) CLogicalIntersect(pmp),
			  GPOS_NEW(pmp) CExpression(
				  pmp,
								...
								CExpression *pexpr) const
{
	...
	CExpression *pexprLeftAgg = CUtils::PexprLogicalGbAggGlobal(
		pmp, (*pdrgpdrgpcrInput)[0], pexprLeftChild, pexprProjList);
	CExpression *pexprRightAgg = CUtils::PexprLogicalGbAggGlobal(
		pmp, (*pdrgpdrgpcrInput)[1], pexprRightChild, pexprProjList);
