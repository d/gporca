...
//
//---------------------------------------------------------------------------
CXformPushDownLeftOuterJoin::CXformPushDownLeftOuterJoin(IMemoryPool *pmp)
	: CXformExploration(
		  // pattern
		  GPOS_NEW(pmp) CExpression(
			  ...
			  (pmp, GPOS_NEW(pmp) CLogicalNAryJoin(pmp),
			   ...
			   GPOS_NEW(pmp) CExpression(
				   pmp,
				   GPOS_NEW(pmp) CPatternTree(pmp))  // NAry-join predicate tree
			   ),
			  GPOS_NEW(pmp) CExpression(
				  pmp,
				  GPOS_NEW(pmp) CPatternLeaf(pmp)),  // inner child is a leaf
			  GPOS_NEW(pmp) CExpression(
				  pmp, GPOS_NEW(pmp) CPatternTree(pmp))  // LOJ predicate tree
			  ))
{
}
									   ...
									   CExpression *pexpr) const
{
	...
	CColRefSet *pcrsLOJUsed =
		CDrvdPropScalar::Pdpscalar(pexprLOJScalarChild->PdpDerive())
			->PcrsUsed();
	...
	if (1 < pdrgpexprLOJChildren->UlLength())
	{
		...
		pexprLOJOuterChild = GPOS_NEW(pmp) CExpression(
			pmp, GPOS_NEW(pmp) CLogicalNAryJoin(pmp), pdrgpexprLOJChildren);
	...
	// build new LOJ
	CExpression *pexprLOJNew = GPOS_NEW(pmp) CExpression(
		pmp, GPOS_NEW(pmp) CLogicalLeftOuterJoin(pmp), pdrgpexprLOJChildren);
	...
	CExpression *pexprNAryJoinNew = GPOS_NEW(pmp) CExpression(
		pmp, GPOS_NEW(pmp) CLogicalNAryJoin(pmp), pdrgpexprNAryJoinChildren);

	// normalize resulting expression and add it to xform results
	CExpression *pexprResult =
		CNormalizer::PexprNormalize(pmp, pexprNAryJoinNew);
