...
//
//---------------------------------------------------------------------------
CXformSplitDQA::CXformSplitDQA(IMemoryPool *pmp)
	: CXformExploration(
		  // pattern
...
CXformSplitDQA::Exfp(CExpressionHandle &exprhdl) const
{
	...
	if (!CLogicalGbAgg::PopConvert(exprhdl.Pop())->FGlobal() ||
		...
		0 < CDrvdPropRelational::Pdprel(exprhdl.Pdp())
				->PcrsOuter()
				->CElements() ||
		CXformUtils::FHasAmbiguousType(
			exprhdl.PexprScalarChild(1 /*ulChildIndex*/),
						  ...
						  CExpression *pexpr) const
{
	...
	ExtractDistinctCols(pmp, pcf, pmda, pexprProjectList, pdrgpexprChildPrEl,
	...
	if (0 < pdrgpexprChildPrEl->UlLength())
	{
		...
		CExpression *pexprChildProject = CUtils::PexprLogicalProject(
			pmp, pexprRelational,
			GPOS_NEW(pmp) CExpression(
				pmp, GPOS_NEW(pmp) CScalarProjectList(pmp), pdrgpexprChildPrEl),
	...
	CExpression *pexprAlt1 =
		PexprSplitHelper(pmp, pcf, pmda, pexpr, pexprRelational, phmexprcr,
						 pdrgpcrArgDQA, false /*fSpillTo2Level*/
		);
	...
	if (!(fForce3StageScalarDQA && fScalarDQA))
	{
		...
		CExpression *pexprAlt2 = PexprSplitIntoLocalDQAGlobalAgg(
			pmp, pcf, pmda, pexpr, pexprRelational, phmexprcr, pdrgpcrArgDQA);
	...
	if (fScalarDQA && !fForce3StageScalarDQA)
	{
		...
		CExpression *pexprAlt3 =
			PexprSplitHelper(pmp, pcf, pmda, pexpr, pexprRelational, phmexprcr,
							 pdrgpcrArgDQA, true /*fSpillTo2Level*/
			);
...
CExpression *
CXformSplitDQA::PexprSplitIntoLocalDQAGlobalAgg(
	IMemoryPool *pmp, CColumnFactory *pcf, CMDAccessor *pmda,
	CExpression *pexpr, CExpression *pexprRelational, HMExprCr *phmexprcr,
	DrgPcr *pdrgpcrArgDQA)
{
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		if (popScAggFunc->FDistinct())
		{
			...
			CScalarAggFunc *popScAggFuncLocal = CUtils::PopAggFunc(
				pmp, popScAggFunc->Pmdid(),
				GPOS_NEW(pmp)
					CWStringConst(pmp, popScAggFunc->PstrAggFunc()->Wsz()),
				true /* fDistinct */, EaggfuncstageLocal /*eaggfuncstage*/,
				true /* fSplit */
			);
		...
		else
		{
			...
			PopulatePrLMultiPhaseAgg(
				pmp, pcf, pmda, pexprPrEl, pdrgpexprPrElFirstStage,
	...
	CExpression *pexprGlobal = PexprMultiLevelAggregation(
		pmp, pexprRelational, pdrgpexprPrElFirstStage,
		NULL, /* pdrgpexprPrElSecondStage */
...
//---------------------------------------------------------------------------
CExpression *
CXformSplitDQA::PexprSplitHelper(IMemoryPool *pmp, CColumnFactory *pcf,
								 CMDAccessor *pmda, CExpression *pexpr,
								 CExpression *pexprRelational,
								 HMExprCr *phmexprcr, DrgPcr *pdrgpcrArgDQA,
								 BOOL fSpillTo2Level)
{
	CExpression *pexprPrL = (*pexpr)[1];
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		else
		{
			...
			PopulatePrLMultiPhaseAgg(pmp, pcf, pmda, pexprPrEl,
	...
	CExpression *pexprGlobal = PexprMultiLevelAggregation(
		pmp, pexprRelational, pdrgpexprPrElFirstStage, pdrgpexprPrElSecondStage,
...
//---------------------------------------------------------------------------
CExpression *
CXformSplitDQA::PexprPrElAgg(IMemoryPool *pmp, CExpression *pexprAggFunc,
...
//---------------------------------------------------------------------------
void
CXformSplitDQA::PopulatePrLMultiPhaseAgg(IMemoryPool *pmp, CColumnFactory *pcf,
										 CMDAccessor *pmda,
										 ...
										 BOOL fSplit2LevelsOnly)
{
	...
	CExpression *pexprPrElFirstStage =
		PexprPrElAgg(pmp, pexprAggFunc, EaggfuncstageLocal,
	...
	CExpression *pexprPrElSecondStage = PexprPrElAgg(
		pmp, pexprAggFunc, eaggfuncstage, pcrLocal, pcrSecondStage);
	if (fSplit2LevelsOnly)
	{
	...
	CExpression *pexprPrElGlobal = PexprPrElAgg(
		pmp, pexprAggFunc, EaggfuncstageGlobal, pcrSecondStage, pcrGlobal);
...
//---------------------------------------------------------------------------
CColRef *
CXformSplitDQA::PcrAggFuncArgument(IMemoryPool *pmp, CMDAccessor *pmda,
								   CColumnFactory *pcf, CExpression *pexprArg,
								   DrgPexpr *pdrgpexprChildPrEl)
{
	...
	CColRef *pcrAdditionalGrpCol =
		pcf->PcrCreate(pmdtype, popScalar->ITypeModifier());

	pexprArg->AddRef();
	CExpression *pexprPrElNew =
		CUtils::PexprScalarProjectElement(pmp, pcrAdditionalGrpCol, pexprArg);
...
CExpression *
CXformSplitDQA::PexprMultiLevelAggregation(
	IMemoryPool *pmp, CExpression *pexprRelational,
	DrgPexpr *pdrgpexprPrElFirstStage, DrgPexpr *pdrgpexprPrElSecondStage,
	DrgPexpr *pdrgpexprPrElThirdStage, DrgPcr *pdrgpcrArgDQA,
	DrgPcr *pdrgpcrLastStage, BOOL fSplit2LevelsOnly,
	BOOL fAddDistinctColToLocalGb)
{
	...
	if (fSplit2LevelsOnly)
	{
		...
		popFirstStage = GPOS_NEW(pmp)
			CLogicalGbAgg(pmp, pdrgpcrLocal, COperator::EgbaggtypeLocal,
						  fLocalAggGeneratesDuplicates, pdrgpcrArgDQA);
		pdrgpcrLastStage->AddRef();
		popSecondStage = GPOS_NEW(pmp)
			CLogicalGbAgg(pmp, pdrgpcrLastStage,
	...
	else
	{
		popFirstStage = GPOS_NEW(pmp) CLogicalGbAgg(
			pmp, pdrgpcrLocal, COperator::EgbaggtypeLocal /* egbaggtype */);
		pdrgpcrLocal->AddRef();
		pdrgpcrArgDQA->AddRef();
		popSecondStage = GPOS_NEW(pmp)
			CLogicalGbAgg(pmp, pdrgpcrLocal, COperator::EgbaggtypeIntermediate,
...
CXformSplitDQA::ExtractDistinctCols(
	IMemoryPool *pmp, CColumnFactory *pcf, CMDAccessor *pmda,
	CExpression *pexpr, DrgPexpr *pdrgpexprChildPrEl, HMExprCr *phmexprcr,
...
)
{
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		if (popScAggFunc->FDistinct() &&
			pmda->Pmdagg(popScAggFunc->Pmdid())->FSplittable())
		{
			GPOS_ASSERT(1 == pexprAggFunc->UlArity());
			...
			if (NULL == pcrDistinctCol)
			{
				...
				pcrDistinctCol = PcrAggFuncArgument(pmp, pmda, pcf, pexprArg,
													pdrgpexprChildPrEl);
