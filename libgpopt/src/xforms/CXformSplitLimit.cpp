...
//
//---------------------------------------------------------------------------
CXformSplitLimit::CXformSplitLimit(IMemoryPool *pmp)
	: CXformExploration(
		  // pattern
		  GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) CLogicalLimit(pmp),
			  GPOS_NEW(pmp) CExpression(
				  pmp, GPOS_NEW(pmp) CPatternLeaf(pmp)),  // relational child
			  GPOS_NEW(pmp) CExpression(
				  pmp,
				  GPOS_NEW(pmp) CPatternLeaf(pmp)),  // scalar child for offset
			  GPOS_NEW(pmp)
				  CExpression(pmp,
							  GPOS_NEW(pmp) CPatternLeaf(
								  pmp))  // scalar child for number of rows
			  ))
{
}
							...
							CExpression *pexpr) const
{
	...
	CDrvdPropRelational *pdprel = CDrvdPropRelational::Pdprel(
		pexprRelational->Pdp(CDrvdProp::EptRelational));
	...
	CExpression *pexprLimitLocal =
		PexprLimit(pmp, pexprRelational, pexprScalarStart, pexprScalarRows, pos,
				   false,  // fGlobal
				   popLimit->FHasCount(), popLimit->FTopLimitUnderDML());

	// assemble global limit operator
	CExpression *pexprLimitGlobal =
		PexprLimit(pmp, pexprLimitLocal, pexprScalarStart, pexprScalarRows, pos,
				   true,  // fGlobal
				   popLimit->FHasCount(), popLimit->FTopLimitUnderDML());
...
//---------------------------------------------------------------------------
CExpression *
CXformSplitLimit::PexprLimit(IMemoryPool *pmp, CExpression *pexprRelational,
							 ...
							 BOOL fTopLimitUnderDML) const
{
	...
	// assemble global limit operator
	CExpression *pexprLimit = GPOS_NEW(pmp)
		CExpression(pmp,
