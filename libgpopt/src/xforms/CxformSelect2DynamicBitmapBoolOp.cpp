...
CXformSelect2DynamicBitmapBoolOp::CXformSelect2DynamicBitmapBoolOp(
	...
	: CXformExploration(GPOS_NEW(pmp) CExpression(
		  ...
		  GPOS_NEW(pmp) CExpression(
			  pmp, GPOS_NEW(pmp) CLogicalDynamicGet(pmp)),  // logical child
		  GPOS_NEW(pmp) CExpression(
											...
											CExpression *pexpr) const
{
	...
	CExpression *pexprResult =
		CXformUtils::PexprSelect2BitmapBoolOp(pmp, pexpr);
	...
	if (NULL != pexprResult)
	{
		...
		CExpression *pexprRedundantSelect =
			CXformUtils::PexprRedundantSelectForDynamicIndex(pmp, pexprResult);
		pexprResult->Release();
