...
protected:
	...
	public:
		// ctor
		explicit CBitSetLink(IMemoryPool *, ULONG ulOffset, ULONG cSizeBits);
	...
	CBitSetLink *
	PbslLocate(ULONG, CBitSetLink * = NULL) const;
...
public:
	...
	// intersect sets
	void
	Intersection(const CBitSet *);

	// difference of sets
	void
	Difference(const CBitSet *);

	// is subset
	BOOL
	FSubset(const CBitSet *) const;

	// equality
	BOOL
	FEqual(const CBitSet *) const;

	// disjoint
	BOOL
	FDisjoint(const CBitSet *) const;
