...
private:
	...
	static INT
	PtrCmp(const void *pv1, const void *pv2)
	{
	...
	// resize function
	void
	Resize(ULONG ulNewSize)
	{
		GPOS_ASSERT(ulNewSize > m_ulAllocated &&
...
public:
	// ctor
	explicit CDynamicPtrArray<T, pfnDestroy>(IMemoryPool *pmp,
											 ULONG ulMinSize = 4,
											 ULONG ulExp = 10)
		: m_pmp(pmp),
		  m_ulAllocated(0),
	...
	void
	AppendArray(const CDynamicPtrArray<T, pfnDestroy> *pdrg)
	{
		...
		if (pdrg->m_ulSize > 0)
		{
			...
			clib::PvMemCpy(m_ppt + m_ulSize, pdrg->m_ppt,
						   pdrg->m_ulSize * sizeof(T *));
		}
	...
	// sort array
	void
	Sort(PfnCompare pfncompare = PtrCmp)
	{
	...
	// equality check
	BOOL
	FEqual(const CDynamicPtrArray<T, pfnDestroy> *pdrg) const
	{
