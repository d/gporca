...
{
// fwd declaration
template <class K, class T, ULONG (*pfnHash)(const K *),
		  BOOL (*pfnEq)(const K *, const K *), void (*pfnDestroyK)(K *),
		  void (*pfnDestroyT)(T *)>
class CHashMapIter;
...
private:
	...
	public:
		// ctor
		CHashMapElem(K *pk, T *pt, BOOL fOwn) : m_pk(pk), m_pt(pt), m_fOwn(fOwn)
		{
			GPOS_ASSERT(NULL != pk);
		}
		...
		BOOL
		operator==(const CHashMapElem &hme) const
		{
	...
	void
	Lookup(const K *pk, CHashMapElem **pphme) const
	{
...
public:
	// ctor
	CHashMap<K, T, pfnHash, pfnEq, pfnDestroyK, pfnDestroyT>(IMemoryPool *pmp,
															 ULONG ulSize = 128)
		: m_pmp(pmp),
		  m_ulSize(ulSize),
		  m_ulEntries(0),
	...
	FInsert(K *pk, T *pt)
	{
		...
		CHashMapElem *phme =
			GPOS_NEW(m_pmp) CHashMapElem(pk, pt, true /*fOwn*/);
