...
private:
	...
	// private copy ctor
	CHashMapIter(
		const CHashMapIter<K, T, pfnHash, pfnEq, pfnDestroyK, pfnDestroyT> &);
...
public:
	...
	CHashMapIter<K, T, pfnHash, pfnEq, pfnDestroyK, pfnDestroyT>(TMap *ptm)
		: m_ptm(ptm), m_ulChain(0), m_ulKey(0)
	{
		GPOS_ASSERT(NULL != ptm);
	}

	// dtor
	virtual ~CHashMapIter<K, T, pfnHash, pfnEq, pfnDestroyK, pfnDestroyT>()
	{
	}
