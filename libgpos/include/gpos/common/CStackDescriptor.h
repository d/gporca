...
private:
	...
	void
	AppendSymbolInfo(CWString *pws, CHAR *szDemangleBuf, SIZE_T size,
					 const Dl_info &rgInfo, ULONG ulIdx) const;

#if (GPOS_sparc)
	//  method called by walkcontext function to store return addresses
	static INT
	IGetStackFrames(ULONG_PTR ulpFp, INT sig __attribute__((unused)),
					void *pvContext);
#endif  // GPOS_sparc
...
public:
	...
	void
	BackTrace(ULONG ulSkip = 0);

	// append trace of stored stack to string
	void
	AppendTrace(CWString *pws, ULONG ulDepth = GPOS_STACK_TRACE_DEPTH) const;

	// append trace of stored stack to stream
	void
	AppendTrace(IOstream &os, ULONG ulDepth = GPOS_STACK_TRACE_DEPTH) const;
