...
private:
	...
	BOOL
	FSetBit(volatile ULONG *ulDest, ULONG ulBitVal)
	{
	...
	BOOL
	FUnsetBit(volatile ULONG *ulDest, ULONG ulBitVal)
	{
		GPOS_ASSERT(NULL != ulDest);
...
public:
	// ctor
	CSyncPool(IMemoryPool *pmp, ULONG ulSize)
		: m_pmp(pmp),
		  m_rgtObjs(NULL),
		...
		if (ULONG_MAX != m_ulIdOffset)
		{
			...
			if (!ITask::PtskSelf()->FPendingExc())
			{
				for (ULONG i = 0; i < m_ulObjs; i++)
				{
					...
					BOOL fReserved =
						(ulBitVal ==
						 (m_rgulBitMapReserved[ulElemOffset] & ulBitVal));
					BOOL fRecycled =
						(ulBitVal ==
						 (m_rgulBitMapRecycled[ulElemOffset] & ulBitVal));

					GPOS_ASSERT((!fReserved || fRecycled) &&
	...
	// unset the reserve bit - must succeed
						FUnsetBit(&m_rgulBitMapReserved[ulElemOffset],
								  ulBitVal);

					GPOS_ASSERT(fRecycled &&
	...
	Recycle(T *pt)
	{
		...
		GPOS_ASSERT((ulBitVal == (ulBitVal & ulReserved)) &&
