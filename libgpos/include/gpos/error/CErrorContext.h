...
public:
	// ctor
	explicit CErrorContext(CMiniDumper *pmdr = NULL);
	...
	void
	Register(CSerializable *pserial)
	{
	...
	void
	Unregister(CSerializable *pserial)
	{
		CAutoSpinlock asl(m_slockSerial);
		asl.Lock();
	...
	virtual void
	CopyPropErrCtxt(const IErrorContext *perrctxt);
