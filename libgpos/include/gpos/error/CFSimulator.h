...
private:
	...
	public:
		...
		struct SStackKey
		{
			...
			// ctor
			SStackKey(ULONG ulMajor, ULONG ulMinor)
				: m_ulMajor(ulMajor), m_ulMinor(ulMinor)
			{
			}

			// simple comparison
			BOOL
			operator==(const SStackKey &skey) const
			{
				return m_ulMajor == skey.m_ulMajor &&
					   m_ulMinor == skey.m_ulMinor;
			}

			// equality function -- needed for hashtable
			static BOOL
			FEqual(const SStackKey &skey, const SStackKey &skeyOther)
			{
		...
		// ctor
		explicit CStackTracker(IMemoryPool *pmp, ULONG cResolution,
							   SStackKey skey);
		...
		// invalid key
		static const SStackKey m_skeyInvalid;
	...
	typedef CSyncHashtable<CStackTracker, CStackTracker::SStackKey, CSpinlockOS>
		CStackTable;
	...
	// insert new tracker
	void
	AddTracker(CStackTracker::SStackKey skey);
...
public:
	...
	BOOL
	FNewStack(ULONG ulMajor, ULONG ulMinor);

	// global instance
	static CFSimulator *m_pfsim;
