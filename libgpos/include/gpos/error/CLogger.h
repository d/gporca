...
private:
	...
	// log message
	void
	Log(const WCHAR *wszMsg, ULONG ulSeverity, const CHAR *szFilename,
		ULONG ulLine);

	// format log message
	void
	Format(const WCHAR *wszMsg, ULONG ulSeverity, const CHAR *szFilename,
		   ULONG ulLine);
...
public:
	// ctor
	explicit CLogger(EErrorInfoLevel eil = ILogger::EeilMsgHeaderStack);
	...
	virtual void
	SetErrorInfoLevel(EErrorInfoLevel eil)
	{
		m_eil = eil;
	}
