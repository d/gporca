...
private:
	// global singleton
	static CMessageRepository *m_pmr;
	...
	typedef CSyncHashtableAccessByKey<CMessageTable, ELocale, CSpinlockOS>
		TMTAccessor;
	...
	void
	InitDirectory(IMemoryPool *pmp);

	// install message table for a given locale
	void
	AddMessageTable(ELocale eloc);
...
public:
	...
	CMessage *
	PmsgLookup(CException exc, ELocale eloc);

	// add individual message
	void
	AddMessage(ELocale eloc, CMessage *pmsg);
