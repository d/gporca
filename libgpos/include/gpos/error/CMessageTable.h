...
class CMessageTable
{
	...
	typedef CSyncHashtable<CMessage, CException, CSpinlockOS> MT;

	// short hand for message table accessor
	typedef CSyncHashtableAccessByKey<CMessage, CException, CSpinlockOS>
		MTAccessor;
...
public:
	...
	CMessage *
	PmsgLookup(CException exc);

	// insert message
	void
	AddMessage(CMessage *pmsg);
	...
	static BOOL
	FEqual(const ELocale &eloc, const ELocale &elocOther)
	{
	...
	// invalid locale
	static const ELocale m_elocInvalid;
