...
#define GPOS_TRACE_FORMAT(szFormat, ...)                     \
	ILogger::TraceFormat(__FILE__, __LINE__, false /*fErr*/, \
						 GPOS_WSZ_LIT(szFormat), __VA_ARGS__)

#define GPOS_TRACE_FORMAT_ERR(szFormat, ...)                \
	ILogger::TraceFormat(__FILE__, __LINE__, true /*fErr*/, \
						 GPOS_WSZ_LIT(szFormat), __VA_ARGS__)
...
private:
	...
	static void
	LogTask(const WCHAR *wszMsg, ULONG ulSeverity, BOOL fErr,
...
protected:
	// write log message
	virtual void
	Write(const WCHAR *wszLogEntry, ULONG ulSev) = 0;
...
public:
	...
	virtual EErrorInfoLevel
	Eil() const = 0;

	// set error info level
	virtual void
	SetErrorInfoLevel(EErrorInfoLevel eil) = 0;

	// retrieve warning message from repository and log it to error log
	static void
	Warning(const CHAR *szFilename, ULONG ulLine, ULONG ulMajor, ULONG ulMinor,
			...);

	// log trace message to current task's output or error log
	static void
	Trace(const CHAR *szFilename, ULONG ulLine, BOOL fErr, const WCHAR *wszMsg);

	// format and log trace message to current task's output or error log
	static void
	TraceFormat(const CHAR *szFilename, ULONG ulLine, BOOL fErr,
