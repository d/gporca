...
private:
	...
	PceInsert(CCacheHashTableEntry *pce)
	{
		...
		if (!m_fUnique || (m_fUnique && NULL == (pceFound = shtacc.PtLookup())))
		{
			shtacc.Insert(pce);
			UllExchangeAdd((volatile ULLONG *) &m_ullCacheSize,
						   pce->Pmp()->UllTotalAllocatedSize());
		}
		else
		{
			pceReturn = pceFound;
	...
	void
	ReleaseEntry(CCacheHashTableEntry *pce)
	{
		GPOS_ASSERT(NULL != pce);
		...
		// scope for hashtable accessor
		{
			...
			if (EXPECTED_REF_COUNT_FOR_DELETE == pce->UlRefCount() &&
				pce->FMarkedForDeletion())
			{
	...
	CCacheHashTableEntry *
	PceNext(CCacheHashTableEntry *pce)
	{
		GPOS_ASSERT(NULL != pce);
	...
	EvictEntries()
	{
		...
		if (FCompareSwap(&m_ulEvictionLock, 0, 1))
		{
			...
			{
				double dToFree =
					static_cast<double>(static_cast<double>(m_ullCacheSize) -
										static_cast<double>(m_ullCacheQuota) *
											(1.0 - m_fEvictionFactor));
				...
				for (ULONG ulRetryCount = 0;
					 ulRetryCount < m_ulGClockInitCounter + 1; ulRetryCount++)
				{
					ullTotalFreed =
						EvictEntriesOnePass(ullTotalFreed, ullToFree);
	...
	static void
	DestroyCacheEntryWithRefCountTest(CCacheHashTableEntry *pce)
	{
		GPOS_ASSERT(NULL != pce);
		...
		GPOS_ASSERT(
			EXPECTED_REF_COUNT_FOR_DELETE == pce->UlRefCount() &&
	...
	static void
	DestroyCacheEntry(CCacheHashTableEntry *pce)
	{
		GPOS_ASSERT(NULL != pce);
	...
	ULLONG
	EvictEntriesOnePass(ULLONG ullTotalFreed, ULLONG ullToFree)
	{
...
public:
	// ctor
	CCache(IMemoryPool *pmp, BOOL fUnique, ULLONG ullCacheQuota,
		   ULONG ulGClockInitCounter, HashFuncPtr pfuncHash,
		  ...
		  m_pfuncEqual(pfuncEqual)
	{
		...
		m_sht.Init(m_pmp, CACHE_HT_NUM_OF_BUCKETS,
				   GPOS_OFFSET(CCacheHashTableEntry, m_linkHash),
				   GPOS_OFFSET(CCacheHashTableEntry, m_pKey),
				   (&CCacheHashTableEntry::m_pInvalidKey), m_pfuncHash,
				   m_pfuncEqual);
	...
	void
	SetCacheQuota(ULLONG ullNewQuota)
	{
