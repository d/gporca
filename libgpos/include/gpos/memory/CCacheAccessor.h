...
public:
	...
	CCacheAccessor(CCache<T, K> *pcache)
		: m_pcache(pcache), m_pmp(NULL), m_pce(NULL), m_fInserted(false)
	...
	T
	PtInsert(K pKey, T pVal)
	{
		GPOS_ASSERT(NULL != m_pmp);

		GPOS_ASSERT(!m_fInserted && "Accessor was already used for insertion");

		GPOS_ASSERT(NULL == m_pce && "Accessor already holds an entry");

		CCacheEntry<T, K> *pce = GPOS_NEW(m_pcache->m_pmp) CCacheEntry<T, K>(
			m_pmp, pKey, pVal, m_pcache->m_ulGClockInitCounter);
