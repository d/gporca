...
private:
	...
	BOOL
	FReserve(CAutoSpinlock &as, ULONG ulAlloc);

	// revert memory reservation
	void
	Unreserve(CAutoSpinlock &as, ULONG ulAlloc, BOOL fAvailableMem);
...
public:
	// ctor
	CMemoryPoolTracker(IMemoryPool *pmpUnderlying, ULLONG ullSize,
					   BOOL fThreadSafe, BOOL fOwnsUnderlying);

	// allocate memory
	virtual void *
	PvAllocate(const ULONG ulBytes, const CHAR *szFile, const ULONG ulLine);
	...
	virtual void
	WalkLiveObjects(gpos::IMemoryVisitor *pmov);

	// check if statistics tracking is supported
	virtual BOOL
	FSupportsStatistics() const
	{
		return true;
	}
	...
	virtual void
	UpdateStatistics(CMemoryPoolStatistics &mps);
