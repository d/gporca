...
public:
	...
	void *
	NewImpl(SIZE_T cSize, const CHAR *szFilename, ULONG ulLine,
			EAllocationType eat);

	// implementation of array-new with memory pool
	template <typename T>
	T *
	NewArrayImpl(SIZE_T cElements, const CHAR *szFilename, ULONG ulLine)
	{
		T *rgTArray = static_cast<T *>(
			NewImpl(sizeof(T) * cElements, szFilename, ulLine, EatArray));
		for (SIZE_T uIdx = 0; uIdx < cElements; ++uIdx)
		{
			...
			catch (...)
			{
				...
				for (SIZE_T uDestroyIdx = uIdx - 1; uDestroyIdx < uIdx;
					 --uDestroyIdx)
				{
...
public:
	...
	DeleteArray(T *object_array)
	{
		...
		const SIZE_T cElements =
			IMemoryPool::UlSizeOfAlloc(object_array) / sizeof(T);
...
inline void *
operator new(gpos::SIZE_T cSize, gpos::IMemoryPool *pmp,
			 const gpos::CHAR *szFilename, gpos::ULONG cLine)
{
	return pmp->NewImpl(cSize, szFilename, cLine,
						gpos::IMemoryPool::EatSingleton);
}
...
inline void
operator delete(void *pv, gpos::IMemoryPool *, const gpos::CHAR *, gpos::ULONG)
{
