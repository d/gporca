...
public:
	...
	BOOL
	FEquals(const CHAR *szBuf) const;

	// appends a string
	void
	Append(const CStringStatic *pstr);

	// appends the contents of a buffer to the current string
	void
	AppendBuffer(const CHAR *szBuf);

	// appends a formatted string
	void
	AppendFormat(const CHAR *szFormat, ...);

	// appends a formatted string based on passed va list
	void
	AppendFormatVA(const CHAR *szFormat, VA_LIST vaArgs);

	// appends wide character string
	void
	AppendConvert(const WCHAR *wsz);
