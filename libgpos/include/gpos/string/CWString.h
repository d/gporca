...
protected:
	...
	virtual void
	AppendBuffer(const WCHAR *wszBuf) = 0;
...
public:
	...
	// appends a string
	void
	Append(const CWStringBase *pstr);

	// appends a formatted string
	virtual void
	AppendFormat(const WCHAR *wszFormat, ...) = 0;

	// appends a string and replaces character with string
	virtual void
	AppendEscape(const CWStringBase *pstr, WCHAR wc,
				 const WCHAR *wszReplace) = 0;

	// appends a null terminated character array
	virtual void
	AppendCharArray(const CHAR *sz) = 0;

	// appends a null terminated wide character array
	virtual void
	AppendWideCharArray(const WCHAR *wsz) = 0;
