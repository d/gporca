...
protected:
	...
	virtual BOOL
	FEquals(const WCHAR *wszBuf) const;

public:
	// ctor
	CWStringBase(ULONG ulLength, BOOL fOwnsMemory)
		: m_ulLength(ulLength), m_fOwnsMemory(fOwnsMemory)
	{
	}
	...
	virtual CWStringConst *
	PStrCopy(IMemoryPool *pmp) const;
	...
	virtual BOOL
	FEquals(const CWStringBase *pStr) const;
	...
	// equality operator
	BOOL
	operator==(const CWStringBase &str) const;
