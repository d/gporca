...
private:
	...
	void
	IncreaseCapacity(ULONG ulRequested);

	// find capacity that fits requested string size
	static ULONG
	UlCapacity(ULONG ulRequested);
...
protected:
	...
	void
	AppendBuffer(const WCHAR *wszBuf);
...
public:
	...
	void
	AppendEscape(const CWStringBase *pstr, WCHAR wc, const WCHAR *wszReplace);

	// appends a formatted string
	void
	AppendFormat(const WCHAR *wszFormat, ...);

	// appends a null terminated character array
	virtual void
	AppendCharArray(const CHAR *sz);

	// appends a null terminated wide character array
	virtual void
	AppendWideCharArray(const WCHAR *wsz);
