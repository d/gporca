...
#define GPOS_MUTEX_CHECK_ABORT_INTERVAL_MSEC \
	(GPOS_CHECK_ABORT_MAX_INTERVAL_MSEC / 4)
...
private:
	...
	ITimedLock()
	{
		...
		do
		{
			...
			if (EBUSY == iRet)
			{
				if (GPOS_MUTEX_CHECK_ABORT_INTERVAL_MSEC <=
					timerElapsed.UlElapsedMS())
				{
					GPOS_CHECK_ABORT;
			...
			{
				GPOS_ASSERT(0 == iRet &&
							"Error occurred while trying to lock mutex");
			}
		} while (0 != iRet);
#else
		// loop until we get the lock
		do
		{
			...
			ULLONG ullExpireUs =
				ullCurrentUs +
				GPOS_MUTEX_CHECK_ABORT_INTERVAL_MSEC * GPOS_USEC_IN_MSEC;
			TIMESPEC ts;
			ts.tv_sec = (ULONG_PTR)(ullExpireUs / GPOS_USEC_IN_SEC);
			ts.tv_nsec = (ULONG_PTR)((ullExpireUs % GPOS_USEC_IN_SEC) *
	...
	IAttemptLock(BOOL fBlocking)
	{
		...
		GPOS_ASSERT_IMP(fOwnedAlready, PTHREAD_MUTEX_RECURSIVE == iMutexType &&
...
public:
	// ctor
	CMutexTyped<iMutexType, fTrackable>() : CMutexBase(fTrackable)
	{
		GPOS_ASSERT(PTHREAD_MUTEX_DEFAULT == iMutexType ||
...
typedef CMutexTyped<PTHREAD_MUTEX_RECURSIVE, false /*fTrackable*/>
	CMutexRecursiveOS;
...
typedef CMutexTyped<PTHREAD_MUTEX_RECURSIVE, true /*fTrackable*/>
	CMutexRecursive;
}  // namespace gpos
