...
ULONG_PTR
UlpExchangeAdd(volatile ULONG_PTR *pul, INT i);

// inter-locked add function; returns original value;
ULLONG
UllExchangeAdd(volatile ULLONG *pullValue, ULLONG ullInc);

// compare-and-swap function for integers
BOOL
FCompareSwap(volatile ULONG *pulDest, ULONG ulOld, ULONG ulNew);

// compare-and-swap function for long integers
BOOL
FCompareSwap(volatile ULLONG *pullDest, ULLONG ullOld, ULLONG ullNew);

#ifdef GPOS_Darwin
// compare-and-swap function for integers holding addresses
BOOL
FCompareSwap(volatile ULONG_PTR *pulpDest, ULONG_PTR ulpOld, ULONG_PTR ulpNew);
#endif  // GPOS_Darwin

// compare-and-swap function for generic pointers
template <class T>
BOOL
FCompareSwap(volatile T **pptDest, T *ptOld, T *ptNew)
{
	return FCompareSwap((ULONG_PTR *) pptDest, (ULONG_PTR) ptOld,
						(ULONG_PTR) ptNew);
}
}  // namespace gpos
