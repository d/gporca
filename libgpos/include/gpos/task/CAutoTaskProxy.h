...
private:
	...
	void
	PropagateError(CTask *ptskSub);

	// check error from sub-task
	void
	CheckError(CTask *ptskSub);

public:
	// ctor
	CAutoTaskProxy(IMemoryPool *pmp, CWorkerPoolManager *m_pwpm,
	...
	void
	SetPropagateError(BOOL fPropagateError)
	{
	...
	// create new task
	CTask *
	PtskCreate(void *(*pfunc)(void *), void *pvArg,
			   volatile BOOL *pfCancel = NULL);
	...
	GPOS_RESULT
	EresTimedWait(CTask *ptsk, ULONG ulTimeoutMs);
	...
	GPOS_RESULT
	EresTimedWaitAny(CTask **pptsk, ULONG ulTimeoutMs);
