...
#define GPOS_UNITTEST_FUNC_THROW(x, ulMajor, ulMinor) \
	gpos::CUnittest(#x, CUnittest::EttStandard, x, ulMajor, ulMinor)
...
public:
	...
	FThrows() const;
	BOOL
	FThrows(ULONG ulMajor, ULONG ulMinor) const;
	...
	BOOL
	FEquals(CHAR *sz) const;

	// find test with given attributes and add to list
	static void
	FindTest(CBitVector &bv, ETestType ett, CHAR *szTestName);

	// parse and set a trace flag
	static void
	SetTraceFlag(const CHAR *szTrace);
