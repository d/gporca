...
IOstream &
HexDump(IOstream &os, const void *pv, ULLONG ullSize);

// generic hash function for byte strings
ULONG
UlHashByteArray(const BYTE *, const ULONG);
...
template <class T>
inline BOOL
FEqualPtr(const T *pt1, const T *pt2)
{
	return pt1 == pt2;
}
...
inline ULONG
UlHashUlp(const ULONG_PTR &ulpKey)
{
	return (ULONG) ulpKey;
}
...
template <class T>
inline BOOL
FEqual(const T *pt1, const T *pt2)
{
	return *pt1 == *pt2;
}
...
inline BOOL
FEqualUlp(const ULONG_PTR &ulpKeyLeft, const ULONG_PTR &ulpKeyRight)
{
