...
private:
	...
	public:
		...
		static ULONG
		UlHash(const ULONG &ul)
		{
			return ul;
		}

		static ULONG
		UlHash(const SElem &elem)
		{
			return elem.m_ulId;
		}
		...
		static BOOL
		FEqual(const SElem &elem, const SElem &elemOther)
		{
			return elem == elemOther;
		}

		// ctor
		SElem(ULONG ulId, ULONG ulKey) : m_ulId(ulId), m_ulKey(ulKey)
		{
		}
	...
	static void
	Unittest_WaitTasks(SElemTest *);

	// inserts a subset of elements
	static void *
	PvUnittest_Inserter(void *);

	// looks up a subset of elements
	static void *
	PvUnittest_Reader(void *);

	// removes a subset of elements
	static void *
	PvUnittest_Remover(void *);

	// checks the number of visited elements
	static void *
	PvUnittest_Iterator(void *);

	// checks the number and contents of visited elements
	static void *
	PvUnittest_IteratorCheck(void *);

	// spawns a number of iterator check tasks
	static void *
	PvUnittest_IteratorsRun(IMemoryPool *, SElemHashtable &, SElem *, ULONG);
...
public:
	...
	EresUnittest_Basics();
	static GPOS_RESULT
	EresUnittest_Accessor();
	static GPOS_RESULT
	EresUnittest_ComplexEquality();
	static GPOS_RESULT
	EresUnittest_SameKeyIteration();
	static GPOS_RESULT
	EresUnittest_NonConcurrentIteration();
	static GPOS_RESULT
	EresUnittest_ConcurrentIteration();
	static GPOS_RESULT
	EresUnittest_Concurrency();
