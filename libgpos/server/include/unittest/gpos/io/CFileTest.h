...
public:
	...
	EresUnittest();
	static GPOS_RESULT
	EresUnittest_FileContent();
	static GPOS_RESULT
	EresUnittest_Invalid();
	static GPOS_RESULT
	EresUnittest_InconsistentSize();

	static void
	Unittest_MkTmpFile(CHAR *szTmpDir, CHAR *szTmpFile);
	static void
	Unittest_DeleteTmpDir(const CHAR *szDir, const CHAR *szFile);

	template <typename T, typename R, typename ARG1, typename ARG2>
	static void
	Unittest_CheckError(T *pt, R (T::*pfunc)(ARG1, ARG2), ARG1 argFirst,
						ARG2 argSec, CException::ExMinor exmi);

	static void
	Unittest_WriteInconsistentSize(const CHAR *szTmpFile);
	static void
	Unittest_ReadInconsistentSize(const CHAR *szTmpFile);
