...
private:
	...
	struct SSimpleObject : public CRefCount
	{
		...
		static BOOL
		FMyEqual(ULONG *const &pvKey, ULONG *const &pvKeySecond);
	...
	static GPOS_RESULT
	EresInsertDuplicates(CCache<SSimpleObject *, ULONG *> *);

	// remove
	static GPOS_RESULT
	EresRemoveDuplicates(CCache<SSimpleObject *, ULONG *> *);
	...
	static ULLONG
	InsertOneElement(CCache<SSimpleObject *, ULONG *> *pCache, ULONG ulKey);
	...
	static ULONG
	ULFillCacheWithoutEviction(CCache<SSimpleObject *, ULONG *> *pCache,
							   ULONG ulKeyStart);

	// checks if after eviction we have more entries from newer generation than the older generation
	static void
	CheckGenerationSanityAfterEviction(CCache<SSimpleObject *, ULONG *> *pCache,
	...
	static void
	TestEvictionForOneCacheSize(ULLONG ullCacheQuota);
	...
	public:
		...
		// hashing  function
		static ULONG
		UlMyHash(CDeepObject::CDeepObjectList *const &plist);

		// key equality function
		static BOOL
		FMyEqual(CDeepObject::CDeepObjectList *const &pvKey,
		...
		void
		AddEntry(IMemoryPool *pmp, ULONG ulKey, ULONG ulVal);
...
public:
	...
	EresUnittest_Basic();
	static GPOS_RESULT
	EresUnittest_Refcount();
	static GPOS_RESULT
	EresUnittest_Eviction();
	static GPOS_RESULT
	EresUnittest_DeepObject();
	static GPOS_RESULT
	EresUnittest_Iteration();
	static GPOS_RESULT
	EresUnittest_IterativeDeletion();
	static GPOS_RESULT
	EresUnittest_ConcurrentAccess();
