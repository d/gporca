...
CExceptionTest::EresUnittest_AdditionOverflow()
{
	GPOS_TRY
	{
		...
		(void) gpos::UllAdd(GPOS_ULLONG_MAX - 2, 1);
		(void) gpos::UllAdd(GPOS_ULLONG_MAX, 0);
		(void) gpos::UllAdd(GPOS_ULLONG_MAX - 2, 2);
	}
	GPOS_CATCH_EX(ex)
	{
	...
	(void) gpos::UllAdd(GPOS_ULLONG_MAX, 1);
...
CExceptionTest::EresUnittest_MultiplicationOverflow()
{
	GPOS_TRY
	{
		...
		(void) gpos::UllMultiply(GPOS_ULLONG_MAX, 1);
		(void) gpos::UllMultiply(GPOS_ULLONG_MAX, 0);
		(void) gpos::UllMultiply(GPOS_ULLONG_MAX / 2, 2);
		(void) gpos::UllMultiply(GPOS_ULLONG_MAX / 2 - 1, 2);
	}
	GPOS_CATCH_EX(ex)
	{
		// no exception is expected here
	...
	(void) gpos::UllMultiply(GPOS_ULLONG_MAX - 4, 2);
