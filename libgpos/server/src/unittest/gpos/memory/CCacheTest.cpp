...
//---------------------------------------------------------------------------
void
CCacheTest::CDeepObject::AddEntry(IMemoryPool *pmp, ULONG ulKey, ULONG ulVal)
{
...
CCacheTest::EresUnittest_Refcount()
{
...

	{
		...
		GPOS_ASSERT(4 == pso->UlpRefCount() &&
					"Expected pso, CCacheEntry CCacheAccessor, and customer to "
					"have ownership");
		// Ideally it shouldn't delete itself because CCache is still holding this object
		pso->Release();
		GPOS_ASSERT(3 == pso->UlpRefCount() &&
					"Expected CCacheEntry, CCacheAccessor and customer to have "
					"ownership");
	}
	GPOS_ASSERT(2 == pso->UlpRefCount() &&
				"Expected refcount to be 1. CCacheEntry and customer have the "
				"ownership");
									   ...
									   ULONG ulKeyStart)
{
	...
	GPOS_ASSERT_IMP(0 == ullInitialCacheSize,
					ullSizeBeforeEviction == ullExpectedCacheSize &&
						ullSizeBeforeEviction + ullOneElemSize >
							ullInitialCacheSize);
...
CCacheTest::EresInsertDuplicates(CCache<SSimpleObject *, ULONG *> *pcache)
{
...

	{
		...
		at.Os() << std::endl
				...
				<< pcache->UllTotalAllocatedSize() << " bytes";
		at.Os() << std::endl
				<< "Total memory consumption by memory manager: "
				<< CMemoryPoolManager::Pmpm()->UllTotalAllocatedSize()
				<< " bytes";
	}
...
CCacheTest::EresUnittest_DeepObject()
{
	...
	apcache = CCacheFactory::PCacheCreate<CDeepObject *,
...
CCacheTest::EresUnittest_ConcurrentAccess()
{
	...
	// scope for cache auto pointer
	{
		...
		// scope for ATP
		{
			...
			CCacheTest::TaskFuncPtr rgPfuncTask[] = {CCacheTest::PvInsertTask,
													 CCacheTest::PvLookupTask,
													 CCacheTest::PvDeleteTask};
			...
			{
				rgPtsk[i] = atp.PtskCreate(rgPfuncTask[i % ulNumberOfTaskTypes],
										   pcache);
