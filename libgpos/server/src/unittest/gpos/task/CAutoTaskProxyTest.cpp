...
CAutoTaskProxyTest::EresUnittest_TimedWait()
{
	...
	// scope for ATP
	{
		...
		GPOS_RESULT eres =
#endif // GPOS_DEBUG
		atp.EresTimedWait(rgPtsk[0], GPOS_ULONG_MAX);
...
CAutoTaskProxyTest::EresUnittest_TimedWaitAny()
{
	...
	// scope for ATP
	{
		...
		GPOS_RESULT eres =
#endif // GPOS_DEBUG
		atp.EresTimedWaitAny(&ptsk, GPOS_ULONG_MAX);
		GPOS_ASSERT(GPOS_OK == eres);
		GPOS_ASSERT(ptsk == rgPtsk[2]);
	...
	)
{
	...
	GPOS_TRY
	{
		...
		case EwtWaitAny:
			...
			break;
		case EwtTimedWait:
			atp.EresTimedWait(ptsk, GPOS_ULONG_MAX);
			break;
		case EwtTimedWaitAny:
			atp.EresTimedWaitAny(&finishedTask, GPOS_ULONG_MAX);
			break;
		case EwtDestroy:
			atp.Destroy(ptsk);
			break;
