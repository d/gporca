...
//
//---------------------------------------------------------------------------
CMessage::CMessage(CException exc, ULONG ulSev, const WCHAR *wszFmt,
...
//---------------------------------------------------------------------------
void
CMessage::FormatMessage(CWStringStatic *pstr, ULONG ulMajor, ULONG ulMinor, ...)
{
...
{
	os << "Message No: " << m_exc.UlMajor() << "-" << m_exc.UlMinor()
	   << std::endl
	   << "Message:   \"" << m_wszFmt << "\" [" << m_ulLenFmt << "]"
	   << std::endl
	   << "Parameters: " << m_ulParams << std::endl
	   << "Comments:  \"" << m_wszComment << "\" [" << m_ulLenComment << "]"
	   << std::endl;
...
CMessage::Pmsg(ULONG ulIndex)
{
	...
	static CMessage rgmsg[CException::ExmiSentinel] = {
		CMessage(CException(CException::ExmaInvalid, CException::ExmiInvalid),
				 CException::ExsevError, GPOS_WSZ_WSZLEN("Unknown error"),
				 0,  // # params
		...
		CMessage(
			...
			GPOS_WSZ_WSZLEN("Interval between successive abort checkpoints "
							"exceeds maximum")),
		...
		CMessage(CException(CException::ExmaSystem, CException::ExmiIOError),
				 ...
				 1,  // # params
				 GPOS_WSZ_WSZLEN("I/O operation failed; use error code to "
		...
		CMessage(CException(CException::ExmaSystem, CException::ExmiNetError),
				 ...
				 1,  // # params
				 GPOS_WSZ_WSZLEN("Networking operation failed; use error code "
		...
		CMessage(
			...
			1,  // # params
			GPOS_WSZ_WSZLEN("Delete operation failed; use error code to "
							"identify the error type")),
		...
		CMessage(CException(CException::ExmaSystem,
				 ...
				 CException::ExsevError,
				 GPOS_WSZ_WSZLEN("Invalid multibyte character for locale "
