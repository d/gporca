...
//
//---------------------------------------------------------------------------
CMemoryPoolManager::CMemoryPoolManager(IMemoryPool *pmpInternal,
									   IMemoryPool *pmpBase)
	: m_pmpBase(pmpBase),
	  ...
	  m_fAllowGlobalNew(true)
{
	...
	GPOS_ASSERT(GPOS_OFFSET(CMemoryPool, m_link) ==
				GPOS_OFFSET(CMemoryPoolAlloc, m_link));
	GPOS_ASSERT(GPOS_OFFSET(CMemoryPool, m_link) ==
				GPOS_OFFSET(CMemoryPoolTracker, m_link));

	m_sht.Init(m_pmpInternal, GPOS_MEMORY_POOL_HT_SIZE,
			   GPOS_OFFSET(CMemoryPool, m_link),
			   GPOS_OFFSET(CMemoryPool, m_ulpKey), &(CMemoryPool::m_ulpInvalid),
			   UlHashUlp, FEqualUlp);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CMemoryPoolManager::EresInit(void *(*pfnAlloc)(SIZE_T), void (*pfnFree)(void *))
{
	...
	if (NULL == pvAllocInternal || NULL == pvAllocBase)
	{
		Free(pvAllocBase);
	...
	IMemoryPool *pmpBase =
		new (pvAllocBase) CMemoryPoolAlloc(pfnAlloc, pfnFree);

	// create internal memory pool
	IMemoryPool *pmpInternal =
		new (pvAllocInternal) CMemoryPoolTracker(pmpBase,
	...
	GPOS_TRY
	{
		CMemoryPoolManager::m_pmpm =
			GPOS_NEW(pmpInternal) CMemoryPoolManager(pmpInternal, pmpBase);
	}
	GPOS_CATCH_EX(ex)
	{
...
//---------------------------------------------------------------------------
IMemoryPool *
CMemoryPoolManager::PmpCreate(EAllocType eat, BOOL fThreadSafe,
							  ULLONG ullCapacity)
{
	IMemoryPool *pmp =
#ifdef GPOS_DEBUG
...
#else
		PmpNew(eat, m_pmpBase, ullCapacity, fThreadSafe,
			   false /*fOwnsUnderlyingPmp*/);
#endif  // GPOS_DEBUG
...
//---------------------------------------------------------------------------
IMemoryPool *
CMemoryPoolManager::PmpNew(EAllocType eat, IMemoryPool *pmpUnderlying,
						   ULLONG ullCapacity, BOOL fThreadSafe,
	...
	switch (eat)
	{
		case CMemoryPoolManager::EatTracker:
			return GPOS_NEW(m_pmpInternal) CMemoryPoolTracker(
				pmpUnderlying, ullCapacity, fThreadSafe, fOwnsUnderlying);
...
//---------------------------------------------------------------------------
IMemoryPool *
CMemoryPoolManager::PmpCreatePoolStack(EAllocType eat, ULLONG ullCapacity,
									   BOOL fThreadSafe)
{
	...
	if (NULL != ITask::PtskSelf() && !fMallocType &&
		GPOS_FTRACE(EtraceTestMemoryPools))
	{
		// put fault injector on top of base pool
		IMemoryPool *pmpFPSimLow = GPOS_NEW(m_pmpInternal)
			CMemoryPoolInjectFault(pmpBase, false /*fOwnsUnderlying*/
			);

		// put tracker on top of fault injector
		pmpBase = PmpNew(EatTracker, pmpFPSimLow, ullCapacity, fThreadSafe,
	...
	if (!fMallocType)
	{
		...
		pmpRequested = PmpNew(eat, pmpBase, ullCapacity, fThreadSafe,
							  pmpBase != m_pmpBase);
	}

	// put fault injector on top of requested pool
	IMemoryPool *pmpFPSim = GPOS_NEW(m_pmpInternal)
		CMemoryPoolInjectFault(pmpRequested, !fMallocType);

	// put tracker on top of the stack
	return PmpNew(EatTracker, pmpFPSim, ullCapacity, fThreadSafe,
				  true /*fOwnsUnderlying*/);
}
...
//---------------------------------------------------------------------------
void
CMemoryPoolManager::DeleteUnregistered(IMemoryPool *pmp)
{
	...
	// accessor's scope
	{
		...
		while (NULL != pmpFound)
		{
			GPOS_ASSERT(pmpFound != pmp &&
						"Attempt to delete a registered memory pool");
...
//---------------------------------------------------------------------------
void
CMemoryPoolManager::Destroy(IMemoryPool *pmp)
{
	...
	while (mpiter.FAdvance())
	{
		...
		if (NULL != pmp)
		{
			...
			if (ullSize > ullSizeThreshold)
			{
				...
				at.Os() << std::endl
						<< "OVERSIZED MEMORY POOL: " << ullSize << " bytes "
						<< std::endl;
			}
		}
	}
...
//---------------------------------------------------------------------------
void
CMemoryPoolManager::DestroyMemoryPoolAtShutdown(CMemoryPool *pmp)
{
	GPOS_ASSERT(NULL != pmp);
