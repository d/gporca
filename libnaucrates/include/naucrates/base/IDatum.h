...
public:
	...
	virtual BOOL
	FMatch(const IDatum *) const = 0;

	// create a copy of the datum
	virtual IDatum *
	PdatumCopy(IMemoryPool *pmp) const = 0;
	...
	virtual BOOL
	FStatsEqual(const IDatum *pdatum) const = 0;

	// stats less than
	virtual BOOL
	FStatsLessThan(const IDatum *pdatum) const = 0;

	// check if the given pair of datums are stats comparable
	virtual BOOL
	FStatsComparable(const IDatum *pdatum) const = 0;

	// stats greater than
	virtual BOOL
	FStatsGreaterThan(const IDatum *pdatum) const
	{
		BOOL fStatsComparable = pdatum->FStatsComparable(this);
		GPOS_ASSERT(fStatsComparable &&
					"Invalid invocation of FStatsGreaterThan");
		return fStatsComparable && pdatum->FStatsLessThan(this);
	}

	// distance function
	virtual CDouble
	DStatsDistance(const IDatum *) const = 0;

	// does the datum need to be padded before statistical derivation
	virtual BOOL
	FNeedsPadding() const = 0;

	// return the padded datum
	virtual IDatum *
	PdatumPadded(IMemoryPool *pmp, ULONG ulColLen) const = 0;

	// does datum support like predicate
	virtual BOOL
	FSupportLikePredicate() const = 0;

	// return the default scale factor of like predicate
	virtual CDouble
	DLikePredicateScaleFactor() const = 0;

	// supports statistical comparisons based on the byte array representation of datum
	virtual BOOL
	FSupportsBinaryComp(const IDatum *pdatum) const = 0;
	...
	virtual BOOL
	FStatsEqualBinary(const IDatum *pdatum) const = 0;

	// statistics less than based on byte array representation of datums
	virtual BOOL
	FStatsLessThanBinary(const IDatum *pdatum) const = 0;

	// comparison function sorting idatums
	static inline INT
	IStatsCmp(const void *pv1, const void *pv2);

};  // class IDatum

// comparison function for statistics operations
INT
IDatum::IStatsCmp(const void *pv1, const void *pv2)
{
