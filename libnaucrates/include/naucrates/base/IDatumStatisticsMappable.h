...
public:
	...
	virtual BOOL
	FHasStatsDoubleMapping() const = 0;

	// map to double for statistics computation
	virtual CDouble
	DStatsMapping() const = 0;

	// can datum be mapped to LINT
	virtual BOOL
	FHasStatsLINTMapping() const = 0;

	// map to LINT for statistics computation
	virtual LINT
	LStatsMapping() const = 0;
	...
	virtual BOOL
	FStatsLessThan(const IDatum *pdatum) const;

	// check if the given pair of datums are stats comparable
	virtual BOOL
	FStatsComparable(const IDatum *pdatum) const;

	// distance function
	virtual CDouble
	DStatsDistance(const IDatum *pdatum) const;
