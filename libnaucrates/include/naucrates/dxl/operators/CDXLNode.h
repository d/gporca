...
public:
	...
	inline CDXLNode *operator[](ULONG ulPos) const
	{
	...
	// setters
	void
	AddChild(CDXLNode *pdxlnChild);

	void
	SetOperator(CDXLOperator *pdxlop);

	void
	SerializeToDXL(CXMLSerializer *) const;

	// replace a given child of this DXL node with the given node
	void
	ReplaceChild(ULONG ulPos, CDXLNode *pdxlnChild);

	void
	SerializeChildrenToDXL(CXMLSerializer *pxmlser) const;

	// setter
	void
	SetProperties(CDXLProperties *pdxlprop);

	// setter for direct dispatch info
	void
	SetDirectDispatchInfo(CDXLDirectDispatchInfo *pdxlddinfo);

	// serialize properties in DXL format
	void
	SerializePropertiesToDXL(CXMLSerializer *pxmlser) const;

#ifdef GPOS_DEBUG
	// checks whether the operator has valid structure, i.e. number and
	// types of child nodes
	void
	AssertValid(BOOL fValidateChildren) const;
#endif  // GPOS_DEBUG
