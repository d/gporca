...
public:
	...
	virtual void
	SerializeToDXL(CXMLSerializer *, const CDXLNode *) const = 0;

	// check if given column is defined by operator
	virtual BOOL FDefinesColumn(ULONG  // ulColId
								) const
	{
	...
	static const CWStringConst *PstrJoinTypeName(EdxlJoinType);

	// Return the index scan direction
	static const CWStringConst *PstrIndexScanDirection(EdxlIndexScanDirection);

#ifdef GPOS_DEBUG
	// checks whether the operator has valid structure, i.e. number and
	// types of child nodes
	virtual void
	AssertValid(const CDXLNode *, BOOL fValidateChildren) const = 0;
#endif  // GPOS_DEBUG
};
