...
CDXLOperatorFactory::PdrgptFromXMLCh(CDXLMemoryManager *pmm,
									 ...
									 Edxltoken edxltokenElement)
{
	...
	CDynamicPtrArray<T, pfnDestroy> *pdrgpt =
		GPOS_NEW(pmp) CDynamicPtrArray<T, pfnDestroy>(pmp);
	...
	for (ULONG ul = 0; ul < ulNumTokens; ul++)
	{
		...
		T *pt = GPOS_NEW(pmp)
			T(ValueFromXmlstr(pmm, xmlszNext, edxltokenAttr, edxltokenElement));
		pdrgpt->Append(pt);
	}
