...
public:
	// ctor/dtor
	CDXLPhysicalMaterialize(IMemoryPool *pmp, BOOL fEager);

	CDXLPhysicalMaterialize(IMemoryPool *pmp, BOOL fEager, ULONG ulSpoolId,
							INT iExecutorSlice, ULONG ulConsumerSlices);
	...
	UlSpoolId() const;
	INT
	IExecutorSlice() const;
	ULONG
	UlConsumerSlices() const;
