...
public:
	// ctor
	CDXLWindowFrame(IMemoryPool *pmp, EdxlFrameSpec edxlfs,
					EdxlFrameExclusionStrategy edxlfes, CDXLNode *pdxlnLeading,
	...
	const CWStringConst *
	PstrES(EdxlFrameExclusionStrategy edxles) const;

	// return the string representation of the frame specification (row or range)
	const CWStringConst *
	PstrFS(EdxlFrameSpec edxlfs) const;
