...
private:
	...
	Pfparse
	FindParseHandler(EDxlParseHandlerType edxlphtype);

	// extract traceflags
	void
	ExtractTraceFlags(CParseHandlerBase *pph);

	// extract optimizer config
	void
	ExtractOptimizerConfig(CParseHandlerBase *pph);

	// extract a physical plan
	void
	ExtractDXLPlan(CParseHandlerBase *pph);

	// extract metadata objects
	void
	ExtractMetadataObjects(CParseHandlerBase *pph);

	// extract statistics
	void
	ExtractStats(CParseHandlerBase *pph);

	// extract DXL query
	void
	ExtractDXLQuery(CParseHandlerBase *pph);

	// extract mdids of requested objects
	void
	ExtractMDRequest(CParseHandlerBase *pph);

	// extract search strategy
	void
	ExtractSearchStrategy(CParseHandlerBase *pph);

	// extract cost params
	void
	ExtractCostParams(CParseHandlerBase *pph);

	// extract a top level scalar expression
	void
	ExtractScalarExpr(CParseHandlerBase *pph);

	// check if given element name is valid for starting DXL document
	static BOOL
	FValidStartElement(const XMLCh *const xmlszName);
