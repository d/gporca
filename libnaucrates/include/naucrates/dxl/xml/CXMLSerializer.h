...
private:
	...
	static void
	WriteEscaped(IOstream &os, const CWStringBase *pstr);

public:
	// ctor/dtor
	CXMLSerializer(IMemoryPool *pmp, IOstream &os, BOOL fIndent = true)
		: m_pmp(pmp),
		  m_os(os),
		  m_fIndent(fIndent),
	...
	void
	OpenElement(const CWStringBase *pstrNamespace,
				const CWStringBase *pstrElem);

	// closes the element with the given name
	void
	CloseElement(const CWStringBase *pstrNamespace,
				 const CWStringBase *pstrElem);

	// adds a string-valued attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, const CWStringBase *pstrValue);

	// adds a character string attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, const CHAR *szValue);

	// adds an unsigned integer-valued attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, ULONG ulValue);

	// adds an unsigned long integer attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, ULLONG ullValue);

	// adds an integer-valued attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, INT iValue);

	// adds an integer-valued attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, LINT lValue);

	// adds a boolean attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, BOOL fValue);

	// add a double-valued attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, CDouble dValue);

	// add a byte array attribute
	void
	AddAttribute(const CWStringBase *pstrAttr, BOOL fNull, const BYTE *pba,
				 ULONG ulLen);
};
