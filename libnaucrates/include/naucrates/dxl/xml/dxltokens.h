...
private:
	...
	struct SStrMapElem
	{
		...
		// ctor
		SStrMapElem(Edxltoken edxlt, CWStringConst *pstr)
			: m_edxlt(edxlt), m_pstr(pstr)
		{
	...
	struct SXMLStrMapElem
	{
		...
		// ctor
		SXMLStrMapElem(Edxltoken edxlt, XMLCh *xmlsz)
			: m_edxlt(edxlt), m_xmlsz(xmlsz)
		{
			GPOS_ASSERT(edxlt < EdxltokenSentinel);
