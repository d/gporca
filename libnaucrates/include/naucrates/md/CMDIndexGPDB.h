...
public:
	// ctor
	CMDIndexGPDB(IMemoryPool *pmp, IMDId *pmdid, CMDName *pmdname,
				 BOOL fClustered, EmdindexType emdindt, IMDId *pmdidItemType,
				 DrgPul *pdrgpulKeyCols, DrgPul *pdrgpulIncludedCols,
				 DrgPmdid *pdrgpmdidOpClasses, IMDPartConstraint *pmdpartcnstr);
	...
	virtual ULONG
	UlKey(ULONG ulPos) const;

	// return the position of the key column
	virtual ULONG
	UlPosInKey(ULONG ulCol) const;

	// number of included columns
	virtual ULONG
	UlIncludedCols() const;

	// return the n-th included column
	virtual ULONG
	UlIncludedCol(ULONG ulPos) const;

	// return the position of the included column
	virtual ULONG
	UlPosInIncludedCol(ULONG ulCol) const;
	...
	virtual BOOL
	FCompatible(const IMDScalarOp *pmdscop, ULONG ulKEyPos) const;
