...
public:
	// ctor
	CMDTypeGenericGPDB(IMemoryPool *pmp, IMDId *pmdid, CMDName *pmdname,
					   BOOL fRedistributable, BOOL fFixedLength, ULONG ulLength,
					   BOOL fByValue, IMDId *pmdidOpEq, IMDId *pmdidOpNEq,
					   IMDId *pmdidOpLT, IMDId *pmdidOpLEq, IMDId *pmdidOpGT,
					   IMDId *pmdidOpGEq, IMDId *pmdidOpComp, IMDId *pmdidMin,
					   IMDId *pmdidMax, IMDId *pmdidAvg, IMDId *pmdidSum,
					   IMDId *pmdidCount, BOOL fHashable, BOOL fComposite,
					   IMDId *pmdidBaseRelation, IMDId *pmdidTypeArray,
					   INT iLength);
	...
	// create a dxl datum
	static CDXLDatum *
	Pdxldatum(IMemoryPool *pmp, IMDId *pmdid, INT iTypeModifier, BOOL fByVal,
			  BOOL fNull, BYTE *pba, ULONG ulLength, LINT lValue,
			  CDouble dValue);

	// create a dxl datum of types having double mapping
	static CDXLDatum *
	PdxldatumStatsDoubleMappable(IMemoryPool *pmp, IMDId *pmdid,
								 INT iTypeModifier, BOOL fByValue, BOOL fNull,
								 BYTE *pba, ULONG ulLength, LINT lValue,
								 CDouble dValue);

	// create a dxl datum of types having lint mapping
	static CDXLDatum *
	PdxldatumStatsLintMappable(IMemoryPool *pmp, IMDId *pmdid,
							   INT iTypeModifier, BOOL fByValue, BOOL fNull,
							   BYTE *pba, ULONG ulLength, LINT lValue,
							   CDouble dValue);
	...
	static BOOL
	FHasByteLintMapping(const IMDId *pmdid);

	// does a datum of this type need bytea to double mapping for statistics computation
	static BOOL
	FHasByteDoubleMapping(const IMDId *pmdid);

	// is this a time-related type
	static BOOL
	FTimeRelatedType(const IMDId *pmdid);

	// is this a network-related type
	static BOOL
	FNetworkRelatedType(const IMDId *pmdid);
};
}  // namespace gpmd
