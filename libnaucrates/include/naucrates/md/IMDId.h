...
public:
	...
	// equality check
	virtual BOOL
	FEquals(const IMDId *pmdid) const = 0;
	...
	// e.g. hashmaps, MD cache, etc.
	static BOOL
	FEqualMDId(const IMDId *pmdidLeft, const IMDId *pmdidRight)
	{
	...
	static BOOL
	FEqualMDid(const VOID_PTR &pvLeft, const VOID_PTR &pvRight)
	{
...
// hash set for mdid
typedef CHashSet<IMDId, IMDId::UlHashMDId, IMDId::FEqualMDId,
				 CleanupRelease<IMDId> >
	HSMDId;

// iterator over the hash set for column id information for missing statistics
typedef CHashSetIter<IMDId, IMDId::UlHashMDId, IMDId::FEqualMDId,
					 CleanupRelease<IMDId> >
	HSIterMDId;
}  // namespace gpmd
