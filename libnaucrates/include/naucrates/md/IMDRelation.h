...
protected:
	...
	PstrColumns(IMemoryPool *pmp, DrgPul *pdrgpul);
...
public:
	...
	virtual BOOL
	FPartitioned() const = 0;

	// number of partition columns
	virtual ULONG
	UlPartColumns() const = 0;

	// number of partitions
	virtual ULONG
	UlPartitions() const = 0;

	// retrieve the partition column at the given position
	virtual const IMDColumn *
	PmdcolPartColumn(ULONG ulPos) const = 0;

	// retrieve list of partition types
	virtual DrgPsz *
	PdrgpszPartTypes() const = 0;

	// retrieve the partition type of the given partition level
	virtual CHAR
	SzPartType(ULONG ulPos) const = 0;
	...
	virtual IMDId *
	PmdidIndex(ULONG ulPos) const = 0;

	// check if index is partial given its mdid
	virtual BOOL
	FPartialIndex(IMDId *mdid) const;

	// retrieve the id of the metadata cache trigger at the given position
	virtual IMDId *
	PmdidTrigger(ULONG ulPos) const = 0;

	// number of check constraints
	virtual ULONG
	UlCheckConstraints() const = 0;

	// retrieve the id of the check constraint cache at the given position
	virtual IMDId *
	PmdidCheckConstraint(ULONG ulPos) const = 0;

	// part constraint
	virtual IMDPartConstraint *
	Pmdpartcnstr() const = 0;

	// relation distribution policy as a string value
	static const CWStringConst *
	PstrDistrPolicy(Ereldistrpolicy ereldistrpolicy);

	// name of storage type
	static const CWStringConst *
	PstrStorageType(IMDRelation::Erelstoragetype erelstorage);
};
