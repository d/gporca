...
public:
	...
	virtual IMDId *
	PmdidCmp(ECmpType ecmpt) const = 0;

	// id of specified specified aggregate type
	virtual IMDId *
	PmdidAgg(EAggType eagg) const = 0;
	...
	virtual BOOL
	FRedistributable() const = 0;

	// id of the array type for the type
	virtual IMDId *
	PmdidTypeArray() const = 0;
	...
	virtual IDatum *
	Pdatum(const CDXLScalarConstValue *pdxlop) const = 0;

	// construct a datum from a DXL datum
	virtual IDatum *
	Pdatum(IMemoryPool *pmp, const CDXLDatum *pdxldatum) const = 0;

	// is type fixed length
	virtual BOOL
	FFixedLength() const = 0;
	...
	PdxlopScConst(IMemoryPool *pmp, IDatum *pdatum) const = 0;

	// generate the DXL datum from IDatum
	virtual CDXLDatum *
	Pdxldatum(IMemoryPool *pmp, IDatum *pdatum) const = 0;

	// generate the DXL datum representing null value
	virtual CDXLDatum *
	PdxldatumNull(IMemoryPool *pmp) const = 0;
