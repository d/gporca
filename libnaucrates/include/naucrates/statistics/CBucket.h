...
private:
	...
	CBucket &
	operator=(const CBucket &);

public:
	// ctor
	CBucket(CPoint *ppointLower, CPoint *ppointUpper, BOOL fLowerClosed,
	...
	// set frequency
	void
	SetFrequency(CDouble dFrequency)
	{
	...
	BOOL
	FIntersects(const CBucket *pbucket) const;

	// does bucket's range subsume another's
	BOOL
	FSubsumes(const CBucket *pbucket) const;

	// does bucket occur before another
	BOOL
	FBefore(const CBucket *pbucket) const;

	// does bucket occur after another
	BOOL
	FAfter(const CBucket *pbucket) const;
	...
	CBucket *
	PbucketGreaterThan(IMemoryPool *pmp, CPoint *ppoint) const;

	// scale down version of bucket adjusting upper boundary
	CBucket *
	PbucketScaleUpper(IMemoryPool *pmp, CPoint *ppointUpper,
					  BOOL fIncludeUpper) const;

	// scale down version of bucket adjusting lower boundary
	CBucket *
	PbucketScaleLower(IMemoryPool *pmp, CPoint *ppointLower,
					  BOOL fIncludeLower) const;

	// extract singleton bucket at given point
	CBucket *
	PbucketSingleton(IMemoryPool *pmp, CPoint *ppointSingleton) const;

	// create a new bucket by intersecting with another and return the percentage of each of the buckets that intersect
	CBucket *
	PbucketIntersect(IMemoryPool *pmp, CBucket *pbucket,
					 CDouble *pdFreqIntersect1,
					 CDouble *pdFreqIntersect2) const;

	// Remove a bucket range. This produces lower and upper split
	void
	Difference(IMemoryPool *pmp, CBucket *pbucketOther, CBucket **pbucketLower,
			   CBucket **pbucketUpper);

	// return copy of bucket
	CBucket *
	PbucketCopy(IMemoryPool *pmp);

	// return a copy of the bucket with updated frequency based on the new total number of rows
	CBucket *
	PbucketUpdateFrequency(IMemoryPool *pmp, CDouble dRowsOld,
						   CDouble dRowsNew);

	// Merge with another bucket and return leftovers
	CBucket *
	PbucketMerge(IMemoryPool *pmp, CBucket *pbucketOther, CDouble dRows,
				 CDouble dRowsOther, CBucket **pbucket1New,
				 CBucket **pbucket2New, BOOL fUnionAll = true);
	...
	CDouble
	DSample(ULONG *pulSeed) const;

	// compare lower bucket boundaries
	static INT
	ICompareLowerBounds(const CBucket *pbucket1, const CBucket *pbucket2);

	// compare upper bucket boundaries
	static INT
	ICompareUpperBounds(const CBucket *pbucket1, const CBucket *pbucket2);

	// compare lower bound of first bucket to upper bound of second bucket
	static INT
	ICompareLowerBoundToUpperBound(const CBucket *pbucket1,
								   const CBucket *pbucket2);

	// create a new singleton bucket with the given datum as it lower and upper bounds
	static CBucket *
	PbucketSingleton(IMemoryPool *pmp, IDatum *pdatum);
};
}  // namespace gpnaucrates
