...
private:
	...
	static CHistogram *
	PhistSimpleFilter(IMemoryPool *pmp, CStatsPred *pstatspred,
					  CBitSet *pbsFilterColIds, CHistogram *phistBefore,
					  CDouble *pdScaleFactorLast, ULONG *pulColIdLast);

	// create a new histogram after applying a point filter
	static CHistogram *
	PhistPointFilter(IMemoryPool *pmp, CStatsPredPoint *pstatspred,
					 CBitSet *pbsFilterColIds, CHistogram *phistBefore,
					 CDouble *pdScaleFactorLast, ULONG *pulColIdLast);

	// create a new histogram after applying a LIKE filter
	static CHistogram *
	PhistLikeFilter(IMemoryPool *pmp, CStatsPredLike *pstatspred,
					CBitSet *pbsFilterColIds, CHistogram *phistBefore,
					CDouble *pdScaleFactorLast, ULONG *pulColIdLast);

	// create a new histogram for an unsupported predicate
	static CHistogram *
	PhistUnsupportedPred(IMemoryPool *pmp, CStatsPredUnsupported *pstatspred,
						 CBitSet *pbsFilterColIds, CHistogram *phistBefore,
						 CDouble *pdScaleFactorLast, ULONG *pulColIdLast);

	// create a new hash map of histograms after applying a conjunctive or disjunctive filter
	static HMUlHist *
	PhmulhistApplyConjOrDisjFilter(IMemoryPool *pmp,
	...
	static HMUlHist *
	PhmulhistApplyConjFilter(IMemoryPool *pmp,
							 const CStatisticsConfig *pstatsconf,
							 HMUlHist *phmulhistIntermediate,
							 CDouble dRowsInput, CStatsPredConj *pstatspredConj,
							 CDouble *pdScaleFactor);

	// create new hash map of histograms after applying the disjunctive predicate
	static HMUlHist *
	PhmulhistApplyDisjFilter(IMemoryPool *pmp,
							 const CStatisticsConfig *pstatsconf,
							 HMUlHist *phmulhistInput, CDouble dRowsInput,
	...
	static BOOL
	FNewStatsColumn(ULONG ulColId, ULONG ulColIdLast);

public:
	// filter
	static CStatistics *
	PstatsFilter(IMemoryPool *pmp, const CStatistics *pstatsInput,
				 CStatsPred *pstatspredBase, BOOL fCapNdvs);

	// derive statistics for filter operation based on given scalar expression
	static IStatistics *
	PstatsFilterForScalarExpr(
		IMemoryPool *pmp, CExpressionHandle &exprhdl, IStatistics *pstatsChild,
		CExpression
			*pexprScalarLocal,  // filter expression on local columns only
		CExpression *
