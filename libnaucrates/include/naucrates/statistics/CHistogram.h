...
class CHistogram
{
	...
	typedef CHashMap<ULONG, CHistogram, gpos::UlHash<ULONG>,
					 gpos::FEqual<ULONG>, CleanupDelete<ULONG>,
					 CleanupDelete<CHistogram> >
		HMUlHist;

	// iterator
	typedef CHashMapIter<ULONG, CHistogram, gpos::UlHash<ULONG>,
						 gpos::FEqual<ULONG>, CleanupDelete<ULONG>,
						 CleanupDelete<CHistogram> >
		HMIterUlHist;
	...
	// iterator
	typedef CHashMapIter<ULONG, CDouble, gpos::UlHash<ULONG>,
						 gpos::FEqual<ULONG>, CleanupDelete<ULONG>,
...
private:
	...
	CHistogram &
	operator=(const CHistogram &);

	// return an array buckets after applying equality filter on the histogram buckets
	DrgPbucket *
	PdrgppbucketEqual(IMemoryPool *pmp, CPoint *ppoint) const;

	// return an array buckets after applying non equality filter on the histogram buckets
	DrgPbucket *
	PdrgppbucketNEqual(IMemoryPool *pmp, CPoint *ppoint) const;

	// less than or less than equal filter
	CHistogram *
	PhistLessThanOrLessThanEqual(IMemoryPool *pmp,
								 CStatsPred::EStatsCmpType escmpt,
								 CPoint *ppoint) const;

	// greater than or greater than equal filter
	CHistogram *
	PhistGreaterThanOrGreaterThanEqual(IMemoryPool *pmp,
									   CStatsPred::EStatsCmpType escmpt,
									   CPoint *ppoint) const;

	// equal filter
	CHistogram *
	PhistEqual(IMemoryPool *pmp, CPoint *ppoint) const;

	// not equal filter
	CHistogram *
	PhistNEqual(IMemoryPool *pmp, CPoint *ppoint) const;

	// IDF filter
	CHistogram *
	PhistIDF(IMemoryPool *pmp, CPoint *ppoint) const;

	// INDF filter
	CHistogram *
	PhistINDF(IMemoryPool *pmp, CPoint *ppoint) const;

	// equality join
	CHistogram *
	PhistJoinEquality(IMemoryPool *pmp, const CHistogram *phist) const;

	// generate histogram based on NDV
	CHistogram *
	PhistJoinEqualityNDV(IMemoryPool *pmp, const CHistogram *phist) const;

	// construct a new histogram for an INDF join predicate
	CHistogram *
	PhistJoinINDF(IMemoryPool *pmp, const CHistogram *phist) const;
	...
	static void
	AddBuckets(IMemoryPool *pmp, DrgPbucket *pdrgppbucketSrc,
			   DrgPbucket *pdrgppbucketDest, CDouble dRowsOld, CDouble dRowsNew,
			   ULONG ulBegin, ULONG ulEnd);
	...
	static BOOL
	FCanComputeJoinNDVRemain(const CHistogram *phist1,
	...
	ComputeJoinNDVRemainInfo(
		...
		CDouble
			dFreqJoinBuckets1,  // frequency of the buckets in input1 that contributed to the join
		CDouble
			dFreqJoinBuckets2,  // frequency of the buckets in input2 that contributed to the join
		CDouble *pdDistinctRemain, CDouble *pFreqRemain);


	// check if the cardinality estimation should be done only via NDVs
	static BOOL
	FNDVBasedCardEstimation(const CHistogram *phist);

public:
	// ctors
	explicit CHistogram(DrgPbucket *pdrgppbucket, BOOL fWellDefined = true);

	CHistogram(DrgPbucket *pdrgppbucket, BOOL fWellDefined, CDouble dNullFreq,
			   CDouble dDistinctRemain, CDouble dFreqRemain,
			   BOOL fColStatsMissing = false);

	// set null frequency
	void
	SetNullFrequency(CDouble dNullFreq);
	...
	CHistogram *
	PhistFilter(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
				CPoint *ppoint) const;

	// filter by comparing with point and normalize
	CHistogram *
	PhistFilterNormalized(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
						  CPoint *ppoint, CDouble *pdScaleFactor) const;

	// join with another histogram
	CHistogram *
	PhistJoin(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
			  const CHistogram *phist) const;

	// LASJ with another histogram
	CHistogram *
	PhistLASJ(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
			  const CHistogram *phist) const;

	// join with another histogram and normalize it.
	// If the join is not an equality join the function returns an empty histogram
	CHistogram *
	PhistJoinNormalized(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
						CDouble dRows, const CHistogram *phistOther,
						CDouble dRowsOther, CDouble *pdScaleFactor) const;

	// scale factor of inequality (!=) join
	CDouble
	DInEqualityJoinScaleFactor(IMemoryPool *pmp, CDouble dRows,
							   const CHistogram *phistOther,
							   CDouble dRowsOther) const;

	// left anti semi join with another histogram and normalize
	CHistogram *
	PhistLASJoinNormalized(
		IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt, CDouble dRows,
		const CHistogram *phistOther, CDouble *pdScaleFactor,
		BOOL
			fIgnoreLasjHistComputation  // except for the case of LOJ cardinality estimation this flag is always
		// "true" since LASJ stats computation is very aggressive
		) const;

	// group by and normalize
	CHistogram *
	PhistGroupByNormalized(IMemoryPool *pmp, CDouble dRows,
						   CDouble *pdScaleFactor) const;

	// union all and normalize
	CHistogram *
	PhistUnionAllNormalized(IMemoryPool *pmp, CDouble dRows,
							const CHistogram *phistOther,
							CDouble dRowsOther) const;

	// union and normalize
	CHistogram *
	PhistUnionNormalized(IMemoryPool *pmp, CDouble dRows,
						 const CHistogram *phistOther, CDouble dRowsOther,
						 CDouble *pdRowsOutput) const;

	// cleanup residual buckets
	void
	CleanupResidualBucket(CBucket *pbucket, BOOL fbucketResidual) const;

	// get the next bucket for union / union all
	CBucket *
	PbucketNext(const CHistogram *phist, CBucket *pbucketNew,
				BOOL *pfbucket2Residual, ULONG *pulBucketIndex) const;

	// create a new histogram with updated bucket frequency
	CHistogram *
	PhistUpdatedFrequency(IMemoryPool *pmp, DrgPbucket *pdrgppbucket,
						  DrgPdouble *pdrgpdouble, CDouble *pdRowsOutput,
						  CDouble dNullRows, CDouble dNDVRemain,
						  CDouble dNDVRemainRows) const;

	// add residual union all buckets after the merge
	ULONG
	UlAddResidualUnionAllBucket(IMemoryPool *pmp, DrgPbucket *pdrgppbucket,
	...
	ULONG
	UlAddResidualUnionBucket(IMemoryPool *pmp, DrgPbucket *pdrgppbucket,
	...
	CHistogram *
	PhistCopy(IMemoryPool *pmp) const;
	...
	Pdxlstatsdercol(IMemoryPool *pmp, CMDAccessor *pmda, ULONG ulColId,
					CDouble dWidth) const;

	// randomly pick a bucket index
	ULONG
	UlRandomBucketIndex(ULONG *pulSeed) const;
	...
	static void
	AddEmptyHistogram(IMemoryPool *pmp, HMUlHist *phmulhistOutput,
