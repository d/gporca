...
typedef CHashMap<ULONG, DrgPul, gpos::UlHash<ULONG>, gpos::FEqual<ULONG>,
				 CleanupDelete<ULONG>, CleanupRelease<DrgPul> >
	HMUlPdrgpul;
...
public:
	...
	void
	AddNotExcludedHistograms(IMemoryPool *pmp, CBitSet *pbsExcludedColIds,
...
private:
	...
	static const ULONG ulStatsEstimationNoRisk;

	// helper method to add histograms where the column ids have been remapped
	static void
	AddHistogramsWithRemap(IMemoryPool *pmp, HMUlHist *phmulhistSrc,
						   HMUlHist *phmulhistDest, HMUlCr *phmulcr,
						   BOOL fMustExist);

	// helper method to add width information where the column ids have been remapped
	static void
	AddWidthInfoWithRemap(IMemoryPool *pmp, HMUlDouble *phmuldoubleSrc,
						  HMUlDouble *phmuldoubleDest, HMUlCr *phmulcr,
						  BOOL fMustExist);

public:
	// ctor
	CStatistics(IMemoryPool *pmp, HMUlHist *phmulhist,
				HMUlDouble *phmuldoubleWidth, CDouble dRows, BOOL fEmpty,
	...
	virtual HMUlDouble *
	CopyWidths(IMemoryPool *pmp) const;

	virtual void
	CopyWidthsInto(IMemoryPool *pmp, HMUlDouble *phmuldouble) const;

	virtual HMUlHist *
	CopyHistograms(IMemoryPool *pmp) const;
	...
	virtual CDouble
	DSkew(ULONG ulColId) const;

	// what is the width in bytes of set of column id's
	virtual CDouble
	DWidth(DrgPul *pdrgpulColIds) const;

	// what is the width in bytes of set of column references
	virtual CDouble
	DWidth(IMemoryPool *pmp, CColRefSet *pcrs) const;
	...
	virtual CDouble
	DNDV(const CColRef *pcr);

	// look up the width of a particular column
	virtual const CDouble *
	PdWidth(ULONG ulColId) const;

	// the risk of errors in cardinality estimation
	virtual ULONG
	UlStatsEstimationRisk() const
	{
	...
	virtual void
	SetStatsEstimationRisk(ULONG ulRisk)
	{
	...
	virtual CStatistics *
	PstatsInnerJoin(IMemoryPool *pmp, const IStatistics *pistatsOther,
					DrgPstatspredjoin *pdrgpstatspredjoin) const;

	// LOJ with another stats structure
	virtual CStatistics *
	PstatsLOJ(IMemoryPool *pmp, const IStatistics *pistatsOther,
			  DrgPstatspredjoin *pdrgpstatspredjoin) const;

	// left anti semi join with another stats structure
	virtual CStatistics *
	PstatsLASJoin(
		IMemoryPool *pmp, const IStatistics *pstatsOther,
	...
	virtual CStatistics *
	PstatsLSJoin(IMemoryPool *pmp, const IStatistics *pstatsInner,
				 DrgPstatspredjoin *pdrgpstatspredjoin) const;

	// return required props associated with stats object
	virtual CReqdPropRelational *
	Prprel(IMemoryPool *pmp) const;

	// append given stats to current object
	virtual void
	AppendStats(IMemoryPool *pmp, IStatistics *pstats);

	// set number of rebinds
	virtual void
	SetRebinds(CDouble dRebinds)
	{
	...
	// conversion function
	static CStatistics *
	PstatsConvert(IStatistics *pistats)
	{
	...
	static CStatistics *
	PstatsDummy(IMemoryPool *pmp, DrgPul *pdrgpulColIds, CDouble dRows);

	// create a dummy statistics object
	static CStatistics *
	PstatsDummy(IMemoryPool *pmp, DrgPul *pdrgpulHistColIds,
				DrgPul *pdrgpulWidthColIds, CDouble dRows);

	// default column width
	static const CDouble DDefaultColumnWidth;

	// default number of rows in relation
	static const CDouble DDefaultRelationRows;

	// minimum number of rows in relation
	static const CDouble DMinRows;

	// epsilon
	static const CDouble DEpsilon;

	// default number of distinct values
	static const CDouble DDefaultDistinctValues;

	// check if the input statistics from join statistics computation empty
	static BOOL
	FEmptyJoinInput(const CStatistics *pstatsOuter,
					const CStatistics *pstatsInner, BOOL fLASJ);

	// add upper bound ndvs information for a given set of columns
	static void
	CreateAndInsertUpperBoundNDVs(IMemoryPool *pmp, CStatistics *pstats,
								  DrgPul *pdrgpulColIds, CDouble dRows);

	// cap the total number of distinct values (NDV) in buckets to the number of rows
	static void
	CapNDVs(CDouble dRows, HMUlHist *phmulhist);
};  // class CStatistics
