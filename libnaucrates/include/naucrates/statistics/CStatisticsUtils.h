...
private:
	...
	struct SMcvPair
	{
		...
		// ctor
		SMcvPair(IDatum *pdatumMCV, CDouble dMcvFreq)
			: m_pdatumMCV(pdatumMCV), m_dMcvFreq(dMcvFreq)
		{
		}
	...
	static DrgPbucket *
	PdrgpbucketMergeBuckets(IMemoryPool *pmp, const DrgPbucket *pdrgpbucketMCV,
							const DrgPbucket *pdrgpbucketHist);

	// split a histogram bucket given an MCV bucket
	static DrgPbucket *
	PdrgpbucketSplitHistBucket(IMemoryPool *pmp, const CBucket *pbucketHist,
	...
	static CBucket *
	PbucketCreateValidBucket(IMemoryPool *pmp, CPoint *ppointLower,
							 CPoint *ppointUpper, BOOL fLowerClosed,
							 BOOL fUpperClosed);

	// given lower and upper bound information and their closedness, test if they can form a valid bucket
	static BOOL
	FValidBucket(CPoint *ppointLower, CPoint *ppointUpper, BOOL fLowerClosed,
				 BOOL fUpperClosed);

	// find the MCVs that fall within the same histogram bucket and perform the split
	static void
	SplitHistDriver(IMemoryPool *pmp, const CBucket *pbucketHist,
					const DrgPbucket *pdrgpbucketMCV,
					DrgPbucket *pdrgpbucketMerged, ULONG *pulMCVIdx,
					ULONG ulMCV);

	// distribute total dDistinct and dFrequency of the histogram bucket into the new buckets
	static void
	DistributeBucketProperties(CDouble dFrequencyTotal, CDouble dDistinctTotal,
							   DrgPbucket *pdrgpbucket);

	// add the NDVs for all of the grouping columns
	static void
	AddNdvForAllGrpCols(IMemoryPool *pmp, const CStatistics *pstatsInput,
	...
	static CDouble
	DMaxGroupsFromSource(IMemoryPool *pmp, const CStatisticsConfig *pstatsconf,
						 CStatistics *pstatsInput, const DrgPul *pdrgpulPerSrc);

	// check to see if any one of the grouping columns has been capped
	static BOOL
	FExistsCappedGrpCol(const CStatistics *pstats, const DrgPul *pdrgpulGrpCol);

	// return the maximum NDV given an array of grouping columns
	static CDouble
	DMaxNdv(const CStatistics *pstats, const DrgPul *pdrgpulGrpCol);

public:
	// get the next data point for generating new bucket boundary
	static CPoint *
	PpointNext(IMemoryPool *pmp, CMDAccessor *pmda, CPoint *ppoint);

	// transform mcv information to optimizer's histogram structure
	static CHistogram *
	PhistTransformMCV(IMemoryPool *pmp, const IMDType *pmdtype,
					  DrgPdatum *pdrgpdatumMCV, DrgPdouble *pdrgpdFreq,
					  ULONG ulNumMCVValues);

	// merge MCVs and histogram
	static CHistogram *
	PhistMergeMcvHist(IMemoryPool *pmp, const CHistogram *phistGPDBMcv,
					  const CHistogram *phistGPDBHist);

	// comparison function for sorting MCVs
	static inline INT
	IMcvPairCmp(const void *pv1, const void *pv2);

	// utility function to print column stats before/after applying filter
	static void
	PrintColStats(IMemoryPool *pmp, CStatsPred *pstatspred, ULONG ulColIdCond,
				  CHistogram *phist, CDouble dScaleFactorLast, BOOL fBefore);

	// extract all the column identifiers used in the statistics filter
	static void
	ExtractUsedColIds(IMemoryPool *pmp, CBitSet *pbsColIds,
					  CStatsPred *pstatspred, DrgPul *pdrgpulColIds);

	// given the previously generated histogram, update the intermediate
	// result of the disjunction
	static void
	UpdateDisjStatistics(IMemoryPool *pmp, CBitSet *pbsStatsNonUpdateableCols,
						 CDouble dRowsInputDisj, CDouble dRowsLocal,
						 CHistogram *phistPrev, HMUlHist *phmulhistResultDisj,
						 ULONG ulColIdLast);
	...
	// disjunction
	static CBitSet *
	PbsNonUpdatableHistForDisj(IMemoryPool *pmp, CStatsPredDisj *pstatspred);

	// helper method to add a histogram to a map
	static void
	AddHistogram(IMemoryPool *pmp, ULONG ulColId, const CHistogram *phist,
				 HMUlHist *phmulhist, BOOL fReplaceOld = false);

	// create a new hash map of histograms after merging
	// the histograms generated by the child of disjunctive predicate
	static HMUlHist *
	PhmulhistMergeAfterDisjChild(IMemoryPool *pmp,
	...
	static CDouble
	DNumOfDistinctVal(const CStatisticsConfig *pstatsconf,
	...
	static HMUlPdrgpul *
	PhmpuldrgpulTblOpIdToGrpColsMap(IMemoryPool *pmp, CStatistics *pstats,
									const CColRefSet *pcrsGrpCols,
									CBitSet *pbsKeys);

	// extract NDVs for the given array of grouping columns
	static DrgPdouble *
	PdrgPdoubleNDV(IMemoryPool *pmp, const CStatisticsConfig *pstatsconf,
	...
	static CDouble
	DGroups(IMemoryPool *pmp, IStatistics *pstats,
			const CStatisticsConfig *pstatsconf, DrgPul *pdrgpulGC,
			CBitSet *pbsKeys);

	// return the default number of distinct values
	static CDouble
	DDefaultDistinctVals(CDouble dRows)
	{
		return std::min(CStatistics::DDefaultDistinctValues.DVal(),
						dRows.DVal());
	}

	// add the statistics (histogram and width) of the grouping columns
	static void
	AddGrpColStats(IMemoryPool *pmp, const CStatistics *pstatsInput,
				   CColRefSet *pcrsGrpCols, HMUlHist *phmulhistOutput,
				   HMUlDouble *phmuldoubleWidthOutput);

	// return the set of grouping columns for statistics computation;
	static CColRefSet *
	PcrsGrpColsForStats(
		IMemoryPool *pmp, const DrgPul *pdrgpulGrpCol,
		CColRefSet *
			pcrsGrpColComputed  // output set of grouping columns that are computed attributes
	);
	...
	static CDouble
	DDistinct(const DrgPbucket *pdrgppbucket);

	// return the cumulative frequency in the given array of buckets
	static CDouble
	DFrequency(const DrgPbucket *pdrgppbucket);

	// true if the given operator increases risk of cardinality misestimation
	static BOOL
	FIncreasesRisk(CLogical *popLogical);

	// return the default column width
	static CDouble
	DDefaultColumnWidth(const IMDType *pmdtype);

	// helper method to add width information
	static void
	AddWidthInfo(IMemoryPool *pmp, HMUlDouble *phmuldoubleSrc,
...
// comparison function for sorting MCVs
INT
CStatisticsUtils::IMcvPairCmp(const void *pv1, const void *pv2)
{
