...
private:
	...
	typedef CStatsPred *(PfPstatspred)(IMemoryPool *pmp, CExpression *pexprPred,
	...
	static CStatsPred::EStatsCmpType
	Estatscmpt(IMDId *pmdid);

	// return the comparison type of an operator for the purpose of statistics computation
	static CStatsPred::EStatsCmpType
	Estatscmpt(const CWStringConst *pstrOpName);

	// extract statistics filtering information from boolean expression
	static CStatsPred *
	PstatspredBoolean(IMemoryPool *pmp, CExpression *pexprPred,
					  CColRefSet *pcrsOuterRefs);

	// extract statistics filtering information from a scalar array compare operator
	static void
	ProcessArrayCmp(IMemoryPool *pmp, CExpression *pexprPred,
					DrgPstatspred *pdrgpstatspred);

	// create and add statistics filtering information for supported filters
	static void
	AddSupportedStatsFilters(IMemoryPool *pmp, DrgPstatspred *pdrgpstatspred,
							 CExpression *pexprPred, CColRefSet *pcrsOuterRefs);

	// create a conjunctive statistics filter composed of the extracted components of the conjunction
	static CStatsPred *
	PstatspredConj(IMemoryPool *pmp, CExpression *pexprScalar,
				   CColRefSet *pcrsOuterRefs);

	// create a disjunction statistics filter composed of the extracted components of the disjunction
	static CStatsPred *
	PstatspredDisj(IMemoryPool *pmp, CExpression *pexprPred,
				   CColRefSet *pcrsOuterRefs);

	// return statistics filter type for the given expression
	static CStatsPredUtils::EPredicateType
	Ept(IMemoryPool *pmp, CExpression *pexprPred);

	// is the condition a conjunctive predicate
	static BOOL
	FConjunction(IMemoryPool *pmp, CExpression *pexprPred);

	// is the condition a boolean predicate
	static BOOL
	FBooleanScIdent(CExpression *pexprPred);

	// is the condition a point predicate
	static BOOL
	FPointPredicate(CExpression *pexprPred);

	// is the condition an IDF point predicate
	static BOOL
	FPointIDF(CExpression *pexprPred);

	// is the condition an INDF point predicate
	static BOOL
	FPointINDF(CExpression *pexprPred);

	// is the condition 'is null' on a scalar ident
	static BOOL
	FScalarIdentIsNull(CExpression *pexprPred);

	// is the condition 'is not null' on a scalar ident
	static BOOL
	FScalarIdentIsNotNull(CExpression *pexprPred);

	// extract statistics filtering information from a point comparison
	static CStatsPred *
	PstatspredPoint(IMemoryPool *pmp, CExpression *pexprPred,
					CColRefSet *pcrsOuterRefs);

	// extract statistics filtering information from a LIKE comparison
	static CStatsPred *
	PstatspredLike(IMemoryPool *pmp, CExpression *pexprPred,
				   CColRefSet *pcrsOuterRefs);

	// extract statistics filtering information from a null test
	static CStatsPred *
	PstatspredNullTest(IMemoryPool *pmp, CExpression *pexprPred,
					   CColRefSet *pcrsOuterRefs);

	// create an unsupported statistics predicate
	static CStatsPred *
	PstatspredUnsupported(IMemoryPool *pmp, CExpression *pexprPred,
						  CColRefSet *pcrsOuterRefs);
	...
	static BOOL
	FCmpColsIgnoreCast(CExpression *pexpr, const CColRef **pcr1,
...
public:
	...
	static CStatsPred *
	PstatspredExtract(IMemoryPool *pmp, CExpression *pexprScalar,
					  CColRefSet *pcrsOuterRefs);

	// helper function to extract array of statistics join filter from an array of join predicates
	static DrgPstatspredjoin *
	PdrgpstatspredjoinExtract(
	...
	static DrgPstatspredjoin *
	Pdrgpstatspredjoin(IMemoryPool *pmp, CExpressionHandle &exprhdl);

	// helper function to extract array of statistics join filter from an expression
	static DrgPstatspredjoin *
	Pdrgpstatspredjoin(IMemoryPool *pmp, CExpressionHandle &exprhdl,
					   CExpression *pexprScalar, DrgPcrs *pdrgpcrsOutput,
					   CColRefSet *pcrsOuterRefs);

	// is the predicate a conjunctive or disjunctive predicate
	static BOOL
	FConjOrDisjPred(CStatsPred *pstatspred);

	// is unsupported predicate on unknown column
	static BOOL
	FUnsupportedPredOnDefinedCol(CStatsPred *pstatspred);
