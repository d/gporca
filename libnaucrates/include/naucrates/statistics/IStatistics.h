...
public:
	...
	// anti semi join
	virtual IStatistics *
	PstatsLASJoin(IMemoryPool *pmp, const IStatistics *pistatsOther,
				  DrgPstatspredjoin *pdrgpstatspredjoin,
				  BOOL fIgnoreLasjHistComputation) const = 0;

	// return required props associated with stats object
	virtual CReqdPropRelational *
	Prprel(IMemoryPool *pmp) const = 0;

	// append given stats to current object
	virtual void
	AppendStats(IMemoryPool *pmp, IStatistics *pstats) = 0;

	// set number of rebinds
	virtual void
	SetRebinds(CDouble dRebinds) = 0;

	// copy stats
	virtual IStatistics *
	PstatsCopy(IMemoryPool *pmp) const = 0;

	// return a copy of this stats object scaled by a given factor
	virtual IStatistics *
	PstatsScale(IMemoryPool *pmp, CDouble dFactor) const = 0;

	// copy stats with remapped column ids
	virtual IStatistics *
	PstatsCopyWithRemap(IMemoryPool *pmp, HMUlCr *phmulcr,
						BOOL fMustExist = true) const = 0;

	// return a set of column references we have stats for
	virtual CColRefSet *
	Pcrs(IMemoryPool *pmp) const = 0;
	...
	Pdxlstatsderrel(IMemoryPool *pmp, CMDAccessor *pmda) const = 0;

	// is the join type either a left semi join or left anti-semi join
	static BOOL
	FSemiJoin(IStatistics::EStatsJoinType esjt)
	{
		return (IStatistics::EsjtLeftAntiSemiJoin == esjt) ||
...
inline IOstream &
operator<<(IOstream &os, IStatistics &stat)
{
	return stat.OsPrint(os);
}
// release istats
inline void
CleanupStats(IStatistics *pstats)
{
	if (NULL != pstats)
	{
