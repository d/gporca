...
//---------------------------------------------------------------------------
GPOS_RESULT
gpdxl::EresExceptionInit(IMemoryPool *pmp)
{
	//---------------------------------------------------------------------------
	...
	CMessage rgmsg[ExmiDXLSentinel] = {
		...
		CMessage(CException(gpdxl::ExmaDXL, gpdxl::ExmiDXLValidationError),
				 CException::ExsevError,
				 GPOS_WSZ_WSZLEN("XML validation exception: XML document does "
								 "not conform to the DXL schema. Details: %s"),
				 1,  // exception details
		...
		CMessage(
			...
			CException::ExsevError,
			GPOS_WSZ_WSZLEN("Query-to-DXL Translation: Attribute number %d not "
							"found in project list"),
			1,  //
			GPOS_WSZ_WSZLEN("Query-to-DXL Translation: Attribute number not "
							"found in project list")),
		...
		CMessage(
			...
			GPOS_WSZ_WSZLEN(
				"Feature not supported by the Pivotal Query Optimizer: %ls"),
			1,  //
			GPOS_WSZ_WSZLEN(
				"Feature not supported by the Pivotal Query Optimizer")),
		...
		CMessage(CException(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLDuplicateRTE),
				 CException::ExsevError,
				 GPOS_WSZ_WSZLEN("DXL-to-Query: Duplicate range table entry at "
								 "query level %d at position %d"),
		...
		CMessage(CException(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLError),
				 CException::ExsevError,
				 GPOS_WSZ_WSZLEN("Query-to-DXL Translation: %ls entry found "
								 "due to incorrect normalization of query"),
				 1,  // entry name
				 GPOS_WSZ_WSZLEN("Query-to-DXL Translation: Entry not found "
								 "due to incorrect normalization of query")),
		...
		CMessage(
			...
			CException::ExsevError,
			GPOS_WSZ_WSZLEN("Expr-to-DXL Translation: Attribute number %d not "
							"found in project list"),
			1,  // attno
			GPOS_WSZ_WSZLEN("Expr-to-DXL Translation: Attribute number not "
							"found in project list")),

		CMessage(
			CException(gpdxl::ExmaDXL, gpdxl::ExmiDXL2PlStmtAttributeNotFound),
			CException::ExsevError,
			GPOS_WSZ_WSZLEN("DXL-to-PlStmt Translation: Attribute number %d "
							"not found in project list"),
			1,  // attno
			GPOS_WSZ_WSZLEN("DXL-to-PlStmt Translation: Attribute number not "
							"found in project list")),

		CMessage(
			CException(gpdxl::ExmaDXL, gpdxl::ExmiDXL2ExprAttributeNotFound),
			CException::ExsevError,
			GPOS_WSZ_WSZLEN("DXL-to-Expr Translation: Attribute number %d not "
							"found in project list"),
			1,  // attno
			GPOS_WSZ_WSZLEN("DXL-to-Expr Translation: Attribute number not "
							"found in project list")),

		CMessage(CException(gpdxl::ExmaDXL, gpdxl::ExmiOptimizerError),
				 CException::ExsevError, GPOS_WSZ_WSZLEN("%s"),
				 1,  // attno
				 GPOS_WSZ_WSZLEN("PQO unable to generate a plan, please see "
								 "the above message for details.")),
		...
		CMessage(
			...
			CException::ExsevError,
			GPOS_WSZ_WSZLEN("Invalid comparison type code. Valid values are "
