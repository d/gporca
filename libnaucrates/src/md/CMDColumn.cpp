...
//
//---------------------------------------------------------------------------
CMDColumn::CMDColumn(CMDName *pmdname, INT iAttNo, IMDId *pmdidType,
					 INT iTypeModifier, BOOL fNullable, BOOL fDropped,
					 CDXLNode *pdxnlDefaultValue, ULONG ulLength)
	: m_pmdname(pmdname),
	  m_iAttNo(iAttNo),
...
//---------------------------------------------------------------------------
void
CMDColumn::Serialize(CXMLSerializer *pxmlser) const
{
	...
	pxmlser->AddAttribute(CDXLTokens::PstrToken(EdxltokenColumnNullable),
						  m_fNullable);
...
CMDColumn::DebugPrint(IOstream &os) const
{
	...
	const CWStringConst *pstrNullsAllowed =
		FNullable() ? CDXLTokens::PstrToken(EdxltokenTrue)
