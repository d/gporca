...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopTblScan(CDXLMemoryManager *pmm,
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopSubqScan(CDXLMemoryManager *pmm,
									const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszAlias =
		XmlstrFromAttrs(attrs, EdxltokenAlias, EdxltokenPhysicalSubqueryScan);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopResult(CDXLMemoryManager *pmm)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopHashJoin(CDXLMemoryManager *pmm,
									const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszJoinType =
		XmlstrFromAttrs(attrs, EdxltokenJoinType, EdxltokenPhysicalHashJoin);

	EdxlJoinType edxljt = EdxljtParseJoinType(
		xmlszJoinType, CDXLTokens::PstrToken(EdxltokenPhysicalHashJoin));
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopNLJoin(CDXLMemoryManager *pmm,
								  const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszJoinType =
		XmlstrFromAttrs(attrs, EdxltokenJoinType, EdxltokenPhysicalNLJoin);

	BOOL fIndexNLJ = false;
	const XMLCh *xmlszIndexNLJ =
		attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenPhysicalNLJoinIndex));
	if (NULL != xmlszIndexNLJ)
	{
		fIndexNLJ =
			FValueFromXmlstr(pmm, xmlszIndexNLJ, EdxltokenPhysicalNLJoinIndex,
							 EdxltokenPhysicalNLJoin);
	}

	EdxlJoinType edxljt = EdxljtParseJoinType(
		xmlszJoinType, CDXLTokens::PstrToken(EdxltokenPhysicalNLJoin));
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopMergeJoin(CDXLMemoryManager *pmm,
									 const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszJoinType =
		XmlstrFromAttrs(attrs, EdxltokenJoinType, EdxltokenPhysicalMergeJoin);

	EdxlJoinType edxljt = EdxljtParseJoinType(
		xmlszJoinType, CDXLTokens::PstrToken(EdxltokenPhysicalMergeJoin));

	BOOL fUniqueOuter = FValueFromAttrs(
		pmm, attrs, EdxltokenMergeJoinUniqueOuter, EdxltokenPhysicalMergeJoin);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopGatherMotion(CDXLMemoryManager *pmm,
										const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	CDXLPhysicalGatherMotion *pdxlop =
		GPOS_NEW(pmp) CDXLPhysicalGatherMotion(pmp);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopBroadcastMotion(CDXLMemoryManager *pmm,
										   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	CDXLPhysicalBroadcastMotion *pdxlop =
		GPOS_NEW(pmp) CDXLPhysicalBroadcastMotion(pmp);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopRedistributeMotion(CDXLMemoryManager *pmm,
											  const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszDuplicateSensitive =
		attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenDuplicateSensitive));
	if (NULL != xmlszDuplicateSensitive)
	{
		fDuplicateSensitive = FValueFromXmlstr(
			pmm, xmlszDuplicateSensitive, EdxltokenDuplicateSensitive,
			EdxltokenPhysicalRedistributeMotion);
	}

	CDXLPhysicalRedistributeMotion *pdxlop =
		GPOS_NEW(pmp) CDXLPhysicalRedistributeMotion(pmp, fDuplicateSensitive);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopRoutedMotion(CDXLMemoryManager *pmm,
										const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	ULONG ulSegmentIdCol = CDXLOperatorFactory::UlValueFromAttrs(
		pmm, attrs, EdxltokenSegmentIdCol,
		EdxltokenPhysicalRoutedDistributeMotion);

	CDXLPhysicalRoutedDistributeMotion *pdxlop =
		GPOS_NEW(pmp) CDXLPhysicalRoutedDistributeMotion(pmp, ulSegmentIdCol);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopRandomMotion(CDXLMemoryManager *pmm,
										const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	if (NULL != xmlszDuplicateSensitive)
	{
		fDuplicateSensitive = FValueFromXmlstr(pmm, xmlszDuplicateSensitive,
											   EdxltokenDuplicateSensitive,
											   EdxltokenPhysicalRandomMotion);
	}

	CDXLPhysicalRandomMotion *pdxlop =
		GPOS_NEW(pmp) CDXLPhysicalRandomMotion(pmp, fDuplicateSensitive);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopAppend(CDXLMemoryManager *pmm,
								  const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	BOOL fIsTarget = FValueFromAttrs(pmm, attrs, EdxltokenAppendIsTarget,
									 EdxltokenPhysicalAppend);

	BOOL fIsZapped = FValueFromAttrs(pmm, attrs, EdxltokenAppendIsZapped,
									 EdxltokenPhysicalAppend);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopLimit(CDXLMemoryManager *pmm,
								 const Attributes &  // attrs
)
{
	// get the memory pool from the memory manager
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopLimitCount(CDXLMemoryManager *pmm,
									  const Attributes &  // attrs
)
{
	// get the memory pool from the memory manager
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopLimitOffset(CDXLMemoryManager *pmm,
									   const Attributes &  // attrs
)
{
	// get the memory pool from the memory manager
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopAgg(CDXLMemoryManager *pmm, const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	const XMLCh *xmlszAggStrategy = XmlstrFromAttrs(attrs, EdxltokenAggStrategy,
	...
	const XMLCh *xmlszStreamSafe =
		attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenAggStreamSafe));
	if (NULL != xmlszStreamSafe)
	{
		fStreamSafe =
			FValueFromXmlstr(pmm, xmlszStreamSafe, EdxltokenAggStreamSafe,
							 EdxltokenPhysicalAggregate);
	}
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopSort(CDXLMemoryManager *pmm, const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	BOOL fDiscardDuplicates = FValueFromAttrs(
		pmm, attrs, EdxltokenSortDiscardDuplicates, EdxltokenPhysicalSort);
...
//---------------------------------------------------------------------------
CDXLPhysical *
CDXLOperatorFactory::PdxlopMaterialize(CDXLMemoryManager *pmm,
									   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	BOOL fEagerMaterialize = FValueFromAttrs(
		pmm, attrs, EdxltokenMaterializeEager, EdxltokenPhysicalMaterialize);

	if (1 == attrs.getLength())
	{
		// no spooling info specified -> create a non-spooling materialize operator
		pdxlopMat =
			GPOS_NEW(pmp) CDXLPhysicalMaterialize(pmp, fEagerMaterialize);
	}
	else
	{
		// parse spool id
		ULONG ulSpoolId = UlValueFromAttrs(pmm, attrs, EdxltokenSpoolId,
										   EdxltokenPhysicalMaterialize);

		// parse id of executor slice
		INT iExecutorSlice = IValueFromAttrs(
			pmm, attrs, EdxltokenExecutorSliceId, EdxltokenPhysicalMaterialize);

		ULONG ulConsumerSlices =
			UlValueFromAttrs(pmm, attrs, EdxltokenConsumerSliceCount,
							 EdxltokenPhysicalMaterialize);

		pdxlopMat = GPOS_NEW(pmp)
			CDXLPhysicalMaterialize(pmp, fEagerMaterialize, ulSpoolId,
									iExecutorSlice, ulConsumerSlices);
	}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopScalarCmp(CDXLMemoryManager *pmm,
									 const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszCmpOp =
		XmlstrFromAttrs(attrs, EdxltokenComparisonOp, EdxltokenScalarComp);

	// parse op no and function id
	IMDId *pmdidOpNo =
		PmdidFromAttrs(pmm, attrs, EdxltokenOpNo, EdxltokenScalarComp);
	...
	CWStringConst *pstrCompOpNameCopy =
		GPOS_NEW(pmp) CWStringConst(pmp, pstrCompOpName->Wsz());
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopDistinctCmp(CDXLMemoryManager *pmm,
									   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	IMDId *pmdidOpNo =
		PmdidFromAttrs(pmm, attrs, EdxltokenOpNo, EdxltokenScalarDistinctComp);
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopOpExpr(CDXLMemoryManager *pmm,
								  const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszOpName =
		XmlstrFromAttrs(attrs, EdxltokenOpName, EdxltokenScalarOpExpr);

	IMDId *pmdidOpNo =
		PmdidFromAttrs(pmm, attrs, EdxltokenOpNo, EdxltokenScalarOpExpr);

	IMDId *pmdidReturnType = NULL;
	const XMLCh *xmlszReturnType =
		attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenOpType));

	if (NULL != xmlszReturnType)
	{
		pmdidReturnType =
			PmdidFromAttrs(pmm, attrs, EdxltokenOpType, EdxltokenScalarOpExpr);
	}

	CWStringDynamic *pstrValue = CDXLUtils::PstrFromXMLCh(pmm, xmlszOpName);
	CWStringConst *pstrValueCopy =
		GPOS_NEW(pmp) CWStringConst(pmp, pstrValue->Wsz());
	GPOS_DELETE(pstrValue);

	return GPOS_NEW(pmp)
		CDXLScalarOpExpr(pmp, pmdidOpNo, pmdidReturnType, pstrValueCopy);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopArrayComp(CDXLMemoryManager *pmm,
									 const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	// get attributes
	const XMLCh *xmlszOpName =
		XmlstrFromAttrs(attrs, EdxltokenOpName, EdxltokenScalarArrayComp);

	const XMLCh *xmlszOpType =
		XmlstrFromAttrs(attrs, EdxltokenOpType, EdxltokenScalarArrayComp);

	// parse operator no and function id
	IMDId *pmdidOpNo =
		PmdidFromAttrs(pmm, attrs, EdxltokenOpNo, EdxltokenScalarArrayComp);

	EdxlArrayCompType edxlarraycomptype = Edxlarraycomptypeany;

	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenOpTypeAll), xmlszOpType))
	{
		edxlarraycomptype = Edxlarraycomptypeall;
	}
	else if (0 != XMLString::compareString(
					  CDXLTokens::XmlstrToken(EdxltokenOpTypeAny), xmlszOpType))
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLInvalidAttributeValue,
	...
	CWStringConst *pstrOpNameCopy =
		GPOS_NEW(pmp) CWStringConst(pmp, pstrOpName->Wsz());
	GPOS_DELETE(pstrOpName);

	return GPOS_NEW(pmp)
		CDXLScalarArrayComp(pmp, pmdidOpNo, pstrOpNameCopy, edxlarraycomptype);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopBoolExpr(CDXLMemoryManager *pmm,
...
CDXLScalar *
CDXLOperatorFactory::PdxlopBooleanTest(
	CDXLMemoryManager *pmm, const EdxlBooleanTestType edxlbooleantesttype)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopSubPlan(CDXLMemoryManager *pmm, IMDId *pmdid,
								   DrgPdxlcr *pdrgdxlcr,
								   ...
								   CDXLNode *pdxlnTestExpr)
{
	...
	return GPOS_NEW(pmp) CDXLScalarSubPlan(pmp, pmdid, pdrgdxlcr,
										   edxlsubplantype, pdxlnTestExpr);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopNullTest(CDXLMemoryManager *pmm, const BOOL fIsNull)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopCast(CDXLMemoryManager *pmm, const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	IMDId *pmdidType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenScalarCast);

	IMDId *pmdidFunc =
		PmdidFromAttrs(pmm, attrs, EdxltokenFuncId, EdxltokenScalarCast);
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopCoerceToDomain(CDXLMemoryManager *pmm,
										  const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	// parse type id and function id
	IMDId *pmdidType = PmdidFromAttrs(pmm, attrs, EdxltokenTypeId,
									  EdxltokenScalarCoerceToDomain);
	INT iTypeModifier = IValueFromAttrs(pmm, attrs, EdxltokenTypeMod,
										EdxltokenScalarCoerceToDomain, true,
										IDefaultTypeModifier);
	ULONG ulCoercionForm = UlValueFromAttrs(pmm, attrs, EdxltokenCoercionForm,
											EdxltokenScalarCoerceToDomain);
	INT iLoc = IValueFromAttrs(pmm, attrs, EdxltokenLocation,
							   EdxltokenScalarCoerceToDomain);

	return GPOS_NEW(pmp) CDXLScalarCoerceToDomain(
		pmp, pmdidType, iTypeModifier, (EdxlCoercionForm) ulCoercionForm, iLoc);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopCoerceViaIO(CDXLMemoryManager *pmm,
									   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	// parse type id and function id
	IMDId *pmdidType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenScalarCoerceViaIO);
	INT iTypeModifier =
		IValueFromAttrs(pmm, attrs, EdxltokenTypeMod,
						EdxltokenScalarCoerceViaIO, true, IDefaultTypeModifier);
	ULONG ulCoercionForm = UlValueFromAttrs(pmm, attrs, EdxltokenCoercionForm,
											EdxltokenScalarCoerceViaIO);
	INT iLoc = IValueFromAttrs(pmm, attrs, EdxltokenLocation,
							   EdxltokenScalarCoerceViaIO);

	return GPOS_NEW(pmp) CDXLScalarCoerceViaIO(
		pmp, pmdidType, iTypeModifier, (EdxlCoercionForm) ulCoercionForm, iLoc);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopArrayCoerceExpr(CDXLMemoryManager *pmm,
										   const Attributes &attrs)
{
	IMemoryPool *pmp = pmm->Pmp();

	IMDId *pmdidElementFunc = PmdidFromAttrs(pmm, attrs, EdxltokenElementFunc,
											 EdxltokenScalarArrayCoerceExpr);
	IMDId *pmdidType = PmdidFromAttrs(pmm, attrs, EdxltokenTypeId,
									  EdxltokenScalarArrayCoerceExpr);
	INT iTypeModifier = IValueFromAttrs(pmm, attrs, EdxltokenTypeMod,
										EdxltokenScalarArrayCoerceExpr, true,
										IDefaultTypeModifier);
	BOOL fIsExplicit = FValueFromAttrs(pmm, attrs, EdxltokenIsExplicit,
									   EdxltokenScalarArrayCoerceExpr);
	ULONG ulCoercionForm = UlValueFromAttrs(pmm, attrs, EdxltokenCoercionForm,
											EdxltokenScalarArrayCoerceExpr);
	INT iLoc = IValueFromAttrs(pmm, attrs, EdxltokenLocation,
	...
	return GPOS_NEW(pmp) CDXLScalarArrayCoerceExpr(
		pmp, pmdidElementFunc, pmdidType, iTypeModifier, fIsExplicit,
		(EdxlCoercionForm) ulCoercionForm, iLoc);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopConstValue(CDXLMemoryManager *pmm,
									  const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopIfStmt(CDXLMemoryManager *pmm,
								  const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	// get the type id
	IMDId *pmdidType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenScalarIfStmt);
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopFuncExpr(CDXLMemoryManager *pmm,
									const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	IMDId *pmdidFunc =
		PmdidFromAttrs(pmm, attrs, EdxltokenFuncId, EdxltokenScalarFuncExpr);

	BOOL fRetset = FValueFromAttrs(pmm, attrs, EdxltokenFuncRetSet,
								   EdxltokenScalarFuncExpr);

	IMDId *pmdidRetType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenScalarFuncExpr);

	INT iTypeModifier =
		IValueFromAttrs(pmm, attrs, EdxltokenTypeMod, EdxltokenScalarCast, true,
						IDefaultTypeModifier);

	return GPOS_NEW(pmp) CDXLScalarFuncExpr(pmp, pmdidFunc, pmdidRetType,
											iTypeModifier, fRetset);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopAggFunc(CDXLMemoryManager *pmm,
								   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	IMDId *pmdidAgg =
		PmdidFromAttrs(pmm, attrs, EdxltokenAggrefOid, EdxltokenScalarAggref);

	const XMLCh *xmlszStage =
		XmlstrFromAttrs(attrs, EdxltokenAggrefStage, EdxltokenScalarAggref);

	BOOL fDistinct = FValueFromAttrs(pmm, attrs, EdxltokenAggrefDistinct,
	...
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenAggrefStageNormal), xmlszStage))
	{
	...
	if (NULL != xmlszReturnType)
	{
		pmdidResolvedRetType =
			PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenScalarAggref);
	}

	return GPOS_NEW(pmp) CDXLScalarAggref(pmp, pmdidAgg, pmdidResolvedRetType,
										  fDistinct, edxlaggstage);
}
...
//---------------------------------------------------------------------------
EdxlFrameBoundary
CDXLOperatorFactory::Edxlfb(const Attributes &attrs, Edxltoken edxltoken)
{
	const XMLCh *xmlszBoundary =
		XmlstrFromAttrs(attrs, edxltoken, EdxltokenWindowFrame);
...
//---------------------------------------------------------------------------
EdxlFrameSpec
CDXLOperatorFactory::Edxlfs(const Attributes &attrs)
{
	const XMLCh *xmlszfs =
		XmlstrFromAttrs(attrs, EdxltokenWindowFrameSpec, EdxltokenWindowFrame);

	EdxlFrameSpec edxlfb = EdxlfsSentinel;
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenWindowFSRow), xmlszfs))
	{
		edxlfb = EdxlfsRow;
	}
	else if (0 == XMLString::compareString(
					  CDXLTokens::XmlstrToken(EdxltokenWindowFSRange), xmlszfs))
	{
		edxlfb = EdxlfsRange;
	}
	else
...
CDXLOperatorFactory::Edxlfes(const Attributes &attrs)
{
	const XMLCh *xmlszfes = XmlstrFromAttrs(
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		Edxltoken edxltk = (Edxltoken) pulElem[1];
		if (0 ==
			XMLString::compareString(CDXLTokens::XmlstrToken(edxltk), xmlszfes))
		{
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopArray(CDXLMemoryManager *pmm,
								 const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	IMDId *pmdidElem = PmdidFromAttrs(pmm, attrs, EdxltokenArrayElementType,
									  EdxltokenScalarArray);
	IMDId *pmdidArray =
		PmdidFromAttrs(pmm, attrs, EdxltokenArrayType, EdxltokenScalarArray);
	BOOL fMultiDimensional = FValueFromAttrs(pmm, attrs, EdxltokenArrayMultiDim,
											 EdxltokenScalarArray);

	return GPOS_NEW(pmp)
		CDXLScalarArray(pmp, pmdidElem, pmdidArray, fMultiDimensional);
}
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopScalarIdent(CDXLMemoryManager *pmm,
									   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopProjElem(CDXLMemoryManager *pmm,
									const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszAlias =
		XmlstrFromAttrs(attrs, EdxltokenAlias, EdxltokenScalarProjElem);

	// parse column id
	ULONG ulId =
		UlValueFromAttrs(pmm, attrs, EdxltokenColId, EdxltokenScalarProjElem);
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopHashExpr(CDXLMemoryManager *pmm,
									const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	IMDId *pmdidType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenScalarHashExpr);
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopSortCol(CDXLMemoryManager *pmm,
								   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	ULONG ulColId =
		UlValueFromAttrs(pmm, attrs, EdxltokenColId, EdxltokenScalarSortCol);

	// get sorting operator name
	const XMLCh *xmlszSortOpName =
		XmlstrFromAttrs(attrs, EdxltokenSortOpName, EdxltokenScalarSortCol);
	CWStringDynamic *pstrSortOpName =
		CDXLUtils::PstrFromXMLCh(pmm, xmlszSortOpName);

	// get null first property
	BOOL fNullsFirst = FValueFromAttrs(pmm, attrs, EdxltokenSortNullsFirst,
									   EdxltokenPhysicalSort);

	// parse sorting operator id
	IMDId *pmdidSortOp =
		PmdidFromAttrs(pmm, attrs, EdxltokenSortOpId, EdxltokenPhysicalSort);

	// copy dynamic string into const string
	CWStringConst *pstrSortOpNameCopy =
		GPOS_NEW(pmp) CWStringConst(pmp, pstrSortOpName->Wsz());

	GPOS_DELETE(pstrSortOpName);

	return GPOS_NEW(pmp) CDXLScalarSortCol(pmp, ulColId, pmdidSortOp,
										   pstrSortOpNameCopy, fNullsFirst);
}
...
//---------------------------------------------------------------------------
CDXLOperatorCost *
CDXLOperatorFactory::Pdxlopcost(CDXLMemoryManager *pmm, const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	const XMLCh *xmlszStartupCost =
		XmlstrFromAttrs(attrs, EdxltokenStartupCost, EdxltokenCost);

	const XMLCh *xmlszTotalCost =
		XmlstrFromAttrs(attrs, EdxltokenTotalCost, EdxltokenCost);

	const XMLCh *xmlszRows =
		XmlstrFromAttrs(attrs, EdxltokenRows, EdxltokenCost);

	const XMLCh *xmlszWidth =
		XmlstrFromAttrs(attrs, EdxltokenWidth, EdxltokenCost);

	CWStringDynamic *pstrStartupCost =
		CDXLUtils::PstrFromXMLCh(pmm, xmlszStartupCost);
	CWStringDynamic *pstrTotalCost =
		CDXLUtils::PstrFromXMLCh(pmm, xmlszTotalCost);
...
//---------------------------------------------------------------------------
CDXLTableDescr *
CDXLOperatorFactory::Pdxltabdesc(CDXLMemoryManager *pmm,
								 const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
...
//---------------------------------------------------------------------------
CDXLIndexDescr *
CDXLOperatorFactory::Pdxlid(CDXLMemoryManager *pmm, const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	// parse index descriptor from attributes
	const XMLCh *xmlszIndexName =
		XmlstrFromAttrs(attrs, EdxltokenIndexName, EdxltokenIndexDescr);

	CWStringDynamic *pstrIndexName =
		CDXLUtils::PstrFromXMLCh(pmm, xmlszIndexName);

	// parse metadata id
	IMDId *pmdid =
		PmdidFromAttrs(pmm, attrs, EdxltokenMdid, EdxltokenIndexDescr);
...
//---------------------------------------------------------------------------
CDXLColDescr *
CDXLOperatorFactory::Pdxlcd(CDXLMemoryManager *pmm, const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();

	// parse column name from attributes
	const XMLCh *xmlszColumnName =
		XmlstrFromAttrs(attrs, EdxltokenColName, EdxltokenColDescr);

	// parse column id
	ULONG ulId =
		UlValueFromAttrs(pmm, attrs, EdxltokenColId, EdxltokenColDescr);

	// parse attno
	INT iAttno = IValueFromAttrs(pmm, attrs, EdxltokenAttno, EdxltokenColDescr);
	...
	IMDId *pmdidType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenColDescr);

	// parse optional type modifier from attributes
	INT iTypeModifier =
		IValueFromAttrs(pmm, attrs, EdxltokenTypeMod, EdxltokenColDescr, true,
						IDefaultTypeModifier);

	BOOL fColDropped = false;

	const XMLCh *xmlszColDropped =
		attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenColDropped));

	if (NULL != xmlszColDropped)
	{
		// attribute is present: get value
		fColDropped = FValueFromXmlstr(pmm, xmlszColDropped,
	...
	const XMLCh *xmlszColumnLength =
		attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenColWidth));

	if (NULL != xmlszColumnLength)
	{
		ulColLen = UlValueFromXmlstr(pmm, xmlszColumnLength, EdxltokenColWidth,
									 EdxltokenColDescr);
	}

	CWStringDynamic *pstrColumnName =
		CDXLUtils::PstrFromXMLCh(pmm, xmlszColumnName);
	...
	return GPOS_NEW(pmp) CDXLColDescr(pmp, pmdname, ulId, iAttno, pmdidType,
									  iTypeModifier, fColDropped, ulColLen);
}
...
//---------------------------------------------------------------------------
CDXLColRef *
CDXLOperatorFactory::Pdxlcr(CDXLMemoryManager *pmm, const Attributes &attrs,
							Edxltoken edxltokenElement)
{
	...
	// parse column name from attributes
	const XMLCh *xmlszColumnName =
		XmlstrFromAttrs(attrs, EdxltokenColName, edxltokenElement);

	// parse column id
	ULONG ulId = 0;
	const XMLCh *xmlszColumnId =
	...
	IMDId *pmdidType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, edxltokenElement);

	// parse optional type modifier
	INT iTypeModifier =
		IValueFromAttrs(pmm, attrs, EdxltokenTypeMod, edxltokenElement, true,
						IDefaultTypeModifier);

	return GPOS_NEW(pmp)
		CDXLColRef(pmp, pmdname, ulId, pmdidType, iTypeModifier);
}
...
//---------------------------------------------------------------------------
INT
CDXLOperatorFactory::IOutputSegId(CDXLMemoryManager *pmm,
								  const Attributes &attrs)
{
	// get output segment index from attributes
	const XMLCh *xmlszSegId =
		XmlstrFromAttrs(attrs, EdxltokenSegId, EdxltokenSegment);
...
//---------------------------------------------------------------------------
const XMLCh *
CDXLOperatorFactory::XmlstrFromAttrs(const Attributes &attrs,
									 Edxltoken edxltokenAttr,
									 Edxltoken edxltokenElement, BOOL fOptional)
{
	const XMLCh *xmlszAttributeVal =
		attrs.getValue(CDXLTokens::XmlstrToken(edxltokenAttr));
...
//---------------------------------------------------------------------------
ULONG
CDXLOperatorFactory::UlValueFromXmlstr(CDXLMemoryManager *pmm,
									   const XMLCh *xmlszAttributeVal,
									   Edxltoken edxltokenAttr,
									   Edxltoken edxltokenElement)
{
...
//---------------------------------------------------------------------------
ULLONG
CDXLOperatorFactory::UllValueFromXmlstr(CDXLMemoryManager *pmm,
										const XMLCh *xmlszAttributeVal,
										Edxltoken edxltokenAttr,
										Edxltoken edxltokenElement)
{
	...
	if ((NULL != ppszEnd && sz == *ppszEnd) || LINT_MAX == liVal ||
		LINT_MIN == liVal || 0 > liVal)
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLInvalidAttributeValue,
				   CDXLTokens::PstrToken(edxltokenAttr)->Wsz(),
				   CDXLTokens::PstrToken(edxltokenElement)->Wsz());
...
//---------------------------------------------------------------------------
BOOL
CDXLOperatorFactory::FValueFromXmlstr(CDXLMemoryManager *pmm,
									  const XMLCh *xmlszAttributeVal,
									  Edxltoken edxltokenAttr,
									  Edxltoken edxltokenElement)
{
...
//---------------------------------------------------------------------------
INT
CDXLOperatorFactory::IValueFromXmlstr(CDXLMemoryManager *pmm,
									  const XMLCh *xmlszAttributeVal,
									  Edxltoken edxltokenAttr,
									  Edxltoken edxltokenElement)
{
...
//---------------------------------------------------------------------------
INT
CDXLOperatorFactory::IValueFromAttrs(CDXLMemoryManager *pmm,
									 const Attributes &attrs,
									 Edxltoken edxltokenAttr,
									 Edxltoken edxltokenElement, BOOL fOptional,
									 INT iDefaultValue)
{
	const XMLCh *xmlszValue = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement, fOptional);
	...
	return IValueFromXmlstr(pmm, xmlszValue, edxltokenAttr, edxltokenElement);
}
...
//---------------------------------------------------------------------------
SINT
CDXLOperatorFactory::SValueFromXmlstr(CDXLMemoryManager *pmm,
									  const XMLCh *xmlszAttributeVal,
									  Edxltoken edxltokenAttr,
									  Edxltoken edxltokenElement)
{
...
//---------------------------------------------------------------------------
SINT
CDXLOperatorFactory::SValueFromAttrs(CDXLMemoryManager *pmm,
									 const Attributes &attrs,
									 Edxltoken edxltokenAttr,
									 Edxltoken edxltokenElement, BOOL fOptional,
									 SINT sDefaultValue)
{
	...
	return SValueFromXmlstr(pmm, xmlszValue, edxltokenAttr, edxltokenElement);
}

// Converts the attribute value from xml string to char
CHAR
CDXLOperatorFactory::CValueFromXmlstr(CDXLMemoryManager *pmm,
...
//---------------------------------------------------------------------------
OID
CDXLOperatorFactory::OidValueFromXmlstr(CDXLMemoryManager *pmm,
										const XMLCh *xmlszAttributeVal,
										Edxltoken edxltokenAttr,
										Edxltoken edxltokenElement)
{
...
//---------------------------------------------------------------------------
OID
CDXLOperatorFactory::OidValueFromAttrs(CDXLMemoryManager *pmm,
									   const Attributes &attrs,
									   Edxltoken edxltokenAttr,
									   Edxltoken edxltokenElement,
									   BOOL fOptional, OID OidDefaultValue)
{
	const XMLCh *xmlszValue = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement, fOptional);
...
//---------------------------------------------------------------------------
CHAR *
CDXLOperatorFactory::SzValueFromXmlstr(CDXLMemoryManager *pmm,
									   const XMLCh *xmlsz,
									   Edxltoken,  // edxltokenAttr,
									   Edxltoken   // edxltokenElement
)
...
//---------------------------------------------------------------------------
CHAR *
CDXLOperatorFactory::SzValueFromAttrs(CDXLMemoryManager *pmm,
									  const Attributes &attrs,
									  Edxltoken edxltokenAttr,
									  Edxltoken edxltokenElement,
									  BOOL fOptional, CHAR *szDefaultValue)
{
	const XMLCh *xmlszValue = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement, fOptional);
	...
	return CDXLOperatorFactory::SzValueFromXmlstr(
		pmm, xmlszValue, edxltokenAttr, edxltokenElement);
}
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLOperatorFactory::PstrValueFromAttrs(CDXLMemoryManager *pmm,
										const Attributes &attrs,
										Edxltoken edxltokenAttr,
										Edxltoken edxltokenElement)
{
	const XMLCh *xmlszValue = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement);
...
//---------------------------------------------------------------------------
BOOL
CDXLOperatorFactory::FValueFromAttrs(CDXLMemoryManager *pmm,
									 const Attributes &attrs,
									 Edxltoken edxltokenAttr,
									 Edxltoken edxltokenElement, BOOL fOptional,
									 BOOL fDefaultValue)
{
	...
	return FValueFromXmlstr(pmm, xmlszValue, edxltokenAttr, edxltokenElement);
}
...
//---------------------------------------------------------------------------
ULONG
CDXLOperatorFactory::UlValueFromAttrs(CDXLMemoryManager *pmm,
									  const Attributes &attrs,
									  Edxltoken edxltokenAttr,
									  Edxltoken edxltokenElement,
									  BOOL fOptional, ULONG ulDefaultValue)
{
	const XMLCh *xmlszValue = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement, fOptional);
	...
	return UlValueFromXmlstr(pmm, xmlszValue, edxltokenAttr, edxltokenElement);
}
...
//---------------------------------------------------------------------------
ULLONG
CDXLOperatorFactory::UllValueFromAttrs(CDXLMemoryManager *pmm,
									   const Attributes &attrs,
									   Edxltoken edxltokenAttr,
									   Edxltoken edxltokenElement,
									   BOOL fOptional, ULLONG ullDefaultValue)
{
	const XMLCh *xmlszValue = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement, fOptional);
	...
	return UllValueFromXmlstr(pmm, xmlszValue, edxltokenAttr, edxltokenElement);
}
...
//---------------------------------------------------------------------------
ULONG
CDXLOperatorFactory::UlGroupingColId(CDXLMemoryManager *pmm,
									 const Attributes &attrs)
{
	const CWStringConst *pstrTokenGroupingCol =
		CDXLTokens::PstrToken(EdxltokenGroupingCol);
	const CWStringConst *pstrTokenColId = CDXLTokens::PstrToken(EdxltokenColId);

	// get grouping column id from attributes
	INT iColId =
		IValueFromAttrs(pmm, attrs, EdxltokenColId, EdxltokenGroupingCol);

	if (iColId < 0)
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLInvalidAttributeValue,
				   pstrTokenColId->Wsz(), pstrTokenGroupingCol->Wsz());
	}
...
//---------------------------------------------------------------------------
IMDId *
CDXLOperatorFactory::PmdidFromAttrs(CDXLMemoryManager *pmm,
									const Attributes &attrs,
									Edxltoken edxltokenAttr,
									Edxltoken edxltokenElement, BOOL fOptional,
									IMDId *pmdidDefault)
{
	...
	const XMLCh *xmlszMdid =
		XmlstrFromAttrs(attrs, edxltokenAttr, edxltokenElement, fOptional);
...
//---------------------------------------------------------------------------
IMDId *
CDXLOperatorFactory::PmdidFromXMLCh(CDXLMemoryManager *pmm,
									const XMLCh *xmlszMdid,
									Edxltoken edxltokenAttr,
									Edxltoken edxltokenElement)
{
	// extract mdid's components: MdidType.Oid.Major.Minor
	XMLStringTokenizer xmlsztok(xmlszMdid,
								CDXLTokens::XmlstrToken(EdxltokenDotSemicolon));
	...
	IMDId::EMDIdType emdt = (IMDId::EMDIdType) UlValueFromXmlstr(
		pmm, xmlszMdidType, edxltokenAttr, edxltokenElement);
	...
	switch (emdt)
	{
		...
		case IMDId::EmdidGPDBCtas:
			pmdid =
				PmdidGPDBCTAS(pmm, pdrgpxmlsz, edxltokenAttr, edxltokenElement);
			break;

		case IMDId::EmdidColStats:
			pmdid =
				PmdidColStats(pmm, pdrgpxmlsz, edxltokenAttr, edxltokenElement);
			break;

		case IMDId::EmdidRelStats:
			pmdid =
				PmdidRelStats(pmm, pdrgpxmlsz, edxltokenAttr, edxltokenElement);
			break;

		case IMDId::EmdidCastFunc:
			pmdid =
				PmdidCastFunc(pmm, pdrgpxmlsz, edxltokenAttr, edxltokenElement);
			break;

		case IMDId::EmdidScCmp:
			pmdid =
				PmdidScCmp(pmm, pdrgpxmlsz, edxltokenAttr, edxltokenElement);
			break;
...
//---------------------------------------------------------------------------
CMDIdGPDB *
CDXLOperatorFactory::PmdidGPDB(CDXLMemoryManager *pmm, DrgPxmlsz *pdrgpxmlsz,
							   Edxltoken edxltokenAttr,
							   Edxltoken edxltokenElement)
{
	...
	ULONG ulOid =
		UlValueFromXmlstr(pmm, xmlszOid, edxltokenAttr, edxltokenElement);

	XMLCh *xmlszVersionMajor = (*pdrgpxmlsz)[1];
	ULONG ulVersionMajor = UlValueFromXmlstr(pmm, xmlszVersionMajor,
											 edxltokenAttr, edxltokenElement);

	XMLCh *xmlszVersionMinor = (*pdrgpxmlsz)[2];
	;
	ULONG ulVersionMinor = UlValueFromXmlstr(pmm, xmlszVersionMinor,
											 edxltokenAttr, edxltokenElement);

	// construct metadata id object
	return GPOS_NEW(pmm->Pmp())
		CMDIdGPDB(ulOid, ulVersionMajor, ulVersionMinor);
}
...
//---------------------------------------------------------------------------
CMDIdGPDB *
CDXLOperatorFactory::PmdidGPDBCTAS(CDXLMemoryManager *pmm,
...
//---------------------------------------------------------------------------
CMDIdColStats *
CDXLOperatorFactory::PmdidColStats(CDXLMemoryManager *pmm,
								   DrgPxmlsz *pdrgpxmlsz,
								   Edxltoken edxltokenAttr,
								   Edxltoken edxltokenElement)
{
	...
	CMDIdGPDB *pmdidRel =
		PmdidGPDB(pmm, pdrgpxmlsz, edxltokenAttr, edxltokenElement);

	XMLCh *xmlszAttno = (*pdrgpxmlsz)[3];
	ULONG ulAttno =
		UlValueFromXmlstr(pmm, xmlszAttno, edxltokenAttr, edxltokenElement);
...
//---------------------------------------------------------------------------
CMDIdRelStats *
CDXLOperatorFactory::PmdidRelStats(CDXLMemoryManager *pmm,
								   DrgPxmlsz *pdrgpxmlsz,
								   Edxltoken edxltokenAttr,
								   Edxltoken edxltokenElement)
{
...
//---------------------------------------------------------------------------
CMDIdCast *
CDXLOperatorFactory::PmdidCastFunc(CDXLMemoryManager *pmm,
								   DrgPxmlsz *pdrgpxmlsz,
								   Edxltoken edxltokenAttr,
								   Edxltoken edxltokenElement)
{
	...
	CMDIdGPDB *pmdidDest =
		PmdidGPDB(pmm, pdrgpxmlszDest, edxltokenAttr, edxltokenElement);
...
//---------------------------------------------------------------------------
CMDIdScCmp *
CDXLOperatorFactory::PmdidScCmp(CDXLMemoryManager *pmm, DrgPxmlsz *pdrgpxmlsz,
								Edxltoken edxltokenAttr,
								Edxltoken edxltokenElement)
{
	...
	CMDIdGPDB *pmdidRight =
		PmdidGPDB(pmm, pdrgpxmlszRight, edxltokenAttr, edxltokenElement);
	...
	IMDType::ECmpType ecmpt = (IMDType::ECmpType) UlValueFromXmlstr(
		pmm, xmlszCmpType, edxltokenAttr, edxltokenElement);
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::Pdxldatum(CDXLMemoryManager *pmm, const Attributes &attrs,
							   Edxltoken edxltokenElement)
{
	...
	BOOL fConstNull =
		FValueFromAttrs(pmm, attrs, EdxltokenIsNull, edxltokenElement);
	BOOL fConstByVal =
		FValueFromAttrs(pmm, attrs, EdxltokenIsByValue, edxltokenElement);


	SDXLDatumFactoryElem rgTranslators[] = {
		// native support
		{CMDIdGPDB::m_mdidInt2.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumInt2},
		{CMDIdGPDB::m_mdidInt4.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumInt4},
		{CMDIdGPDB::m_mdidInt8.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumInt8},
		{CMDIdGPDB::m_mdidBool.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumBool},
		{CMDIdGPDB::m_mdidOid.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumOid},
		// types with long int mapping
		{CMDIdGPDB::m_mdidBPChar.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsLintMappable},
		{CMDIdGPDB::m_mdidVarChar.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsLintMappable},
		{CMDIdGPDB::m_mdidText.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsLintMappable},
		{CMDIdGPDB::m_mdidCash.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsLintMappable},
		// non-integer numeric types
		{CMDIdGPDB::m_mdidNumeric.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidFloat4.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidFloat8.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		// network-related types
		{CMDIdGPDB::m_mdidInet.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidCidr.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidMacaddr.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		// time-related types
		{CMDIdGPDB::m_mdidDate.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidTime.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidTimeTz.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidTimestamp.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidTimestampTz.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidAbsTime.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidRelTime.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidInterval.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable},
		{CMDIdGPDB::m_mdidTimeInterval.OidObjectId(),
		 &CDXLOperatorFactory::PdxldatumStatsDoubleMappable}};
	...
	if (NULL == pf)
	{
		...
		return PdxldatumGeneric(pmm, attrs, edxltokenElement, pmdid, fConstNull,
								fConstByVal);
	}
	else
	{
		return (*pf)(pmm, attrs, edxltokenElement, pmdid, fConstNull,
					 fConstByVal);
	}
}
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumOid(CDXLMemoryManager *pmm,
								  const Attributes &attrs,
								  Edxltoken edxltokenElement, IMDId *pmdid,
								  BOOL fConstNull,
								  BOOL
#ifdef GPOS_DEBUG
									  fConstByVal
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumInt2(CDXLMemoryManager *pmm,
								   const Attributes &attrs,
								   Edxltoken edxltokenElement, IMDId *pmdid,
								   BOOL fConstNull,
								   BOOL
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumInt4(CDXLMemoryManager *pmm,
								   const Attributes &attrs,
								   Edxltoken edxltokenElement, IMDId *pmdid,
								   BOOL fConstNull,
								   BOOL
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumInt8(CDXLMemoryManager *pmm,
								   const Attributes &attrs,
								   Edxltoken edxltokenElement, IMDId *pmdid,
								   BOOL fConstNull,
								   BOOL
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumBool(CDXLMemoryManager *pmm,
								   const Attributes &attrs,
								   Edxltoken edxltokenElement, IMDId *pmdid,
								   BOOL fConstNull,
								   BOOL
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumGeneric(CDXLMemoryManager *pmm,
									  const Attributes &attrs,
									  Edxltoken edxltokenElement, IMDId *pmdid,
									  BOOL fConstNull, BOOL fConstByVal)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	if (!fConstNull)
	{
		...
		if (NULL == pba)
		{
			...
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLInvalidAttributeValue,
					   ...
					   CDXLTokens::PstrToken(edxltokenElement));
		}
	}
	...
	return GPOS_NEW(pmp) CDXLDatumGeneric(
		pmp, pmdid, iTypeModifier, fConstByVal, fConstNull, pba, ulPbaLength);
}
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumStatsLintMappable(CDXLMemoryManager *pmm,
												const Attributes &attrs,
												Edxltoken edxltokenElement,
												IMDId *pmdid, BOOL fConstNull,
												BOOL fConstByVal)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	return GPOS_NEW(pmp)
		CDXLDatumStatsLintMappable(pmp, pmdid, iTypeModifier, fConstByVal,
								   fConstNull, pba, ulPbaLength, lValue);
}
...
//---------------------------------------------------------------------------
LINT
CDXLOperatorFactory::LValue(CDXLMemoryManager *pmm, const Attributes &attrs,
							Edxltoken edxltokenElement, BYTE *pba)
{
	if (NULL == pba)
	{
		// unable to decode value. probably not Base64 encoded.
...
//---------------------------------------------------------------------------
BYTE *
CDXLOperatorFactory::Pba(CDXLMemoryManager *pmm, const Attributes &attrs,
						 Edxltoken edxltokenElement, ULONG *pulLength)
{
	const XMLCh *xmlszValue =
		XmlstrFromAttrs(attrs, EdxltokenValue, edxltokenElement);
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::PdxldatumStatsDoubleMappable(CDXLMemoryManager *pmm,
												  const Attributes &attrs,
												  Edxltoken edxltokenElement,
												  IMDId *pmdid, BOOL fConstNull,
												  BOOL fConstByVal)
{
	...
	if (!fConstNull)
	{
		...
		dValue =
			DValueFromAttrs(pmm, attrs, EdxltokenDoubleValue, edxltokenElement);
	}
	INT iTypeModifier =
		IValueFromAttrs(pmm, attrs, EdxltokenTypeMod, EdxltokenScalarCast, true,
						-1 /* default value */
	...
	return GPOS_NEW(pmp)
		CDXLDatumStatsDoubleMappable(pmp, pmdid, iTypeModifier, fConstByVal,
									 fConstNull, pba, ulPbaLength, dValue);
}
...
//---------------------------------------------------------------------------
DrgPul *
CDXLOperatorFactory::PdrgpulFromAttrs(CDXLMemoryManager *pmm,
									  const Attributes &attrs,
									  Edxltoken edxltokenAttr,
									  Edxltoken edxltokenElement)
{
	const XMLCh *xmlsz = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement);
...
//---------------------------------------------------------------------------
DrgPmdid *
CDXLOperatorFactory::PdrgpmdidFromXMLCh(CDXLMemoryManager *pmm,
										...
										Edxltoken edxltokenElement)
{
	...
	XMLStringTokenizer xmlsztok(xmlszUlList,
	...
	for (ULONG ul = 0; ul < ulNumTokens; ul++)
	{
		...
		IMDId *pmdid =
			PmdidFromXMLCh(pmm, xmlszMdid, edxltokenAttr, edxltokenElement);
...
// Will raise an exception if list is not well-formed
DrgPsz *
CDXLOperatorFactory::PdrgpszFromXMLCh(CDXLMemoryManager *pmm,
									  const XMLCh *xmlsz,
									  Edxltoken edxltokenAttr,
									  Edxltoken edxltokenElement)
{
	return PdrgptFromXMLCh<CHAR, CleanupDelete, CValueFromXmlstr>(
		pmm, xmlsz, edxltokenAttr, edxltokenElement);
}
...
//---------------------------------------------------------------------------
DrgPdrgPul *
CDXLOperatorFactory::PdrgpdrgpulFromXMLCh(CDXLMemoryManager *pmm,
										  const XMLCh *xmlsz,
										  Edxltoken edxltokenAttr,
										  Edxltoken edxltokenElement)
{
	...
	XMLStringTokenizer xmlsztok(xmlsz,
								CDXLTokens::XmlstrToken(EdxltokenSemicolon));
	const ULONG ulNumTokens = xmlsztok.countTokens();
	...
	for (ULONG ul = 0; ul < ulNumTokens; ul++)
	{
		...
		DrgPul *pdrgpul =
			PdrgpulFromXMLCh(pmm, xmlszList, edxltokenAttr, edxltokenElement);
...
//---------------------------------------------------------------------------
DrgPi *
CDXLOperatorFactory::PdrgpiParseSegmentIdList(CDXLMemoryManager *pmm,
											  ...
											  Edxltoken edxltokenElement)
{
	...
	XMLStringTokenizer xmlsztok(xmlszSegIdList,
								CDXLTokens::XmlstrToken(EdxltokenComma));
	...
	for (ULONG ul = 0; ul < ulNumSegments; ul++)
	{
		...
		INT *piSegId = GPOS_NEW(pmp) INT(
			IValueFromXmlstr(pmm, xmlszSegId, edxltokenAttr, edxltokenElement));
...
//---------------------------------------------------------------------------
DrgPstr *
CDXLOperatorFactory::PdrgPstrFromXMLCh(CDXLMemoryManager *pmm,
									   const XMLCh *xmlsz)
{
	IMemoryPool *pmp = pmm->Pmp();
...
//---------------------------------------------------------------------------
void
CDXLOperatorFactory::SetSegmentInfo(CDXLMemoryManager *pmm,
									CDXLPhysicalMotion *pdxlopMotion,
									const Attributes &attrs,
									Edxltoken edxltokenElement)
{
	const XMLCh *xmlszInputSegList =
		XmlstrFromAttrs(attrs, EdxltokenInputSegments, edxltokenElement);
	DrgPi *pdrgpiInputSegments = PdrgpiParseSegmentIdList(
		pmm, xmlszInputSegList, EdxltokenInputSegments, edxltokenElement);
	pdxlopMotion->SetInputSegIds(pdrgpiInputSegments);

	const XMLCh *xmlszOutputSegList =
		XmlstrFromAttrs(attrs, EdxltokenOutputSegments, edxltokenElement);
	DrgPi *pdrgpiOutputSegments = PdrgpiParseSegmentIdList(
		pmm, xmlszOutputSegList, EdxltokenOutputSegments, edxltokenElement);
...
//---------------------------------------------------------------------------
EdxlJoinType
CDXLOperatorFactory::EdxljtParseJoinType(const XMLCh *xmlszJoinType,
										 const CWStringConst *pstrJoinName)
{
	EdxlJoinType edxljt = EdxljtSentinel;

	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenJoinInner), xmlszJoinType))
	{
		edxljt = EdxljtInner;
	}
	else if (0 ==
			 XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenJoinLeft), xmlszJoinType))
	{
		edxljt = EdxljtLeft;
	}
	else if (0 ==
			 XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenJoinFull), xmlszJoinType))
	{
		edxljt = EdxljtFull;
	}
	else if (0 ==
			 XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenJoinRight), xmlszJoinType))
	{
		edxljt = EdxljtRight;
	}
	else if (0 == XMLString::compareString(
					  CDXLTokens::XmlstrToken(EdxltokenJoinIn), xmlszJoinType))
	{
		edxljt = EdxljtIn;
	}
	else if (0 == XMLString::compareString(
...
CDXLOperatorFactory::EdxljtParseIndexScanDirection(
	const XMLCh *xmlszIndexScanDirection, const CWStringConst *pstrIndexScan)
{
	if (0 == XMLString::compareString(
...
//---------------------------------------------------------------------------
CDXLLogical *
CDXLOperatorFactory::PdxlopLogicalJoin(CDXLMemoryManager *pmm,
									   const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	...
	const XMLCh *xmlszJoinType =
		XmlstrFromAttrs(attrs, EdxltokenJoinType, EdxltokenLogicalJoin);
	EdxlJoinType edxljt = EdxljtParseJoinType(
		xmlszJoinType, CDXLTokens::PstrToken(EdxltokenLogicalJoin));
...
//---------------------------------------------------------------------------
CDouble
CDXLOperatorFactory::DValueFromXmlstr(CDXLMemoryManager *pmm,
									  const XMLCh *xmlszAttributeVal,
									  Edxltoken,  //edxltokenAttr,
									  Edxltoken   //edxltokenElement
)
...
//---------------------------------------------------------------------------
CDouble
CDXLOperatorFactory::DValueFromAttrs(CDXLMemoryManager *pmm,
									 const Attributes &attrs,
									 Edxltoken edxltokenAttr,
									 Edxltoken edxltokenElement)
{
	const XMLCh *xmlszValue = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, edxltokenAttr, edxltokenElement);
	return DValueFromXmlstr(pmm, xmlszValue, edxltokenAttr, edxltokenElement);
}
...
//---------------------------------------------------------------------------
LINT
CDXLOperatorFactory::LValueFromXmlstr(CDXLMemoryManager *pmm,
									  const XMLCh *xmlszAttributeVal,
									  Edxltoken,  //edxltokenAttr,
									  Edxltoken   //edxltokenElement
)
...
//---------------------------------------------------------------------------
LINT
CDXLOperatorFactory::LValueFromAttrs(CDXLMemoryManager *pmm,
									 const Attributes &attrs,
									 Edxltoken edxltokenAttr,
									 Edxltoken edxltokenElement, BOOL fOptional,
									 LINT lDefaultValue)
...
//---------------------------------------------------------------------------
CSystemId
CDXLOperatorFactory::Sysid(CDXLMemoryManager *pmm, const Attributes &attrs,
						   Edxltoken edxltokenAttr, Edxltoken edxltokenElement)
{
	// extract systemids
	const XMLCh *xmlsz =
		XmlstrFromAttrs(attrs, edxltokenAttr, edxltokenElement);

	// get sysid components
	XMLStringTokenizer xmlsztokSysid(xmlsz,
...
//---------------------------------------------------------------------------
CDXLScalar *
CDXLOperatorFactory::PdxlopWindowRef(CDXLMemoryManager *pmm,
									 const Attributes &attrs)
{
	// get the memory pool from the memory manager
	IMemoryPool *pmp = pmm->Pmp();
	IMDId *pmdidFunc = PmdidFromAttrs(pmm, attrs, EdxltokenWindowrefOid,
									  EdxltokenScalarWindowref);
	IMDId *pmdidRetType =
		PmdidFromAttrs(pmm, attrs, EdxltokenTypeId, EdxltokenScalarWindowref);
	BOOL fDistinct = FValueFromAttrs(pmm, attrs, EdxltokenWindowrefDistinct,
									 EdxltokenScalarWindowref);
	BOOL fStarArg = FValueFromAttrs(pmm, attrs, EdxltokenWindowrefStarArg,
									EdxltokenScalarWindowref);
	BOOL fSimpleAgg = FValueFromAttrs(pmm, attrs, EdxltokenWindowrefSimpleAgg,
									  EdxltokenScalarWindowref);
	ULONG ulWinspecPos = UlValueFromAttrs(
		pmm, attrs, EdxltokenWindowrefWinSpecPos, EdxltokenScalarWindowref);

	const XMLCh *xmlszStage = XmlstrFromAttrs(attrs, EdxltokenWindowrefStrategy,
											  EdxltokenScalarWindowref);
	...
	return GPOS_NEW(pmp)
		CDXLScalarWindowRef(pmp, pmdidFunc, pmdidRetType, fDistinct, fStarArg,
							fSimpleAgg, edxlwinstage, ulWinspecPos);
}
...
//---------------------------------------------------------------------------
IMDType::ECmpType
CDXLOperatorFactory::Ecmpt(const XMLCh *xmlszCmpType)
{
	ULONG rgrgulMapping[][2] = {{EdxltokenCmpEq, IMDType::EcmptEq},
	...
	GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLInvalidAttributeValue,
			   CDXLTokens::PstrToken(EdxltokenGPDBScalarOpCmpType)->Wsz(),
			   CDXLTokens::PstrToken(EdxltokenGPDBScalarOp)->Wsz());
...
CDXLOperatorFactory::EreldistrpolicyFromXmlstr(const XMLCh *xmlsz)
{
	GPOS_ASSERT(NULL != xmlsz);
	...
	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelDistrMasterOnly)))
	{
		ereldistrpolicy = IMDRelation::EreldistrMasterOnly;
	}
	else if (0 == XMLString::compareString(
					  xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelDistrHash)))
	{
		ereldistrpolicy = IMDRelation::EreldistrHash;
	}
	else if (0 == XMLString::compareString(
					  xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelDistrRandom)))
	{
...
CDXLOperatorFactory::ErelstoragetypeFromXmlstr(const XMLCh *xmlsz)
{
	GPOS_ASSERT(NULL != xmlsz);

	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelStorageHeap)))
	{
	...
	if (0 ==
		XMLString::compareString(
			xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelStorageAppendOnlyCols)))
	{
	...
	if (0 ==
		XMLString::compareString(
			xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelStorageAppendOnlyRows)))
	{
	...
	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelStorageExternal)))
	{
		return IMDRelation::ErelstorageExternal;
	}

	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenRelStorageVirtual)))
	{
...
CDXLOperatorFactory::EctascommitFromAttr(const Attributes &attrs)
{
	const XMLCh *xmlsz =
	...
	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenOnCommitPreserve)))
	{
		return CDXLCtasStorageOptions::EctascommitPreserve;
	}

	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenOnCommitDelete)))
	{
		return CDXLCtasStorageOptions::EctascommitDelete;
	}

	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenOnCommitDrop)))
	{
		return CDXLCtasStorageOptions::EctascommitDrop;
	}

	if (0 != XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenOnCommitNOOP)))
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLInvalidAttributeValue,
				   CDXLTokens::PstrToken(EdxltokenOnCommitAction)->Wsz(),
				   CDXLTokens::PstrToken(EdxltokenCTASOptions)->Wsz());
...
CDXLOperatorFactory::EmdindtFromAttr(const Attributes &attrs)
{
	const XMLCh *xmlsz =
	...
	if (NULL == xmlsz ||
		0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenIndexTypeBtree)))
	{
		// default is btree
	...
	if (0 == XMLString::compareString(
				 xmlsz, CDXLTokens::XmlstrToken(EdxltokenIndexTypeBitmap)))
	{
