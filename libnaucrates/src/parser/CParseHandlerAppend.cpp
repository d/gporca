...
//
//---------------------------------------------------------------------------
CParseHandlerAppend::CParseHandlerAppend(IMemoryPool *pmp,
...
CParseHandlerAppend::SetupInitialHandlers(const Attributes &attrs)
{
	...
	GPOS_ASSERT(this->UlLength() == 0 &&
				"No handlers should have been added yet");

	m_pdxlop = (CDXLPhysicalAppend *) CDXLOperatorFactory::PdxlopAppend(
		m_pphm->Pmm(), attrs);
...
//---------------------------------------------------------------------------
void
CParseHandlerAppend::StartElement(const XMLCh *const xmlszUri,
...
//---------------------------------------------------------------------------
void
CParseHandlerAppend::EndElement(const XMLCh *const,  // xmlszUri,
								const XMLCh *const xmlszLocalname,
								const XMLCh *const  // xmlszQname
)
{
	if (0 !=
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenPhysicalAppend), xmlszLocalname))
	{
		CWStringDynamic *pstr =
			CDXLUtils::PstrFromXMLCh(m_pphm->Pmm(), xmlszLocalname);
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiDXLUnexpectedTag, pstr->Wsz());
