...
//
//---------------------------------------------------------------------------
CParseHandlerAssert::CParseHandlerAssert(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerAssert::StartElement(const XMLCh *const,  // xmlszUri,
								  ...
								  const Attributes &attrs)
{
	if (0 !=
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenPhysicalAssert), xmlszLocalname))
	{
	...
	CHAR *szErrorCode = CDXLOperatorFactory::SzValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenErrorCode, EdxltokenPhysicalAssert);
	if (NULL == szErrorCode ||
		GPOS_SQLSTATE_LENGTH != clib::UlStrLen(szErrorCode))
	{
	...
	CParseHandlerBase *pphAssertPredicate = CParseHandlerFactory::Pph(
		m_pmp, CDXLTokens::XmlstrToken(EdxltokenScalarAssertConstraintList),
		m_pphm, this);
...
//---------------------------------------------------------------------------
void
CParseHandlerAssert::EndElement(const XMLCh *const,  // xmlszUri,
								const XMLCh *const xmlszLocalname,
								const XMLCh *const  // xmlszQname
)
{
	...
	CParseHandlerScalarAssertConstraintList *pphAssertPredicate =
