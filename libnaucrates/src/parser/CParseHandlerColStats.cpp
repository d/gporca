...
//
//---------------------------------------------------------------------------
CParseHandlerColStats::CParseHandlerColStats(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerColStats::StartElement(const XMLCh *const xmlszUri,
									const XMLCh *const xmlszLocalname,
									const XMLCh *const xmlszQname,
									const Attributes &attrs)
{
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenColumnStats), xmlszLocalname))
	{
		...
		IMDId *pmdid = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenColumnStats);
		...
		m_dWidth = CDXLOperatorFactory::DValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenWidth, EdxltokenColumnStats);

		const XMLCh *xmlszNullFreq =
			attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenColNullFreq));
		if (NULL != xmlszNullFreq)
		{
			m_dNullFreq = CDXLOperatorFactory::DValueFromXmlstr(
				m_pphm->Pmm(), xmlszNullFreq, EdxltokenColNullFreq,
				EdxltokenColumnStats);
		}

		const XMLCh *xmlszDistinctRemain =
			attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenColNdvRemain));
		if (NULL != xmlszDistinctRemain)
		{
			m_dDistinctRemain = CDXLOperatorFactory::DValueFromXmlstr(
				m_pphm->Pmm(), xmlszDistinctRemain, EdxltokenColNdvRemain,
				EdxltokenColumnStats);
		}

		const XMLCh *xmlszFreqRemain =
			attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenColFreqRemain));
		if (NULL != xmlszFreqRemain)
		{
			m_dFreqRemain = CDXLOperatorFactory::DValueFromXmlstr(
				m_pphm->Pmm(), xmlszFreqRemain, EdxltokenColFreqRemain,
				EdxltokenColumnStats);
		}

		const XMLCh *xmlszColStatsMissing =
			attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenColStatsMissing));
		if (NULL != xmlszColStatsMissing)
		{
			m_fColStatsMissing = CDXLOperatorFactory::FValueFromXmlstr(
				m_pphm->Pmm(), xmlszColStatsMissing, EdxltokenColStatsMissing,
				EdxltokenColumnStats);
		}
	}
	...
	{
		// new bucket
		CParseHandlerBase *pphStatsBucket = CParseHandlerFactory::Pph(
			m_pmp, CDXLTokens::XmlstrToken(EdxltokenColumnStatsBucket), m_pphm,
			this);
		this->Append(pphStatsBucket);

		m_pphm->ActivateParseHandler(pphStatsBucket);
		pphStatsBucket->startElement(xmlszUri, xmlszLocalname, xmlszQname,
									 attrs);
	}
	else
	{
...
//---------------------------------------------------------------------------
void
CParseHandlerColStats::EndElement(const XMLCh *const,  // xmlszUri,
								  const XMLCh *const xmlszLocalname,
								  const XMLCh *const  // xmlszQname
)
{
	...
	{
		CParseHandlerColStatsBucket *pphBucket =
	...
	m_pimdobj = GPOS_NEW(m_pmp) CDXLColStats(
		m_pmp, m_pmdidColStats, m_pmdname, m_dWidth, m_dNullFreq,
		m_dDistinctRemain, m_dFreqRemain, pdrgpdxlbucket, m_fColStatsMissing);
