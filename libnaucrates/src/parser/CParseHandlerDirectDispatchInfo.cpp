	...
	const Attributes &attrs)
{
					  ...
					  xmlstrLocalname))
	{
		...
		m_pdrgpdxldatum = GPOS_NEW(m_pmp) DrgPdxldatum(m_pmp);
					  ...
					  CDXLTokens::XmlstrToken(EdxltokenDatum), xmlstrLocalname))
	{
		GPOS_ASSERT(NULL != m_pdrgpdxldatum);

		CDXLDatum *pdxldatum = CDXLOperatorFactory::Pdxldatum(
			m_pphm->Pmm(), attrs, EdxltokenDirectDispatchInfo);
...
//---------------------------------------------------------------------------
void
CParseHandlerDirectDispatchInfo::EndElement(const XMLCh *const,  // xmlstrUri,
											const XMLCh *const xmlstrLocalname,
											const XMLCh *const  // xmlstrQname
)
{
	...
	{
		m_pdxlddinfo =
			GPOS_NEW(m_pmp) CDXLDirectDispatchInfo(m_pdrgpdrgpdxldatum);
