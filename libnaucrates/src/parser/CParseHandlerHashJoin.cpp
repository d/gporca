...
//
//---------------------------------------------------------------------------
CParseHandlerHashJoin::CParseHandlerHashJoin(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerHashJoin::StartElement(const XMLCh *const,  // xmlszUri,
									...
									const Attributes &attrs)
{
	if (0 !=
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenPhysicalHashJoin), xmlszLocalname))
	{
	...
	m_pdxlop = (CDXLPhysicalHashJoin *) CDXLOperatorFactory::PdxlopHashJoin(
		m_pphm->Pmm(), attrs);
	...
	CParseHandlerBase *pphHashCondList = CParseHandlerFactory::Pph(
		m_pmp, CDXLTokens::XmlstrToken(EdxltokenScalarHashCondList), m_pphm,
		this);
	m_pphm->ActivateParseHandler(pphHashCondList);

	// parse handler for the join filter
	CParseHandlerBase *ppHJFilter = CParseHandlerFactory::Pph(
...
//---------------------------------------------------------------------------
void
CParseHandlerHashJoin::EndElement(const XMLCh *const,  // xmlszUri,
								  const XMLCh *const xmlszLocalname,
								  const XMLCh *const  // xmlszQname
)
{
	...
	CParseHandlerFilter *ppHJFilter =
		dynamic_cast<CParseHandlerFilter *>((*this)[3]);
	CParseHandlerCondList *pphHashCondList =
		dynamic_cast<CParseHandlerCondList *>((*this)[4]);
	CParseHandlerPhysicalOp *pphLeft =
		dynamic_cast<CParseHandlerPhysicalOp *>((*this)[5]);
	CParseHandlerPhysicalOp *pphRight =
