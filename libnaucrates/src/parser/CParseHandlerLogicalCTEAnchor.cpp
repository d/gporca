...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalCTEAnchor::StartElement(const XMLCh *const,  // xmlszUri,
											...
											const Attributes &attrs)
{
	if (0 !=
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenLogicalCTEAnchor), xmlszLocalname))
	{
	...
	ULONG ulId = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenCTEId, EdxltokenLogicalCTEAnchor);

	m_pdxln = GPOS_NEW(m_pmp)
		CDXLNode(m_pmp, GPOS_NEW(m_pmp) CDXLLogicalCTEAnchor(m_pmp, ulId));
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalCTEAnchor::EndElement(const XMLCh *const,  // xmlszUri,
										  const XMLCh *const xmlszLocalname,
										  const XMLCh *const  // xmlszQname
)
{
