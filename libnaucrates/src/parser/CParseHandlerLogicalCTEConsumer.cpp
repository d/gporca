...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalCTEConsumer::StartElement(const XMLCh *const,  // xmlszUri,
											  ...
											  const Attributes &attrs)
{
	...
	ULONG ulId = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenCTEId, EdxltokenLogicalCTEConsumer);

	DrgPul *pdrgpulColIds = CDXLOperatorFactory::PdrgpulFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenColumns, EdxltokenLogicalCTEConsumer);

	m_pdxln = GPOS_NEW(m_pmp)
		CDXLNode(m_pmp, GPOS_NEW(m_pmp)
							CDXLLogicalCTEConsumer(m_pmp, ulId, pdrgpulColIds));
}
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalCTEConsumer::EndElement(const XMLCh *const,  // xmlszUri,
											const XMLCh *const xmlszLocalname,
											const XMLCh *const  // xmlszQname
)
{
