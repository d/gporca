...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalCTEProducer::StartElement(const XMLCh *const,  // xmlszUri,
											  ...
											  const Attributes &attrs)
{
	...
	ULONG ulId = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenCTEId, EdxltokenLogicalCTEProducer);

	DrgPul *pdrgpulColIds = CDXLOperatorFactory::PdrgpulFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenColumns, EdxltokenLogicalCTEProducer);

	m_pdxln = GPOS_NEW(m_pmp)
		CDXLNode(m_pmp, GPOS_NEW(m_pmp)
							CDXLLogicalCTEProducer(m_pmp, ulId, pdrgpulColIds));
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalCTEProducer::EndElement(const XMLCh *const,  // xmlszUri,
											const XMLCh *const xmlszLocalname,
											const XMLCh *const  // xmlszQname
)
{
