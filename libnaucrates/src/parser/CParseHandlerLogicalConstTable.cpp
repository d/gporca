...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalConstTable::StartElement(const XMLCh *const,  // xmlszUri,
				 ...
				 xmlszLocalname))
	{
		...
		this->Append(pphColDescr);
			 ...
			 XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenConstTuple), xmlszLocalname))
	{
		GPOS_ASSERT(
			NULL !=
			m_pdrgpdrgpdxldatum);  // we must have already seen a logical const table
		...
		m_pdrgpdxldatum = GPOS_NEW(m_pmp) DrgPdxldatum(m_pmp);
					  ...
					  CDXLTokens::XmlstrToken(EdxltokenDatum), xmlszLocalname))
	{
		...
		CDXLDatum *pdxldatum = CDXLOperatorFactory::Pdxldatum(
			m_pphm->Pmm(), attrs, EdxltokenScalarConstValue);
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalConstTable::EndElement(const XMLCh *const,  // xmlszUri,
										   const XMLCh *const xmlszLocalname,
										   const XMLCh *const  // xmlszQname
)
{
				 ...
				 xmlszLocalname))
	{
		...
		CDXLLogicalConstTable *pdxlopConstTable = GPOS_NEW(m_pmp)
			CDXLLogicalConstTable(m_pmp, pdrgpdxlcd, m_pdrgpdrgpdxldatum);
