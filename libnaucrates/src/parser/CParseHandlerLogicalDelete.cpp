...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalDelete::StartElement(const XMLCh *const,  // xmlszUri,
										 ...
										 const Attributes &attrs)
{
	if (0 !=
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenLogicalDelete), xmlszLocalname))
	{
	...
	m_ulCtid = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenCtidColId, EdxltokenLogicalDelete);
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalDelete::EndElement(const XMLCh *const,  // xmlszUri,
									   const XMLCh *const xmlszLocalname,
									   const XMLCh *const  // xmlszQname
)
{
	...
	m_pdxln = GPOS_NEW(m_pmp)
		CDXLNode(m_pmp, GPOS_NEW(m_pmp)
							CDXLLogicalDelete(m_pmp, pdxltabdesc, m_ulCtid,
											  m_ulSegmentId, m_pdrgpulDelete));
