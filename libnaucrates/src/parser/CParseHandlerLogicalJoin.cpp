...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalJoin::CParseHandlerLogicalJoin(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalJoin::StartElement(const XMLCh *const xmlszUri,
									   const XMLCh *const xmlszLocalname,
									   const XMLCh *const xmlszQname,
									   const Attributes &attrs)
{
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenLogicalJoin), xmlszLocalname))
	{
		if (NULL == m_pdxln)
		{
			// parse and create logical join operator
			CDXLLogicalJoin *pdxlopJoin =
				(CDXLLogicalJoin *) CDXLOperatorFactory::PdxlopLogicalJoin(
					m_pphm->Pmm(), attrs);
		...
		else
		{
			...
			CParseHandlerBase *pphLgJoin = CParseHandlerFactory::Pph(
				m_pmp, CDXLTokens::XmlstrToken(EdxltokenLogicalJoin), m_pphm,
				this);
			...
			pphLgJoin->startElement(xmlszUri, xmlszLocalname, xmlszQname,
									attrs);
		}
	}
	else
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalJoin::EndElement(const XMLCh *const,  // xmlszUri,
									 const XMLCh *const xmlszLocalname,
									 const XMLCh *const  // xmlszQname
)
{
