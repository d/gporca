...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalLimit::CParseHandlerLogicalLimit(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalLimit::StartElement(const XMLCh *const,  // xmlszUri,
										...
										const Attributes &attrs)
{
	if (0 ==
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalLimit),
								 xmlszLocalname))
	{
		const XMLCh *xmlszNonRemovableLimit =
		...
		if (xmlszNonRemovableLimit)
		{
			fKeepLimit = CDXLOperatorFactory::FValueFromXmlstr(
				m_pphm->Pmm(), xmlszNonRemovableLimit,
				EdxltokenTopLimitUnderDML, EdxltokenLogicalLimit);
		}

		m_pdxln = GPOS_NEW(m_pmp) CDXLNode(
			m_pmp, GPOS_NEW(m_pmp) CDXLLogicalLimit(m_pmp, fKeepLimit));
		...
		CParseHandlerBase *pphOffset = CParseHandlerFactory::Pph(
			m_pmp, CDXLTokens::XmlstrToken(EdxltokenScalarLimitOffset), m_pphm,
			this);
		m_pphm->ActivateParseHandler(pphOffset);

		CParseHandlerBase *pphCount = CParseHandlerFactory::Pph(
			m_pmp, CDXLTokens::XmlstrToken(EdxltokenScalarLimitCount), m_pphm,
			this);
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalLimit::EndElement(const XMLCh *const,  // xmlszUri,
									  const XMLCh *const xmlszLocalname,
									  const XMLCh *const  // xmlszQname
)
{
	...
	CParseHandlerScalarOp *pphCount =
		dynamic_cast<CParseHandlerScalarOp *>((*this)[1]);
	CParseHandlerScalarOp *pphOffSet =
