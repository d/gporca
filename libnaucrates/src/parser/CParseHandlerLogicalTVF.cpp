...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalTVF::CParseHandlerLogicalTVF(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalTVF::StartElement(const XMLCh *const xmlszUri,
									  const XMLCh *const xmlszLocalname,
									  const XMLCh *const xmlszQname,
									  const Attributes &attrs)
{
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenLogicalTVF), xmlszLocalname))
	{
		// parse function id
		m_pmdidFunc = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenFuncId, EdxltokenLogicalTVF);
		...
		m_pmdidRetType = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenTypeId, EdxltokenLogicalTVF);
	}
	else if (0 ==
...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalTVF::EndElement(const XMLCh *const,  // xmlszUri,
									const XMLCh *const xmlszLocalname,
									const XMLCh *const  // xmlszQname
)
{
	...
	CDXLLogicalTVF *pdxlopTVF = GPOS_NEW(m_pmp) CDXLLogicalTVF(
		m_pmp, m_pmdidFunc, m_pmdidRetType, m_pmdname, pdrgpdxlcd);
