...
void
CParseHandlerMDArrayCoerceCast::StartElement(const XMLCh *const,  // xmlszUri,
											 ...
											 const Attributes &attrs)
{
	...
	CMDName *pmdname =
		CDXLUtils::PmdnameFromXmlsz(m_pphm->Pmm(), xmlszFuncName);

	// parse cast properties
	IMDId *pmdid = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenGPDBArrayCoerceCast);

	IMDId *pmdidSrc = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenGPDBCastSrcType,
		EdxltokenGPDBArrayCoerceCast);

	IMDId *pmdidDest = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenGPDBCastDestType,
		EdxltokenGPDBArrayCoerceCast);

	IMDId *pmdidCastFunc = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenGPDBCastFuncId,
		EdxltokenGPDBArrayCoerceCast);

	// parse whether func returns a set
	BOOL fBinaryCoercible = CDXLOperatorFactory::FValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenGPDBCastBinaryCoercible,
		EdxltokenGPDBArrayCoerceCast);

	// parse coercion path type
	IMDCast::EmdCoercepathType eCoercePathType =
		(IMDCast::EmdCoercepathType) CDXLOperatorFactory::IValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenGPDBCastCoercePathType,
			EdxltokenGPDBArrayCoerceCast);

	INT iTypeModifier = CDXLOperatorFactory::IValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenTypeMod, EdxltokenGPDBArrayCoerceCast,
		true, IDefaultTypeModifier);

	BOOL fIsExplicit = CDXLOperatorFactory::FValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenIsExplicit,
		EdxltokenGPDBArrayCoerceCast);

	EdxlCoercionForm edcf =
		(EdxlCoercionForm) CDXLOperatorFactory::IValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenCoercionForm,
			EdxltokenGPDBArrayCoerceCast);

	INT iLoc = CDXLOperatorFactory::IValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenLocation, EdxltokenGPDBArrayCoerceCast);

	m_pimdobj = GPOS_NEW(m_pmp) CMDArrayCoerceCastGPDB(
		m_pmp, pmdid, pmdname, pmdidSrc, pmdidDest, fBinaryCoercible,
		pmdidCastFunc, eCoercePathType, iTypeModifier, fIsExplicit, edcf, iLoc);
}

// invoked by Xerces to process a closing tag
void
CParseHandlerMDArrayCoerceCast::EndElement(const XMLCh *const,  // xmlszUri,
										   const XMLCh *const xmlszLocalname,
										   const XMLCh *const  // xmlszQname
)
{
