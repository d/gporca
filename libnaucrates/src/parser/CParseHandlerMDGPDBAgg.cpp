...
//
//---------------------------------------------------------------------------
CParseHandlerMDGPDBAgg::CParseHandlerMDGPDBAgg(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerMDGPDBAgg::StartElement(const XMLCh *const,  // xmlszUri,
									 ...
									 const Attributes &attrs)
{
									  ...
									  xmlszLocalname))
	{
		...
		CWStringDynamic *pstrAggName =
			CDXLUtils::PstrFromXMLCh(m_pphm->Pmm(), xmlszAggName);
		...
		m_pmdid = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenGPDBAgg);

		// parse ordered aggregate info
		const XMLCh *xmlszOrderedAgg =
		...
		if (NULL != xmlszOrderedAgg)
		{
			m_fOrdered = CDXLOperatorFactory::FValueFromXmlstr(
				m_pphm->Pmm(), xmlszOrderedAgg, EdxltokenGPDBIsAggOrdered,
				EdxltokenGPDBAgg);
		}

		// parse splittable aggregate info
		const XMLCh *xmlszSplittableAgg =
		...
		if (NULL != xmlszSplittableAgg)
		{
			m_fSplittable = CDXLOperatorFactory::FValueFromXmlstr(
				m_pphm->Pmm(), xmlszSplittableAgg, EdxltokenGPDBAggSplittable,
				EdxltokenGPDBAgg);
		}

		// parse hash capable aggragate info
		const XMLCh *xmlszHashAggCapableAgg = attrs.getValue(
		...
		if (NULL != xmlszHashAggCapableAgg)
		{
			m_fHashAggCapable = CDXLOperatorFactory::FValueFromXmlstr(
				m_pphm->Pmm(), xmlszHashAggCapableAgg,
				EdxltokenGPDBAggHashAggCapable, EdxltokenGPDBAgg);
		}
	}
					  ...
					  xmlszLocalname))
	{
		...
		m_pmdidTypeResult = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenGPDBAggResultTypeId);
	}
	else if (0 == XMLString::compareString(
...
//---------------------------------------------------------------------------
void
CParseHandlerMDGPDBAgg::EndElement(const XMLCh *const,  // xmlszUri,
								   const XMLCh *const xmlszLocalname,
								   const XMLCh *const  // xmlszQname
)
{
									  ...
									  xmlszLocalname))
	{
		...
		m_pimdobj = GPOS_NEW(m_pmp)
			CMDAggregateGPDB(m_pmp, m_pmdid, m_pmdname, m_pmdidTypeResult,
							 m_pmdidTypeIntermediate, m_fOrdered, m_fSplittable,
							 m_fHashAggCapable);
