...
//
//---------------------------------------------------------------------------
CParseHandlerMDGPDBFunc::CParseHandlerMDGPDBFunc(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerMDGPDBFunc::StartElement(const XMLCh *const,  // xmlszUri,
									  const XMLCh *const xmlszLocalname,
									  const XMLCh *const,  // xmlszQname
									  const Attributes &attrs)
{
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFunc), xmlszLocalname))
	{
		// parse func name
		const XMLCh *xmlszFuncName = CDXLOperatorFactory::XmlstrFromAttrs(
			attrs, EdxltokenName, EdxltokenGPDBFunc);

		CWStringDynamic *pstrFuncName =
			CDXLUtils::PstrFromXMLCh(m_pphm->Pmm(), xmlszFuncName);
					  ...
					  xmlszLocalname))
	{
		...
		m_pmdidTypeResult = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenGPDBFuncResultTypeId);
	}
	else if (0 ==
			 XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenOutputCols), xmlszLocalname))
	{
		...
		m_pdrgpmdidTypes = CDXLOperatorFactory::PdrgpmdidFromXMLCh(
			m_pphm->Pmm(), xmlszTypes, EdxltokenTypeIds, EdxltokenOutputCols);
	}
}
...
//---------------------------------------------------------------------------
void
CParseHandlerMDGPDBFunc::EndElement(const XMLCh *const,  // xmlszUri,
									const XMLCh *const xmlszLocalname,
									const XMLCh *const  // xmlszQname
)
{
				 ...
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFunc), xmlszLocalname))
	{
		...
		m_pimdobj = GPOS_NEW(m_pmp) CMDFunctionGPDB(
			m_pmp, m_pmdid, m_pmdname, m_pmdidTypeResult, m_pdrgpmdidTypes,
			m_fReturnsSet, m_efuncstbl, m_efuncdataacc, m_fStrict);
...
CParseHandlerMDGPDBFunc::EFuncStability(const XMLCh *xmlsz)
{
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFuncStable), xmlsz))
	{
		return CMDFunctionGPDB::EfsStable;
	}

	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFuncImmutable), xmlsz))
	{
		return CMDFunctionGPDB::EfsImmutable;
	}

	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFuncVolatile), xmlsz))
	{
...
CParseHandlerMDGPDBFunc::EFuncDataAccess(const XMLCh *xmlsz)
{
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFuncNoSQL), xmlsz))
	{
		return CMDFunctionGPDB::EfdaNoSQL;
	}

	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFuncContainsSQL), xmlsz))
	{
		return CMDFunctionGPDB::EfdaContainsSQL;
	}

	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBFuncReadsSQLData), xmlsz))
	{
	...
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenGPDBFuncModifiesSQLData), xmlsz))
	{
