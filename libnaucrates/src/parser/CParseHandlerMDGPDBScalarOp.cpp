...
//---------------------------------------------------------------------------
void
CParseHandlerMDGPDBScalarOp::StartElement(const XMLCh *const xmlszUri,
										  ...
										  const Attributes &attrs)
{
								 ...
								 xmlszLocalname))
	{
		...
		CWStringDynamic *pstrOpName =
			CDXLUtils::PstrFromXMLCh(m_pphm->Pmm(), xmlszOpName);
		...
		m_pmdid = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOp);
		...
		const XMLCh *xmlszReturnsNullOnNullInput = attrs.getValue(
		...
		if (NULL != xmlszReturnsNullOnNullInput)
		{
			m_fReturnsNullOnNullInput = CDXLOperatorFactory::FValueFromAttrs(
				m_pphm->Pmm(), attrs, EdxltokenReturnsNullOnNullInput,
					  ...
					  xmlszLocalname))
	{
		...
		m_pmdidTypeLeft = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid,
					  ...
					  xmlszLocalname))
	{
		...
		m_pmdidFunc = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpFuncId);
	}
	else if (0 == XMLString::compareString(
					  ...
					  xmlszLocalname))
	{
		...
		m_pmdidOpCommute = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenGPDBScalarOpCommOpId);
	}
	else if (0 == XMLString::compareString(
				 ...
				 CDXLTokens::XmlstrToken(EdxltokenOpClasses), xmlszLocalname))
	{
		...
		CParseHandlerBase *pphOpClassList = CParseHandlerFactory::Pph(
			m_pmp, CDXLTokens::XmlstrToken(EdxltokenMetadataIdList), m_pphm,
			this);
		m_pphm->ActivateParseHandler(pphOpClassList);
		this->Append(pphOpClassList);
		pphOpClassList->startElement(xmlszUri, xmlszLocalname, xmlszQname,
									 attrs);
	}
	else
	{
...
//---------------------------------------------------------------------------
void
CParseHandlerMDGPDBScalarOp::EndElement(const XMLCh *const,  // xmlszUri,
										const XMLCh *const xmlszLocalname,
										const XMLCh *const  // xmlszQname
)
{
								 ...
								 xmlszLocalname))
	{
		...
		m_pimdobj = GPOS_NEW(m_pmp) CMDScalarOpGPDB(
			m_pmp, m_pmdid, m_pmdname, m_pmdidTypeLeft, m_pmdidTypeRight,
			m_pmdidTypeResult, m_pmdidFunc, m_pmdidOpCommute, m_pmdidOpInverse,
			m_ecmpt, m_fReturnsNullOnNullInput, pdrgpmdidOpClasses);
...
//---------------------------------------------------------------------------
BOOL
CParseHandlerMDGPDBScalarOp::FSupportedChildElem(const XMLCh *const xmlsz)
{
	return (
		...
		0 == XMLString::compareString(
				 ...
				 xmlsz) ||
		0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpFuncId), xmlsz) ||
		0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpCommOpId),
				 xmlsz) ||
		0 == XMLString::compareString(
