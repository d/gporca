...
CParseHandlerMDIndexInfoList::CParseHandlerMDIndexInfoList(
	...
	: CParseHandlerBase(pmp, pphm, pphRoot), m_pdrgpmdIndexInfo(NULL)
{
}
...
void
CParseHandlerMDIndexInfoList::StartElement(const XMLCh *const,  // xmlszUri,
										   ...
										   const Attributes &attrs)
{
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenIndexInfoList), xmlszLocalname))
	{
		m_pdrgpmdIndexInfo = GPOS_NEW(m_pmp) DrgPmdIndexInfo(m_pmp);
	}
	else if (0 ==
			 XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenIndexInfo), xmlszLocalname))
	{
		// parse mdid
		IMDId *pmdid = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenIndexInfo);

		// parse index partial info
		BOOL fPartial = CDXLOperatorFactory::FValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenIndexPartial, EdxltokenIndexInfo);

		CMDIndexInfo *pmdIndexInfo =
			GPOS_NEW(m_pmp) CMDIndexInfo(pmdid, fPartial);
...
void
CParseHandlerMDIndexInfoList::EndElement(const XMLCh *const,  // xmlszUri,
										 const XMLCh *const xmlszLocalname,
										 const XMLCh *const  // xmlszQname
)
{
