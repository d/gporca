...
//
//---------------------------------------------------------------------------
CParseHandlerMDType::CParseHandlerMDType(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerMDType::StartElement(const XMLCh *const,  // xmlszUri,
								  ...
								  const Attributes &attrs)
{
									  ...
									  xmlszLocalname))
	{
		...
		m_pmdid = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenMDType);
		...
		if (!FBuiltInType(m_pmdid))
		{
			...
			CWStringDynamic *pstrTypeName =
				CDXLUtils::PstrFromXMLCh(m_pphm->Pmm(), xmlszTypeName);
			...
			m_fRedistributable = CDXLOperatorFactory::FValueFromAttrs(
				m_pphm->Pmm(), attrs, EdxltokenMDTypeRedistributable,
				EdxltokenMDType);

			// parse if type is passed by value
			m_fByValue = CDXLOperatorFactory::FValueFromAttrs(
				m_pphm->Pmm(), attrs, EdxltokenMDTypeByValue, EdxltokenMDType);

			// parse if type is hashable
			m_fHashable = CDXLOperatorFactory::FValueFromAttrs(
				m_pphm->Pmm(), attrs, EdxltokenMDTypeHashable, EdxltokenMDType);

			// parse if type is composite
			const XMLCh *xmlszAttributeVal = attrs.getValue(
			...
			else
			{
				m_fComposite = CDXLOperatorFactory::FValueFromXmlstr(
					m_pphm->Pmm(), xmlszAttributeVal, EdxltokenMDTypeComposite,
					EdxltokenMDType);
			}
...
//---------------------------------------------------------------------------
void
CParseHandlerMDType::ParseMdid(const XMLCh *xmlszLocalname,
							   const Attributes &attrs)
{
	...
	if (m_pmdidMin == *ppmdid || m_pmdidMax == *ppmdid ||
		m_pmdidAvg == *ppmdid || m_pmdidSum == *ppmdid ||
		m_pmdidCount == *ppmdid)
	{
		(*ppmdid)->Release();
	}

	*ppmdid = CDXLOperatorFactory::PmdidFromAttrs(m_pphm->Pmm(), attrs,
												  EdxltokenMdid, edxltoken);
}
...
//---------------------------------------------------------------------------
BOOL
CParseHandlerMDType::FBuiltInType(const IMDId *pmdid) const
{
...
//---------------------------------------------------------------------------
IMDId **
CParseHandlerMDType::Ppmdid(Edxltoken edxltoken)
{
	switch (edxltoken)
	{
...
//---------------------------------------------------------------------------
void
CParseHandlerMDType::EndElement(const XMLCh *const,  // xmlszUri,
								const XMLCh *const xmlszLocalname,
								const XMLCh *const  // xmlszQname
)
{
		...
		switch (pmdidGPDB->OidObjectId())
		{
			...
			default:
				...
				m_pimdobj = GPOS_NEW(m_pmp) CMDTypeGenericGPDB(
					m_pmp, m_pmdid, m_pmdname, m_fRedistributable,
					m_fFixedLength, ulLen, m_fByValue, m_pmdidOpEq,
					m_pmdidOpNEq, m_pmdidOpLT, m_pmdidOpLEq, m_pmdidOpGT,
					m_pmdidOpGEq, m_pmdidOpComp, m_pmdidMin, m_pmdidMax,
					m_pmdidAvg, m_pmdidSum, m_pmdidCount, m_fHashable,
					m_fComposite, m_pmdidBaseRelation, m_pmdidTypeArray,
					m_iLength);
				break;
		}
