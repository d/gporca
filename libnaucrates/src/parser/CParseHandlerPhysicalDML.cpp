...
//
//---------------------------------------------------------------------------
CParseHandlerPhysicalDML::CParseHandlerPhysicalDML(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerPhysicalDML::StartElement(const XMLCh *const,  // xmlszUri,
									   ...
									   const Attributes &attrs)
{
	...
	const XMLCh *xmlszSourceColIds = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, EdxltokenColumns, edxltoken);
	m_pdrgpul = CDXLOperatorFactory::PdrgpulFromXMLCh(
		m_pphm->Pmm(), xmlszSourceColIds, EdxltokenColumns, edxltoken);

	m_ulAction = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenActionColId, edxltoken);
	m_ulOid = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenOidColId, edxltoken);
	m_ulCtid = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenCtidColId, edxltoken);
	m_ulSegmentId = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenGpSegmentIdColId, edxltoken);

	const XMLCh *xmlszPreserveOids =
	...
	if (NULL != xmlszPreserveOids)
	{
		m_fPreserveOids = CDXLOperatorFactory::FValueFromXmlstr(
			m_pphm->Pmm(), xmlszPreserveOids, EdxltokenUpdatePreservesOids,
			EdxltokenPhysicalDMLUpdate);
	}

	if (m_fPreserveOids)
	{
		m_ulTupleOidColId = CDXLOperatorFactory::UlValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenTupleOidColId,
			EdxltokenPhysicalDMLUpdate);
	}

	const XMLCh *xmlszInputSorted =
	...
	if (NULL != xmlszInputSorted)
	{
		m_fInputSorted = CDXLOperatorFactory::FValueFromXmlstr(
			m_pphm->Pmm(), xmlszInputSorted, EdxltokenInputSorted,
...
//---------------------------------------------------------------------------
void
CParseHandlerPhysicalDML::EndElement(const XMLCh *const,  // xmlszUri,
									 const XMLCh *const xmlszLocalname,
									 const XMLCh *const  // xmlszQname
)
{
	EdxlDmlType edxldmltype =
		CParseHandlerPhysicalDML::EdxlDmlOpType(xmlszLocalname);
	...
	CParseHandlerDirectDispatchInfo *pphDirectDispatch =
		dynamic_cast<CParseHandlerDirectDispatchInfo *>((*this)[1]);
	GPOS_ASSERT(NULL != pphDirectDispatch->Pdxlddinfo() &&
	...
	pdxlddinfo->AddRef();
	CDXLPhysicalDML *pdxlop = GPOS_NEW(m_pmp) CDXLPhysicalDML(
		m_pmp, m_edxldmltype, pdxltabdesc, m_pdrgpul, m_ulAction, m_ulOid,
		m_ulCtid, m_ulSegmentId, m_fPreserveOids, m_ulTupleOidColId, pdxlddinfo,
		m_fInputSorted);
...
//---------------------------------------------------------------------------
EdxlDmlType
CParseHandlerPhysicalDML::EdxlDmlOpType(const XMLCh *xmlszDmlType)
{
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenPhysicalDMLInsert), xmlszDmlType))
	{
	...
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenPhysicalDMLDelete), xmlszDmlType))
	{
	...
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenPhysicalDMLUpdate), xmlszDmlType))
	{
