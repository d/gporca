...
//
//---------------------------------------------------------------------------
CParseHandlerRelStats::CParseHandlerRelStats(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerRelStats::StartElement(const XMLCh *const,  // xmlszUri,
									...
									const Attributes &attrs)
{
	if (0 !=
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenRelationStats), xmlszLocalname))
	{
	...
	IMDId *pmdid = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenMdid, EdxltokenRelationStats);

	// parse rows

	CDouble dRows = CDXLOperatorFactory::DValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenRows, EdxltokenRelationStats);
	...
	m_pimdobj = GPOS_NEW(m_pmp) CDXLRelStats(
		m_pmp, CMDIdRelStats::PmdidConvert(pmdid), pmdname, dRows, fEmpty);
}
...
//---------------------------------------------------------------------------
void
CParseHandlerRelStats::EndElement(const XMLCh *const,  // xmlszUri,
								  const XMLCh *const xmlszLocalname,
								  const XMLCh *const  // xmlszQname
)
{
