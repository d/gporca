...
//---------------------------------------------------------------------------
void
CParseHandlerScalarArrayRef::StartElement(const XMLCh *const xmlszUri,
										  ...
										  const Attributes &attrs)
{
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenScalarArrayRef), xmlszLocalname))
	{
		// initialize the arrayref node
		GPOS_ASSERT(NULL == m_pdxln);

		// parse types
		IMDId *pmdidElem = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenArrayElementType,
			EdxltokenScalarArrayRef);
		IMDId *pmdidArray = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenArrayType, EdxltokenScalarArrayRef);
		IMDId *pmdidReturn = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenTypeId, EdxltokenScalarArrayRef);
		INT iTypeModifier = CDXLOperatorFactory::IValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenTypeMod, EdxltokenScalarArrayRef,
			true, IDefaultTypeModifier);

		m_pdxln = GPOS_NEW(m_pmp) CDXLNode(
			m_pmp,
			GPOS_NEW(m_pmp) CDXLScalarArrayRef(m_pmp, pmdidElem, iTypeModifier,
											   pmdidArray, pmdidReturn));
	}
					  ...
					  xmlszLocalname))
	{
		...
		CParseHandlerBase *pphChild = CParseHandlerFactory::Pph(
			m_pmp, CDXLTokens::XmlstrToken(EdxltokenScalarArrayRefIndexList),
			m_pphm, this);
...
//---------------------------------------------------------------------------
void
CParseHandlerScalarArrayRef::EndElement(const XMLCh *const,  // xmlszUri,
										const XMLCh *const xmlszLocalname,
										const XMLCh *const  // xmlszQname
)
{
