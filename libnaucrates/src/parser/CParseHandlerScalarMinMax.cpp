...
//
//---------------------------------------------------------------------------
CParseHandlerScalarMinMax::CParseHandlerScalarMinMax(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerScalarMinMax::StartElement(const XMLCh *const xmlszUri,
		...
		CDXLScalarMinMax::EmmtSentinel == m_emmt)
	{
		...
		if (0 ==
			XMLString::compareString(
				CDXLTokens::XmlstrToken(EdxltokenScalarMax), xmlszLocalname))
		{
		...
		m_pmdidType = CDXLOperatorFactory::PmdidFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenTypeId, edxltoken);
	}
	else
	{
		// parse child
...
//---------------------------------------------------------------------------
void
CParseHandlerScalarMinMax::EndElement(const XMLCh *const,  // xmlszUri
									  const XMLCh *const xmlszLocalname,
									  const XMLCh *const  // xmlszQname
)
{
	...
	m_pdxln = GPOS_NEW(m_pmp) CDXLNode(
		m_pmp, GPOS_NEW(m_pmp) CDXLScalarMinMax(m_pmp, m_pmdidType, m_emmt));
...
CParseHandlerScalarMinMax::Emmt(const XMLCh *xmlszLocalname)
{
	if (0 == XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenScalarMin), xmlszLocalname))
	{
