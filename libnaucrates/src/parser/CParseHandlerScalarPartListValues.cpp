	...
	const Attributes &attrs)
{
	...
	ULONG ulLevel = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenPartLevel,
		EdxltokenScalarPartListValues);
	IMDId *pmdidResult = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenGPDBScalarOpResultTypeId,
		EdxltokenScalarPartListValues);
	IMDId *pmdidElement = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenArrayElementType,
		EdxltokenScalarPartListValues);
	m_pdxln = GPOS_NEW(m_pmp)
		CDXLNode(m_pmp, GPOS_NEW(m_pmp) CDXLScalarPartListValues(
							m_pmp, ulLevel, pmdidResult, pmdidElement));
}

// Invoked by Xerces to process a closing tag
void
CParseHandlerScalarPartListValues::EndElement(const XMLCh *const,  // xmlszUri,
											  const XMLCh *const xmlszLocalname,
											  const XMLCh *const  // xmlszQname
)
{
