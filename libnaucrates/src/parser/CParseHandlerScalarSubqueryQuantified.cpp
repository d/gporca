	...
	const Attributes &attrs)
{
	...
	IMDId *pmdidOp = CDXLOperatorFactory::PmdidFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenOpNo, edxltokenElement);

	// parse operator name
	const XMLCh *xmlszScalarOpName = CDXLOperatorFactory::XmlstrFromAttrs(
		attrs, EdxltokenOpName, edxltokenElement);

	CWStringDynamic *pstrScalarOpName =
		CDXLUtils::PstrFromXMLCh(m_pphm->Pmm(), xmlszScalarOpName);
	...
	ULONG ulColId = CDXLOperatorFactory::UlValueFromAttrs(
		m_pphm->Pmm(), attrs, EdxltokenColId, edxltokenElement);

	if (EdxltokenScalarSubqueryAny == edxltokenElement)
	{
		m_pdxlop = GPOS_NEW(m_pmp)
			CDXLScalarSubqueryAny(m_pmp, pmdidOp, pmdnameScalarOp, ulColId);
	}
	else
	{
		m_pdxlop = GPOS_NEW(m_pmp)
			CDXLScalarSubqueryAll(m_pmp, pmdidOp, pmdnameScalarOp, ulColId);
	}

	// parse handler for the child nodes
	CParseHandlerBase *pphLgChild = CParseHandlerFactory::Pph(
		m_pmp, CDXLTokens::XmlstrToken(EdxltokenLogical), m_pphm, this);
	m_pphm->ActivateParseHandler(pphLgChild);

	CParseHandlerBase *pphScChild = CParseHandlerFactory::Pph(
...
)
{
	...
	CParseHandlerScalarOp *pphScChild =
		dynamic_cast<CParseHandlerScalarOp *>((*this)[0]);
	CParseHandlerLogicalOp *pphLgChild =
