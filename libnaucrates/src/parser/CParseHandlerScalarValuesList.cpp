...
void
CParseHandlerScalarValuesList::StartElement(const XMLCh *const xmlszUri,
											...
											const Attributes &attrs)
{
	if (0 ==
		XMLString::compareString(
			CDXLTokens::XmlstrToken(EdxltokenScalarValuesList), xmlszLocalname))
	{
		CDXLScalarValuesList *pdxlop =
			GPOS_NEW(m_pmp) CDXLScalarValuesList(m_pmp);
		m_pdxln = GPOS_NEW(m_pmp) CDXLNode(m_pmp, pdxlop);
					  ...
					  xmlszLocalname))
	{
		CParseHandlerBase *pphScConstValue =
		...
		pphScConstValue->startElement(xmlszUri, xmlszLocalname, xmlszQname,
									  attrs);
	}
	else
	{
...
void
CParseHandlerScalarValuesList::EndElement(const XMLCh *const,  // xmlszUri,
										  const XMLCh *const,  //xmlszLocalname,
										  const XMLCh *const   // xmlszQname
)
{
