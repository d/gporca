...
//---------------------------------------------------------------------------
void
CParseHandlerStatsDerivedColumn::StartElement(const XMLCh *const xmlszUri,
											  ...
											  const Attributes &attrs)
{
				 ...
				 xmlszLocalname))
	{
		...
		m_ulColId = CDXLOperatorFactory::UlValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenColId, EdxltokenStatsDerivedColumn);

		// parse column width
		m_dWidth = CDXLOperatorFactory::DValueFromAttrs(
			m_pphm->Pmm(), attrs, EdxltokenWidth, EdxltokenStatsDerivedColumn);
...
//---------------------------------------------------------------------------
void
CParseHandlerStatsDerivedColumn::EndElement(const XMLCh *const,  // xmlszUri,
											const XMLCh *const xmlszLocalname,
											const XMLCh *const  // xmlszQname
)
{
	...
	{
		CParseHandlerColStatsBucket *pph =
	...
	m_pstatsdercol = GPOS_NEW(m_pmp) CDXLStatsDerivedColumn(
		m_ulColId, m_dWidth, m_dNullFreq, m_dDistinctRemain, m_dFreqRemain,
		pdrgpdxlbucket);
