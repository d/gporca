...
//
//---------------------------------------------------------------------------
CParseHandlerWindowSpec::CParseHandlerWindowSpec(IMemoryPool *pmp,
...
//---------------------------------------------------------------------------
void
CParseHandlerWindowSpec::StartElement(const XMLCh *const xmlszUri,
									  ...
									  const Attributes &attrs)
{
				 ...
				 CDXLTokens::XmlstrToken(EdxltokenWindowSpec), xmlszLocalname))
	{
		...
		if (NULL != xmlszAlias)
		{
			CWStringDynamic *pstrAlias =
				CDXLUtils::PstrFromXMLCh(m_pphm->Pmm(), xmlszAlias);
			m_pmdname = GPOS_NEW(m_pmp) CMDName(m_pmp, pstrAlias);
			GPOS_DELETE(pstrAlias);
		}

		const XMLCh *xmlszPartCols = CDXLOperatorFactory::XmlstrFromAttrs(
			attrs, EdxltokenPartKeys, EdxltokenPhysicalWindow);
		m_pdrgpulPartCols = CDXLOperatorFactory::PdrgpulFromXMLCh(
			m_pphm->Pmm(), xmlszPartCols, EdxltokenPartKeys,
					  ...
					  xmlszLocalname))
	{
		...
		CParseHandlerBase *pphSortColList = CParseHandlerFactory::Pph(
			m_pmp, CDXLTokens::XmlstrToken(EdxltokenScalarSortColList), m_pphm,
			this);
		m_pphm->ActivateParseHandler(pphSortColList);

		// store parse handler
		this->Append(pphSortColList);
		pphSortColList->startElement(xmlszUri, xmlszLocalname, xmlszQname,
									 attrs);
	}
	else if (0 ==
			 XMLString::compareString(
				 CDXLTokens::XmlstrToken(EdxltokenWindowFrame), xmlszLocalname))
	{
		m_fHasWindowFrame = true;

		// parse handler for the leading and trailing scalar values
		CParseHandlerBase *pphWf = CParseHandlerFactory::Pph(
			m_pmp, CDXLTokens::XmlstrToken(EdxltokenWindowFrame), m_pphm, this);
...
//---------------------------------------------------------------------------
void
CParseHandlerWindowSpec::EndElement(const XMLCh *const,  // xmlszUri,
									const XMLCh *const xmlszLocalname,
									const XMLCh *const  // xmlszQname
)
{
	...
	if (1 == this->UlLength())
	{
		if (m_fHasWindowFrame)
		{
			...
			CParseHandlerWindowFrame *pphWf =
				dynamic_cast<CParseHandlerWindowFrame *>((*this)[0]);
			pdxlwf = pphWf->Pdxlwf();
		}
		else
		{
			CParseHandlerSortColList *pphSortColList =
	...
	m_pdxlws = GPOS_NEW(m_pmp) CDXLWindowSpec(
		m_pmp, m_pdrgpulPartCols, m_pmdname, pdxlnSortColList, pdxlwf);
