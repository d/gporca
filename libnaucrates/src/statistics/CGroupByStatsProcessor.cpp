...
CStatistics *
CGroupByStatsProcessor::PstatsGroupBy(IMemoryPool *pmp,
									  const CStatistics *pstatsInput,
									  DrgPul *pdrgpulGC, DrgPul *pdrgpulAgg,
									  CBitSet *pbsKeys)
{
	...
	if (pstatsInput->FEmpty())
	{
		...
		CHistogram::AddDummyHistogramAndWidthInfo(pmp, pcf, phmulhist,
												  phmuldoubleWidth, pdrgpulAgg,
												  true /* fEmpty */);
		CHistogram::AddDummyHistogramAndWidthInfo(pmp, pcf, phmulhist,
												  phmuldoubleWidth, pdrgpulGC,
												  true /* fEmpty */);

		pstatsAgg = GPOS_NEW(pmp) CStatistics(pmp, phmulhist, phmuldoubleWidth,
											  dRowsAgg, true /* fEmpty */);
	}
	else
	{
		...
		CColRefSet *pcrsGrpColsForStats = CStatisticsUtils::PcrsGrpColsForStats(
			pmp, pdrgpulGC, pcrsGrpColComputed);
		...
		CStatisticsUtils::AddGrpColStats(pmp, pstatsInput, pcrsGrpColsForStats,
										 phmulhist, phmuldoubleWidth);
		CStatisticsUtils::AddGrpColStats(pmp, pstatsInput, pcrsGrpColComputed,
										 phmulhist, phmuldoubleWidth);

		const CStatisticsConfig *pstatsconf = pstatsInput->PStatsConf();

		DrgPdouble *pdrgpdNDV = CStatisticsUtils::PdrgPdoubleNDV(
			pmp, pstatsconf, pstatsInput, pcrsGrpColsForStats, pbsKeys);
		CDouble dGroups =
			CStatisticsUtils::DNumOfDistinctVal(pstatsconf, pdrgpdNDV);
		...
		dRowsAgg =
			std::min(std::max(CStatistics::DMinRows.DVal(), dGroups.DVal()),
					 pstatsInput->DRows().DVal());

		// create a new stats object for the output
		pstatsAgg = GPOS_NEW(pmp) CStatistics(pmp, phmulhist, phmuldoubleWidth,
											  dRowsAgg, pstatsInput->FEmpty());
	}
	...
	CStatisticsUtils::ComputeCardUpperBounds(
		pmp, pstatsInput, pstatsAgg, dRowsAgg, CStatistics::EcbmMin /* ecbm */);
