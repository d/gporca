...
// ctor
CHistogram::CHistogram(DrgPbucket *pdrgppbucket, BOOL fWellDefined)
...
// ctor
CHistogram::CHistogram(DrgPbucket *pdrgppbucket, BOOL fWellDefined,
					   CDouble dNullFreq, CDouble dDistinctRemain,
					   CDouble dFreqRemain, BOOL fColStatsMissing)
	: m_pdrgppbucket(pdrgppbucket),
	  m_fWellDefined(fWellDefined),
	  ...
	  m_fColStatsMissing(fColStatsMissing)
{
	...
	GPOS_ASSERT_IMP(dDistinctRemain < CStatistics::DEpsilon,
					dFreqRemain < CStatistics::DEpsilon);
}

// set histograms null frequency
void
CHistogram::SetNullFrequency(CDouble dNullFreq)
{
...
CHistogram::OsPrint(IOstream &os) const
{
	...
	os << "Was NDVs re-scaled Based on Row Estimate: " << m_fNDVScaled
	   << std::endl;
...
BOOL
CHistogram::FEmpty() const
{
	return (0 == m_pdrgppbucket->UlLength() &&
			CStatistics::DEpsilon > m_dNullFreq &&
			CStatistics::DEpsilon > m_dDistinctRemain);
}

// construct new histogram with less than or less than equal to filter
CHistogram *
CHistogram::PhistLessThanOrLessThanEqual(IMemoryPool *pmp,
										 CStatsPred::EStatsCmpType escmpt,
										 CPoint *ppoint) const
{
	GPOS_ASSERT(CStatsPred::EstatscmptL == escmpt ||
				CStatsPred::EstatscmptLEq == escmpt);
	...
	for (ULONG ulBucketIdx = 0; ulBucketIdx < ulNumBuckets; ulBucketIdx++)
	{
		...
		else
		{
			...
			CBucket *pbucketLast = pbucket->PbucketScaleUpper(
				pmp, ppoint,
				CStatsPred::EstatscmptLEq == escmpt /*fIncludeUpper*/);
			if (NULL != pbucketLast)
			{
	...
	return GPOS_NEW(pmp) CHistogram(pdrgppbucketNew,
									...
									dDistinctRemain, dFreqRemain);
}

// return an array buckets after applying non equality filter on the histogram buckets
DrgPbucket *
CHistogram::PdrgppbucketNEqual(IMemoryPool *pmp, CPoint *ppoint) const
{
	...
	for (ULONG ulBucketIdx = 0; ulBucketIdx < ulNumBuckets; ulBucketIdx++)
	{
		...
		{
			CBucket *pbucketLT = pbucket->PbucketScaleUpper(
				pmp, ppoint, false /*fIncludeUpper */);
			if (NULL != pbucketLT)
			{
...
CHistogram::PhistNEqual(IMemoryPool *pmp, CPoint *ppoint) const
{
	...
	return GPOS_NEW(pmp)
		CHistogram(pdrgppbucket, true /*fWellDefined*/, dNullFreq,
				   m_dDistinctRemain, m_dFreqRemain);
}
...
DrgPbucket *
CHistogram::PdrgppbucketEqual(IMemoryPool *pmp, CPoint *ppoint) const
{
	GPOS_ASSERT(NULL != ppoint);
	...
	if (ppoint->Pdatum()->FNull())
	{
		return GPOS_NEW(pmp)
			CHistogram(GPOS_NEW(pmp) DrgPbucket(pmp), true /* fWellDefined */,
					   m_dNullFreq, DDefaultNDVRemain, DDefaultNDVFreqRemain);
	}

	DrgPbucket *pdrgppbucket = PdrgppbucketEqual(pmp, ppoint);

	if (CStatistics::DEpsilon < m_dDistinctRemain &&
		0 == pdrgppbucket->UlLength())  // no match is found in the buckets
	{
		return GPOS_NEW(pmp) CHistogram(
			...
			1.0,   // dDistinctRemain
			std::min(CDouble(1.0),
					 m_dFreqRemain / m_dDistinctRemain)  // dFreqRemain
		);
	}
...
CHistogram::PhistINDF(IMemoryPool *pmp, CPoint *ppoint) const
{
	...
	if (CStatistics::DEpsilon < m_dDistinctRemain &&
		0 == ulBuckets)  // no match is found in the buckets
	{
		return GPOS_NEW(pmp) CHistogram(
			pdrgppbucket,
			true,  // fWellDefined
	...
	return GPOS_NEW(pmp) CHistogram(pdrgppbucket, true /* fWellDefined */,
									dNullFreq, CHistogram::DDefaultNDVRemain,
									CHistogram::DDefaultNDVFreqRemain);
}

// construct new histogram with greater than or greater than equal filter
CHistogram *
CHistogram::PhistGreaterThanOrGreaterThanEqual(IMemoryPool *pmp,
											   CStatsPred::EStatsCmpType escmpt,
											   CPoint *ppoint) const
{
	GPOS_ASSERT(CStatsPred::EstatscmptGEq == escmpt ||
				CStatsPred::EstatscmptG == escmpt);
	...
	for (ulBucketIdx = 0; ulBucketIdx < ulNumBuckets; ulBucketIdx++)
	{
		...
		if (pbucket->FContains(ppoint))
		{
			if (CStatsPred::EstatscmptGEq == escmpt)
			{
				...
				CBucket *pbucketFirst = pbucket->PbucketScaleLower(
					pmp, ppoint, true /* fIncludeLower */);
...
void
CHistogram::CapNDVs(CDouble dRows)
{
	const ULONG ulBuckets = m_pdrgppbucket->UlLength();
	...
	for (ULONG ul = 0; ul < ulBuckets; ul++)
	{
		...
		pbucket->SetDistinct(std::max(CHistogram::DMinDistinct.DVal(),
									  (dDistinctBucket * dScaleRatio).DVal()));
	}
...
CHistogram::FValid() const
{
	...
	for (ULONG ulBucketIdx = 1; ulBucketIdx < m_pdrgppbucket->UlLength();
		 ulBucketIdx++)
	{
		CBucket *pbucket = (*m_pdrgppbucket)[ulBucketIdx];
...
CHistogram *
CHistogram::PhistFilterNormalized(IMemoryPool *pmp,
								  CStatsPred::EStatsCmpType escmpt,
								  CPoint *ppoint, CDouble *pdScaleFactor) const
{
	// if histogram is not well-defined, then result is not well defined
	if (!FWellDefined())
	{
		CHistogram *phistAfter = GPOS_NEW(pmp)
			CHistogram(GPOS_NEW(pmp) DrgPbucket(pmp), false /* fWellDefined */);
...
CHistogram *
CHistogram::PhistJoinNormalized(IMemoryPool *pmp,
								CStatsPred::EStatsCmpType escmpt, CDouble dRows,
								const CHistogram *phistOther,
								CDouble dRowsOther,
								CDouble *pdScaleFactor) const
{
	...
	if (CStatsPred::EstatscmptEqNDV == escmpt)
	{
		*pdScaleFactor = std::max(
			...
			std::max(CHistogram::DMinDistinct.DVal(),
					 phistOther->DDistinct().DVal()));
		return PhistJoinEqualityNDV(pmp, phistOther);
	}

	BOOL fEqOrINDF = (CStatsPred::EstatscmptEq == escmpt ||
					  CStatsPred::EstatscmptINDF == escmpt);
	if (!fEqOrINDF)
	{
		*pdScaleFactor = CScaleFactorUtils::DDefaultScaleFactorNonEqualityJoin;

		if (CStatsPred::EstatscmptNEq == escmpt ||
			CStatsPred::EstatscmptIDF == escmpt)
		{
			*pdScaleFactor =
				DInEqualityJoinScaleFactor(pmp, dRows, phistOther, dRowsOther);
		}
	...
	*pdScaleFactor =
		std::max(std::max(DMinDistinct.DVal(), DDistinct().DVal()),
	...
	*pdScaleFactor =
		std::min((*pdScaleFactor).DVal(), dCartesianProduct.DVal());
...
CDouble
CHistogram::DInEqualityJoinScaleFactor(IMemoryPool *pmp, CDouble dRows,
									   const CHistogram *phistOther,
									   CDouble dRowsOther) const
{
	GPOS_ASSERT(NULL != phistOther);
	...
	CHistogram *phistJoin =
		PhistJoinNormalized(pmp, CStatsPred::EstatscmptEq, dRows, phistOther,
							dRowsOther, &dScaleFactor);
...
// output histogram
CHistogram *
CHistogram::PhistLASJoinNormalized(IMemoryPool *pmp,
								   CStatsPred::EStatsCmpType escmpt,
								   CDouble dRows, const CHistogram *phistOther,
								   CDouble *pdScaleFactor,
								   BOOL fIgnoreLasjHistComputation) const
{
	// if either histogram is not well-defined, the result is not well defined
	if (!FWellDefined() || !phistOther->FWellDefined())
	{
...
CHistogram *
CHistogram::PhistFilter(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
						CPoint *ppoint) const
{
	...
	switch (escmpt)
	{
		...
		{
			phistAfter =
				PhistGreaterThanOrGreaterThanEqual(pmp, escmpt, ppoint);
			break;
		}
...
CHistogram *
CHistogram::PhistJoin(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
					  const CHistogram *phist) const
{
...
CHistogram *
CHistogram::PhistLASJ(IMemoryPool *pmp, CStatsPred::EStatsCmpType escmpt,
					  const CHistogram *phist) const
{
	...
	while (ul1 < ulBuckets1 && ul2 < ulBuckets2)
	{
		...
		{
			pbucketCandidate = (*m_pdrgppbucket)[ul1]->PbucketCopy(
				pmp);  // candidate bucket in result histogram
			ul1++;
		}
		...
		pbucketCandidate->Difference(pmp, pbucket2, &pbucketLowerSplit,
									 &pbucketUpperSplit);
	...
	if (ul2 == ulBuckets2)
	{
		...
		CStatisticsUtils::AddRemainingBuckets(pmp, m_pdrgppbucket,
											  pdrgppbucketNew, &ul1);
	}
	else
	{
	...
	CDouble dNullFreq =
		CLeftAntiSemiJoinStatsProcessor::DNullFreqLASJ(escmpt, this, phist);

	return GPOS_NEW(pmp)
		CHistogram(pdrgppbucketNew, true /*fWellDefined*/, dNullFreq,
				   m_dDistinctRemain, m_dFreqRemain);
}
...
CHistogram::DNormalize()
{
	...
	if (UlBuckets() == 0 && CStatistics::DEpsilon > m_dNullFreq &&
		CStatistics::DEpsilon > m_dDistinctRemain)
	{
		return CDouble(GPOS_FP_ABS_MAX);
	}

	CDouble dScaleFactor =
		std::max(DOUBLE(1.0), (CDouble(1.0) / DFrequency()).DVal());
	...
	CDouble dFreqRemain =
		std::min((m_dFreqRemain * dScaleFactor).DVal(), DOUBLE(1.0));
	if (CStatistics::DEpsilon > m_dDistinctRemain)
	{
		dFreqRemain = 0.0;
	}
...
CHistogram::PhistCopy(IMemoryPool *pmp) const
{
	...
	CHistogram *phistCopy =
		GPOS_NEW(pmp) CHistogram(pdrgpbucket, m_fWellDefined, m_dNullFreq,
								 m_dDistinctRemain, m_dFreqRemain);
	if (FScaledNDV())
	{
...
BOOL
CHistogram::FSupportsFilter(CStatsPred::EStatsCmpType escmpt)
{
...
BOOL
CHistogram::FSupportsJoinPred(CStatsPred::EStatsCmpType escmpt)
{
...
CHistogram::PhistJoinEquality(IMemoryPool *pmp, const CHistogram *phist) const
{
	...
	while (ul1 < ulBuckets1 && ul2 < ulBuckets2)
	{
		...
		if (pbucket1->FIntersects(pbucket2))
		{
			...
			CBucket *pbucketNew = pbucket1->PbucketIntersect(
				pmp, pbucket2, &dFreqIntersect1, &dFreqIntersect2);
	...
	ComputeJoinNDVRemainInfo(this, phist, pdrgppbucketJoin, dFreqJoinBuckets1,
							 dFreqJoinBuckets2, &dDistinctRemain, &dFreqRemain);

	return GPOS_NEW(pmp)
		CHistogram(pdrgppbucketJoin, true /*fWellDefined*/, 0.0 /*dNullFreq*/,
				   dDistinctRemain, dFreqRemain);
}

// construct a new histogram for NDV based cardinality estimation
CHistogram *
CHistogram::PhistJoinEqualityNDV(IMemoryPool *pmp,
								 const CHistogram *phist) const
{
	CDouble dDistinctRemain(0.0);
	CDouble dFreqRemain(0.0);
...
CHistogram::PhistJoinINDF(IMemoryPool *pmp, const CHistogram *phist) const
{
	...
	CDouble dNullFreq = DNullFreq() * phist->DNullFreq() * DOUBLE(1.0) /
						std::max(DDistinct(), phist->DDistinct());
...
BOOL
CHistogram::FCanComputeJoinNDVRemain(const CHistogram *phist1,
									 const CHistogram *phist2)
{
	...
	BOOL fHasDistinctRemain1 =
		CStatistics::DEpsilon < phist1->DDistinctRemain();
	BOOL fHasDistinctRemain2 =
		CStatistics::DEpsilon < phist2->DDistinctRemain();
	...
	if ((fHasDistinctRemain1 || fHasBuckets1) &&
		(fHasDistinctRemain2 || fHasBuckets2))
	{
		//
		return true;
	}
...
CHistogram::ComputeJoinNDVRemainInfo(
	const CHistogram *phist1, const CHistogram *phist2,
	DrgPbucket *pdrgppbucketJoin, CDouble dFreqJoinBuckets1,
	CDouble dFreqJoinBuckets2, CDouble *pdDistinctRemain, CDouble *pdFreqRemain)
{
	GPOS_ASSERT(NULL != phist1);
	GPOS_ASSERT(NULL != phist2);
	...
	*pdFreqRemain = 0.0;
	...
	CDouble dFreqNonJoinBuckets1 =
		std::max(CDouble(0), (dFreqBuckets1 - dFreqJoinBuckets1));
	CDouble dFreqNonJoinBuckets2 =
		std::max(CDouble(0), (dFreqBuckets2 - dFreqJoinBuckets2));

	// compute the NDV of the non-joining buckets
	CDouble dNDVNonJoinBuckets1 =
		CStatisticsUtils::DDistinct(phist1->Pdrgpbucket()) - dNDVJoin;
	CDouble dNDVNonJoinBuckets2 =
		CStatisticsUtils::DDistinct(phist2->Pdrgpbucket()) - dNDVJoin;
	...
	CDouble dFreqJoinRemain =
		dFreqRemain1 * dFreqRemain2 /
		std::max(phist1->DDistinctRemain(), phist2->DDistinctRemain());
	dFreqJoinRemain =
		dFreqJoinRemain + dFreqRemain1 * dFreqNonJoinBuckets2 /
							  std::max(dDistinctNonNull1, dNDVNonJoinBuckets2);
	dFreqJoinRemain =
		dFreqJoinRemain + dFreqRemain2 * dFreqNonJoinBuckets1 /
							  std::max(dDistinctNonNull2, dNDVNonJoinBuckets1);
...
CHistogram *
CHistogram::PhistGroupByNormalized(IMemoryPool *pmp,
								   CDouble,  // dRows,
								   CDouble *pdDistinctValues) const
{
	// if either histogram is not well-defined, the result is not well defined
	if (!FWellDefined())
	{
	...
	for (ULONG ul = 0; ul < ulBuckets; ul++)
	{
		...
		CBucket *pbucketNew = GPOS_NEW(pmp)
			CBucket(ppLower, ppUpper, true /* fClosedLower */, fUpperClosed,
					pbucket->DDistinct() / dDistinct, pbucket->DDistinct());
	...
	CHistogram *phistAfter =
		GPOS_NEW(pmp) CHistogram(pdrgppbucketNew, true /*fWellDefined*/,
								 dNullFreqNew, m_dDistinctRemain, dFreqRemain);
...
CHistogram *
CHistogram::PhistUnionAllNormalized(IMemoryPool *pmp, CDouble dRows,
									...
									CDouble dRowsOther) const
{
	...
	while (NULL != pbucket1 && NULL != pbucket2)
	{
		...
		{
			pdrgppbucketNew->Append(
				pbucket1->PbucketUpdateFrequency(pmp, dRows, dRowsNew));
		...
		{
			pdrgppbucketNew->Append(
				pbucket2->PbucketUpdateFrequency(pmp, dRowsOther, dRowsNew));
		...
		else
		{
			...
			CBucket *pbucketMerge = pbucket1->PbucketMerge(
				pmp, pbucket2, dRows, dRowsOther, &pbucket1New, &pbucket2New);
	...
	ul1 =
		UlAddResidualUnionAllBucket(pmp, pdrgppbucketNew, pbucket1, dRowsOther,
									dRowsNew, fbucket1Residual, ul1);
	ul2 = UlAddResidualUnionAllBucket(pmp, pdrgppbucketNew, pbucket2, dRows,
									  dRowsNew, fbucket2Residual, ul2);

	CleanupResidualBucket(pbucket1, fbucket1Residual);
	CleanupResidualBucket(pbucket2, fbucket2Residual);

	// add any leftover buckets from other histogram
	AddBuckets(pmp, phist->m_pdrgppbucket, pdrgppbucketNew, dRowsOther,
			   dRowsNew, ul2, ulBuckets2);

	// add any leftover buckets from this histogram
	AddBuckets(pmp, m_pdrgppbucket, pdrgppbucketNew, dRows, dRowsNew, ul1,
			   ulBuckets1);

	CDouble dNullFreqNew =
		(m_dNullFreq * dRows + phist->m_dNullFreq * dRowsOther) / dRowsNew;

	CDouble dDistinctRemain =
		std::max(m_dDistinctRemain, phist->m_dDistinctRemain);
	CDouble dFreqRemain =
		(m_dFreqRemain * dRows + phist->m_dFreqRemain * dRowsOther) / dRowsNew;

	CHistogram *phistResult =
		GPOS_NEW(pmp) CHistogram(pdrgppbucketNew, true /*fWellDefined*/,
								 dNullFreqNew, dDistinctRemain, dFreqRemain);
...
ULONG
CHistogram::UlAddResidualUnionAllBucket(IMemoryPool *pmp,
										DrgPbucket *pdrgppbucket,
										CBucket *pbucket, CDouble dRowsOld,
										CDouble dRowsNew, BOOL fbucketResidual,
										ULONG ulIndex) const
{
	GPOS_ASSERT(NULL != pdrgppbucket);
	...
	{
		pdrgppbucket->Append(
			pbucket->PbucketUpdateFrequency(pmp, dRowsOld, dRowsNew));
		return ulIndex + 1;
	}
...
void
CHistogram::AddBuckets(IMemoryPool *pmp, DrgPbucket *pdrgppbucketSrc,
					   DrgPbucket *pdrgppbucketDest, CDouble dRowsOld,
					   CDouble dRowsNew, ULONG ulBegin, ULONG ulEnd)
{
	...
	{
		pdrgppbucketDest->Append(
			((*pdrgppbucketSrc)[ul])
				->PbucketUpdateFrequency(pmp, dRowsOld, dRowsNew));
	}
}

// cleanup residual buckets
void
CHistogram::CleanupResidualBucket(CBucket *pbucket, BOOL fbucketResidual) const
{
...
CBucket *
CHistogram::PbucketNext(const CHistogram *phist, CBucket *pbucketNew,
						BOOL *pfbucketResidual, ULONG *pulBucketIndex) const
{
	GPOS_ASSERT(NULL != phist);
	if (NULL != pbucketNew)
	{
...
CHistogram *
CHistogram::PhistUnionNormalized(IMemoryPool *pmp, CDouble dRows,
								 const CHistogram *phistOther,
								 CDouble dRowsOther,
								 CDouble *pdRowsOutput) const
{
	GPOS_ASSERT(NULL != phistOther);
	...
	while (NULL != pbucket1 && NULL != pbucket2)
	{
		if (pbucket1->FBefore(pbucket2))
		{
			...
			pdrgpdoubleRows->Append(
				GPOS_NEW(pmp) CDouble(pbucket1->DFrequency() * dRows));
			CleanupResidualBucket(pbucket1, fbucket1Residual);
			ul1++;
			pbucket1 = (*this)[ul1];
			fbucket1Residual = false;
		...
		else if (pbucket2->FBefore(pbucket1))
		{
			...
			pdrgpdoubleRows->Append(
				GPOS_NEW(pmp) CDouble(pbucket2->DFrequency() * dRowsOther));
			CleanupResidualBucket(pbucket2, fbucket2Residual);
			ul2++;
		...
		else
		{
			...
			pdrgpdoubleRows->Append(
				GPOS_NEW(pmp) CDouble(pbucketMerge->DFrequency() * dRows));
			...
			pbucket1 = PbucketNext(this, pbucket1New, &fbucket1Residual, &ul1);
			pbucket2 =
				PbucketNext(phistOther, pbucket2New, &fbucket2Residual, &ul2);
		}
	}
	...
	ul1 = UlAddResidualUnionBucket(pmp, pdrgppbucket, pbucket1, dRowsOther,
								   fbucket1Residual, ul1, pdrgpdoubleRows);
	ul2 = UlAddResidualUnionBucket(pmp, pdrgppbucket, pbucket2, dRows,
								   fbucket2Residual, ul2, pdrgpdoubleRows);

	CleanupResidualBucket(pbucket1, fbucket1Residual);
	CleanupResidualBucket(pbucket2, fbucket2Residual);

	// add any leftover buckets from other histogram
	AddBuckets(pmp, phistOther->m_pdrgppbucket, pdrgppbucket, dRowsOther,
			   pdrgpdoubleRows, ul2, ulBuckets2);

	// add any leftover buckets from this histogram
	AddBuckets(pmp, m_pdrgppbucket, pdrgppbucket, dRows, pdrgpdoubleRows, ul1,
			   ulBuckets1);

	// compute the total number of null values from both histograms
	CDouble dNullRows = std::max((this->DNullFreq() * dRows),
								 (phistOther->DNullFreq() * dRowsOther));

	// compute the total number of distinct values (NDV) that are not captured by the buckets in both the histograms
	CDouble dNDVRemain =
		std::max(m_dDistinctRemain, phistOther->DDistinctRemain());

	// compute the total number of rows having distinct values not captured by the buckets in both the histograms
	CDouble dNDVRemainRows = std::max((this->DFreqRemain() * dRows),
									  (phistOther->DFreqRemain() * dRowsOther));

	CHistogram *phistResult =
		PhistUpdatedFrequency(pmp, pdrgppbucket, pdrgpdoubleRows, pdRowsOutput,
							  dNullRows, dNDVRemain, dNDVRemainRows);
	// clean up
...
CHistogram *
CHistogram::PhistUpdatedFrequency(IMemoryPool *pmp, DrgPbucket *pdrgppbucket,
								  DrgPdouble *pdrgpdouble, CDouble *pdRowOutput,
								  CDouble dNullRows, CDouble dNDVRemain,
								  CDouble dNDVRemainRows) const
{
	GPOS_ASSERT(NULL != pdrgppbucket);
	...
	*pdRowOutput = std::max(CStatistics::DMinRows, dRowCummulative);
	...
	for (ULONG ul = 0; ul < ulLen; ul++)
	{
		...
		CBucket *pbucketNew = GPOS_NEW(pmp) CBucket(
			pbucket->PpLower(), pbucket->PpUpper(), pbucket->FLowerClosed(),
			pbucket->FUpperClosed(), dFrequency, pbucket->DDistinct());
	...
	return GPOS_NEW(pmp)
		CHistogram(pdrgppbucketNew, true /* fWellDefined */, dNullFreq,
				   dNDVRemain, dNDVRemainFreq, false /* fColStatsMissing */
		);
}

// add residual bucket in an union operation to the array of buckets in the histogram
ULONG
CHistogram::UlAddResidualUnionBucket(IMemoryPool *pmp, DrgPbucket *pdrgppbucket,
									 CBucket *pbucket, CDouble dRows,
									 BOOL fbucketResidual, ULONG ulIndex,
									 DrgPdouble *pdrgpdouble) const
{
	GPOS_ASSERT(NULL != pdrgppbucket);
	GPOS_ASSERT(NULL != pdrgpdouble);
	...
	pdrgpdouble->Append(GPOS_NEW(pmp)
							CDouble(pbucketNew->DFrequency() * dRows));
...
void
CHistogram::AddBuckets(IMemoryPool *pmp, DrgPbucket *pdrgppbucketSrc,
					   DrgPbucket *pdrgppbucketDest, CDouble dRows,
					   DrgPdouble *pdrgpdouble, ULONG ulBegin, ULONG ulEnd)
{
	GPOS_ASSERT(NULL != pdrgppbucketSrc);
	GPOS_ASSERT(NULL != pdrgppbucketDest);
	GPOS_ASSERT(ulBegin <= ulEnd);
...
CBucket *CHistogram::operator[](ULONG ulPos) const
{
	if (ulPos < UlBuckets())
	{
...
CHistogram::Pdxlstatsdercol(IMemoryPool *pmp, CMDAccessor *pmda, ULONG ulColId,
							CDouble dWidth) const
{
	...
	for (ULONG ul = 0; ul < ulBuckets; ul++)
	{
		...
		CDXLBucket *pdxlbucket = GPOS_NEW(pmp)
			CDXLBucket(pdxldatumLower, pdxldatumUpper, pbucket->FLowerClosed(),
					   pbucket->FUpperClosed(), dFreq, dDistinct);

		pdrgpdxlbucket->Append(pdxlbucket);
	}

	return GPOS_NEW(pmp)
		CDXLStatsDerivedColumn(ulColId, dWidth, m_dNullFreq, m_dDistinctRemain,
							   m_dFreqRemain, pdrgpdxlbucket);
}

// randomly pick a bucket index based on bucket frequency values
ULONG
CHistogram::UlRandomBucketIndex(ULONG *pulSeed) const
{
...
CHistogram::ComputeSkew()
{
	...
	if (!FNormalized() || 0 == m_pdrgppbucket->UlLength() ||
		!(*m_pdrgppbucket)[0]->FCanSample())
	{
		return;
	}
...
CHistogram::PhistDefaultBoolColStats(IMemoryPool *pmp)
{
	...
	return GPOS_NEW(pmp)
		CHistogram(pdrgpbucket, true /* fWellDefined */, dNullFreq,
				   dDistinctRemain, dFreqRemain, true /*fColStatsMissing */
		);
}

// check if the join cardinality estimation can be done based on NDV alone
BOOL
CHistogram::FNDVBasedCardEstimation(const CHistogram *phist)
{
	GPOS_ASSERT(NULL != phist);
	...
	if (IMDType::EtiInt2 == eti || IMDType::EtiInt4 == eti ||
		IMDType::EtiInt8 == eti || IMDType::EtiBool == eti ||
		IMDType::EtiOid == eti)
	{
		return false;
	}
	...
	{
		IDatumStatisticsMappable *pdatumMappable =
			(IDatumStatisticsMappable *) pdatum;
...
void
CHistogram::AddHistograms(IMemoryPool *pmp, HMUlHist *phmulhistSrc,
...
void
CHistogram::AddDummyHistogramAndWidthInfo(IMemoryPool *pmp, CColumnFactory *pcf,
										  HMUlHist *phmulhistOutput,
										  HMUlDouble *phmuldoubleWidthOutput,
										  const DrgPul *pdrgpul, BOOL fEmpty)
{
	...
	for (ULONG ul = 0; ul < ulCount; ul++)
	{
		...
		phmuldoubleWidthOutput->FInsert(GPOS_NEW(pmp) ULONG(ulColId),
...
void
CHistogram::AddEmptyHistogram(IMemoryPool *pmp, HMUlHist *phmulhistOutput,
