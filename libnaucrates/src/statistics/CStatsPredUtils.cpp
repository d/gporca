...
CStatsPredUtils::Estatscmpt(const CWStringConst *pstrOpName)
{
...
//---------------------------------------------------------------------------
CStatsPred::EStatsCmpType
CStatsPredUtils::Estatscmpt(IMDId *pmdid)
{
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredUnsupported(IMemoryPool *pmp,
									   CExpression *,  // pexprPred,
									   CColRefSet *	//pcrsOuterRefs
)
{
	return GPOS_NEW(pmp)
		CStatsPredUnsupported(ULONG_MAX, CStatsPred::EstatscmptOther);
}
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredNullTest(IMemoryPool *pmp, CExpression *pexprPred,
									CColRefSet *  //pcrsOuterRefs
)
{
	GPOS_ASSERT(NULL != pexprPred);
	GPOS_ASSERT(FScalarIdentIsNull(pexprPred) ||
				FScalarIdentIsNotNull(pexprPred));
	...
	CScalarIdent *popScalarIdent =
		CScalarIdent::PopConvert((*pexprNullTest)[0]->Pop());
	...
	CStatsPredPoint *pstatspred =
		GPOS_NEW(pmp) CStatsPredPoint(pcr->UlId(), escmpt, ppoint);
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredPoint(IMemoryPool *pmp, CExpression *pexprPred,
								 CColRefSet *  //pcrsOuterRefs,
)
{
	GPOS_ASSERT(NULL != pexprPred);
...
//---------------------------------------------------------------------------
CStatsPred::EStatsCmpType
CStatsPredUtils::Estatscmptype(IMDId *pmdid)
{
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::Pstatspred(IMemoryPool *pmp, CExpression *pexpr)
{
	...
	GPOS_ASSERT(COperator::EopScalarConst == pexprRight->Pop()->Eopid() ||
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FCmpColsIgnoreCast(CExpression *pexpr,
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredExtract(IMemoryPool *pmp, CExpression *pexprScalar,
								   CColRefSet *pcrsOuterRefs)
{
	GPOS_ASSERT(NULL != pexprScalar);
	if (CPredicateUtils::FOr(pexprScalar))
	{
		CStatsPred *pstatspredDisj =
			PstatspredDisj(pmp, pexprScalar, pcrsOuterRefs);
	...
	{
		CStatsPred *pstatspredConj =
			PstatspredConj(pmp, pexprScalar, pcrsOuterRefs);
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredConj(IMemoryPool *pmp, CExpression *pexprScalar,
								CColRefSet *pcrsOuterRefs)
{
	GPOS_ASSERT(NULL != pexprScalar);
	...
	for (ULONG ul = 0; ul < ulLen; ul++)
	{
		...
		{
			CStatsPred *pstatspredDisj =
				PstatspredDisj(pmp, pexprPred, pcrsOuterRefs);
			if (NULL != pstatspredDisj)
			{
		...
		{
			AddSupportedStatsFilters(pmp, pdrgpstatspred, pexprPred,
									 pcrsOuterRefs);
		}
	}
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredDisj(IMemoryPool *pmp, CExpression *pexprPred,
								CColRefSet *pcrsOuterRefs)
{
	GPOS_ASSERT(NULL != pexprPred);
	...
	CExpression *pexprNew =
		CExpressionUtils::PexprDedupChildren(pmp, pexprPred);
	...
	for (ULONG ul = 0; ul < ulLen; ul++)
	{
		...
		AddSupportedStatsFilters(pmp, pdrgpstatspredDisjChild, pexpr,
								 pcrsOuterRefs);
	}
...
//---------------------------------------------------------------------------
void
CStatsPredUtils::AddSupportedStatsFilters(IMemoryPool *pmp,
										  ...
										  CColRefSet *pcrsOuterRefs)
{
	...
	if (COperator::EopScalarConst == pexprPred->Pop()->Eopid())
	{
		pdrgpstatspred->Append(GPOS_NEW(pmp) CStatsPredUnsupported(
			ULONG_MAX, CStatsPred::EstatscmptOther,
...
CStatsPredUtils::Ept(IMemoryPool *pmp, CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
	if (CPredicateUtils::FOr(pexprPred))
	{
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FConjunction(IMemoryPool *pmp, CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
	DrgPexpr *pdrgpexprConjuncts =
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FBooleanScIdent(CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
	return CPredicateUtils::FBooleanScalarIdent(pexprPred) ||
		   CPredicateUtils::FNegatedBooleanScalarIdent(pexprPred);
}
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FPointPredicate(CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
	return (CPredicateUtils::FIdentCompareConstIgnoreCast(
		pexprPred, COperator::EopScalarCmp));
}
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FPointIDF(CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FPointINDF(CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FScalarIdentIsNull(CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
	...
	return CUtils::FScalarNullTest(pexprPred) &&
		   CUtils::FScalarIdent((*pexprPred)[0]);
}
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FScalarIdentIsNotNull(CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprPred);
	...
	return CUtils::FScalarBoolOp(pexprPred, CScalarBoolOp::EboolopNot) &&
		   FScalarIdentIsNull(pexprNullTest);
}
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredLike(IMemoryPool *pmp, CExpression *pexprPred,
								CColRefSet *  //pcrsOuterRefs,
)
{
	GPOS_ASSERT(NULL != pexprPred);
	...
	CPredicateUtils::ExtractLikePredComponents(pexprPred, &pexprScIdent,
											   &pexprScConst);

	if (NULL == pexprScIdent || NULL == pexprScConst)
	{
		return GPOS_NEW(pmp)
			CStatsPredUnsupported(ULONG_MAX, CStatsPred::EstatscmptLike);
	}

	CScalarIdent *popScalarIdent =
		CScalarIdent::PopConvert(pexprScIdent->Pop());
	ULONG ulColId = popScalarIdent->Pcr()->UlId();

	CScalarConst *popScalarConst =
		CScalarConst::PopConvert(pexprScConst->Pop());
	...
	if (!IMDType::FStatsComparable(pcr->Pmdtype(), pdatumLiteral))
	{
		...
		return GPOS_NEW(pmp)
			CStatsPredUnsupported(pcr->UlId(), CStatsPred::EstatscmptLike);
	}
	...
	return GPOS_NEW(pmp)
		CStatsPredLike(ulColId, pexprLeft, pexprRight, dDefaultScaleFactor);
}
...
//---------------------------------------------------------------------------
void
CStatsPredUtils::ProcessArrayCmp(IMemoryPool *pmp, CExpression *pexprPred,
								 DrgPstatspred *pdrgpstatspred)
{
	GPOS_ASSERT(NULL != pdrgpstatspred);
	GPOS_ASSERT(NULL != pexprPred);
	...
	BOOL fCompareToConstAndScalarIdents =
		CPredicateUtils::FCompareCastIdentToConstArray(pexprPred) ||
	...
	{
		// unsupported predicate for stats calculations
		pdrgpstatspred->Append(GPOS_NEW(pmp)
								   CStatsPredUnsupported(pcr->UlId(), escmpt));
	...
	for (ULONG ul = 0; ul < ulConstants; ul++)
	{
		CExpression *pexprConst =
			CUtils::PScalarArrayExprChildAt(pmp, pexprArray, ul);
		if (COperator::EopScalarConst == pexprConst->Pop()->Eopid())
		{
			CScalarConst *popScalarConst =
				CScalarConst::PopConvert(pexprConst->Pop());
			IDatum *pdatumLiteral = popScalarConst->Pdatum();
			...
			if (!pdatumLiteral->FStatsComparable(pdatumLiteral))
			{
				// stats calculations on such datums unsupported
				pstatspredChild =
					GPOS_NEW(pmp) CStatsPredUnsupported(pcr->UlId(), escmpt);
			}
			else
			{
				pstatspredChild = GPOS_NEW(pmp)
					CStatsPredPoint(pmp, pcr, escmpt, pdatumLiteral);
			}
	...
	if (fAny)
	{
		CStatsPredDisj *pstatspredOr =
			GPOS_NEW(pmp) CStatsPredDisj(pdrgpstatspredChild);
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::PstatspredBoolean(IMemoryPool *pmp, CExpression *pexprPred,
								   CColRefSet *  //pcrsOuterRefs
)
{
	GPOS_ASSERT(NULL != pexprPred);
	GPOS_ASSERT(CPredicateUtils::FBooleanScalarIdent(pexprPred) ||
				CPredicateUtils::FNegatedBooleanScalarIdent(pexprPred));
	...
	if (CPredicateUtils::FBooleanScalarIdent(pexprPred))
	{
		...
		pdatum = pmda->PtMDType<IMDTypeBool>()->PdatumBool(
			pmp, true /* fValue */, false /* fNull */);
	...
	if (!pdatum->FStatsComparable(pdatum))
	{
		...
		return GPOS_NEW(pmp)
			CStatsPredUnsupported(ulColId, CStatsPred::EstatscmptEq);
	}


	GPOS_ASSERT(NULL != pdatum && ULONG_MAX != ulColId);

	return GPOS_NEW(pmp) CStatsPredPoint(ulColId, CStatsPred::EstatscmptEq,
...
CStatsPredJoin *
CStatsPredUtils::PstatsjoinExtract(
	IMemoryPool *pmp, CExpression *pexprJoinPred,
	DrgPcrs
		*pdrgpcrsOutput,  // array of output columns of join's relational inputs
	CColRefSet *pcrsOuterRefs, DrgPexpr *pdrgpexprUnsupported)
{
	...
	CColRefSet *pcrsUsed =
		CDrvdPropScalar::Pdpscalar(pexprJoinPred->PdpDerive())->PcrsUsed();
	...
	BOOL fSupportedScIdentComparison =
		FCmpColsIgnoreCast(pexprJoinPred, &pcrLeft, &escmpt, &pcrRight);
	if (fSupportedScIdentComparison && CStatsPred::EstatscmptOther != escmpt)
	{
		...
		ULONG ulIndexLeft =
			CUtils::UlPcrIndexContainingSet(pdrgpcrsOutput, pcrLeft);
		ULONG ulIndexRight =
			CUtils::UlPcrIndexContainingSet(pdrgpcrsOutput, pcrRight);

		if (ULONG_MAX != ulIndexLeft && ULONG_MAX != ulIndexRight &&
			ulIndexLeft != ulIndexRight)
		{
			if (ulIndexLeft < ulIndexRight)
			{
				return GPOS_NEW(pmp)
					CStatsPredJoin(pcrLeft->UlId(), escmpt, pcrRight->UlId());
			}

			return GPOS_NEW(pmp)
				CStatsPredJoin(pcrRight->UlId(), escmpt, pcrLeft->UlId());
		}
	}
...
DrgPstatspredjoin *
CStatsPredUtils::PdrgpstatspredjoinExtract(
	IMemoryPool *pmp, CExpression *pexprScalar,
	DrgPcrs
		*pdrgpcrsOutput,  // array of output columns of join's relational inputs
	CColRefSet *pcrsOuterRefs, CStatsPred **ppstatspredUnsupported)
{
	GPOS_ASSERT(NULL != pexprScalar);
	GPOS_ASSERT(NULL != pdrgpcrsOutput);

	DrgPstatspredjoin *pdrgpstatspredjoin =
		GPOS_NEW(pmp) DrgPstatspredjoin(pmp);
	...
	for (ULONG ul = 0; ul < ulSize; ul++)
	{
		...
		CStatsPredJoin *pstatsjoin =
			PstatsjoinExtract(pmp, pexprPred, pdrgpcrsOutput, pcrsOuterRefs,
	...
	if (1 == ulUnsupported)
	{
		*ppstatspredUnsupported = CStatsPredUtils::PstatspredExtract(
			pmp, (*pdrgpexprUnsupported)[0], pcrsOuterRefs);
	}
	else if (1 < ulUnsupported)
	{
		pdrgpexprUnsupported->AddRef();
		CExpression *pexprConj = CPredicateUtils::PexprConjDisj(
			pmp, pdrgpexprUnsupported, true /* fConjunction */);
		*ppstatspredUnsupported =
			CStatsPredUtils::PstatspredExtract(pmp, pexprConj, pcrsOuterRefs);
		pexprConj->Release();
	}
...
DrgPstatspredjoin *
CStatsPredUtils::Pdrgpstatspredjoin(
	IMemoryPool *pmp, CExpressionHandle &exprhdl, CExpression *pexprScalarInput,
	DrgPcrs
		*pdrgpcrsOutput,  // array of output columns of join's relational inputs
	CColRefSet *pcrsOuterRefs)
{
	...
	CExpression *pexprScalar = CPredicateUtils::PexprRemoveImpliedConjuncts(
		pmp, pexprScalarInput, exprhdl);

	// extract all the conjuncts
	CStatsPred *pstatspredUnsupported = NULL;
	DrgPstatspredjoin *pdrgpstatspredjoin =
		PdrgpstatspredjoinExtract(pmp, pexprScalar, pdrgpcrsOutput,
								  pcrsOuterRefs, &pstatspredUnsupported);
...
//---------------------------------------------------------------------------
DrgPstatspredjoin *
CStatsPredUtils::Pdrgpstatspredjoin(IMemoryPool *pmp,
									CExpressionHandle &exprhdl)
{
	...
	DrgPstatspredjoin *pdrgpstats = Pdrgpstatspredjoin(
		pmp, exprhdl, pexprScalar, pdrgpcrsOutput, pcrsOuterRefs);
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FConjOrDisjPred(CStatsPred *pstatspred)
{
	return ((CStatsPred::EsptConj == pstatspred->Espt()) ||
...
//---------------------------------------------------------------------------
BOOL
CStatsPredUtils::FUnsupportedPredOnDefinedCol(CStatsPred *pstatspred)
{
	return ((CStatsPred::EsptUnsupported == pstatspred->Espt()) &&
			(ULONG_MAX == pstatspred->UlColId()));
}
