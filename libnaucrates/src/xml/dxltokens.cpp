...
IMemoryPool *CDXLTokens::m_pmp = NULL;

CDXLMemoryManager *CDXLTokens::m_pmm = NULL;
...
//---------------------------------------------------------------------------
void
CDXLTokens::Init(IMemoryPool *pmp)
{
	...
	for (ULONG ul = 0; ul < GPOS_ARRAY_SIZE(rgStrMap); ul++)
	{
		...
		m_pstrmap[mapelem.m_edxlt].m_pstr =
			GPOS_NEW(m_pmp) CWStringConst(m_pmp, mapelem.m_wsz);
...
//---------------------------------------------------------------------------
const CWStringConst *
CDXLTokens::PstrToken(Edxltoken edxltoken)
{
...
//---------------------------------------------------------------------------
const XMLCh *
CDXLTokens::XmlstrToken(Edxltoken edxltoken)
{
