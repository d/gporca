...
private:
	...
	static GPOS_RESULT
	EresUnittest_CStatistics(SStatsFilterSTestCase rgstatsdisjtc[],
							 ULONG ulTestCases);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj1(IMemoryPool *pmp);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj2(IMemoryPool *pmp);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj3(IMemoryPool *pmp);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj4(IMemoryPool *pmp);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj5(IMemoryPool *pmp);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj6(IMemoryPool *pmp);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj7(IMemoryPool *pmp);

	// disjunction filters
	static CStatsPred *
	PstatspredDisj8(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredNestedPredDiffCol1(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredNestedPredDiffCol2(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredNestedPredCommonCol1(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredNestedPredCommonCol2(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredNestedSharedCol(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredDisjOverConjSameCol1(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredDisjOverConjSameCol2(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredDisjOverConjSameCol3(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredDisjOverConjSameCol4(IMemoryPool *pmp);

	// nested AND and OR predicates
	static CStatsPred *
	PstatspredDisjOverConjDifferentCol1(IMemoryPool *pmp);

	static CStatsPred *
	PstatspredDisjOverConjMultipleIdenticalCols(IMemoryPool *pmp);

	// conjunctive predicates
	static CStatsPred *
	PstatspredConj(IMemoryPool *pmp);

	// generate an array of filter given a column identifier, comparison type, and array of integer point
	static DrgPstatspred *
	PdrgpstatspredInteger(IMemoryPool *pmp, ULONG ulColId,
						  CStatsPred::EStatsCmpType escmpt, INT *piVals,
						  ULONG ulVals);

	// create a numeric predicate on a particular column
	static DrgPstatspred *
	PdrgppredfilterNumeric(IMemoryPool *pmp, ULONG ulColId,
						   SStatsCmpValElem statsCmpValElem);

	// create a filter on a column with null values
	static CStatsPred *
	PstatspredNullableCols(IMemoryPool *pmp);

	// create a point filter where the constant is null
	static CStatsPred *
	PstatspredWithNullConstant(IMemoryPool *pmp);

	// create a 'is not null' point filter
	static CStatsPred *
	PstatspredNotNull(IMemoryPool *pmp);

	// compare the derived statistics with the statistics in the outputfile
	static GPOS_RESULT
	EresUnittest_CStatisticsCompare(IMemoryPool *pmp, CMDAccessor *pmda,
...
public:
	...
	static GPOS_RESULT
	EresUnittest_CStatisticsBasicsFromDXL();
