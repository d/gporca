...
private:
	...
	static void
	StatsFilterInt4(IMemoryPool *pmp, ULONG ulColId, INT iLower, INT iUpper,
					DrgPstatspred *pgrgpstatspred);

	// create filter on boolean types
	static void
	StatsFilterBool(IMemoryPool *pmp, ULONG ulColId, BOOL fValue,
					DrgPstatspred *pgrgpstatspred);

	// create filter on numeric types
	static void
	StatsFilterNumeric(IMemoryPool *pmp, ULONG ulColId,
					   CWStringDynamic *pstrLowerEncoded,
					   CWStringDynamic *pstrUpperEncoded, CDouble dValLower,
					   CDouble dValUpper, DrgPstatspred *pdrgpstatspred);

	// create filter on generic types
	static void
	StatsFilterGeneric(IMemoryPool *pmp, ULONG ulColId, OID oid,
					   CWStringDynamic *pstrLowerEncoded,
					   CWStringDynamic *pstrUpperEncoded, LINT lValLower,
					   LINT lValUpper, DrgPstatspred *pgrgpstatspred);

	static CHistogram *
	PhistExampleInt4Dim(IMemoryPool *pmp);

	// helper function that generates an array of ULONG pointers
	static DrgPul *
	Pdrgpul(IMemoryPool *pmp, ULONG ul1, ULONG ul2 = ULONG_MAX)
	{
	...
	PtabdescTwoColumnSource(IMemoryPool *pmp, const CName &nameTable,
...
public:
	...
	Pdrgpstatspred1(IMemoryPool *pmp);

	static DrgPstatspred *
	Pdrgpstatspred2(IMemoryPool *pmp);
	...
	// union all tests
	static GPOS_RESULT
	EresUnittest_UnionAll();

	// statistics basic tests
	static GPOS_RESULT
	EresUnittest_CStatisticsBasic();

	// exercise stats derivation during optimization
	static GPOS_RESULT
	EresUnittest_CStatisticsSelectDerivation();

	// GbAgg test when grouping on repeated columns
	static GPOS_RESULT
	EresUnittest_GbAggWithRepeatedGbCols();
