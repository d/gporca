...
private:
	// local memory pool
	static IMemoryPool *m_pmp;

	// construct an array of segment ids
	static gpdxl::DrgPi *
	PdrgpiSegments(IMemoryPool *pmp);

	// generate minidump file name from passed file name
	static CHAR *
	SzMinidumpFileName(IMemoryPool *pmp, const CHAR *szFileName);
...
public:
	...
	static void
	InitProviderFile(IMemoryPool *pmp);
	...
	static CTableDescriptor *
	PtabdescPlain(IMemoryPool *pmp, ULONG ulCols, IMDId *pmdid,
				  const CName &nameTable, BOOL fNullable = false);

	// generate a plain table descriptor, where the column names are generated using a format string containing %d
	static CTableDescriptor *
	PtabdescPlainWithColNameFormat(IMemoryPool *pmp, ULONG ulCols, IMDId *pmdid,
	...
	static CExpression *
	PexprLogicalNAryJoin(IMemoryPool *pmp, DrgPexpr *pdrgpexpr);

	// generate an n-ary join expression using given array of relation names
	static CExpression *
	PexprLogicalNAryJoin(IMemoryPool *pmp, CWStringConst *pstrRel,
						 ULONG *pulRel, ULONG ulRels, BOOL fCrossProduct);

	// generate a random n-ary expression
	static CExpression *
	PexprLogicalNAryJoin(IMemoryPool *pmp);

	// generate left outer join on top of n-ary join expression
	static CExpression *
	PexprLeftOuterJoinOnNAryJoin(IMemoryPool *pmp);

	// generate n-ary join on top of left outer join expression
	static CExpression *
	PexprNAryJoinOnLeftOuterJoin(IMemoryPool *pmp);

	// generate a project expression
	static CExpression *
	PexprLogicalProject(IMemoryPool *pmp, CExpression *pexpr, CColRef *pcr,
						CColRef *pcrNew);

	// generate a random project expression
	static CExpression *
	PexprLogicalProject(IMemoryPool *pmp);

	// generate a Project over GbAgg with correlation beneath it
	static CExpression *
	PexprLogicalProjectGbAggCorrelated(IMemoryPool *pmp);

	// generate a limit expression
	static CExpression *
	PexprLogicalLimit(IMemoryPool *pmp, CExpression *pexpr, LINT iStart,
					  LINT iRows, BOOL fGlobal, BOOL fHasCount);

	// generate a random limit expression
	static CExpression *
	PexprLogicalLimit(IMemoryPool *pmp);

	// generate a random group by expression
	static CExpression *
	PexprLogicalGbAgg(IMemoryPool *pmp);

	// generate a random group by expression
	static CExpression *
	PexprLogicalGbAggWithInput(IMemoryPool *pmp, CExpression *pexprInput);

	// generate a random group by with a sum expression
	static CExpression *
	PexprLogicalGbAggWithSum(IMemoryPool *pmp);

	// generate a random group by over join expression
	static CExpression *
	PexprLogicalGbAggOverJoin(IMemoryPool *pmp);

	// generate GbAgg with correlation beneath it
	static CExpression *
	PexprLogicalGbAggCorrelated(IMemoryPool *pmp);

	// generate a dedup group by over inner join expression
	static CExpression *
	PexprLogicalGbAggDedupOverInnerJoin(IMemoryPool *pmp);

	// generate a top-level apply expression
	template <class T>
	static CExpression *
	PexprLogicalApply(IMemoryPool *pmp);

	// generate a top-level apply expression with an outer reference
	template <class T>
	static CExpression *
	PexprLogicalApplyWithOuterRef(IMemoryPool *pmp);

	// generate a const table get expression
	static CExpression *
	PexprConstTableGet(IMemoryPool *pmp, ULONG ulElements);

	// generate a const table get expression with 5 elements
	static CExpression *
	PexprConstTableGet5(IMemoryPool *pmp);

	// generate a logical insert expression
	static CExpression *
	PexprLogicalInsert(IMemoryPool *pmp);

	// generate a logical delete expression
	static CExpression *
	PexprLogicalDelete(IMemoryPool *pmp);

	// generate a logical update expression
	static CExpression *
	PexprLogicalUpdate(IMemoryPool *pmp);

	// generate a dynamic get expression
	static CExpression *
	PexprLogicalDynamicGet(IMemoryPool *pmp, CTableDescriptor *ptabdesc,
						   const CWStringConst *pstrTableAlias,
						   ULONG ulPartIndex);

	// generate a dynamic get expression
	static CExpression *
	PexprLogicalDynamicGet(IMemoryPool *pmp);

	// generate a select with an equality predicate over a dynamic get expression
	static CExpression *
	PexprLogicalSelectWithEqPredicateOverDynamicGet(IMemoryPool *pmp);

	// generate a select with a less than predicate over a dynamic get expression
	static CExpression *
	PexprLogicalSelectWithLTPredicateOverDynamicGet(IMemoryPool *pmp);

	// generate a logical table-valued function expression with 2 constant arguments
	static CExpression *
	PexprLogicalTVFTwoArgs(IMemoryPool *pmp);

	// generate a logical table-valued function expression with 3 constant arguments
	static CExpression *
	PexprLogicalTVFThreeArgs(IMemoryPool *pmp);

	// generate a logical table-valued function expression with no arguments
	static CExpression *
	PexprLogicalTVFNoArgs(IMemoryPool *pmp);

	// generate a logical table-valued function expression
	static CExpression *
	PexprLogicalTVF(IMemoryPool *pmp, ULONG ulArgs);

	// generate a cte producer on top of a select
	static CExpression *
	PexprLogicalCTEProducerOverSelect(IMemoryPool *pmp, ULONG ulCTEId);

	// generate a CTE producer on top of a select
	static CExpression *
	PexprLogicalCTEProducerOverSelect(IMemoryPool *pmp);

	// generate an expression with CTE producer and consumer
	static CExpression *
	PexprCTETree(IMemoryPool *pmp);

	// generate a logical sequence expression
	static CExpression *
	PexprLogicalSequence(IMemoryPool *pmp, DrgPexpr *pdrgpexpr);

	// generate a logical sequence expression
	static CExpression *
	PexprLogicalSequence(IMemoryPool *pmp);

	// generate a random union expression
	static CExpression *
	PexprLogicalUnion(IMemoryPool *pmp, ULONG ulDepth);

	// generate a sequence project expression
	static CExpression *
	PexprLogicalSequenceProject(IMemoryPool *pmp, OID oidFunc,
								CExpression *pexprInput);

	// generate a random expression with one window function
	static CExpression *
	PexprOneWindowFunction(IMemoryPool *pmp);

	// generate a random expression with two window functions
	static CExpression *
	PexprTwoWindowFunctions(IMemoryPool *pmp);

	// generate a dynamic array of join expressions
	static DrgPexprJoins *
	PdrgpexprJoins(IMemoryPool *pmp, CWStringConst *pstrRel, ULONG *pulRel,
				   ULONG ulRels, BOOL fCrossProduct);

	// generate a query context from an array of required column references
	static CQueryContext *
	PqcGenerate(IMemoryPool *pmp, CExpression *pexpr, DrgPcr *pdrgpcr);

	// generate a dummy query context for testing
	static CQueryContext *
	PqcGenerate(IMemoryPool *pmp, CExpression *pexpr);

	// generate a nested AND/OR tree
	static CExpression *
	PexprScalarNestedPreds(IMemoryPool *pmp, CExpression *pexpr,
						   CScalarBoolOp::EBoolOperator eboolop);

	// find the first child expression with the given operator id
	static CExpression *
	PexprFindFirstExpressionWithOpId(CExpression *pexpr,
									 COperator::EOperatorId eopid);

	// equality predicate shortcut
	static void
	EqualityPredicate(IMemoryPool *pmp, CColRefSet *pcrsLeft,
					  CColRefSet *pcrsRight, DrgPexpr *pdrgpexpr);

	// int2 point
	static CPoint *
	PpointInt2(IMemoryPool *pmp, INT i);

	// int4 point
	static CPoint *
	PpointInt4(IMemoryPool *pmp, INT i);

	// create an INT4 point with null value
	static CPoint *
	PpointInt4NullVal(IMemoryPool *pmp);

	// int8 point
	static CPoint *
	PpointInt8(IMemoryPool *pmp, INT li);

	// bool point
	static CPoint *
	PpointBool(IMemoryPool *pmp, BOOL fVal);
	...
	static GPOS_RESULT
	EresTranslate(IMemoryPool *pmp, const CHAR *szQueryFileName,
				  const CHAR *szPlanFileName, BOOL fIgnoreMismatch);

	// return query logical expression from minidump
	static CExpression *
	PexprReadQuery(IMemoryPool *pmp, const CHAR *szQueryFileName);

	// compare expected and actual output
	static GPOS_RESULT
	EresCompare(IOstream &os, CWStringDynamic *pstrActual,
				CWStringDynamic *pstrExpected, BOOL fIgnoreMismatch = true);

	// match given two plans using string comparison
	static BOOL
	FPlanMatch(IMemoryPool *pmp, IOstream &os, const CDXLNode *pdxlnFst,
			   ULLONG ullPlanIdFst, ULLONG ullPlanSpaceSizeFst,
			   const CDXLNode *pdxlnSnd, ULLONG ullPlanIdSnd,
			   ULLONG ullPlanSpaceSizeSnd);

	// compare given DXL plans
	static BOOL
	FPlanCompare(IMemoryPool *pmp, IOstream &os, const CDXLNode *pdxlnFst,
				 ULLONG ullPlanIdFst, ULLONG ullPlanSpaceSizeFst,
				 const CDXLNode *pdxlnSnd, ULLONG ullPlanIdSnd,
				 ULLONG ullPlanSpaceSizeSnd, BOOL fMatchPlans,
				 INT iCmpSpaceSize);

	// run minidumps while loading metadata from passed file
	static GPOS_RESULT
	EresRunMinidumps(IMemoryPool *pmp, const CHAR *rgszFileNames[],
					 ULONG ulTests, ULONG *pulTestCounter, ULONG ulSessionId,
	...
	static GPOS_RESULT
	EresRunMinidumpsUsingOneMDFile(IMemoryPool *pmp, const CHAR *szMDFilePath,
	...
	static GPOS_RESULT
	EresCheckOptimizedPlan(const CHAR *rgszFileNames[], ULONG ulTests,
						   ULONG *pulTestCounter, ULONG ulSessionId,
	...
	static IDatum *
	PdatumGeneric(IMemoryPool *pmp, CMDAccessor *pmda, IMDId *pmdidType,
	...
	static CConstraintInterval *
	PciGenericInterval(IMemoryPool *pmp, CMDAccessor *pmda,
	...
	static CExpression *
	PexprScalarCmpIdentToConstant(IMemoryPool *pmp, CExpression *pexpr);

	// helper for generating an exists subquery
	static CExpression *
	PexprExistsSubquery(IMemoryPool *pmp, CExpression *pexprOuter);

	// helper for generating a not exists subquery
	static CExpression *
	PexprNotExistsSubquery(IMemoryPool *pmp, CExpression *pexprOuter);

	// helper for generating an ANY subquery
	static CExpression *
	PexpSubqueryAny(IMemoryPool *pmp, CExpression *pexprOuter);

	// helper for generating an ALL subquery
	static CExpression *
	PexpSubqueryAll(IMemoryPool *pmp, CExpression *pexprOuter);
	...
	PexprFirst(const CExpression *pexpr, const COperator::EOperatorId eopid);

	// generate a scalar expression comparing scalar identifier to a constant
	static CExpression *
	PexprScIdentCmpConst(IMemoryPool *pmp, CExpression *pexprRelational,
						 IMDType::ECmpType ecmpt, ULONG ulVal);

	// generate a scalar expression comparing scalar identifiers
	static CExpression *
	PexprScIdentCmpScIdent(IMemoryPool *pmp, CExpression *pexprLeft,
						   CExpression *pexprRight, IMDType::ECmpType ecmpt);

	// generate a scalar AND expression
	static CExpression *
	PexprAnd(IMemoryPool *pmp, CExpression *pexprFst, CExpression *pexprSnd);

	// generate a scalar OR expression
	static CExpression *
	PexprOr(IMemoryPool *pmp, CExpression *pexprFst, CExpression *pexprSnd);
	...
	static GPOS_RESULT
	EresUnittest_RunTests(const CHAR **rgszFileNames, ULONG *pulTestCounter,
						  ULONG ulTests);

	static GPOS_RESULT
	EresUnittest_RunTestsWithoutAdditionalTraceFlags(const CHAR **rgszFileNames,
	...
	static ULONG
	UlSegments(COptimizerConfig *poconf);

	// create Equivalence Class based on the breakpoints
	static DrgPcrs *
	createEquivalenceClasses(IMemoryPool *pmp, CColRefSet *pcrs,
							 INT setBoundary[]);
};  // class CTestUtils
}  // namespace gpopt
