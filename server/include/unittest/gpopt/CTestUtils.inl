...
template <class T>
CExpression *
CTestUtils::PexprJoinPartitionedOuter(IMemoryPool *pmp)
{
...
template <class T>
CExpression *
CTestUtils::PexprJoinPartitionedInner(IMemoryPool *pmp)
{
...
template <class T>
CExpression *
CTestUtils::PexprJoinPartitioned(IMemoryPool *pmp, BOOL fOuterPartitioned)
{
	...
	CExpression *pexprScalar =
		CUtils::PexprScalarEqCmp(pmp, CUtils::PexprScalarIdent(pmp, pcr),
								 CUtils::PexprScalarIdent(pmp, pcrPartKey));

	if (fOuterPartitioned)
	{
		return CUtils::PexprLogicalJoin<T>(pmp, pexprGetPartitioned, pexprGet,
										   pexprScalar);
	}

	return CUtils::PexprLogicalJoin<T>(pmp, pexprGet, pexprGetPartitioned,
									   pexprScalar);
}
...
template <class T>
CExpression *
CTestUtils::PexprLogicalJoin(IMemoryPool *pmp, CExpression *pexprLeft,
							 CExpression *pexprRight)
{
	...
	return CUtils::PexprLogicalJoin<T>(pmp, pexprLeft, pexprRight,
									   pexprEquality);
}
...
template <class T>
CExpression *
CTestUtils::PexprLogicalJoin(IMemoryPool *pmp)
{
...
template <class T>
CExpression *
CTestUtils::PexprLogicalApply(IMemoryPool *pmp)
{
	...
	return GPOS_NEW(pmp)
		CExpression(pmp, pop, pexprOuter, pexprInner, pexprPredicate);
}
...
template <class T>
CExpression *
CTestUtils::PexprLogicalApplyWithOuterRef(IMemoryPool *pmp)
{
	CExpression *pexprOuter = PexprLogicalGet(pmp);
	CColRefSet *pcrsOuter =
		CDrvdPropRelational::Pdprel(pexprOuter->PdpDerive())->PcrsOutput();
	...
	CColRefSet *pcrsOuterRef =
		CDrvdPropRelational::Pdprel(pexprInner->PdpDerive())->PcrsOuter();
	...
	pdrgpcr->Append(pcr);
	COperator *pop =
		GPOS_NEW(pmp) T(pmp, pdrgpcr, COperator::EopScalarSubquery);
	return GPOS_NEW(pmp)
		CExpression(pmp, pop, pexprOuter, pexprInner, pexprPredicate);
}
