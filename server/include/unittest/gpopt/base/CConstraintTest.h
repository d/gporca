...
private:
	...
	static DrgPrng *
	Pdrgprng(IMemoryPool *pmp, IMDId *pmdid, const SRangeInfo rgRangeInfo[],
			 ULONG ulRanges);

	static CConstraintInterval *
	PciFirstInterval(IMemoryPool *pmp, IMDId *pmdid, CColRef *pcr);

	static CConstraintInterval *
	PciSecondInterval(IMemoryPool *pmp, IMDId *pmdid, CColRef *pcr);

	// interval from scalar comparison
	static GPOS_RESULT
	EresUnittest_CIntervalFromScalarCmp(IMemoryPool *pmp, CMDAccessor *pmda,
										CColRef *pcr);

	// generate comparison expression
	static CExpression *
	PexprScalarCmp(IMemoryPool *pmp, CMDAccessor *pmda, CColRef *pcr,
				   IMDType::ECmpType ecmpt, LINT lVal);

	// interval from scalar bool op
	static GPOS_RESULT
	EresUnittest_CIntervalFromScalarBoolOp(IMemoryPool *pmp, CMDAccessor *pmda,
										   CColRef *pcr);

	// debug print
	static void
	PrintConstraint(IMemoryPool *pmp, CConstraint *pcnstr);

	// build a conjunction
	static CConstraintConjunction *
	Pcstconjunction(IMemoryPool *pmp, IMDId *pmdid, CColRef *pcr);

	// build a disjunction
	static CConstraintDisjunction *
	Pcstdisjunction(IMemoryPool *pmp, IMDId *pmdid, CColRef *pcr);
...
public:
	...
	EresUnittest();
	static GPOS_RESULT
	EresUnittest_CInterval();
	static GPOS_RESULT
	EresUnittest_CIntervalFromScalarExpr();
	static GPOS_RESULT
	EresUnittest_CConjunction();
	static GPOS_RESULT
	EresUnittest_CDisjunction();
	static GPOS_RESULT
	EresUnittest_CNegation();
	static GPOS_RESULT
	EresUnittest_CConstraintFromScalarExpr();
	static GPOS_RESULT
	EresUnittest_CConstraintIntervalConvertsTo();
	static GPOS_RESULT
	EresUnittest_CConstraintIntervalPexpr();
	static GPOS_RESULT
	...
	static GPOS_RESULT
	EresUnittest_NegativeTests();
#endif  // GPOS_DEBUG

	// test constraints on date intervals
	static GPOS_RESULT
	EresUnittest_ConstraintsOnDates();

	// print equivalence classes
	static void
	PrintEquivClasses(IMemoryPool *pmp, DrgPcrs *pdrgpcrs,
