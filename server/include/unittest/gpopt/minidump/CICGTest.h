...
private:
	...
	static BOOL
	FDXLOpSatisfiesPredicate(CDXLNode *pdxl, FnDXLOpPredicate fdop);

	// check if the given dxl fragment does not contains Index Join
	static BOOL
	FIsNotIndexJoin(CDXLOperator *pdxlop);

	// check that the given dxl fragment does not contain an Index Join
	static BOOL
	FHasNoIndexJoin(CDXLNode *pdxl);
...
public:
	...
	static GPOS_RESULT
	EresUnittest_RunUnsupportedMinidumpTests();

	static GPOS_RESULT
	EresUnittest_NegativeIndexApplyTests();
