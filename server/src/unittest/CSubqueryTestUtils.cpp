...
//---------------------------------------------------------------------------
void
CSubqueryTestUtils::GenerateGetExpressions(IMemoryPool *pmp,
										   ...
										   CExpression **ppexprInner)
{
	...
	CTableDescriptor *ptabdescR =
		CTestUtils::PtabdescCreate(pmp, 3 /*ulCols*/, pmdidR, CName(&strNameR));
	*ppexprOuter = CTestUtils::PexprLogicalGet(pmp, ptabdescR, &strNameR);
	...
	CTableDescriptor *ptabdescS =
		CTestUtils::PtabdescCreate(pmp, 3 /*ulCols*/, pmdidS, CName(&strNameS));
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprJoinWithAggSubquery(IMemoryPool *pmp, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	...
	CExpression *pexprSelect =
		PexprSelectWithAggSubquery(pmp, pexprLeft, pexprInner, fCorrelated);

	(*pexprSelect)[0]->AddRef();
	(*pexprSelect)[1]->AddRef();

	CExpression *pexpr = GPOS_NEW(pmp)
		CExpression(pmp, GPOS_NEW(pmp) CLogicalInnerJoin(pmp),
					(*pexprSelect)[0], pexprRight, (*pexprSelect)[1]);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubquery(IMemoryPool *pmp,
											   ...
											   BOOL fCorrelated)
{
	...
	CExpression *pexprSubq =
		PexprSubqueryAgg(pmp, pexprOuter, pexprInner, fCorrelated);

	// generate equality predicate
	CExpression *pexprPredicate =
		CUtils::PexprScalarEqCmp(pmp, pcrLeft, pexprSubq);
	...
	BOOL fCorrelated)
{
	...
	// generate equality predicate
	CExpression *pexprPredicate =
		CUtils::PexprScalarEqCmp(pmp, pexprConst, pexprSubq);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithAggSubquery(IMemoryPool *pmp,
												CExpression *pexprOuter,
												CExpression *pexprInner,
												BOOL fCorrelated)
{
	...
	CColRef *pcrComputed =
		pcf->PcrCreate(pmdtype, popSubquery->ITypeModifier());

	// generate a scalar project list
	CExpression *pexprPrjElem =
		CUtils::PexprScalarProjectElement(pmp, pcrComputed, pexprSubq);
	CExpression *pexprPrjList = GPOS_NEW(pmp)
		CExpression(pmp, GPOS_NEW(pmp) CScalarProjectList(pmp), pexprPrjElem);

	return CUtils::PexprLogicalProject(pmp, pexprOuter, pexprPrjList,
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubqueryConstComparison(IMemoryPool *pmp,
															  BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	...
	return PexprSelectWithAggSubqueryConstComparison(pmp, pexprOuter,
												...
												BOOL fCorrelated)
{
	...
	return PexprProjectWithAggSubquery(pmp, pexprOuter, pexprInner,
									   fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAggSubqueryOverJoin(IMemoryPool *pmp,
													   BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	...
	CTableDescriptor *ptabdescT =
		CTestUtils::PtabdescCreate(pmp, 3 /*ulCols*/, pmdidT, CName(&strNameT));
	CExpression *pexprT =
		CTestUtils::PexprLogicalGet(pmp, ptabdescT, &strNameT);
	CColRef *pcrInner = CDrvdPropRelational::Pdprel(pexprR->PdpDerive())
							->PcrsOutput()
							->PcrAny();
	CColRef *pcrOuter = CDrvdPropRelational::Pdprel(pexprT->PdpDerive())
							->PcrsOutput()
							->PcrAny();
	...
	CExpression *pexprSubq = GPOS_NEW(pmp) CExpression(
		pmp,
		GPOS_NEW(pmp)
			CScalarSubquery(pmp, pcrInner, false /*fGeneratedByExist*/,
							false /*fGeneratedByQuantified*/),
		pexprJoin);

	CExpression *pexprPredOuter =
		CUtils::PexprScalarEqCmp(pmp, pcrOuter, pexprSubq);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAnySubquery(IMemoryPool *pmp,
											   BOOL fCorrelated)
{
	return PexprSelectWithSubqueryQuantified(
		pmp, COperator::EopScalarSubqueryAny, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAnySubqueryOverWindow(IMemoryPool *pmp,
														 BOOL fCorrelated)
{
...
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryQuantifiedOverWindow(
	IMemoryPool *pmp, COperator::EOperatorId eopid, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	...
	CExpression *pexprSubqueryQuantified = PexprSubqueryQuantified(
		pmp, eopid, pexprOuter, pexprInner, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAllSubquery(IMemoryPool *pmp,
											   BOOL fCorrelated)
{
	return PexprSelectWithSubqueryQuantified(
		pmp, COperator::EopScalarSubqueryAll, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAllSubqueryOverWindow(IMemoryPool *pmp,
														 BOOL fCorrelated)
{
	return PexprSelectWithSubqueryQuantifiedOverWindow(
		pmp, COperator::EopScalarSubqueryAll, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAnyAggSubquery(IMemoryPool *pmp,
												  BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithAllAggSubquery(IMemoryPool *pmp,
												  BOOL fCorrelated)
{
	return PexprSelectWithQuantifiedAggSubquery(
		pmp, COperator::EopScalarSubqueryAll, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithAnySubquery(IMemoryPool *pmp,
												BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
	...
	return PexprProjectWithSubqueryQuantified(pmp, pexprOuter, pexprInner,
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithAllSubquery(IMemoryPool *pmp,
												BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueriesInDifferentContexts(IMemoryPool *pmp,
													   BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
	GenerateGetExpressions(pmp, &pexprOuter, &pexprInner);
	CExpression *pexprSelect =
		PexprSelectWithAggSubquery(pmp, pexprOuter, pexprInner, fCorrelated);

	CExpression *pexprGet = CTestUtils::PexprLogicalGet(pmp);
	return PexprProjectWithSubqueryQuantified(pmp, pexprSelect, pexprGet,
											  COperator::EopScalarSubqueryAny,
											  fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueriesInNullTestContext(IMemoryPool *pmp,
													 BOOL fCorrelated)
{
	CExpression *pexprOuter = NULL;
	CExpression *pexprInner = NULL;
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithExistsSubquery(IMemoryPool *pmp,
												  BOOL fCorrelated)
{
	return PexprSelectWithSubqueryExistential(
		pmp, COperator::EopScalarSubqueryExists, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNotExistsSubquery(IMemoryPool *pmp,
													 BOOL fCorrelated)
{
	return PexprSelectWithSubqueryExistential(
		pmp, COperator::EopScalarSubqueryNotExists, fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryDisjuncts(IMemoryPool *pmp,
													 BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	...
	return PexprSelectWithSubqueryBoolOp(pmp, pexprOuter, pexprInner,
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithTrimmableExists(IMemoryPool *pmp,
												   BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithTrimmableNotExists(IMemoryPool *pmp,
													  BOOL fCorrelated)
{
	return PexprSelectWithTrimmableExistentialSubquery(
		pmp, COperator::EopScalarSubqueryNotExists, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithExistsSubquery(IMemoryPool *pmp,
												   BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithNotExistsSubquery(IMemoryPool *pmp,
													  BOOL fCorrelated)
{
	return PexprProjectWithSubqueryExistential(
		pmp, COperator::EopScalarSubqueryNotExists, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedCmpSubquery(IMemoryPool *pmp,
													 BOOL fCorrelated)
{
	CExpression *pexprSelectWithSubquery =
		PexprSelectWithAggSubquery(pmp, fCorrelated);
	...
	CExpression *pexprPredicate1 = CUtils::PexprScalarEqCmp(
		pmp, CUtils::PexprScalarConstBool(pmp, true /*fVal*/),
		pexprSubqueryPred);

	// add another nesting level
	CExpression *pexprPredicate = CUtils::PexprScalarEqCmp(
		pmp, CUtils::PexprScalarConstBool(pmp, true /*fVal*/), pexprPredicate1);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithCmpSubqueries(IMemoryPool *pmp,
												 BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	CExpression *pexprOuter = NULL;
	...
	CExpression *pexprScalarSubquery2 =
		PexprSubqueryAgg(pmp, pexprOuter, pexprT, fCorrelated);

	// generate equality predicate between both subqueries
	CExpression *pexprPredicate = CUtils::PexprScalarEqCmp(
		pmp, pexprScalarSubquery1, pexprScalarSubquery2);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedSubquery(IMemoryPool *pmp,
												  BOOL fCorrelated)
{
	CExpression *pexprInner = PexprSelectWithAggSubquery(pmp, fCorrelated);
	CColRef *pcrInner = CDrvdPropRelational::Pdprel(pexprInner->PdpDerive())
							->PcrsOutput()
							->PcrAny();

	CExpression *pexprOuter = CTestUtils::PexprLogicalGet(pmp);
	CColRef *pcrOuter = CDrvdPropRelational::Pdprel(pexprOuter->PdpDerive())
							->PcrsOutput()
							->PcrAny();
	...
	IMemoryPool *pmp, COperator::EOperatorId eopid, BOOL fCorrelated)
{
	...
	CExpression *pexprOuter1 = CTestUtils::PexprLogicalGetNullable(
		pmp, GPOPT_TEST_REL_OID1, &strName1, &strAlias1);
	...
	CExpression *pexprOuter2 = CTestUtils::PexprLogicalGetNullable(
		pmp, GPOPT_TEST_REL_OID2, &strName2, &strAlias2);
	...
	CExpression *pexprOuter3 = CTestUtils::PexprLogicalGetNullable(
		pmp, GPOPT_TEST_REL_OID3, &strName3, &strAlias3);
	...
	CExpression *pexprInner = CTestUtils::PexprLogicalGetNullable(
		pmp, GPOPT_TEST_REL_OID4, &strName4, &strAlias4);

	CExpression *pexprSubqueryQuantified1 = PexprSubqueryQuantified(
		pmp, eopid, pexprOuter3, pexprInner, fCorrelated);
	CExpression *pexprSelect1 =
		CUtils::PexprLogicalSelect(pmp, pexprOuter3, pexprSubqueryQuantified1);
	CExpression *pexprSubqueryQuantified2 = PexprSubqueryQuantified(
		pmp, eopid, pexprOuter2, pexprSelect1, fCorrelated);
	CExpression *pexprSelect2 =
		CUtils::PexprLogicalSelect(pmp, pexprOuter2, pexprSubqueryQuantified2);
	CExpression *pexprSubqueryQuantified3 = PexprSubqueryQuantified(
		pmp, eopid, pexprOuter1, pexprSelect2, fCorrelated);

	return CUtils::PexprLogicalSelect(pmp, pexprOuter1,
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedAnySubqueries(IMemoryPool *pmp,
													   BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithNestedAllSubqueries(IMemoryPool *pmp,
													   BOOL fCorrelated)
{
	return PexprSelectWithNestedQuantifiedSubqueries(
		pmp, COperator::EopScalarSubqueryAll, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWith2LevelsCorrSubquery(IMemoryPool *pmp,
													   BOOL fCorrelated)
{
	...
	if (fCorrelated)
	{
		...
		CColRef *pcrOuter = CDrvdPropRelational::Pdprel(pexpr->PdpDerive())
								->PcrsOutput()
								->PcrAny();
		CColRef *pcrInner =
			CDrvdPropRelational::Pdprel(pexprInnerSelect->PdpDerive())
				->PcrsOutput()
				->PcrAny();
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSelectWithSubqueryConjuncts(IMemoryPool *pmp,
													 BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	...
	return PexprSelectWithSubqueryBoolOp(
		pmp, pexprOuter, pexprInner, fCorrelated, CScalarBoolOp::EboolopAnd);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprProjectWithSubqueries(IMemoryPool *pmp,
											   BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
...
)
{
	...
	if (fCorrelated)
	{
		...
		pexprPred = CUtils::PexprScalarBoolOp(pmp, CScalarBoolOp::EboolopAnd,
											  pdrgpexpr);
	}
	else
	{
...
CExpression *
CSubqueryTestUtils::PexprSubqueryQuantified(
	IMemoryPool *pmp, COperator::EOperatorId eopid, CExpression *pexprOuter,
	CExpression *pexprInner,
	BOOL
		fCorrelated  // add a predicate to inner expression correlated with outer expression?
)
{
	...
	CExpression *pexprSelect =
		PexprSubquery(pmp, pexprOuter, pexprInner, fCorrelated);
	...
	if (COperator::EopScalarSubqueryAny == eopid)
	{
		...
		return GPOS_NEW(pmp) CExpression(
			...
			pexprSelect, CUtils::PexprScalarIdent(pmp, pcrOuter));
	}
	...
	return GPOS_NEW(pmp) CExpression(
		...
		GPOS_NEW(pmp) CScalarSubqueryAll(
			pmp, GPOS_NEW(pmp) CMDIdGPDB(GPDB_INT4_NEQ_OP), pstr, pcrInner),
		pexprSelect, CUtils::PexprScalarIdent(pmp, pcrOuter));
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableSubquery(IMemoryPool *pmp,
												  ...
												  BOOL fCorrelated)
{
	...
	CWStringConst strNameR(GPOS_WSZ_LIT("Rel1"));
	CMDIdGPDB *pmdidR = GPOS_NEW(pmp) CMDIdGPDB(
		GPOPT_TEST_REL_OID1, 1 /*ulVersionMajor*/, 1 /*ulVersionMinor*/);
	CTableDescriptor *ptabdescR =
		CTestUtils::PtabdescPlain(pmp, 3 /*ulCols*/, pmdidR, CName(&strNameR));
	CExpression *pexprOuter =
		CTestUtils::PexprLogicalGet(pmp, ptabdescR, &strNameR);
	...
	switch (eopid)
	{
		...
		case COperator::EopScalarSubqueryAll:
			pexprSubquery = PexprSubqueryQuantified(pmp, eopid, pexprOuter,
													pexprInner, fCorrelated);
			break;

		case COperator::EopScalarSubqueryExists:
		case COperator::EopScalarSubqueryNotExists:
			pexprSubquery = PexprSubqueryExistential(pmp, eopid, pexprOuter,
													 pexprInner, fCorrelated);
			break;
	...
	CExpression *pexprPred = CUtils::PexprScalarEqCmp(
		pmp, pcrOuter, CUtils::PexprScalarConstInt4(pmp, 5 /*iVal*/));
	...
	return CUtils::PexprLogicalSelect(
		pmp, pexprOuter, CPredicateUtils::PexprDisjunction(pmp, pdrgpexpr));
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableAnySubquery(IMemoryPool *pmp,
													 BOOL fCorrelated)
{
	return PexprUndecorrelatableSubquery(pmp, COperator::EopScalarSubqueryAny,
										 fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableAllSubquery(IMemoryPool *pmp,
													 BOOL fCorrelated)
{
	return PexprUndecorrelatableSubquery(pmp, COperator::EopScalarSubqueryAll,
										 fCorrelated);
}
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableExistsSubquery(IMemoryPool *pmp,
														BOOL fCorrelated)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableNotExistsSubquery(IMemoryPool *pmp,
														   BOOL fCorrelated)
{
	return PexprUndecorrelatableSubquery(
		pmp, COperator::EopScalarSubqueryNotExists, fCorrelated);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprUndecorrelatableScalarSubquery(IMemoryPool *pmp,
														BOOL fCorrelated)
{
	CExpression *pexprOuter = CTestUtils::PexprLogicalGet(pmp);
	CExpression *pexprInner = CTestUtils::PexprLogicalGet(pmp);
	...
	CColRef *pcrOuter = CDrvdPropRelational::Pdprel(pexprOuter->PdpDerive())
							...
							->PcrAny();
	CExpression *pexprPred =
		CUtils::PexprScalarEqCmp(pmp, pcrOuter, pexprSubquery);
...
)
{
	...
	{
		return GPOS_NEW(pmp) CExpression(
			pmp, GPOS_NEW(pmp) CScalarSubqueryExists(pmp), pexprSelect);
	}

	return GPOS_NEW(pmp) CExpression(
		pmp, GPOS_NEW(pmp) CScalarSubqueryNotExists(pmp), pexprSelect);
}
...
)
{
	...
	CExpression *pexprLogicalGbAgg = CUtils::PexprLogicalGbAggGlobal(
		pmp, pdrgpcr, pexprSelect, pexprProjList);

	// return a subquery expression on top of group by
	return GPOS_NEW(pmp) CExpression(
		pmp,
		GPOS_NEW(pmp)
			CScalarSubquery(pmp, pcrComputed, false /*fGeneratedByExist*/,
							false /*fGeneratedByQuantified*/),
		pexprLogicalGbAgg);
}
	...
	BOOL fCorrelated, CScalarBoolOp::EBoolOperator eboolop)
{
	...
	CExpression *pexprPred1 =
		CUtils::PexprScalarEqCmp(pmp, pcrLeft, pexprAggSubquery);

	// generate a regular predicate
	CExpression *pexprPred2 = CUtils::PexprScalarEqCmp(
		pmp, pcrLeft, CUtils::PexprScalarConstInt4(pmp, 5 /*iVal*/));

	// generate ALL subquery
	CExpression *pexprGet = CTestUtils::PexprLogicalGet(pmp);
	CExpression *pexprSubqueryAll =
		PexprSubqueryQuantified(pmp, COperator::EopScalarSubqueryAll,
	...
	CExpression *pexprSubqueryExists =
		PexprSubqueryExistential(pmp, COperator::EopScalarSubqueryExists,
											   ...
											   BOOL fCorrelated)
{
	...
	pexprPrjElem =
		CUtils::PexprScalarProjectElement(pmp, pcrComputed, pexprAggSubquery);
	...
	pexprPrjElem =
		CUtils::PexprScalarProjectElement(pmp, pcrComputed, pexprSubqueryAll);
	pdrgpexpr->Append(pexprPrjElem);
	...
	CExpression *pexprSubqueryExists =
		PexprSubqueryExistential(pmp, COperator::EopScalarSubqueryExists,
								 pexprOuter, pexprGet, fCorrelated);
	pcrComputed = pcf->PcrCreate(pmdtypebool, IDefaultTypeModifier);
	pexprPrjElem = CUtils::PexprScalarProjectElement(pmp, pcrComputed,
													 pexprSubqueryExists);
	pdrgpexpr->Append(pexprPrjElem);
	...
	IMemoryPool *pmp, COperator::EOperatorId eopid, BOOL fCorrelated)
{
	...
	CExpression *pexprSubq = PexprSubqueryAgg(
		pmp, pexprOuter, CTestUtils::PexprLogicalGet(pmp), fCorrelated);
	CExpression *pexprGb = (*pexprSubq)[0];
	pexprGb->AddRef();
	pexprSubq->Release();

	CColRef *pcrInner = CDrvdPropRelational::Pdprel(pexprGb->PdpDerive())
							->PcrsOutput()
							->PcrAny();
	CColRef *pcrOuter = CDrvdPropRelational::Pdprel(pexprOuter->PdpDerive())
							->PcrsOutput()
	...
	if (COperator::EopScalarSubqueryAny == eopid)
	{
		...
		pexprSubqueryQuantified = GPOS_NEW(pmp) CExpression(
			...
			GPOS_NEW(pmp) CScalarSubqueryAny(
				pmp, GPOS_NEW(pmp) CMDIdGPDB(GPDB_INT4_EQ_OP), pstr, pcrInner),
			pexprGb, CUtils::PexprScalarIdent(pmp, pcrOuter));
	}
...
CSubqueryTestUtils::PexprProjectWithSubqueryQuantified(
	IMemoryPool *pmp, CExpression *pexprOuter, CExpression *pexprInner,
	COperator::EOperatorId eopid, BOOL fCorrelated)
{
	GPOS_ASSERT(NULL != pmp);
	GPOS_ASSERT(COperator::EopScalarSubqueryAny == eopid ||
				COperator::EopScalarSubqueryAll == eopid);
	...
	CExpression *pexprPrjElem = CUtils::PexprScalarProjectElement(
		pmp, pcrComputed, pexprSubqueryQuantified);
	CExpression *pexprPrjList = GPOS_NEW(pmp)
		CExpression(pmp, GPOS_NEW(pmp) CScalarProjectList(pmp), pexprPrjElem);
	...
	IMemoryPool *pmp, COperator::EOperatorId eopid, BOOL fCorrelated)
{
	...
	return CUtils::PexprLogicalSelect(pmp, pexprOuter,
									  pexprSubqueryExistential);
}
...
CExpression *
CSubqueryTestUtils::PexprSelectWithTrimmableExistentialSubquery(
	IMemoryPool *pmp, COperator::EOperatorId eopid,
	BOOL  // fCorrelated
)
{
	GPOS_ASSERT(NULL != pmp);
	...
	CExpression *pexprGbAgg = CUtils::PexprLogicalGbAggGlobal(
		pmp, GPOS_NEW(pmp) DrgPcr(pmp), (*pexprInner)[0], (*pexprInner)[1]);
	pexprInner->Release();
	...
	if (COperator::EopScalarSubqueryExists == eopid)
	{
		pexprSubqueryExistential = GPOS_NEW(pmp) CExpression(
			pmp, GPOS_NEW(pmp) CScalarSubqueryExists(pmp), pexprGbAgg);
	}
	else
	{
		pexprSubqueryExistential = GPOS_NEW(pmp) CExpression(
			pmp, GPOS_NEW(pmp) CScalarSubqueryNotExists(pmp), pexprGbAgg);
	}
	...
	CExpression *pexprEqPred = CUtils::PexprScalarEqCmp(
		pmp, pcrs->PcrAny(), CUtils::PexprScalarConstInt4(pmp, 5 /*iVal*/));

	CExpression *pexprConjunction = CPredicateUtils::PexprConjunction(
		pmp, pexprSubqueryExistential, pexprEqPred);
	pexprEqPred->Release();
	...
	IMemoryPool *pmp, COperator::EOperatorId eopid, BOOL fCorrelated)
{
	...
	// generate a scalar project list
	CExpression *pexprPrjElem = CUtils::PexprScalarProjectElement(
		pmp, pcrComputed, pexprSubqueryExistential);
	CExpression *pexprPrjList = GPOS_NEW(pmp)
		CExpression(pmp, GPOS_NEW(pmp) CScalarProjectList(pmp), pexprPrjElem);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueryWithConstTableGet(IMemoryPool *pmp,
												   COperator::EOperatorId eopid)
{
	GPOS_ASSERT(NULL != pmp);
	GPOS_ASSERT(COperator::EopScalarSubqueryAny == eopid ||
	...
	CExpression *pexprConstTableGet =
		CTestUtils::PexprConstTableGet(pmp, 3 /* ulElements */);
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryTestUtils::PexprSubqueryWithDisjunction(IMemoryPool *pmp)
{
	GPOS_ASSERT(NULL != pmp);
