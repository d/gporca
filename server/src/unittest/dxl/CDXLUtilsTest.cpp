...
CDXLUtilsTest::EresUnittest_SerializeQuery()
{
	...
	CQueryToDXLResult *presult =
		CDXLUtils::PdxlnParseDXLQuery(pmp, szDXL, NULL /*szXSDPath*/);
		 ...
		 ulHeaders++)
	{
			 ...
			 ulIndent++)
		{
			...
			CDXLUtils::SerializeQuery(
				pmp, oss, presult->Pdxln(), presult->PdrgpdxlnOutputCols(),
				presult->PdrgpdxlnCTE(), rgfHeaders[ulHeaders],
...
CDXLUtilsTest::EresUnittest_SerializePlan()
{
	...
	CDXLNode *pdxln = CDXLUtils::PdxlnParsePlan(pmp, szDXL, NULL /*szXSDPath*/,
		 ...
		 ulHeaders++)
	{
			 ...
			 ulIndent++)
		{
			oss << "Headers: " << rgfHeaders[ulHeaders]
				<< ", indentation: " << rgfIndentation[ulIndent] << std::endl;
			CDXLUtils::SerializePlan(pmp, oss, pdxln, ullPlanId,
									 ullPlanSpaceSize, rgfHeaders[ulHeaders],
									 rgfIndentation[ulIndent]);
			oss << std::endl;
		}
	}
...
CDXLUtilsTest::EresUnittest_Encoding()
{
	...
	const CHAR *sz =
		"{\"{FUNCEXPR :funcid 1967 :funcresulttype 1184 :funcretset false "
		":funcformat 1 :args ({FUNCEXPR :funcid 1191 :funcresulttype 1184 "
		":funcretset false :funcformat 2 :args ({CONST :consttype 25 :constlen "
		"-1 :constbyval false :constisnull false :constvalue 7 [ 0 0 0 7 110 "
		"111 119 ]})} {CONST :consttype 23 :constlen 4 :constbyval true "
	...
	a_pxmlbyteEncoded =
		Base64::encode(pxmlbyte, (XMLSize_t) ulLen, &outputLength, a_pmm.Pt());
	...
	a_pxmlbyteDecoded =
		Base64::decode(a_pxmlbyteEncoded.Rgt(), &outputLength, a_pmm.Pt());
	...
	ULONG ulOutputLen = 0;
	BYTE *pba =
		CDXLUtils::PbaFromBase64XMLStr(a_pmm.Pt(), pxmlch, &ulOutputLen);
	CHAR *szPba = (CHAR *) pba;
