...
CBucket *
CCardinalityTestUtils::PbucketIntegerClosedLowerBound(IMemoryPool *pmp,
													  ...
													  CDouble dDistinct)
{
	...
	return GPOS_NEW(pmp) CBucket(ppLower, ppUpper, true /* fLowerClosed */,
								 fUpperClosed, dFrequency, dDistinct);
}

// create an integer bucket with the provider upper/lower bound, frequency and NDV information
CBucket *
CCardinalityTestUtils::PbucketInteger(IMemoryPool *pmp, INT iLower, INT iUpper,
									  BOOL fLowerClosed, BOOL fUpperClosed,
									  CDouble dFrequency, CDouble dDistinct)
{
	CPoint *ppLower = CTestUtils::PpointInt4(pmp, iLower);
	CPoint *ppUpper = CTestUtils::PpointInt4(pmp, iUpper);

	return GPOS_NEW(pmp) CBucket(ppLower, ppUpper, fLowerClosed, fUpperClosed,
								 dFrequency, dDistinct);
}

// create a singleton bucket containing a boolean value
CBucket *
CCardinalityTestUtils::PbucketSingletonBoolVal(IMemoryPool *pmp, BOOL fValue,
											   CDouble dFrequency)
{
	...
	ppLower->AddRef();
	return GPOS_NEW(pmp) CBucket(ppLower, ppLower, true /* fClosedUpper */,
								 true /* fClosedUpper */, dFrequency, 1.0);
}

// helper function to generate integer histogram based on the NDV and bucket information provided
CHistogram *
CCardinalityTestUtils::PhistInt4Remain(IMemoryPool *pmp, ULONG ulBuckets,
									   ...
									   CDouble dNDVRemain)
{
	...
	for (ULONG ulIdx = 0; ulIdx < ulBuckets; ulIdx++)
	{
		...
		CBucket *pbucket = PbucketIntegerClosedLowerBound(
	...
	return GPOS_NEW(pmp)
		CHistogram(pdrgppbucket, true, dNullFreq, dNDVRemain, dFreqRemain);
}

// helper function to generate an example int histogram
CHistogram *
CCardinalityTestUtils::PhistExampleInt4(IMemoryPool *pmp)
{
	...
	pdrgppbucket->Append(CCardinalityTestUtils::PbucketIntegerClosedLowerBound(
		pmp, 100, 100, 0.1, 1.0));
...
CHistogram *
CCardinalityTestUtils::PhistExampleBool(IMemoryPool *pmp)
{
	DrgPbucket *pdrgppbucket = GPOS_NEW(pmp) DrgPbucket(pmp);
	CBucket *pbucketFalse =
		CCardinalityTestUtils::PbucketSingletonBoolVal(pmp, false, 0.1);
	CBucket *pbucketTrue =
		CCardinalityTestUtils::PbucketSingletonBoolVal(pmp, true, 0.2);
...
CPoint *
CCardinalityTestUtils::PpointGeneric(IMemoryPool *pmp, OID oid,
									 ...
									 LINT lValue)
{
	...
	IDatum *pdatum =
		CTestUtils::PdatumGeneric(pmp, pmda, pmdid, pstrEncodedValue, lValue);
...
CPoint *
CCardinalityTestUtils::PpointNumeric(IMemoryPool *pmp,
									 CWStringDynamic *pstrEncodedValue,
									 CDouble dValue)
{
	CMDAccessor *pmda = COptCtxt::PoctxtFromTLS()->Pmda();
	...
	CDXLDatumStatsDoubleMappable *pdxldatum = GPOS_NEW(pmp)
		CDXLDatumStatsDoubleMappable(pmp, pmdid, IDefaultTypeModifier,
...
void
CCardinalityTestUtils::PrintBucket(IMemoryPool *pmp, const char *pcPrefix,
...
void
CCardinalityTestUtils::PrintHist(IMemoryPool *pmp, const char *pcPrefix,
...
void
CCardinalityTestUtils::PrintStats(IMemoryPool *pmp, const CStatistics *pstats)
{
	CWStringDynamic str(pmp);
	COstreamString oss(&str);
