...
CHistogramTest::EresUnittest_CHistogramInt4()
{
	...
	CHistogram *phist0 =
		phist->PhistFilter(pmp, CStatsPred::EstatscmptG, ppoint0);
	...
	CHistogram *phist1 =
		phist->PhistFilter(pmp, CStatsPred::EstatscmptL, ppoint1);
	...
	CHistogram *phist2 =
		phist->PhistFilter(pmp, CStatsPred::EstatscmptL, ppoint2);
	...
	CHistogram *phist3 =
		phist->PhistFilter(pmp, CStatsPred::EstatscmptEq, ppoint3);
	...
	CHistogram *phist4 = phist->PhistFilterNormalized(
		pmp, CStatsPred::EstatscmptEq, ppoint4, &dScaleFactor);
	CCardinalityTestUtils::PrintHist(pmp, "phist4", phist4);
	GPOS_RTL_ASSERT(phist4->FValid());

	// lasj
	CHistogram *phist5 =
		phist->PhistLASJ(pmp, CStatsPred::EstatscmptEq, phist2);
	CCardinalityTestUtils::PrintHist(pmp, "phist5", phist5);
	GPOS_RTL_ASSERT(phist5->UlBuckets() == 5);

	// inequality check
	CHistogram *phist6 =
		phist->PhistFilter(pmp, CStatsPred::EstatscmptNEq, ppoint2);
	...
	CHistogram *phist8 =
		phist7->PhistFilter(pmp, CStatsPred::EstatscmptEq, ppoint3);
	GPOS_RTL_ASSERT(fabs((phist8->DFrequency() - 0.2).DVal()) <
					CStatistics::DEpsilon);
	GPOS_RTL_ASSERT(fabs((phist8->DDistinct() - 1.0).DVal()) <
					CStatistics::DEpsilon);

	// greater than, hitting remaining tuples
	CHistogram *phist9 =
		phist7->PhistFilter(pmp, CStatsPred::EstatscmptG, ppoint1);
	CCardinalityTestUtils::PrintHist(pmp, "phist9", phist9);
	GPOS_RTL_ASSERT(fabs((phist9->DFrequency() - 0.26).DVal()) <
					CStatistics::DEpsilon);
	GPOS_RTL_ASSERT(fabs((phist9->DDistinct() - 1.8).DVal()) <
					CStatistics::DEpsilon);

	// equality join, hitting remaining tuples
	CHistogram *phist10 =
		phist7->PhistJoin(pmp, CStatsPred::EstatscmptEq, phist7);
	GPOS_RTL_ASSERT(phist10->UlBuckets() == 5);
	GPOS_RTL_ASSERT(fabs((phist10->DDistinctRemain() - 2.0).DVal()) <
					CStatistics::DEpsilon);
	GPOS_RTL_ASSERT(fabs((phist10->DFreqRemain() - 0.08).DVal()) <
					CStatistics::DEpsilon);
...
CHistogramTest::EresUnittest_CHistogramBool()
{
	...
	CBucket *pbucketFalse =
		CCardinalityTestUtils::PbucketSingletonBoolVal(pmp, false, 0.1);
	CBucket *pbucketTrue =
		CCardinalityTestUtils::PbucketSingletonBoolVal(pmp, false, 0.9);
	...
	CDouble dScaleFactor(0.0);
	CHistogram *phist1 = phist->PhistFilterNormalized(
		pmp, CStatsPred::EstatscmptEq, ppoint1, &dScaleFactor);
	CCardinalityTestUtils::PrintHist(pmp, "phist1", phist1);
...
CHistogramTest::EresUnittest_CHistogramValid()
{
	...
	CBucket *pbucket1 = CCardinalityTestUtils::PbucketIntegerClosedLowerBound(
		pmp, 0, 10, 0.1, 2.0);
	pdrgppbucket->Append(pbucket1);
	CBucket *pbucket2 = CCardinalityTestUtils::PbucketIntegerClosedLowerBound(
		pmp, 9, 20, 0.1, 2.0);
...
CHistogram *
CHistogramTest::PhistExampleInt4Remain(IMemoryPool *pmp)
{
	...
	return GPOS_NEW(pmp)
		CHistogram(pdrgppbucket, true, 0.1 /*dNullFreq*/,
				   2.0 /*dDistinctRemain*/, 0.4 /*dFreqRemain*/);
}
