...
public:
		  ...
		  m_pdrgpstat(GPOS_NEW(m_amp.Pmp()) DrgPstat(m_amp.Pmp()))
	{
		...
		DrgPstats *pdrgpstats = CDXLUtils::PdrgpstatsTranslateStats(
			Pmp(), &m_mda, pdrgpdxlstatsderrel);
...
const CHAR *szFileName =
	"../data/dxl/statistics/"
	"Join-Statistics-NDVBasedCardEstimation-EqPred-Input.xml";
...
{
	CDouble dRowsExpected(
		10000);  // the minimum cardinality is min(NDV a, NDV b)
	...
	CExpression *pexprScalarIdentRight =
	...
	CExpression *pexprScConst =
		CUtils::PexprScalarConstInt4(pmp, 10 /* iVal */);
	...
	CExpression *pScalarCmp =
		CUtils::PexprScalarEqCmp(pmp, pexprScOp, pexprScalarIdentRight);

	IStatistics *pstatsJoin = CJoinStatsProcessor::PstatsJoinArray(
		pmp, pdrgpstat, pScalarCmp, IStatistics::EsjtInnerJoin);
...
GPOS_RESULT
CJoinCardinalityNDVBasedEqPredTest::
	EresUnittest_NDVCardEstimationNotApplicableMultipleIdents()
{
	...
	CExpression *pexprScalarIdentRight =
		CUtils::PexprScalarIdent(pmp, pcrRight);
	CExpression *pexprScalarIdentLeft2 =
		CUtils::PexprScalarIdent(pmp, pcrLeft2);
	...
	CExpression *pexprScOp = CUtils::PexprScalarOp(
		pmp, pcrLeft1, pexprScalarIdentLeft2, CWStringConst(GPOS_WSZ_LIT("+")),
		GPOS_NEW(pmp) CMDIdGPDB(GPDB_INT4_ADD_OP));
...
GPOS_RESULT
CJoinCardinalityNDVBasedEqPredTest::
	EresUnittest_NDVCardEstimationNotApplicableInequalityRange()
{
	// cartesian product / 2.5
	// 2.5 = 1/.4 -- where .4 is default selectivity
	CDouble dRowsExpected(76004000);
	...
	//	|  +--CScalarConst (10)
	//	+--CScalarIdent "column_0000" (0)
	CExpression *pScalarCmp = CUtils::PexprScalarCmp(
		pmp, pexprScOp, pexprScalarIdentRight, IMDType::EcmptLEq);
