...
CStatisticsTest::EresUnittest()
{
	...
	GPOS_RESULT eres = GPOS_FAILED;
	{
		...
		pmdp->AddRef();
		CMDAccessor mda(pmp, CMDCache::Pcache(), CTestUtils::m_sysidDefault,
						pmdp);
...
CStatisticsTest::EresUnittest_UnionAll()
{
	...
	for (ULONG ul = 0; ul < ulTestCases; ul++)
	{
		...
		CStatistics *pstatsOutput = CUnionAllStatsProcessor::PstatsUnionAll(
			pmp, pstats1, pstats2, pdrgpulColIdOutput, pdrgpulColIdInput1,
			pdrgpulColIdInput2);
...
CStatisticsTest::EresUnittest_GbAggWithRepeatedGbCols()
{
	...
	CExpression *pexprPrj = CUtils::PexprAddProjection(
		pmp, pexpr, CUtils::PexprScalarIdent(pmp, pcrs->PcrFirst()));
	CColRef *pcrComputed =
	...
	BOOL fRows1EqualRows2 =
		(pexprGbAgg1->Pstats()->DRows() == pexprGbAgg2->Pstats()->DRows());
	BOOL fRows2EqualRows3 =
		(pexprGbAgg2->Pstats()->DRows() == pexprGbAgg3->Pstats()->DRows());
...
CHistogram *
CStatisticsTest::PhistExampleInt4Dim(IMemoryPool *pmp)
{
	...
	for (ULONG ulIdx = 0; ulIdx < 9; ulIdx++)
	{
		...
		CBucket *pbucket =
			CCardinalityTestUtils::PbucketIntegerClosedLowerBound(
				pmp, iLower, iUpper, dFrequency, dDistinct);
...
CTableDescriptor *
CStatisticsTest::PtabdescTwoColumnSource(IMemoryPool *pmp,
										 ...
										 const CWStringConst &strColB)
{
	CTableDescriptor *ptabdesc = GPOS_NEW(pmp) CTableDescriptor(
		pmp, GPOS_NEW(pmp) CMDIdGPDB(GPOPT_TEST_REL_OID1, 1, 1), nameTable,
	...
	for (ULONG ul = 0; ul < 2; ul++)
	{
		...
		CColumnDescriptor *pcoldesc = GPOS_NEW(pmp)
			CColumnDescriptor(pmp, pmdtype, IDefaultTypeModifier, nameColumn,
							  ul + 1, false /*fNullable*/
			);
...
CStatisticsTest::EresUnittest_CStatisticsBasic()
{
	...
	CTableDescriptor *ptabdesc = PtabdescTwoColumnSource(
		pmp, CName(&strRelAlias), pmdtypeint4, strColA, strColB);
	CExpression *pexprGet =
		CTestUtils::PexprLogicalGet(pmp, ptabdesc, &strRelAlias);

	if (NULL == pcf->PcrLookup(1 /*ulId*/))
	{
		// create column references for grouping columns
		(void) pcf->PcrCreate(pmdtypeint4, IDefaultTypeModifier, 0 /* iAttno */,
							  false /*FNullable*/, 1 /* ulId */,
							  CName(&strColA), pexprGet->Pop()->UlOpId());
	}

	if (NULL == pcf->PcrLookup(2 /*ulId*/))
	{
		(void) pcf->PcrCreate(pmdtypeint4, IDefaultTypeModifier, 1 /* iAttno */,
							  false /*FNullable*/, 2 /* ulId */,
							  CName(&strColB), pexprGet->Pop()->UlOpId());
	}
	...
	phmulhist->FInsert(GPOS_NEW(pmp) ULONG(1),
					   CCardinalityTestUtils::PhistExampleBool(pmp));

	// generate int histogram for column 2
	phmulhist->FInsert(GPOS_NEW(pmp) ULONG(2),
	...
	phmuldoubleWidth->FInsert(GPOS_NEW(pmp) ULONG(1),
							  GPOS_NEW(pmp) CDouble(1.0));

	// width for int
	phmuldoubleWidth->FInsert(GPOS_NEW(pmp) ULONG(2),
							  GPOS_NEW(pmp) CDouble(4.0));

	CStatistics *pstats =
		GPOS_NEW(pmp) CStatistics(pmp, phmulhist, phmuldoubleWidth,
								  1000.0 /* dRows */, false /* fEmpty */);
	pstats->DRows();
	...
	CStatistics *pstats1 = CFilterStatsProcessor::PstatsFilter(
		pmp, pstats, pstatspred, true /* fCapNdvs */);
	pstats1->DRows();
	...
	phmuldoubleWidth2->FInsert(GPOS_NEW(pmp) ULONG(10),
							   GPOS_NEW(pmp) CDouble(4.0));

	CStatistics *pstats2 =
		GPOS_NEW(pmp) CStatistics(pmp, phmulhist2, phmuldoubleWidth2,
								  100.0 /* dRows */, false /* fEmpty */);
	...
	CStatsPredJoin *pstatspredjoin =
		GPOS_NEW(pmp) CStatsPredJoin(2, CStatsPred::EstatscmptEq, 10);
	DrgPstatspredjoin *pdrgpstatspredjoin =
		GPOS_NEW(pmp) DrgPstatspredjoin(pmp);
	pdrgpstatspredjoin->Append(pstatspredjoin);
	CStatistics *pstats3 =
		pstats->PstatsInnerJoin(pmp, pstats2, pdrgpstatspredjoin);
	...
	CStatistics *pstats4 = CGroupByStatsProcessor::PstatsGroupBy(
		pmp, pstats, pdrgpulGC, pdrgpulAgg, NULL /*pbsKeys*/);
	...
	// LASJ stats
	CStatistics *pstats5 =
		pstats->PstatsLASJoin(pmp, pstats2, pdrgpstatspredjoin,
	...
	CStatistics *pstats6 = CUnionAllStatsProcessor::PstatsUnionAll(
		pmp, pstats, pstats, pdrgpulColIds, pdrgpulColIds, pdrgpulColIds);

	GPOS_TRACE(GPOS_WSZ_LIT("pstats6 = pstats1 union all pstats1"));
	CCardinalityTestUtils::PrintStats(pmp, pstats6);

	CStatistics *pstats7 =
		CLimitStatsProcessor::PstatsLimit(pmp, pstats, CDouble(4.0));
...
DrgPstatspred *
CStatisticsTest::Pdrgpstatspred1(IMemoryPool *pmp)
{
...
// create a filter clause
DrgPstatspred *
CStatisticsTest::Pdrgpstatspred2(IMemoryPool *pmp)
{
	// contain for filters
	DrgPstatspred *pdrgpstatspred = GPOS_NEW(pmp) DrgPstatspred(pmp);
	...
	CWStringDynamic *pstrLowerNumeric =
		GPOS_NEW(pmp) CWStringDynamic(pmp, GPOS_WSZ_LIT("AAAACgAAAQABAA=="));
	CWStringDynamic *pstrUpperNumeric =
		GPOS_NEW(pmp) CWStringDynamic(pmp, GPOS_WSZ_LIT("AAAACgAAAQACAA=="));

	StatsFilterNumeric(pmp, 3, pstrLowerNumeric, pstrUpperNumeric, CDouble(1.0),
	...
	CWStringDynamic *pstrLowerDate =
		GPOS_NEW(pmp) CWStringDynamic(pmp, GPOS_WSZ_LIT("HxEAAA=="));
	CWStringDynamic *pstrUpperDate =
		GPOS_NEW(pmp) CWStringDynamic(pmp, GPOS_WSZ_LIT("LREAAA=="));
	LINT lLowerDate =
		LINT(4383) * LINT(INT64_C(86400000000));  // microseconds per day
	LINT lUpperDate =
		LINT(4397) * LINT(INT64_C(86400000000));  // microseconds per day
	StatsFilterGeneric(pmp, 4, GPDB_DATE, pstrLowerDate, pstrUpperDate,
	...
	CWStringDynamic *pstrLowerTS =
		GPOS_NEW(pmp) CWStringDynamic(pmp, GPOS_WSZ_LIT("ACcI7mpYAQA="));
	CWStringDynamic *pstrUpperTS =
		GPOS_NEW(pmp) CWStringDynamic(pmp, GPOS_WSZ_LIT("AAg5THNYAQA="));
	LINT lLowerTS = LINT(INT64_C(378691260000000));  // microseconds
	LINT lUpperTS = LINT(INT64_C(378727200000000));  // microseconds

	StatsFilterGeneric(pmp, 5, GPDB_TIMESTAMP, pstrLowerTS, pstrUpperTS,
...
void
CStatisticsTest::StatsFilterInt4(IMemoryPool *pmp, ULONG ulColId, INT iLower,
								 INT iUpper, DrgPstatspred *pdrgpstatspred)
{
	CStatsPredPoint *pstatspred1 =
		GPOS_NEW(pmp) CStatsPredPoint(ulColId, CStatsPred::EstatscmptGEq,
									  CTestUtils::PpointInt4(pmp, iLower));

	CStatsPredPoint *pstatspred2 = GPOS_NEW(pmp) CStatsPredPoint(
		ulColId, CStatsPred::EstatscmptL, CTestUtils::PpointInt4(pmp, iUpper));
...
void
CStatisticsTest::StatsFilterBool(IMemoryPool *pmp, ULONG ulColId, BOOL fValue,
								 DrgPstatspred *pdrgpstatspred)
{
	CStatsPredPoint *pstatspred1 = GPOS_NEW(pmp) CStatsPredPoint(
		ulColId, CStatsPred::EstatscmptEq, CTestUtils::PpointBool(pmp, fValue));
...
void
CStatisticsTest::StatsFilterNumeric(IMemoryPool *pmp, ULONG ulColId,
...
void
CStatisticsTest::StatsFilterGeneric(IMemoryPool *pmp, ULONG ulColId, OID oid,
									CWStringDynamic *pstrLowerEncoded,
									CWStringDynamic *pstrUpperEncoded,
									LINT lValueLower, LINT lValueUpper,
									DrgPstatspred *pdrgpstatspred)
{
	CStatsPredPoint *pstatspred1 = GPOS_NEW(pmp)
		CStatsPredPoint(ulColId, CStatsPred::EstatscmptGEq,
						CCardinalityTestUtils::PpointGeneric(
							pmp, oid, pstrLowerEncoded, lValueLower));

	CStatsPredPoint *pstatspred2 = GPOS_NEW(pmp)
		CStatsPredPoint(ulColId, CStatsPred::EstatscmptL,
						CCardinalityTestUtils::PpointGeneric(
							pmp, oid, pstrUpperEncoded, lValueUpper));
...
CStatisticsTest::EresUnittest_CStatisticsSelectDerivation()
{
	...
	return CTestUtils::EresTranslate(
		pmp, szQuerySelect, szPlanSelect,
