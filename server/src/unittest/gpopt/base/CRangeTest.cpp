...
//---------------------------------------------------------------------------
IDatum *
CRangeTest::PdatumInt2(gpos::IMemoryPool *pmp, INT i)
{
	return GPOS_NEW(pmp)
...
//---------------------------------------------------------------------------
IDatum *
CRangeTest::PdatumInt4(gpos::IMemoryPool *pmp, INT i)
{
	return GPOS_NEW(pmp)
...
//---------------------------------------------------------------------------
IDatum *
CRangeTest::PdatumInt8(gpos::IMemoryPool *pmp, INT li)
{
	return GPOS_NEW(pmp)
...
CRangeTest::EresUnittest_CRangeFromScalar()
{
	...
	CDatumInt4GPDB *pdatumint4 =
		GPOS_NEW(pmp) CDatumInt4GPDB(CTestUtils::m_sysidDefault, 10 /*iVal*/);
	...
	{
		pdatumint4->AddRef();
		CRange *prange = GPOS_NEW(pmp) CRange(&comp, rgecmpt[ul], pdatumint4);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CRangeTest::EresInitAndCheckRanges(IMemoryPool *pmp, IMDId *pmdid, PfPdatum pf)
{
	CConstExprEvaluatorDefault *pceeval =
		GPOS_NEW(pmp) CConstExprEvaluatorDefault();
	CDefaultComparator comp(pceeval);
	...
	// generate ranges
	pmdid->AddRef();
	CRange *prange1 = GPOS_NEW(pmp)
		CRange(pmdid, &comp, (*pf)(pmp, 10), CRange::EriIncluded, NULL,
			   CRange::EriExcluded);  // [10, inf)

	pmdid->AddRef();
	CRange *prange2 = GPOS_NEW(pmp)
		CRange(pmdid, &comp, NULL, CRange::EriExcluded, (*pf)(pmp, 20),
			   CRange::EriIncluded);  // (-inf, 20]

	pmdid->AddRef();
	CRange *prange3 = GPOS_NEW(pmp) CRange(pmdid, &comp, (*pf)(pmp, -20),
										   CRange::EriExcluded, (*pf)(pmp, 0),
										   CRange::EriIncluded);  // (-20, 0]

	pmdid->AddRef();
	CRange *prange4 = GPOS_NEW(pmp) CRange(pmdid, &comp, (*pf)(pmp, -10),
										   CRange::EriIncluded, (*pf)(pmp, 10),
										   CRange::EriExcluded);  // [-10, 10)

	pmdid->AddRef();
	CRange *prange5 = GPOS_NEW(pmp)
		CRange(pmdid, &comp, (*pf)(pmp, 0), CRange::EriIncluded, (*pf)(pmp, 0),
...
//---------------------------------------------------------------------------
void
CRangeTest::TestRangeRelationship(IMemoryPool *pmp, CRange *prange1,
								  ...
								  CRange *prange4, CRange *prange5)
{
	...
	GPOS_ASSERT_MSG(!prange1->FContains(prange2),
					"[10, inf) does not contain (-inf, 20]");
	GPOS_ASSERT_MSG(prange2->FContains(prange4),
...
//---------------------------------------------------------------------------
void
CRangeTest::PrintRange(IMemoryPool *pmp, CColRef *pcr, CRange *prange)
{
