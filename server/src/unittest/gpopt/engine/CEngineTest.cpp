...
)
{
	...
	// scope for optimization context
	{
		...
		DrgPexprJoins *pdrgpexprCrossProducts = CTestUtils::PdrgpexprJoins(
			pmp, pstr, pul, ulRels, true /*fCrossProduct*/);

		// generate join expressions
		DrgPexprJoins *pdrgpexpr = CTestUtils::PdrgpexprJoins(
			pmp, pstr, pul, ulRels, false /*fCrossProduct*/);
...
//---------------------------------------------------------------------------
void
CEngineTest::BuildMemoRecursive(IMemoryPool *pmp, CExpression *pexprInput,
								DrgPss *pdrgpss)
{
...
//---------------------------------------------------------------------------
GPOS_RESULT
CEngineTest::EresTestEngine(Pfpexpr rgpf[], ULONG ulSize)
{
	CAutoMemoryPool amp;
	IMemoryPool *pmp = amp.Pmp();
...
CEngineTest::EresUnittest_BuildMemoWithCTE()
{
	...
	CColRefSet *pcrsLeft =
		CDrvdPropRelational::Pdprel(pexprCTE->PdpDerive())->PcrsOutput();
