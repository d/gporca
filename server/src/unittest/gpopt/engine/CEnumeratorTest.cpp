...
const CHAR *rgszUnsatisfiedRequiredPropertiesPlans[] = {
	...
	"../data/dxl/minidump/"
	"InvalidPlan_MotionGatherFromMasterToMaster-ScalarDQA.mdp",
	"../data/dxl/minidump/"
	"InvalidPlan_IncompatibleDistributionOnJoinBranches.mdp",
};
		...
		GPOS_TRY
		{
			...
			);
			{
				...
				at.Os() << std::endl
						<< "Minidump "
						<< rgszUnsatisfiedRequiredPropertiesPlans[ul];
				at.Os() << " did not raise UnsatisfiedRequiredProperties, as "
						   "expected"
						<< std::endl;
			}
...
CEnumeratorTest::EresUnittest_RunCompatibleDistributions()
{
	...
	return CTestUtils::EresRunMinidumps(pmp, rgszCompatibleDistributions,
										ulTests,
