...
CPredicateUtilsTest::EresUnittest_Conjunctions()
{
	...
	DrgPexpr *pdrgpexprExtract =
		CPredicateUtils::PdrgpexprConjuncts(pmp, pexprConjunction);
	GPOS_ASSERT(pdrgpexprExtract->UlLength() == ulConjs);

	// collapse into single conjunct
	CExpression *pexpr =
		CPredicateUtils::PexprConjunction(pmp, pdrgpexprExtract);
	...
	CExpression *pexprCmp1 =
		CUtils::PexprScalarCmp(pmp, pcr1, pcr2, IMDType::EcmptEq);
	CExpression *pexprCmp2 = CUtils::PexprScalarCmp(
		pmp, pcr1, CUtils::PexprScalarConstInt4(pmp, 1 /*iVal*/),
		IMDType::EcmptEq);
...
CPredicateUtilsTest::EresUnittest_Disjunctions()
{
	...
	DrgPexpr *pdrgpexprExtract =
		CPredicateUtils::PdrgpexprDisjuncts(pmp, pexprDisjunction);
	GPOS_ASSERT(pdrgpexprExtract->UlLength() == ulDisjs);

	// collapse into single disjunct
	CExpression *pexpr =
		CPredicateUtils::PexprDisjunction(pmp, pdrgpexprExtract);
	GPOS_ASSERT(NULL != pexpr);
	...
	CExpression *pexprSingleton =
		CPredicateUtils::PexprDisjunction(pmp, NULL /*pdrgpexpr*/);
	GPOS_ASSERT(NULL != pexprSingleton);
	pexprSingleton->Release();
...

	{
		...
		CExpression *pexprCmp3 =
			CUtils::PexprScalarCmp(pmp, pcr2, pcr1, IMDType::EcmptG);
		CExpression *pexprCmp4 = CUtils::PexprScalarCmp(
			pmp, CUtils::PexprScalarConstInt4(pmp, 200 /*iVal*/), pcr3,
			IMDType::EcmptL);
		pexprCmp1->AddRef();
		pexprCmp2->AddRef();
...
CPredicateUtilsTest::EresUnittest_PlainEqualities()
{
	...
	CExpression *pexprScIdentInequality = CUtils::PexprScalarCmp(
		pmp, pcrLeft, pcrRight, CWStringConst(GPOS_WSZ_LIT("<")),
		GPOS_NEW(pmp) CMDIdGPDB(GPDB_INT4_LT_OP));
	...
	CExpression *pexprScalarConstInt4 =
		CUtils::PexprScalarConstInt4(pmp, 10 /*fValue*/);
	CExpression *pexprScIdentConstEquality =
		CUtils::PexprScalarEqCmp(pmp, pexprScalarConstInt4, pcrRight);
...
CPredicateUtilsTest::EresUnittest_Implication()
{
	...
	CExpression *pexprConstraints =
		CExpressionPreprocessor::PexprAddPredicatesFromConstraints(pmp,
																   pexprJoin2);
	...
	CExpression *pexprMinimizedPred =
		CPredicateUtils::PexprRemoveImpliedConjuncts(
			pmp, (*pexprConstraints)[2], exprhdl);
	...
	DrgPexpr *pdrgpexprOriginalConjuncts =
		CPredicateUtils::PdrgpexprConjuncts(pmp, (*pexprConstraints)[2]);
	DrgPexpr *pdrgpexprNewConjuncts =
		CPredicateUtils::PdrgpexprConjuncts(pmp, pexprMinimizedPred);

	GPOS_ASSERT(pdrgpexprNewConjuncts->UlLength() <
				pdrgpexprOriginalConjuncts->UlLength());
