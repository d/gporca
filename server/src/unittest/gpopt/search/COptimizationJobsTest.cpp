	...
	// install opt context in TLS
	{
...

		{
			...
			(void) jgi.OsDiagramToGraphviz(
				pmp, at.Os(), GPOS_WSZ_LIT("GroupImplementationJob"));
...

		{
			...
			// dumping state graph
			at.Os() << std::endl;
			(void) jge.OsDiagramToGraphviz(pmp, at.Os(),
...

		{
			...
			(void) jgeo.OsDiagramToGraphviz(
				pmp, at.Os(), GPOS_WSZ_LIT("GroupExpressionOptimizationJob"));
...

		{
			...
			(void) jgei.OsDiagramToGraphviz(
				pmp, at.Os(), GPOS_WSZ_LIT("GroupExpressionImplementationJob"));
...

		{
			...
			(void) jgee.OsDiagramToGraphviz(
				pmp, at.Os(), GPOS_WSZ_LIT("GroupExpressionExplorationJob"));
...

		{
			CAutoTrace at(pmp);
			CXformSet *pxfs =
				CLogical::PopConvert(pgexprLogical->Pop())->PxfsCandidates(pmp);
			...
			while (xsi.FAdvance())
			{
				...
				(void) jt.OsDiagramToGraphviz(
					pmp, at.Os(), GPOS_WSZ_LIT("TransformationJob"));
