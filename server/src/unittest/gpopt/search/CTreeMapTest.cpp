...
//---------------------------------------------------------------------------
IOstream &
CTreeMapTest::CNode::OsPrint(IOstream &os, ULONG ulIndent) const
{
...
//
//---------------------------------------------------------------------------
CTreeMapTest::CNode::CNode(IMemoryPool *,  // pmp
						   ULONG *pulData, DrgPnd *pdrgpnd)
	: m_ulData(ULONG_MAX), m_pdrgpnd(pdrgpnd)
{
	if (NULL != pulData)
	{
		m_ulData = *pulData;
...
//---------------------------------------------------------------------------
CTreeMapTest::CNode *
CTreeMapTest::Pnd(IMemoryPool *pmp, ULONG *pul, DrgPnd *pdrgpnd,
				  BOOL *fTestTrue)
{
...
CTreeMapTest::PtmapLoad(IMemoryPool *pmp)
{
...
CTreeMapTest::EresUnittest_Memo()
{
	...
	CExpression *pexprPlan = NULL;
	{
		...
		peng->Trace();
		{
			...
			for (ULONG ulRank = 0; ulRank < ullCount; ulRank++)
			{
				CDrvdPropCtxtPlan *pdpctxtplan = GPOS_NEW(pmp)
					CDrvdPropCtxtPlan(pmp, false /*fUpdateCTEMap*/);
				CExpression *pexprAlt = NULL;
				GPOS_TRY
				{
					pexprAlt =
						peng->Pmemotmap()->PrUnrank(pmp, pdpctxtplan, ulRank);
					at.Os() << std::endl
	...
	for (ULONG ul = 0; eres == GPOS_OK && ul < ulTests; ul++)
	{
		GPOS_TRY
		{
			eres = CTestUtils::EresRunMinidumps(
				pmp, &rgszFailedPlanEnumerationTests[ul],
				1,  // ulTests
				&m_ulTestCounter,
				1,  // ulSessionId
				1,  // ulCmdId
