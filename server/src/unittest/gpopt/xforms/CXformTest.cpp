...
CXformTest::EresUnittest_ApplyXforms_CTE()
{
	...
	CExpression *pexprSelect =
		CTestUtils::PexprLogicalSelect(pmp, pexprConsumer);
	pdrgpexpr->Append(pexprSelect);

	pexprSelect->AddRef();
	CExpression *pexprAnchor = GPOS_NEW(pmp) CExpression(
		pmp, GPOS_NEW(pmp) CLogicalCTEAnchor(pmp, ulCTEId), pexprSelect);
...
//---------------------------------------------------------------------------
void
CXformTest::ApplyExprXforms(IMemoryPool *pmp, IOstream &os, CExpression *pexpr)
{
	...
	for (ULONG ulXformId = 0; ulXformId < CXform::ExfSentinel; ulXformId++)
	{
		...
		if (pxform->FCheckPattern(pexpr) &&
			CXform::FPromising(pmp, pxform, pexpr))
		{
			if (CXform::ExfExpandNAryJoinMinCard == pxform->Exfid())
			{
				...
				exprhdl.DeriveStats(pmp, pmp, NULL /*prprel*/,
									NULL /*pdrgpstatCtxt*/);
			}
...
//---------------------------------------------------------------------------
CExpression *
CXformTest::PexprStarJoinTree(IMemoryPool *pmp, ULONG ulTabs)
{
	...
	for (ULONG ul = 1; ul < ulTabs; ul++)
	{
		...
		pexprLeft = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(
			pmp, pexprLeft, pexprRight, pexprPred);
	}
...
//---------------------------------------------------------------------------
CExpression *
CXformTest::PexprJoinTree(IMemoryPool *pmp)
{
